...
class TestGetBot:
    ...
    @pytest.mark.asyncio
    async def test_get_existing_bot(
    ...
    @pytest.mark.asyncio
    async def test_get_bot_detail_includes_devices(
        self, api: APITestHelper, created_bot: dict[str, Any]
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_bot_detail_device_info_fields(
        self, api: APITestHelper, created_bot: dict[str, Any]
    ) -> None:
...
class TestUpdateDevices:
    ...
    async def _fetch_device(self, api: APITestHelper, bot_uuid: str) -> dict:
        resp = await api.client.get(
    ...
    async def test_update_devices_returns_publish_id(
        ...
        device = await self._fetch_device(api, bot_uuid)
        ...
        response = await api.client.post(
            ...
            json={
                ...
                "request_id": f"e2e-upd-dev--{unique_id}-xxxxxxxxxxxx",
            },
        )
    ...
    async def test_update_devices_with_config(
        ...
        response = await api.client.post(
            ...
            json={
                ...
                "device_uuids": [device["device_uuid"]],
                "auto_approve_publish": True,
                "request_id": f"e2e-upd-dev-cfg-{unique_id}-xxxxxxxxxx",
                "config": {
                    "entity_id": entity_id,
                    "entity_type": "staff",
                },
...
class TestGetBotDevices:
    ...
    @pytest.mark.asyncio
    async def test_get_bot_devices_by_uuid_endpoint(
        self, api: APITestHelper, created_bot: dict[str, Any]
    ) -> None:
        bot = created_bot
    ...
    @pytest.mark.asyncio
    async def test_get_bot_devices_by_id_paginated(
        self, api: APITestHelper, created_bot: dict[str, Any]
    ) -> None:
        bot = created_bot
