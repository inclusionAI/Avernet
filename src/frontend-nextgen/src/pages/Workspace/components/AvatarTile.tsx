...
export function AvatarTile({ src, label, className }: AvatarTileProps) {
  ...
  if (isImage) {
    return <img src={src} alt={label} className={cn('h-11 w-11 shrink-0 rounded-xl object-cover', className)} />;
  }
  ...
  return (
    <span
      aria-hidden="true"
      className={cn(
        'flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[var(--color-primary-soft)] text-lg font-semibold text-[var(--color-primary)]',
        className,
      )}
    >
