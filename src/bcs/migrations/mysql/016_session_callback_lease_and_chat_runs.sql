-- Consolidated migration 016: Session callback leases and Direct Chat runs.
-- The two former 016 files are preserved byte-for-byte under ../legacy/mysql.
-- Existing split-016 installations require explicit reconciliation; see
-- ../reconciliation/016-session-callback-and-chat-runs.md before applying.
-- This canonical DDL targets the pre-016 schema and uses MySQL 8-compatible
-- ADD COLUMN syntax. Apply through the versioned runner; do not replay raw DDL.
...
ALTER TABLE `bcs_group_sessions`
  ADD COLUMN `callback_lease_owner` varchar(128) DEFAULT NULL,
  ADD COLUMN `callback_lease_token` bigint DEFAULT NULL,
  ADD COLUMN `callback_lease_until_ms` bigint unsigned DEFAULT NULL,
