...
class AvernetStarWorkflowTest(unittest.TestCase):
    ...
    def test_workflow_pins_skill_and_runs_codex_read_only_with_image(self):
        ...
        self.assertEqual(
            workflow.count('git -C efficiency apply --check "$GITHUB_WORKSPACE/scripts/ci/avernet_star_growth.patch"'),
            2,
        )
        self.assertEqual(
            workflow.count('git -C efficiency apply --reverse --check "$GITHUB_WORKSPACE/scripts/ci/avernet_star_growth.patch"'),
            2,
        )
        self.assertIn(
