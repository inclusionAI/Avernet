...
export const TASK_GRANT_ENDPOINTS = {
  grant: '/api/v1/collaboration/tasks/grant',
  revoke: '/api/v1/collaboration/tasks/revoke',
};
...
export function grantTaskClaim(body: { bcs_bot_id: string }, signal?: AbortSignal) {
  return backendRequest<BackendApiEnvelope<TaskClaimGrantResultDto>>(TASK_GRANT_ENDPOINTS.grant, {
    method: 'POST',
    data: body,
    injectUserId: false,
    signal,
...
export function revokeTaskClaim(body: { bcs_bot_id: string }, signal?: AbortSignal) {
  return backendRequest<BackendApiEnvelope<TaskClaimGrantResultDto>>(TASK_GRANT_ENDPOINTS.revoke, {
    method: 'POST',
    data: body,
    injectUserId: false,
    signal,
