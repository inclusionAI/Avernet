...
async def rebind_architect_bot(
    ...
    try:
        ...
        logger.warning("[bot_router.rebind_architect_bot] permission error: %s", e)
        return ApiResponse(success=False, message=str(e), error_code=403, data=None)
    except BotServiceError as e:
        logger.warning("[bot_router.rebind_architect_bot] service error: %s", e)
        return ApiResponse(success=False, message=str(e), error_code=400, data=None)
    except Exception as e:
        logger.error(f"[bot_router.rebind_architect_bot] Error: {e}", exc_info=True)
        return ApiResponse(
            success=False,
