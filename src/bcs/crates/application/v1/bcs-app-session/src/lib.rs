...
impl SessionServiceImpl {
    ...
    fn can_manage_session_actor(actor_id: &str, session: &Session, group: &DomainGroup) -> bool {
        Self::can_manage_group_actor(actor_id, group)
            || session
                .created_by
                .as_deref()
                .is_some_and(|creator| creator == actor_id)
    }

    fn can_manage_session(principal: &Principal, session: &Session, group: &DomainGroup) -> bool {
        Self::can_manage_session_actor(&principal.actor_id(), session, group)
    }
...
impl SessionService for SessionServiceImpl {
    ...
    async fn delete(&self, command: DeleteSession) -> Result<DeleteResult, ApplicationError> {
        ...
        let acting_actor_id = self
            ...
            .await?;
        if !Self::can_manage_session_actor(&acting_actor_id, &session, &group) {
            return Err(ApplicationError::forbidden(
