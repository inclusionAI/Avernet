...
describe("Stage Skill integration test", () => {
  it("rejects a test Case that does not satisfy the selected Stage input", async () => {
    const test = await startRouter();
    const response = await fetch(`${test.baseUrl}/api/evolve/stage-skills/IMPL-1/integration-tests`, {
      method: "POST",
    ...
    expect(await response.json()).toMatchObject({
      error: expect.stringContaining("input.diagnose_goal"),
    });
    expect(test.evolveRepo.createTaskWithStep).not.toHaveBeenCalled();
    expect(test.dispatch).not.toHaveBeenCalled();
  });

  it("dispatches the real Stage Skill directly when the case has no target Skill", async () => {
    const test = await startRouter();
    const response = await fetch(`${test.baseUrl}/api/evolve/stage-skills/IMPL-1/integration-tests`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-User-Id": "user-1" },
      body: JSON.stringify({ botId: "bot-1", caseInput: { diagnose_goal: "定位失败原因" } }),
    });

    expect(response.status).toBe(201);
    expect(test.evolveRepo.createTaskWithStep).toHaveBeenCalledWith(expect.objectContaining({
      task: expect.objectContaining({
        configJson: expect.stringContaining('"session_source":{"mode":"local"}'),
      }),
      step: expect.objectContaining({ stepType: "stage_extension" }),
    }));
    expect(test.stageRepo.createExtensionRun).toHaveBeenCalledWith(expect.objectContaining({
      initialInput: {
        diagnose_goal: "定位失败原因",
        session_source: { mode: "local" },
      },
    }));
    expect(test.dispatch).toHaveBeenCalledWith(expect.objectContaining({ stepType: "stage_extension" }));
    expect(test.ocbLocalSkills.exportLocalSkill).not.toHaveBeenCalled();
  });

  it("freezes a real OCB Skill and prepares the isolated candidate before the Stage Skill", async () => {
    const test = await startRouter();
    const response = await fetch(`${test.baseUrl}/api/evolve/stage-skills/IMPL-1/integration-tests`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-User-Id": "user-1" },
      body: JSON.stringify({
        botId: "bot-1",
        targetSkillAssetId: "SKILL-1",
        caseInput: { diagnose_goal: "定位失败原因" },
      }),
    });
    const body = await response.json() as { stepId: string; extensionStepId: string };

    expect(response.status).toBe(201);
    expect(test.ocbLocalSkills.exportLocalSkill).toHaveBeenCalledWith(expect.objectContaining({
      botId: "bot-1", skillId: "local-skill",
    }));
    expect(test.putObject).toHaveBeenCalledWith(
      expect.stringMatching(/evolve\/skills\/tasks\/EVT-.*\/baseline\/package\.zip/),
      Buffer.from("real skill package"),
      "application/zip",
    );
    expect(test.evolveRepo.createTaskWithStep).toHaveBeenCalledWith(expect.objectContaining({
      step: expect.objectContaining({ stepType: "skill_prepare", stepId: body.stepId }),
    }));
    expect(body.stepId).not.toBe(body.extensionStepId);
    expect(test.evolveRepo.createStep).toHaveBeenCalledWith(expect.objectContaining({
      stepId: body.extensionStepId, stepType: "stage_extension", stepNo: 2,
    }));
    expect(test.stageRepo.createExtensionRun).toHaveBeenCalledWith(expect.objectContaining({
      stepId: body.extensionStepId,
      implementationId: "IMPL-1",
    }));
    expect(test.dispatch).toHaveBeenCalledWith(expect.objectContaining({ stepType: "skill_prepare" }));
  });
});
