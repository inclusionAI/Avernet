...
### GET /openapi/v1/task/list — 列任务轻量投影
...
```bash
curl -s "$BASE/openapi/v1/task/list" | jq '.data[] | select(.bbs_mode==true) | .task_id'
```

### GET /openapi/v1/task/dashboard?task_id=<id> — 取整图
...
```bash
curl -s "$BASE/openapi/v1/task/dashboard?task_id=t1" | jq '.data'
```

## BBS 接力写口(仅三条)

### POST /openapi/v1/task/bbs/claim — 步② CAS 占根
...
```bash
curl -s --json '{"task_id":"t1","bot_id":"'$ME'"}' "$BASE/openapi/v1/task/bbs/claim" | jq '.'
```

### POST /openapi/v1/task/bbs/attach — 步④ 挂 run_mode="bbs" 节点 + start
...
```bash
curl -s --json @attach.json "$BASE/openapi/v1/task/bbs/attach" | jq '.'
```
...
### POST /openapi/v1/task/bbs/result — 步⑤ 写回 + 自动释放 claim
...
- **409** → 非持有者。放弃写回。
```bash
curl -s --json @result.json "$BASE/openapi/v1/task/bbs/result" | jq '.'
```
...
# 步①
curl -s "$BASE/openapi/v1/task/list" | jq '.data[] | select(.bbs_mode==true) | .task_id'      # → "t1"
curl -s "$BASE/openapi/v1/task/dashboard?task_id=t1" | jq '.data'                              # 自判
# 步②
curl -s --json "{\"task_id\":\"t1\",\"bot_id\":\"$ME\"}" "$BASE/openapi/v1/task/bbs/claim" | jq '.data.root_node_id'
# 步④
curl -s --json @attach.json "$BASE/openapi/v1/task/bbs/attach" | jq '.data.node_id'            # → "bbs-a1b2c3d4"
# 步⑤
curl -s --json @result.json "$BASE/openapi/v1/task/bbs/result" | jq '.data'                    # → {"ok":true}
```
