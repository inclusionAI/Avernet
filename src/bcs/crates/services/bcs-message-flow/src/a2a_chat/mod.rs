...
pub struct A2aChat {
    ...
    session_management: Option<Arc<dyn bcs_service_api::SessionManagementService>>,
    queue_owner: Arc<std::sync::OnceLock<std::sync::Weak<crate::BcsMessageFlow>>>,
...
impl A2aChat {
    ...
    pub fn new_with_run_ports(
        ...
        Self {
            ...
            default_timeout_ms,
            session_management: None, queue_owner: Arc::new(std::sync::OnceLock::new()),
    ...
    async fn fail_run_if_open_with_reason(
        ...
        if record.delivery_id.is_some() { return Ok(false); }
...
impl A2aChatRunService for A2aChat {
    async fn start_async_chat(
        ...
        let queued = self.queue_enabled(&cmd.target_bot_id).await;
        if !queued {
        ...
        let chat_result = bcs_observability::observe_result("chat.prepare_and_deliver", self
            .chat_with_queue_mode(A2aChatCommand {
                caller: cmd.caller,
        ...
        let stored = self.run_store.try_get(&cmd.run_id).await?;
        let expires_at_ms = stored.as_ref().map_or(expires_at_ms, |r| r.expires_at_ms);
        let managed = stored.as_ref().is_some_and(|r| r.delivery_id.is_some());
        if managed || chat_outcome.status == "completed" {
            self.chat_run_events.unregister(&cmd.run_id).await;
        } else {
        ...
        let delivery = if managed {
            let record = stored.ok_or_else(|| queued::invalid("direct run missing"))?;
            self.managed_status(record, None).await?.response.and_then(|v| v.get("delivery").cloned())
        } else { None };
    ...
    async fn cancel_run(&self, cmd: ChatRunCancelCommand) -> ServiceResult<A2aRunStatus> {
        ...
        if status.response.as_ref().is_some_and(|p| p["is_terminal"] == true) { self.chat_run_cleanup.unregister(&cmd.run_id).await; }
        Ok(status)
    }
}
...
impl A2aChatService for A2aChat {
    async fn chat(&self, cmd: A2aChatCommand) -> ServiceResult<A2aChatOutcome> {
        self.chat_with_queue_mode(cmd, None).await
    }
    ...
    async fn get_run(&self, caller: CallerContext, run_id: &str) -> ServiceResult<A2aRunStatus> {
        ...
        self.managed_status(record, None).await
    }
    ...
    async fn wait_run(
        ...
        if record.delivery_id.is_some() {
            ...
            loop {
                let current = self.run_store.try_get(run_id).await?.ok_or_else(|| ServiceError::BotNotFound(run_id.into()))?;
                let status = self.managed_status(current, None).await?;
                let payload = status.response.as_ref().ok_or_else(|| ServiceError::InternalError("chat status missing".into()))?;
                if payload["version"].as_u64().unwrap_or(0) > since_version || payload["is_terminal"] == true || tokio::time::Instant::now() >= deadline { return Ok(status); }
                tokio::time::sleep(Duration::from_millis(1_000).min(deadline.saturating_duration_since(tokio::time::Instant::now()))).await;
            }
        }
    ...
    async fn record_run_event(&self, run_id: &str, event_json: &str) -> ServiceResult<bool> {
        let mut record = self
            ...
            .await
            .ok_or_else(|| ServiceError::BotNotFound(format!("chat run {run_id}")))?;
        if record.delivery_id.is_some() { return Err(queued::invalid("managed events require scoped delivery handling")); }
        if record.state.is_terminal() {
            return Ok(true);
        }
    ...
    async fn cancel_run(&self, caller: CallerContext, run_id: &str) -> ServiceResult<A2aRunStatus> {
        ...
        let record = self
            ...
            .ok_or_else(|| ServiceError::BotNotFound(format!("chat run {run_id}")))?;
        ensure_run_owner(&record, &from_bot_id)?;
        if record.delivery_id.is_some() { return self.cancel_managed(record, &from_bot_id).await; }
        let client_kind = direct_chat_client_kind(record.client.as_deref());
    ...
    async fn cleanup_expired(
        ...
        self.recover_managed_runs().await?;
