...
def _static_relay_closure() -> str:
    ...
    return "\n".join([
        ...
        "【协作群多轮接力(仅当上方【## 群组成】列出除你之外的其它 Bot 成员;你为 master/driver)】上述三步按多轮执行,不得合并成一轮:",
        "1) 先单独回复承接(接力上下文):复述接自谁、上游交你的下一步任务、你的承接边界,并说明本群各成员 Bot 分工;此轮只发承接,不执行、不派发;",
        "2) 派发执行:用 bcs_assign_task(主从群)或 bcs_route(chat 群)把各自职责任务派发给【## 群组成】中其它非人类 Bot 成员执行(human 仅为观察者,不得向 human 派发);等待各成员 Bot 执行并回复;",
        "3) 汇总与交接:收齐各成员 Bot 回复后,综合成本群执行产出(执行产出步),再算 gap 给出交接(gap与交接步);在正文描述下一步交给谁、做什么,不路由群外。",
