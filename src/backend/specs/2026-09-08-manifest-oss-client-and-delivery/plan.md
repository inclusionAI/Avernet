...
`identity.py` ×1, `cli_tools/service.py` ×1 — replaced by **one** `is_tree()`
branch in `skills`, which runs two validators by design (`validate_zip`
byte-for-byte vs `validate_directory`) and cannot be reduced further without
discarding one of them. `resources`, `identity` and `cli_tools` end with no
branch at all.

`archive_refusal(unpack, strip_components)` moves to `delivery.py` as a pure
function taking the two *values* rather than the entry dict, so `resources` can
still call it **before** the fetch (its whole point — a doomed entry costs no
network) while `BlobDelivery.members` calls the same function after. One rule,
two callers, as it is today.
