 ...
 * - 与 BCS 状态机副屏（协作群 YAML workflow，bcsPanel.StateMachineRunView 经 manifest CDN 加载）互不干扰。
 *
...
/**
 * 注册任务副屏模块。由 `src/extensions/empty.ts` 在 app 初始化时调用一次。
 */
export function registerTaskPanel(): void {
  // 任务副屏挂载用全路径组件名 taskPanel.TaskLoopView（lib.Component
  // 点号 key，引擎 resolveBusinessEntry 对点号 key CDN 优先）。aixLibraryCdnMap['taskPanel'] 命中
  // → 远程 UMD 热更；当前无 TaskPanel UMD → CDN 通道休眠 → 回退本地注册。
