...
from agentclaw.community.core.service_bot.services.publish_flow.release_phase_mixin import ReleasePhaseMixin
    ...
    def __init__(
        ...
        *,
        ...
        self._build_stage_runner = BuildStageRunner(
            ...
            employee_publication_provider=employee_publication_provider,
        )
        ...
        if current_status == PublishStatus.VALIDATING:
            if self._employee_publication_provider is not None:
                ...
                permitted = await asyncio.to_thread(
                    self._employee_publication_provider().prepare_online, publish_id, operator
                )
                if not permitted:
                    return PublishFlowResult(publish_id=publish_id, status=current_status,
                                             message="数字员工审批中", data={"digital_employee_approval": True})
