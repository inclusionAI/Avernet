...
**Track C 把 engine 面向客户端的 HTTP 包装到 `/openapi/v1/bots/{bot_id}/<component>/…` 之下，
...
- socket 集合是**由能力推导出来的**，因此本端点与 `…/{bot_id}/engine/capabilities`
  永远不允许互相矛盾。
...
| `api/web_shell` | — | 2 | 1 | ⛔ **C3 / 非 v1** | `GET /terminal`、`/terminal/health`、`WS /ws/terminal`。也**不能**经 `…/{bot_id}/connection` 触达 —— 终端 socket 曾经实现过又被移除（见上文 connection 条目）；那两个 HTTP 路由是 shell 自身的引导 |
...
| `api/app`（模块级） | — | 6 | 2 | ⛔ / 🔌 | `/health`、`/readiness`、`/config`、`/test-connection`、`/disconnect`、`/api/evaluation/report` 属运维面。`WS /ws` 与 `WS /api/{engine}/ws` 是通用对话 socket → `…/{bot_id}/connection` |
...
- **engine 的 `warning` 被刻意丢弃。** 它是能力*受限*信号，但那些字符串是内部
  ...
  保持不变；`…/{bot_id}/engine/capabilities` 才是调用方发现限制的地方。_2026-07-30 决定。_
...
而 get 与 set 都返回 501。公共侧的 `…/{bot_id}/approvals/modes` 以 `APPROVAL_SET` 为门禁，
...
这些组位于 `/openapi/v1/bots/{bot_id}/{sessions,engine,models,approvals,connection}`
—— 各自处在 Agent 之后、在再下一段上分叉，因此彼此之间不可能互相遮蔽，相互顺序是自由的。
但它们仍必须注册在 bots 组之前，与待退役的组件在前的旧地址、以及账户级字面量
（`check-name`、`ceiling`、`authorized`、`mcp`、`logs`、`loadtest`）一起 —— 真正依赖
先于 `/openapi/v1/bots/{bot_id}` 命中的正是后面这些。

_沿革：这些组最初就比 `{bot_id}` 通配符深一段，2026-08-03 被移到它之上，2026-08-15 又
移了回来 —— 整个面现在都是 Agent 在前，它们顺序自由的理由回到了最初那个。本节原先提示
的那处近似冲突两种形态下都不存在：按 bot 收敛的 MCP 是
`/openapi/v1/bots/mcp/{bot_id}/...`，嵌套在市场组自己的字面量**之下**而不是从另一侧与
之竞争，因为 `mcp` 正是那些不指向单一 Agent 的组之一。Track C 不新增它（规则 C2）。_
