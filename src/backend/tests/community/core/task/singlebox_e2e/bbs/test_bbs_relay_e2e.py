...
def test_c_two_bots_claim_same_bbs_task_exactly_one_wins(client):
    ...
    r1 = c.post("/openapi/v1/collaboration/tasks/bbs/claim", json={"task_id": task_id, "bot_id": "botA"})
    r2 = c.post("/openapi/v1/collaboration/tasks/bbs/claim", json={"task_id": task_id, "bot_id": "botB"})
...
def test_b_partial_handoff_relay(client):
    ...
    assert c.post("/openapi/v1/collaboration/tasks/bbs/claim", json={"task_id": task_id, "bot_id": "botA"}).status_code == 200
    r_attach_a = c.post("/openapi/v1/collaboration/tasks/bbs/attach", json=_attach_body(task_id, task_id, "botA"))
    assert r_attach_a.status_code == 200, r_attach_a.text
    node_a = r_attach_a.json()["data"]["node_id"]
    assert node_a.startswith("bbs-")
    r_fail = c.post("/openapi/v1/collaboration/tasks/bbs/result", json=_result_body(
        task_id, node_a, "botA", verdict="FAIL", gaps=["need_data"], output_patch={"progress": 30}))
    assert r_fail.status_code == 200, r_fail.text

    # botA 已释放 claim(on_bbs_report finally 清根 bbs_owner):dashboard 见 scoped FAILED + checkpoint
    d1, nodes1 = _dashboard_tasks(c, task_id)
    assert nodes1[node_a]["status"] == "FAILED"
    assert (nodes1[node_a]["run_info"]["acceptance_result"] or {})["gaps"] == ["need_data"]
    assert nodes1[node_a]["run_info"]["output"] == {"progress": 30}
    assert _root_owner(nodes1, task_id) is None, "botA result 后 claim 应已释放"
    assert d1["extend_props"].get("bbs_relay_count") == 1, "botA attach 后 relay_count 应 +1"

    # botB 接力第二段:claim(owner 已清,CAS 成功)→ attach 新 scoped 节点(接力不重做,新 node_id)
    assert c.post("/openapi/v1/collaboration/tasks/bbs/claim", json={"task_id": task_id, "bot_id": "botB"}).status_code == 200
    r_attach_b = c.post("/openapi/v1/collaboration/tasks/bbs/attach", json=_attach_body(task_id, task_id, "botB"))
    ...
    r_pass = c.post("/openapi/v1/collaboration/tasks/bbs/result", json=_result_body(
...
def test_d_crash_lease_relay(client):
    ...
    assert c.post("/openapi/v1/collaboration/tasks/bbs/claim", json={"task_id": task_id, "bot_id": "botA"}).status_code == 200
    r_attach = c.post("/openapi/v1/collaboration/tasks/bbs/attach", json=_attach_body(task_id, task_id, "botA"))
    ...
    assert r_attach_b.status_code == 200, r_attach_b.text
    node_b = r_attach_b.json()["data"]["node_id"]
    assert c.post("/openapi/v1/collaboration/tasks/bbs/result", json=_result_body(
...
def test_g_graph_hung_attach_rejected(client):
    ...
    r_claim = c.post("/openapi/v1/collaboration/tasks/bbs/claim", json={"task_id": task_id, "bot_id": "botA"})
