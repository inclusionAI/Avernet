...
def _execute_body(owner_id: str) -> dict:
    """``POST /openapi/v1/task/execute`` 请求体(存储行业尽调案例)。MAX_DEPTH=1 → N_field_interview MISS@MAX 升 BBS。"""
    return {
        "task_spec": {
            "metadata": {
                "task_id": TASK_ID,
...
def _wake_prompt() -> str:
    ...
    return (
        ...
        f"例 curl {_BACKEND}/openapi/v1/task/list --json ...)。\n"
        "按 bbs-relay-pickup SKILL.md 6 步自驱:\n"
        "  步① GET /openapi/v1/task/list 客户端筛 bbs_mode==true,GET /openapi/v1/task/dashboard 取整图;\n"
        "  步② POST /openapi/v1/task/bbs/claim 占根;\n"
        "  步③ 读根 goal + 已 DONE 叶子 + 前序 scoped 节点 checkpoint 自判 full/partial/skip;\n"
        "  步④ POST /openapi/v1/task/bbs/attach 挂一个 run_mode=bbs 节点 + 用你自身能力执行该节点指令;\n"
        "  步⑤ POST /openapi/v1/task/bbs/result 写回(verdict=PASS、acceptances_metric 列出达成的 AC、"
...
class TestTaskIntegrationE2E(unittest.TestCase):
    ...
    async def _run(self, loop: asyncio.AbstractEventLoop) -> None:
        ...
        async with httpx.AsyncClient(timeout=300.0, headers=_HDRS) as cli:
            # 2) POST /openapi/v1/task/execute → backend 进程内真实 engine 后台推进首帧
            ...
            while time.monotonic() < deadline:
                r = await cli.get(f"{_BACKEND}/openapi/v1/task/dashboard", params={"task_id": TASK_ID})
                r.raise_for_status()
