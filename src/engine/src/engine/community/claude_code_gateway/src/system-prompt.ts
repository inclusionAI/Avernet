...
export type ResolvedSystemPrompt = { prompt: string; fileCount: number; sha256: string };
...
function expandPromptFile(filePath: string, root: string, depth: number, active: Set<string>, loaded: Set<string>): string {
  if (depth > MAX_IMPORT_DEPTH) throw new Error(`system prompt import depth exceeds ${MAX_IMPORT_DEPTH}`);
  const resolved = resolveWithinRoot(root, filePath);
  if (active.has(resolved)) throw new Error('system prompt import cycle detected');
  active.add(resolved);
  loaded.add(resolved);
  try {
    ...
    return source.replace(IMPORT_LINE, (_match, importPath: string) => {
      ...
      return expandPromptFile(path.join(path.dirname(resolved), importPath), root, depth + 1, active, loaded);
    });
  } finally {
...
export function resolveSystemPromptFile(promptFile: string, promptRoot?: string): ResolvedSystemPrompt {
  ...
  const prompt = expandPromptFile(promptFile, root, 0, new Set<string>(), loaded).trim();
  if (!prompt) throw new Error('system prompt file is empty');
  if (prompt.length > MAX_PROMPT_CHARS) throw new Error(`system prompt exceeds ${MAX_PROMPT_CHARS} characters after imports`);
  return { prompt, fileCount: loaded.size, sha256: createHash('sha256').update(prompt).digest('hex') };
}
...
export function resolveConfiguredSystemPrompt(env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  log.debug('system-prompt:file-loaded', { fileCount: resolved.fileCount, promptChars: resolved.prompt.length, sha256: resolved.sha256 });
