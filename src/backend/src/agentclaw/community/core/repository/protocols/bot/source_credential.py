...
class SourceCredentialRepositoryProtocol(Protocol):
    ...
    def upsert(
        ...
        *,
        ...
        - ``access_key_id`` / ``region`` / ``endpoint`` — the ``oss_aksk``
          mechanism's non-secret halves, ``None`` for every other type.
          Written on both branches, including with ``None``: a rotation from
          one mechanism to another must clear the fields the old one used, or
          the row would half-describe two mechanisms at once.
        """
        ...
