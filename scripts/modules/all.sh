...
# Nextgen consumes Gateway OpenAPI/auth routes; the legacy UI keeps its
# existing direct-service topology and startup order unchanged.
if [ "${FRONTEND_VARIANT:-legacy}" = nextgen ]; then
