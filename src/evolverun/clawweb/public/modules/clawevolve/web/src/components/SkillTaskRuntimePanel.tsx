...
function InteractionCard({ taskId, interaction, canOperate, onUpdated }: {
  ...
  useEffect(() => {
    if (interaction.question.format !== 'html') return
  ...
  return <div className="rounded-xl border border-amber-200 bg-amber-50/60 p-4">
    <div className="flex items-center justify-between gap-3">
      <p className="text-xs font-semibold text-amber-900">需要补充信息</p>
      <span className="text-[10px] text-amber-700">{interaction.status === 'waiting' ? '等待回答' : '已回答'}</span>
    </div>
    {interaction.question.format === 'html'
      ...
      : <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-gray-800">{interaction.question.content}</p>}
    ...
    {interaction.status === 'answered' && <pre className="mt-3 overflow-x-auto rounded-lg bg-white p-3 text-xs text-gray-600">{JSON.stringify(interaction.answer, null, 2)}</pre>}
...
function CandidateDecision({ task, canOperate, onUpdated }: {
  ...
  const target = task.config.targetSkill as { name?: string; assetId?: string } | undefined
  ...
  useEffect(() => {
    ...
    void api.evolve.getTaskSkillDiff(task.task_id)
      .then((value) => { setDiff(value); setSelected(value.files[0]?.path ?? '') })
      .catch((reason) => setError(reason instanceof Error ? reason.message : '候选差异加载失败'))
  }, [task.task_id, target?.assetId])
  ...
  return <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
    <div className="flex flex-wrap items-start justify-between gap-3">
      ...
      {task.status === 'waiting_acceptance' && canOperate && <div className="flex gap-2">
...
export default function SkillTaskRuntimePanel({ task, canOperate, onUpdated }: {
  ...
  if (!task.config.targetSkill && interactions.length === 0) return null
  return <div className="space-y-5">
    {interactions.length > 0 && <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
      <div><h2 className="text-sm font-semibold text-gray-900">用户交互</h2><p className="mt-1 text-xs text-gray-500">回答后，平台会把内容带回同一个 Stage 继续执行。</p></div>
      <div className="mt-4 space-y-3">{interactions.map((item) => <InteractionCard key={item.interactionId} taskId={task.task_id} interaction={item} canOperate={canOperate} onUpdated={onUpdated} />)}</div>
    </section>}
    <CandidateDecision task={task} canOperate={canOperate} onUpdated={onUpdated} />
  </div>
}
