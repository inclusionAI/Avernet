...
- Relay Runner 指令只注入 `EXECUTION_RESULT → S6本地next_task_spec（不上报）→ /search → 确定执行者/执行模态 → PLAN_RESULT → DISPATCH_RESULT → /dispatch/BBS` 事件协议；Relay 任务拒绝中心化 `status/output/acceptance_result` 节点终态回投，避免错误协议把节点推成一次性终态。
