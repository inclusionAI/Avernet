...
impl MySqlMessageStore {
    async fn delivery_transaction(
        ...
        for command in admission {
            ...
            let table = if direct { "bcs_session_registry" } else { "bcs_group_sessions" };
            let type_filter = if direct { " AND session_type = 'direct_a2a' AND current_msg_seq IS NOT NULL" } else { "" };
            ...
            let sequence = self.db.transaction(vec![DbTransactionStep::Query(
                DbStatement::with_params(
                    format!("SELECT current_msg_seq FROM {table} WHERE env = ? AND session_id = ?{type_filter}{suffix}"),
                    vec![self.env.as_str().into(), command.message.session_id.as_str().into()],
                ),
            )]).await.map_err(storage)?;
            ...
            db_get_column(
                rows.first().ok_or_else(|| if direct { MessageDeliveryRepoError::Invalid("session_registry_missing".into()) } else { storage("canonical session is missing") })?,
                "current_msg_seq",
            ).map_err(storage)? };
            let seq = old_seq
            ...
            steps.push(DbTransactionStep::ExecuteChecked {
                statement: DbStatement::with_params(format!("UPDATE {table} SET current_msg_seq = ? WHERE env = ? AND session_id = ? AND current_msg_seq = ?{type_filter}"),
...
impl MessageDeliveryRepoPort for MySqlMessageStore {
    ...
    async fn lookup(&self, scope: DeliveryLookup) -> Result<Vec<PersistedMessageDelivery>, MessageDeliveryRepoError> {
        ...
        let predicate = match scope {
            DeliveryLookup::Ids(ids) => {
                if ids.len() > 8 { return Err(MessageDeliveryRepoError::Invalid("delivery recovery batch too large".into())); }
                if ids.is_empty() { return Ok(Vec::new()); }
                let placeholders = vec!["?"; ids.len()].join(",");
                params.extend(ids.into_iter().map(DbValue::from));
                return self.db.query(DbStatement::with_params(format!("SELECT * FROM bcs_message_deliveries WHERE env = ? AND delivery_id IN ({placeholders})"), params)).await.map_err(storage)?.into_iter().map(row_to_delivery).collect();
            }
