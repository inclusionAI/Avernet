...
2026-09-16 修订：移除 `CLAWWEB_MONITORING_STALE_SECONDS`，运行时不再读取监控环境变量。
状态新鲜度固定为 300 秒，以最新 `checkedAt` 而非最近成功时间判断；超过阈值显示 UNKNOWN，
PAUSED 保持不变。删除环境配置不会删除过期判定，也不会隐藏数据库/装配故障。
`createMonitoringRuntime(getDb?, now?)` 不再接受环境参数；模块内调用及测试同步调整，
现有 OCB 无参默认装配及本地 Host 的 getDb 注入不变。
...
- 三个旧监控环境变量残留均不影响新版本；原自定义 stale 阈值统一变为 300 秒。旧版本回滚仍可能依赖名单及合法 stale 配置，应按旧版本部署要求恢复配置。

## Validation / Rollout

1. 无旧变量、残留 false/非法 JSON、残留旧名单、stale 变量为空/非法/旧自定义值均可启动监控；GET/POST 正常，状态始终按固定 300 秒过期。
