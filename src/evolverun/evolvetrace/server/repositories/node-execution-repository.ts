...
export class NodeExecutionRepository {
  ...
  /**
   * Find node executions for all flow runs of a given workflow_id.
   * Used by health/node-stats endpoints.
   */
  async findByWorkflowId(workflowId: string, options: { limit?: number; offset?: number } = {}): Promise<NodeExecutionRow[]> {
    const limit = options.limit ?? 10000;
    const offset = options.offset ?? 0;
    try {
      // Find all flow_ids for this workflow, then find node_executions for those flow_ids
      return await this.db.query<NodeExecutionRow>(
        `SELECT ne.* FROM node_executions ne
         INNER JOIN flow_runs fr ON ne.flow_id = fr.flow_id
         WHERE fr.workflow_id = ?
         ORDER BY ne.started_at DESC LIMIT ? OFFSET ?`,
        [workflowId, limit, offset],
      );
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      console.warn(`[db] NodeExecutionRepository.findByWorkflowId failed: ${msg}`);
      return [];
    }
  }
}