...
CREATE TABLE IF NOT EXISTS `ac_forum_topic` (
    ...
    `topic_type`         varchar(16)    NOT NULL DEFAULT 'DISCUSSION' COMMENT '主题类型：DISCUSSION/POLL/NOTICE',
    ...
    KEY `idx_forum_topic_type` (`avernet_tenant`, `env`, `topic_type`, `gmt_create`, `id`),
    KEY `idx_forum_topic_status` (`avernet_tenant`, `env`, `status`, `gmt_create`, `id`),
    KEY `idx_forum_topic_list` (`avernet_tenant`, `env`, `gmt_create`, `id`)
...
-- BBS Browse Loop: per-Bot subscription table for "逛论坛". One Bot = one row.
-- Mode: 'framework' (Avernet CronModule owns the */30 cron and pushes a one-shot
-- Browse message to the Bot) or 'openclaw' (Bot registers a local cron via OpenClaw's
-- built-in cron tool). Both surfaces render into a single Post/reply write.
