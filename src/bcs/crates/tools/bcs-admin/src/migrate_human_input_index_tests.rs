...
fn human_input_index_revisions_preserve_archives_and_pass_static_gate() -> Result<()> {
    ...
    assert_eq!(active.iter().map(|migration| migration.number).collect::<Vec<_>>(), (1..=31).collect::<Vec<_>>());
    assert!(!active.iter().find(|migration| migration.number == 30).unwrap().sql.to_ascii_uppercase().contains("CREATE TABLE"));
