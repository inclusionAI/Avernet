    ...
    if relay_execution:
        ...
        return "\n".join(
            [
                ...
                "响应 data.relay_turn 是后续 PLAN/搜索/派发的唯一接力凭证，必须原样保存为 RELAY_TURN 变量，不得自行生成。后续所有请求必须把它作为顶层 relay_turn 字段提交，严禁放入 payload。ACCEPTED 和 DECLINED 都一样：HTTP 成功只是第4步完成，必须立即使用该凭证继续第5步；不得输出「等待引擎」后停止。",
                f"5. 再次 GET {backend}/api/v1/collaboration/tasks/{task_id}/context，基于根 TaskSpec、all_done_output、本节点实际产出与验收事实重新计算 gaps。将 event_type=PLAN_RESULT POST 到 callback/report；请求顶层字段必须包含 task_id、node_id、event_type、event_id、holder_id、relay_turn、progress_reason，其中 relay_turn 使用已保存的 RELAY_TURN；payload={{gaps,next_task_spec}}。gaps 非空时 next_task_spec 必须是唯一下一棒 context+goal，节点 ID 由 Graph 生成；gaps=[] 时 next_task_spec=null，任务结束。PLAN_RESULT HTTP 成功且 gaps 非空时必须原样保存响应中的 target_node_id；未成功读取该值时只能用相同 event_id 原样重试本事件，不得进入搜索或派发。在最终 PLAN_RESULT(gaps=[], next_task_spec=null) HTTP 200 前，不得向用户宣称任务已完成，也不得把 EXECUTION_RESULT 成功当作图状态已完成。",
                ...
                "7. 根据搜索返回的真实字段，由 Skill 判断 HIT_SINGLE、HIT_MULTI_BOTS 或 MISS。向 callback/report 上报 event_type=DISPATCH_RESULT；请求顶层同样必须携带 relay_turn；node_id 必须是 PLAN_RESULT 返回的 target_node_id。payload 使用 outcome、run_mode、driver_bot_id、next_relay_bots；协作群 driver 必须属于 next_relay_bots，当前棒不进入下一棒群，Human 默认作为 observer。MISS 必须提供 miss_reason。",
                f"8. HIT_SINGLE/HIT_MULTI_BOTS 后 POST {backend}/api/v1/collaboration/tasks/dispatch，传 task_id、origin_node_id、target_node_id、holder_id、relay_turn（当前棒的 RELAY_TURN）、唯一 dispatch_id。dispatch 是最后一步：必须先满足 target_node_id 来自 PLAN_RESULT 原始响应且不等于 origin_node_id、同一 target_node_id 的 DISPATCH_RESULT 已 HTTP 200、relay_turn 未过期且属于当前 origin。缺少任一前置变量时停止等待恢复，不得用 root/task_id 猜测 target，也不得直接调用 /dispatch。只有 HTTP 200 才算交接成功；MISS 自动发布 BBS。",
