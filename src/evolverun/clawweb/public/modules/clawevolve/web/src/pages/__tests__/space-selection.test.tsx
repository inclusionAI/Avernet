...
beforeEach(() => {
  ...
  api.tclog.bots.mockResolvedValue({ bots: [{ botId: 'bot-1', botName: '我的 Bot' }] })
...
describe('space selection interactions', () => {
  ...
  it('shows failure, disables the selector and retries instead of claiming an empty successful load', async () => {
    api.evolve.listSpaces.mockRejectedValueOnce(new Error('OCB unavailable'))
    render(<Selector />)
    expect((await screen.findByRole('alert')).textContent).toContain('OCB unavailable')
  ...
  it.each(['', 'team-1'])('registers a Skill with selected space %s and keeps list ownership visible', async (spaceId) => {
    ...
    fireEvent.change(screen.getByRole('combobox', { name: '所属 Bot' }), { target: { value: 'bot-1' } })
