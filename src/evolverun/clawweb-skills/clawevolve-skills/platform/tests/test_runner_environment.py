...
def test_local_requires_existing_workspace_inside_selected_bot(tmp_path):
    ...
    assert resolve_runner_environment(env).startup_environment()["OPENCLAW_WORKSPACE"] == str(workspace)
...
def test_container_keeps_admin_requirement_and_container_defaults():
    ...
    assert env.startup_environment()["OPENCLAW_WORKSPACE"] == "/home/admin/.openclaw/workspace"
