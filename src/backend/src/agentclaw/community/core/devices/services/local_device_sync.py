...
- **Pathlib fallback** (contract tests / standalone host / lifecycle-injected
  ...
  creates/removes symlinks directly on the local filesystem under
  ``~/.openclaw/workspace/skills/``.
- **BaaS mode** (singlebox prod / concrete-device-context callers): both
  ``baas_service`` and ``binding_ctx`` set, plus an injected
  ``Annotated[HttpClient, QUALIFIER_GENERAL]`` instance — ``sync_symlinks``
  POSTs the bindpath body to the container's ``/api/skills/symlink/bindpath``
  via ``BaasService.get_http_info``. This is NOT the Corp
  ``x-proxypass-token`` ``BaasInvokeTransport`` — it reuses the existing
...
class LocalDeviceSyncService(DeviceSync):
    """Local mode: creates/removes symlinks directly on the local filesystem
    (pathlib mode) or pushes to a BaaS container (BaaS mode).

    ``skills_dir`` should point to ``~/.openclaw/workspace/skills/``
    (the directory OpenClaw actually reads via workspaceSkills priority).

    When ``skills_dir`` is *None*, all pathlib operations are no-ops so the
    service can be safely used as a fallback.
    """
    ...
    def __init__(
        ...
        *,
        ...
        - **BaaS mode** (singlebox prod): both ``baas_service`` and
          ``binding_ctx`` set (plus ``http_client`` — the injected
          ``Annotated[HttpClient, QUALIFIER_GENERAL]`` full-absolute-URL
          client) → ``sync_symlinks`` POSTs to the container's
          ``/api/skills/symlink/bindpath`` via ``get_http_info``.
    ...
    def _baas_sync_symlinks(
        ...
        ``BaasInvokeTransport``). The container URL ``info.http_url`` is
        passed as the full absolute URL per call; the ``openclawToken``
        header is sent per call; ``clean_target_dir=False`` is preserved.
        Empty list short-circuits before ``get_http_info``.
        """
        ...
        for sm in symlinks:
            ...
            if not src or not tgt:
                logger.warning(
                    "[LocalDeviceSyncService.baas.sync_symlinks] skip invalid: %s", sm
                )
                continue
        ...
        # - LocalDeviceSyncService 只服务 singlebox,宿主 skills/ 下除了 skill 软链还有
        ...
        if not cleaned:
            logger.info(
                "[LocalDeviceSyncService.baas.sync_symlinks] empty list, "
        ...
        logger.info(
            "[LocalDeviceSyncService.baas.sync_symlinks] %d mappings → bindpath, binding=%s",
        ...
        try:
            ...
            logger.error(
                "[LocalDeviceSyncService.baas.sync_symlinks] get_http_info: %s", e
            )
            return {
                "success": False,
        ...
        def _post() -> Any:
            # 30s client timeout on the injected general HttpClient (full
            # absolute URL passed per call). The outer 60s wait covers the
            # whole container-call attempt.
        ...
        try:
            ...
            logger.error(
                "[LocalDeviceSyncService.baas.sync_symlinks] container call: %s", e
            )
    ...
    def _pathlib_sync_symlinks(
        ...
        if self._skills_dir is None:
            logger.info("[LocalDeviceSyncService.sync_symlinks] No skills_dir — skipping (non-local device)")
            return {"success": True, "message": "no skills_dir configured, skip"}

        logger.info(
            "[LocalDeviceSyncService.sync_symlinks] %d mappings, skills_dir=%s",
        ...
        for mapping in symlinks:
            ...
            if not source or not target:
                logger.warning("[LocalDeviceSyncService] skipping invalid mapping: %s", mapping)
                continue
            ...
            if not source_path.exists():
                logger.warning("[LocalDeviceSyncService] source does not exist, skipping: %s (original: %s)", source_path, source)
                skipped += 1
                continue
            ...
            try:
                ...
                try:
                    ...
                    logger.debug("[LocalDeviceSyncService] created: %s -> ./%s", target_name, rel_source)
                except ValueError:
                    # 无法计算相对路径，使用绝对路径
                    link_path.symlink_to(source_path)
                    logger.debug("[LocalDeviceSyncService] created: %s -> %s", target_name, source_path)
                created += 1
            except OSError as e:
                logger.error("[LocalDeviceSyncService] failed to create symlink %s -> %s: %s", target_name, source_path, e)
                skipped += 1
        ...
        for item in self._skills_dir.iterdir():
            ...
            if item.is_symlink():
                item.unlink()
                removed += 1
                logger.debug("[LocalDeviceSyncService] removed stale symlink: %s", item.name)

        logger.info(
            "[LocalDeviceSyncService.sync_symlinks] done: created=%d, removed=%d, skipped=%d",
    ...
    def _ensure_skills_repo(self) -> None:
        ...
        if source is None:
            ...
            if not target.exists():
                ...
                logger.info("[LocalDeviceSyncService] Created empty skills-repo directory (local dev mode)")
            return

        # 已正确软链到源 → 幂等返回
        if target.is_symlink() and target.resolve() == source.resolve():
            logger.info("[LocalDeviceSyncService] skills-repo already symlinked -> %s", source)
            return
        ...
        logger.info("[LocalDeviceSyncService] symlink skills-repo: %s -> %s", target, source)
