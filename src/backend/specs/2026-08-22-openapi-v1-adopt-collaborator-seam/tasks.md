...
## Task 1: Declare where a Check row's handler reads the bot
- **Goal:** Add `Check.bot_from` so a row can say whether its route publishes `bot_id` on the path or in the query string.
- **Files:** `src/backend/src/agentclaw/community/adapters/http/openapi_v1/authorization.py`
- **Done when:**
  - [ ] `Check` carries `bot_from: Literal["path", "query"] = "path"`, documented as "where this route's own handler reads the bot", with the six legacy skills addresses named as the only expected `"query"` users.
  - [ ] `Check.__post_init__` rejects any other value, raising while the module imports — same fail-at-import posture as the existing `PermissionLevel.NONE` refusal.
  - [ ] Existing rows and `repr` are unchanged; defaulting to `"path"` means no row edits in this task.
- **Depends on:** —

## Task 2: Build the gate for whichever source the row names
- **Goal:** Let the seam adjudicate a bot named in the query string, without letting any route be adjudicated from two sources.
- **Files:** `.../openapi_v1/bot_access.py`
- **Done when:**
  - [ ] `require_check` builds one of two closures on `rule.bot_from` — `BotIdPath` or the legacy `LegacyBotIdQuery` — rather than one gate with an optional parameter, so a request naming `bot_id` in both places cannot be adjudicated against the wrong one.
  - [ ] Both closures resolve the owner through `OwnerIdDep` and share `_level`, `_is_audited`, `_audit` and the bare `yield` unchanged.
  - [ ] The module docstring's "reads only `{bot_id}` from the path" claim is rewritten to the real invariant: the gate reads the bot from the same place the handler does.
  - [ ] `test_bot_access.py` covers: query-addressed gate refuses below the bar; refusal body is byte-identical to an absent bot's; a request omitting `bot_id` entirely is refused, not admitted.
- **Depends on:** Task 1

## Task 3: Refuse a Check row whose bot_from disagrees with its route
- **Goal:** Stop `bot_from` becoming a second declaration free to drift from the route.
...
- **Done when:**
  ...
  - [ ] Its inverse exists: every remaining `ServiceChecked` row cites the harness module and nothing else, so the burn-down is asserted rather than described. It fails today (11 other modules still cited) and is marked `xfail` with a reason naming this feature, flipping to a real assertion in the last group.
  - [ ] `scaffolding_row_count()` and `SCAFFOLDING_MODES` are untouched.
- **Depends on:** Task 2, Task 3

## Task 5: Record what the bot-chat operations actually do
...
## Task 6: Move the diagnostics group onto the seam
...
## Task 7: Move the render-screens group onto the seam
...
## Task 8: Give the authorized-apps handlers the owner they adjudicate
...
## Task 9: Move the authorized-apps group onto the seam
...
## Task 10: Move the skill-centre capability hook onto the seam
...
- **Goal:** Keep the legacy skills twins checked once their shared service check is gone — the first users of `bot_from="query"`.
- **Files:** `.../openapi_v1/authorization.py`
- **Done when:**
  - [ ] All six legacy rows (`/bots/skills`, `/bots/skills/upload`, `/bots/skills/{skill_id}` GET and DELETE, `activate`, `deactivate`) are `Check(MEMBER, bot_from="query")` instead of `INHERITED`.
  ...
  - [ ] `INHERITED` falls from 42 to 36, and the module comment explaining the mode is updated to say which twins are now adjudicated and why the rest are not.
- **Depends on:** Task 11

## Task 13: Move the engine-runtime groups onto the seam
...
## Task 14: Adjudicate the sixteen retiring engine-runtime addresses
...
- **Done when:**
  ...
  - [ ] `INHERITED` falls from 36 to 20 — all remaining rows twin an `OWNER_SCOPED` address.
- **Depends on:** Task 13

## Task 15: Decide and record what guards the operator credential
...
## Task 16: Move the service-publication facade onto the seam
...
## Task 18: Move the editors group onto the seam
...
- **Done when:**
  ...
  - [ ] `scaffolding_row_count()` has fallen by 91 flips plus 2 corrections plus 22 twins; the number is asserted, not described.
  - [ ] Every `Check` row's handler declares `OwnerIdDep`, and every `bot_from` matches its route — both already enforced at assembly, now also asserted over the live surface.
...
- **Group A — The seam learns where to look:** Tasks 1, 2, 3, 4
  - Theme: The prerequisite. `Check` can say where its route publishes the bot, the gate honours it, assembly refuses a row that lies about it, and the no-adopter assertion is replaced by a burn-down one. No row changes mode; nothing about the surface changes yet.
- **Group B — Say what the bot-chat operations really do:** Task 5
  - Theme: A correction, not a migration. Two rows stop claiming a check that does not exist.
- **Group C — First adopters:** Tasks 6, 7
  - Theme: The smallest, least entangled groups — no lock, no audit, no twins — put the `Check` path in front of real callers.
- **Group D — Authorized apps:** Tasks 8, 9
...
- **Group E — Skill centre:** Tasks 10, 11, 12
  - Theme: 29 rows behind two MEMBER checks, plus the six retiring skills addresses that become the first `bot_from="query"` rows.
- **Group F — Engine runtime:** Tasks 13, 14, 15
  - Theme: The largest group and its sixteen twins, keeping bot-type gating intact, and the argued decision about what guards the operator credential.
- **Group G — The entangled three:** Tasks 16, 17, 18
