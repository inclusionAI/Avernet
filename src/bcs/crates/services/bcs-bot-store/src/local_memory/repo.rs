...
impl BotRepoPort for MemoryBotRepo {
    ...
    async fn list_bots_by_name_and_cooperatable_with(
        ...
        self.repo_list_bots_by_name_and_cooperatable_with(name, bot_uuid, cooperatable_only, friend_uuids, offset, limit).await
    }
    ...
    async fn get_protocol_version(&self, bot_id: &str) -> u32 {
        self.repo_get_protocol_version(bot_id).await
    }

    async fn set_protocol_version(&self, bot_id: &str, version: u32) {
        self.repo_set_protocol_version(bot_id, version).await
    }
