-- ClawFlow Seed Data (MySQL) — Sample workflow runs and events for development/testing
-- Usage: mysql -u <user> -p <database> < seed_data_mysql.sql

-- ── Sample Flow Run ──
-- A completed workflow run of the "full-demo" workflow

INSERT IGNORE INTO schema_version (version, description, gmt_create, gmt_modified) VALUES
  (1, 'Initial schema: flow_events, flow_metrics, triggered_alerts', UNIX_TIMESTAMP(), UNIX_TIMESTAMP()),
  (2, 'Add node_executions and flow_runs tables for state persistence and query API', UNIX_TIMESTAMP(), UNIX_TIMESTAMP()),
  (3, 'Add scheduled_triggers table for cron scheduler', UNIX_TIMESTAMP(), UNIX_TIMESTAMP()),
  (4, 'Add webhook_triggers and webhook_events tables for webhook trigger system', UNIX_TIMESTAMP(), UNIX_TIMESTAMP()),
  (5, 'Add workflow_specs table for ClawWeb browser-persisted edits', UNIX_TIMESTAMP(), UNIX_TIMESTAMP()),
  (6, 'Add triggered_by to flow_runs, node_title and triggered_by to node_executions', UNIX_TIMESTAMP(), UNIX_TIMESTAMP()),
  (7, 'Fix schema_version and workflow_specs column names for trigger compatibility', UNIX_TIMESTAMP(), UNIX_TIMESTAMP());
