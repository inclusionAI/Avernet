...
Checks performed:
  ...
  2. Cross-layer imports — flag adapter files that import from BOTH
     ``secbaas.api`` and ``secbaas.core`` (concern mixing).
...
_CONTRACT_LAYERS = {"secbaas.api", "secbaas.spi"}
_CORE_LAYERS = {"secbaas.core", "secbaas.domain"}
_PLUGIN_LAYERS = {"secbaas.plugins"}
_ADAPTER_LAYERS = {"secbaas.adapters"}
...
def _imported_layers(tree: ast.AST) -> set[str]:
    ...
    For ``from secbaas.X.Y import ...`` we record ``secbaas.X``.
    For ``import secbaas.X.Y`` we record ``secbaas.X``.
    """
    layers: set[str] = set()
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            if node.module is not None and node.module.startswith("secbaas."):
                parts = node.module.split(".")
                if len(parts) >= 2:
                    layers.add(f"{parts[0]}.{parts[1]}")
        elif isinstance(node, ast.Import):
            for alias in node.names:
                if alias.name.startswith("secbaas."):
...
def test_no_mixed_architectural_imports():
    ...
    If a file imports from BOTH ``secbaas.api`` AND ``secbaas.core``, it
