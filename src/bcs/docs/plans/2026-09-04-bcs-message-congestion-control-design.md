...
final 不要求 Provider 统一语义。已结束可见段 H（按原始字节连接，不添加分隔符）、当前段 C、final F 按顺序
...
本轮不更改 task/A2A 专有完成语义。重建正文与原始 final 分开：只有队列准入和内部
`run_reply` 使用重建结果；前端、channel 和普通转发保持原始事件，不注入重建全文或最后展示段。
非受管 final 先发布原始事件，再查询会话、路由和按需重建队列正文；群路由优先使用非空会话成员，
缺少会话、未配置会话服务、查询失败或会话成员为空时保留原有群成员回退。`delta_text` 仅用于 delta，final 正文使用 `message`。
受管终态仍先提交再发布。空 final 的队列路由可使用已重建正文，非队列目标不补发该正文。
