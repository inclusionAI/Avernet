...
if _os.environ.get("SECBAAS_RUN_MODE", "bare").lower() == "sofa":
    from secbaas_enterprise.bootstrap import (  # type: ignore[import-untyped]
        EnterprisePluginContainer as PluginContainer,
    )
else:
...
async def initialize_services(container: containers.DeclarativeContainer) -> None:
    ...
    container.wire(packages=["secbaas.adapters.web"])
