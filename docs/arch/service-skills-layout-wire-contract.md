...
for versioned service-bot Skills artifacts. It is deliberately narrower than a
general engine-layout descriptor.

## Contract v1

The Backend derives these immutable environment variables from the target
publish record's `skills_artifact` on first release, update, restart, rollback,
and recreate:

| Variable | Allowed value | Meaning |
|---|---|---|
| `AGENTCLAW_SKILLS_LAYOUT` | `legacy` or `pool` | Repo mount layout frozen in the service artifact |
...
1. The Backend validates the artifact engine against the Bot engine before
   emitting either value.
2. Pool artifacts must declare exactly `skills-pool-p3-v1`; missing or unknown
...
6. Old images may ignore these added environment variables. Consequently a new
   Backend remains compatible with old images for Legacy artifacts; Pool
