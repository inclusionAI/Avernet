...
- 一个逻辑退出可能覆盖当前和历史多个 active root。Engine 按 `(action, mapping)`
  汇总为一项，使用最差状态，`retryable` 保留任一子结果的重试需要；逐目录结果
  留在诊断 evidence 中。同名替换覆盖的历史目录退出归属对应 APPLY，任何目录
  失败都不能被新入口的成功掩盖。Backend 仍拒绝重复逻辑项，不放宽结果校验。
