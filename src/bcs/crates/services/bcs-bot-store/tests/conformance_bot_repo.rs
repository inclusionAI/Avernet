...
#[tokio::test]
async fn persistent_registry_passes_identity_operation_conformance() {
    let repo = PersistentBotRepo::new(sqlite_db().await);
    bcs_test_support::contract::repo::bot_identity::bot_identity_operation_port_contract_tests(&repo).await;
}

#[tokio::test]
async fn local_registry_passes_identity_operation_conformance() {
