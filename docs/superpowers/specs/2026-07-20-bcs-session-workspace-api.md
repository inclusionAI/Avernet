...
- `presign_download=true`：`GET .../content`（会话内与分享下载均）会 302 跳转到后端预签名 GET URL（baas 经 `POST /share-link` 同步返 `share_url`），字节不经 BCS。会话内与分享共用同一 trait 方法 `presign_get`，差异仅在 BCS service 层鉴权（会话内 member 校验 vs 分享 token 校验）；**两者统一 TTL**（`share_link_ttl`，默认 3600），不区分短/长。
- `presign_download=false`（local）：`GET .../content` 由 BCS 流式返回 body；`presign_get` 返 `Unsupported`（local 文件不经预签名、分享为 presign 后端能力）。
...
PUT 各分片后调一次 `complete`，由 `StoragePlugin::complete_upload` 在后端组装（baas/OSS：`complete`
合并分片后直接进 `DONE`；local：按 `part_number` 顺序拼接段文件），客户端无需收集 ETag。**分段上传须所有分片 PUT 完成后再 `complete`；缺段或累计 size ≠ prepare `size` 时 `complete` 返 `409 INVALID_TRANSITION`（字节尚未上传完全），local 在 `complete_upload` 校验各段存在与累计大小。**`part_number` 为 `u16`（1–65535）；后端 `part_size` 选择须保证 `part_count ≤ 65535`，否则 prepare 返 `413 PAYLOAD_TOO_LARGE`。中途可 `DELETE` 取消
（后端 abort 分段会话）。
...
- baas：调 baas `complete`（`POST .../upload-url/{transfer_id}/complete`），ticket **同步**进 `DONE`（session 场景无需轮询），取回 `StorageObjectMeta`。
...
**后端行为：** 调 `StoragePlugin::delete`（`StorageHandle` 由 `object_handle` 重建；baas 凭 `transfer_id`）
- 本地：unlink 终态文件（幂等，文件已不存在返 `Ok`）；
- baas：`DELETE .../transfers/{transfer_id}`（按 `transfer_id` 删 ticket + OSS staging 对象，仅终态
  ticket 有效；BCS 保证调用前文件 `Ready`=`DONE`，已 `DELETED` 重复删返幂等 `Ok`）。
...
`delete`（`DELETE .../transfers/{transfer_id}` 要求终态 ticket，对未完成的上传不适用）。

**后端行为：** 调 `StoragePlugin::abort_upload`（`UploadHandle` 由 `object_handle` 重建）
- 本地：unlink 临时 `.part` 段文件（单片）/ 所有分片段文件（分段），幂等；
- baas：`DELETE .../upload-url/{transfer_id}`，ticket 转 `CANCELLED` 终态（MULTIPART 同时中止 OSS 分片会话，幂等）。
...
- 默认按 `gmt_create` **升序**（`created_at` 同序）返回（保证分页稳定、不漏页不重页）。
...
- 同一会话**允许存在多个同名文件**：`file_id` 是唯一标识（DB 唯一索引在 `(env, session_id, file_id)` 与 `(env, file_id)`，
  不在 `file_name`）。`file_id` 为 ULID 全局唯一，故 share token 仅凭 `file_id` 解析；客户端不得按 `file_name` 去重。
...
**带鉴权**（member 校验）的会话内下载。调 `download_route` → `StoragePlugin::presign_get`。

- 预签名后端（`supports_presign_download=true`）：**302 跳转** 到后端预签名 GET URL。字节在客户端 <-> 后端直接流转。
  baas 下 `download_route` → `presign_get` 调 `POST .../share-link` 同步取 `share_url`（带 `expire_seconds` 时效，BCS 用统一 `share_link_ttl`，默认 3600），前端无感、不传 `ttl`。
- 本地后端（`supports_presign_download=false`）：流式返回 body，含 `Content-Type`、`Content-Length`、
  `Content-Disposition: attachment; filename="..."`。

可选查询 `?ttl=<秒>`：**对前端隐藏**（接受但忽略）—— TTL 完全由 BCS 内部 + 后端决定。分享下载的有效期见 §1.9.c。
...
（1 分钟至 7 天）。此 TTL 决定 **share token 的 `exp`**（BCS 自管 HMAC 签名过期），与下载链接的
`share_link_ttl`（§1.8/§1.9.c，默认 3600，控制 baas `share_url` 的 `expire_seconds`）是**两个独立 TTL**：
前者管「分享链接是否还有效」，后者管「每次取字节的 OSS URL 多久过期」。持有人凭未过期 token
重调下载端点即换新 `share_url`（插件不缓存）。
...
**权限：无**（token 校验后跳过成员鉴权）。`token` query 参数为**必填**。流程：`share_consume(token)`
校验 share token 的 `exp` 未过期（任何失败模式统一 404，见错误节）→ 取出文件行重建 `StorageHandle`
→ `StoragePlugin::presign_get(handle, share_link_ttl)`（与 §1.8 会话内下载**共用同一 trait 方法**、
**同一 TTL=`share_link_ttl`（默认 3600）**，不设短/长二分）→ 302 跳转。**不引入 `download_route_shared`**，
分享路径复用现有 `share_consume` + `presign_get` 组合；token 过期由 BCS（`share_consume`）校验，与
`share_url` 的 `expire_seconds`(OSS URL 自身过期) 独立。
- 预签名后端：`presign_get` 调 `POST .../transfers/{transfer_id}/share-link` 同步取 `share_url`，
  **302 跳转** 到它，TTL=3600（`expire_seconds`）。**安全性两层保证**：BCS share token 的 `exp`
  （过期 → `share_consume` 直接 404，不再换 `share_url`）+ `share_url` 的 `expire_seconds`（OSS URL 自身过期）。
  `share_url` 先于 token `exp` 过期时，持有人凭未过期 token 重调本端点即换新 URL（插件不缓存 share_url）。
  跨主机重定向**必须剥离 `Authorization`** 头（同 §1.8）。
- 本地后端：分享为预签名后端能力，本地后端 `presign_get` 返 `Unsupported`/不支持分享下载
  （本地文件不适合无鉴权跨会话分享）。
...
pub trait StoragePlugin: Send + Sync + 'static {
    ...
    ///
    /// **Single method serves both download paths** (in-session + share): the BCS
    /// SessionFileService distinguishes them at the service layer (in-session via
    /// `download_route` with member auth; share via `share_consume(token)` with
    /// token verification), each calling this same method with the **same
    /// `ttl_secs = share_link_ttl` (default 3600)** — no short/long TTL split.
    /// baas (Session File Sharing API v1.1): both
    /// routes map to `POST .../transfers/{transfer_id}/share-link` (sync, returns
    /// an OSS presigned GET URL `share_url` with `expire_seconds` 60-604800);
    /// `ttl_secs` maps to `expire_seconds` (clamped), and `PresignGetTicket.expires_at`
    /// takes the backend response `expires_at`. Implementations MUST NOT cache the
    /// issued URL — the same `transfer_id` may be re-minted on each call (short TTL
    /// URLs are refreshed by re-calling). Only callable when the object is `DONE`;
    /// calls on non-`DONE` yield `Conflict`. Local: `Unsupported` (streams via
    /// get_stream instead; sharing is a presign-backend capability).
    ...
    /// Permanently remove a finalized object. Idempotent: missing/already-deleted
    /// object is Ok. baas: `DELETE .../transfers/{transfer_id}` (by transfer_id,
    /// not OSS key) — terminal-state-guarded (Ready=DONE satisfies it).
...
/// type) — NOT the per-part OSS direct URLs (those go to the client via
/// PreparedUpload.client_target and are not persisted), and NOT oss_key (delete
/// is by transfer_id, not OSS key).
pub struct UploadHandle {
    pub backend: &'static str,
    pub key: String,
    // local (single):    { temp_path, final_path }
    // local (multipart): { final_path, parts: [{ part_number, temp_path }] }
    // baas  (pending):   { transfer_id, type:"SINGLE"|"MULTIPART", expires_at }   // type DIAG only
    //     — transfer_id is the SOLE credential to baas (complete/abort/share-link/delete);
    //       no oss_key persisted (delete is by transfer_id).
...
/// baas Ready: { transfer_id } only (slimmed; type/expires_at dropped).
...
/// Presigned GET URL result of `presign_get`. baas: `download_url`=share_url from
/// `POST /share-link`, `expires_at`=backend `expires_at` (ISO 8601 → unix secs).
/// Same ticket type serves both in-session and share downloads.
...
- `complete_upload` 后 `get_stream`（或 `supports_presign_download` 时 `presign_get` + GET 下载字节）
  能往返原始字节
- `supports_presign_download` 后端须覆盖**下载/分享往返**：`presign_get` 签发预签名 GET URL（baas 经
  `POST /share-link` 同步返 `share_url`），GET 往返原始字节；断言 `presign_get` 不缓存（连续两次调用得到
  可各自独立过期的 URL）、对非 `DONE`/非法 `transfer_id` 返回 `Conflict`/`NotFound`（baas 行为，local 返
  `Unsupported`）。会话内与分享下载共用同一 trait 方法。
...
- **基础路径**（v1.1 session 维度）：`{endpoint}/api/v1/sessions/{tenant}/{session_id}/files/...` —— `endpoint`（仅 host）+ 配置 `tenant` + 请求级 `session_id`（用 BCS 的）拼接。**`transfer_id` 是与 baas 对接的唯一凭证**。
- 上传（留存模式，无设备）：`prepare_upload` 调 `POST .../upload-url`（body：`filename`(必填)、`file_size`、`expire_seconds`、`operator`=BCS caller、`staging_subdir=null`）取 baas 签发的 OSS 直传 URL + `transfer_id`
  （单片 `type:"SINGLE"` 返顶层 `upload_url`；分段 `type:"MULTIPART"` 顶层 `upload_url`=null + `parts[]`），
  返回 `PreparedUpload{ client_target: Direct{...}, handle }` —— BCS 把真 OSS 直传 URL 原样交给客户端（§1.2），
  客户端直接 PUT 到 OSS、**字节不经 BCS**，`stream_upload` 不被调用。`complete_upload` 调
  `POST .../upload-url/{transfer_id}/complete`（空 body）→ ticket 直接进 `DONE`（**同步，无需轮询**；
  分段 baas 自行合并分片，客户端/BCS 都不收集 ETag）；`abort_upload` 调 `DELETE .../upload-url/{transfer_id}`。
- **`object_handle` 仅持久化 `transfer_id`**（+ Pending `type`/`expires_at`，Ready 后瘦到仅 `transfer_id`）——
  不持久化短命的 per-part OSS 直传 URL，也**不需要 `oss_key`**（删除按 `transfer_id`，不按 OSS key）。详见 baas 插件文档。
- 下载 **统一走 `presign_get` → `POST .../transfers/{transfer_id}/share-link`**（v1.1 同步返 OSS 预签名 GET URL，无独立 `download_url`）。会话内下载 `download_route`（member 鉴权）与分享下载 `share_consume(token)`→`presign_get` 均**统一 `share_link_ttl`（默认 3600）**，不设短/长 TTL、不引入 `download_route_shared`；**前端均不传 `ttl`、BCS 内部决定**。`presign_get` 不缓存、每次重签。
- `delete`（`Ready`）调 `DELETE .../transfers/{transfer_id}`（**按 `transfer_id`**，删 ticket + OSS staging 对象，终态守卫由 baas 把关、Ready=DONE 满足），已 `DELETED` 重复删幂等 `Ok`。
- `health_check` 仅探测 baas `endpoint`（仅 host，凭证由插件持有不进 URL）可达性，不依赖真实 `transfer_id`。
- 会话隔离：路径含 `{tenant}/{session_id}`（用 BCS 的 session_id），不同会话天然隔离；BCS 列表权威来自自身
  DB，不依赖 baas 列表。删除按 `transfer_id` 且须在该 session 路径下，避免跨会话误删。
- 客户端直传 OSS 要求客户端能网络可达 OSS（baas 流量隔离前提）；仅能连 BCS 的客户端应使用
  `supports_presign_put=false` 后端（local）。跨主机 PUT/GET 时客户端剥离 `Authorization`，OSS 预签名 URL 自带签名。
- 错误映射（v1.1）、`UploadHandle`/`StorageHandle` 形态、完整上传/下载流程、身份/租户、配置、测试、分段细节均见 baas 插件文档。
