...
import { createLocalMonitoringIntegration } from "./local-integration.js";
...
const runtime = createMonitoringRuntime(() => db, Date.now, createLocalMonitoringIntegration());
...
const app = express();
// Only this explicit loopback demo grants legacy discovery; production authentication is unchanged.
app.use((req, _res, next) => { req.isClawInsightAdmin = true; next(); });
