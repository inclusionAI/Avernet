...
The deploy env var name is config-driven (ConfigPath.DEPLOY_ENV_VAR).
Tests that need the third fallback env var must patch both the config
(to point to a known env var name) and the OS environment.
"""

import socket
from unittest.mock import patch

from secbaas.community.config._models import Config, UserConfig
from secbaas.community.core.utils import env_utils

_DEPLOY_CFG = Config(
    user_config=UserConfig(env={"deploy_env_var": "CUSTOM_DEPLOY_ENV"})
)
...
class TestIsEmptyEnv:
    ...
    def test_is_empty_env_true_when_no_env_vars(self, monkeypatch):
        """WHEN no SERVER_ENV, REAL_SERVER_ENV, or custom deploy env are set,
        ...
        monkeypatch.delenv("CUSTOM_DEPLOY_ENV", raising=False)

        with patch(
            "secbaas.community.core.utils.env_utils.get_config",
            return_value=_DEPLOY_CFG,
        ):
    ...
    def test_is_empty_env_false_when_app_env_set(self, monkeypatch):
        """WHEN the configured deploy env var is set, THEN is_empty_env() returns False."""
        monkeypatch.delenv("SERVER_ENV", raising=False)
        monkeypatch.delenv("REAL_SERVER_ENV", raising=False)
        monkeypatch.setenv("CUSTOM_DEPLOY_ENV", "gray")
