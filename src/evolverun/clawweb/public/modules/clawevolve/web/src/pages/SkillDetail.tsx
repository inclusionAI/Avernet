      ...
      {timeline.map((item) => {
        ...
        return <section key={`version:${version.versionId}`} className="border-l border-gray-200 pb-2">
          <button type="button" onClick={() => onSelect(`version:${version.versionId}`, 'content')} className={`relative ml-4 block w-[calc(100%_-_1rem)] rounded-xl border px-3 py-3 text-left transition ${active ? 'border-blue-300 bg-white shadow-sm' : 'border-gray-200 bg-white hover:border-blue-200'}`}>
            ...
            {version.creationKind === 'rollback' && version.sourceVersion && <span className="mt-1 block font-mono text-[10px] text-amber-700" title={`来源版本 ID：${version.sourceVersion.versionId}`}>回滚自 {version.sourceVersion.version} · {version.sourceVersion.versionId}</span>}
            {version.creationKind === 'edit' && <span className="mt-1 block text-[10px] text-blue-600">手动编辑</span>}
            {version.creationKind === 'upload' && <span className="mt-1 block text-[10px] text-blue-600">上传升级</span>}
          </button>
        </section>
      })}
...
export default function SkillDetail() {
  ...
  const currentVersion = asset?.versions?.find((item) => item.version === asset.currentVersion) ?? null
  ...
  const refreshAfterVersionCreated = async (created: Version) => {
    const [nextAsset, history] = await Promise.all([
      api.evolve.getSkillAsset(assetId), api.evolve.getSkillAssetHistory(assetId),
    ])
    setAsset(nextAsset); setEvents(history.events)
    const next = new URLSearchParams(searchParams)
    next.set('selected', `version:${created.versionId}`)
    next.set('view', 'content')
    setSearchParams(next)
  }
  const confirmRollback = async () => {
    if (!rollbackVersion || !currentVersion) return
    setRollbackSubmitting(true); setRollbackError('')
    try {
      const created = await api.evolve.createSkillVersion(assetId, {
        mode: 'rollback', baseVersionId: currentVersion.versionId, sourceVersionId: rollbackVersion.versionId,
      })
      await refreshAfterVersionCreated(created)
      setRollbackVersion(null)
    } catch (reason) {
      setRollbackError(reason instanceof Error ? reason.message : '回滚失败')
    } finally {
  ...
  return <div className="mx-auto max-w-[1480px] px-4 py-7 sm:px-6 lg:px-8">
    ...
    {asset && selection && <section className="mt-5 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm lg:grid lg:grid-cols-[340px_minmax(0,1fr)]">
      ...
      <div className="min-w-0">
        ...
        {tab === 'content' && <><div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-5 py-3"><div><p className="text-xs text-gray-500">{selectedVersion?.version ?? '未关联版本'} 的冻结内容</p>{selectedVersion?.creationKind === 'rollback' && selectedVersion.sourceVersion && <p className="mt-1 font-mono text-[10px] text-amber-700">来源：回滚自 {selectedVersion.sourceVersion.version}（{selectedVersion.sourceVersion.versionId}）</p>}</div><div className="flex items-center gap-3">{asset?.canEdit !== false && selectedVersion && currentVersion && selectedVersion.versionId !== currentVersion.versionId && <button type="button" onClick={() => { setRollbackError(''); setRollbackVersion(selectedVersion) }} className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-1.5 text-xs font-medium text-amber-700 hover:bg-amber-100">回滚到此版本</button>}{selectedVersion?.sourceTaskId && <Link className="text-xs font-medium text-blue-600" to={taskDetailPath(selectedVersion.sourceTaskId, returnTo)}>查看来源任务 ↗</Link>}</div></div><VersionContent content={content} path={path} onPathChange={loadPath} /></>}
