...
description: 计算任务 gap 并产出下一步可执行子任务 List[TaskSpec];gap 已闭返回空数组。对齐 arch 场景(架构师名册/技术栈概览/双视角分析)确定式分解——按 root node_id 前缀 + done_children 查表(参照 task-planning storage 特例,非自由 LLM 分解)。
version: 1.0.0
...
按 **arch 场景确定式剧本**产出下一批子任务——参照 `task-planning` 的 storage 特例:**按 root `node_id`
前缀 + `done_children` 查表**,不靠自由 LLM 分解(避免拆子数/拆法飘忽)。
...
| 字段 | 含义 |
|---|---|
| `node_id` | 当前计算 gap 的目标节点(node_id=... 形式;arch 根以 `t_arch_`/`t_2mode_`/`t_3mode_` 前缀) |
...
## 确定式分解剧本(arch 场景:按 root `node_id` 前缀 + `done_children`)

框架二轮起 target 恒为根(根从初始规划后一直 PLANNING)。按根 `node_id` **前缀** + 快照 `done_children`
(已 DONE 子节点 + 其 `output` 产出)联合返回下一批:

### `t_arch_*`(单一交付物:架构师名册,预期 MISS→升 BBS 中继)

| 目标 node_id | done_children | 返回 children |
|---|---|---|
| `t_arch_*` | `[]`(初始) | `[N_architects]`  <!-- 无 bot → MISS@MAX→升 BBS,金庸中继收口 --> |
| `t_arch_*` | done 产出**已含架构师名册**(可能来自 `run_mode=="bbs"` 中继 scoped 节点) | `[]`(`has_gap=false`,gap 闭) |
| `t_arch_*` | 仍缺架构师名册 | `[]`(`has_gap=true`,`gap_detail="缺架构师名册"`,等 BBS 中继) |

### `t_2mode_*`(两份交付物:技术栈概览 + 架构师名册)

| 目标 node_id | done_children | 返回 children |
|---|---|---|
| `t_2mode_*` | `[]`(初始) | `[N_tech_stack, N_architects]`  <!-- tech_stack 命中 bot(single_bot);architects MISS→BBS --> |
| `t_2mode_*` | done 产出**已含技术栈概览 + 架构师名册** | `[]`(`has_gap=false`,gap 闭) |
| `t_2mode_*` | 仍缺任一份 | `[]`(`has_gap=true`,`gap_detail="<缺哪份>"`) |

### `t_3mode_*`(三份交付物:技术栈概览 + 业务/数据双视角分析 + 架构师名册)

| 目标 node_id | done_children | 返回 children |
|---|---|---|
| `t_3mode_*` | `[]`(初始) | `[N_tech_stack, N_dual_view, N_architects]`  <!-- tech_stack single_bot;dual_view coop_group;architects MISS→BBS --> |
| `t_3mode_*` | done 产出**已含技术栈概览 + 双视角分析 + 架构师名册** | `[]`(`has_gap=false`,gap 闭) |
| `t_3mode_*` | 仍缺任一份 | `[]`(`has_gap=true`,`gap_detail="<缺哪份>"`) |
...
```
> 各 case 初始批次按需取其中若干:`t_arch_`→`[N_architects]`;`t_2mode_`→`[N_tech_stack, N_architects]`;
> `t_3mode_`→`[N_tech_stack, N_dual_view, N_architects]`。
