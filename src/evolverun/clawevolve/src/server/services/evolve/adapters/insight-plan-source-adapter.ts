...
export type InsightPlanSourceAdapterOptions = {
  producer?: string;
  evidenceAccess?: {
    instruction?: string;
...
const DEFAULT_PRODUCER = "avernet-insight-plan-source-adapter";
const DEFAULT_EVIDENCE_INSTRUCTION =
  "优先通过 Evidence Access API 读取完整 Session 和 Task 证据，不要只根据问题标题判断。";
const DEFAULT_EVIDENCE_ENDPOINT =
  "/api/insight/v1/evidence-access/{ownerUserId}/{improvementId}/{sessionId}/{taskIndex}";

function resolveAdapterOptions(options: InsightPlanSourceAdapterOptions) {
  const producer = options.producer?.trim() || DEFAULT_PRODUCER;
  const instruction = options.evidenceAccess?.instruction?.trim() || DEFAULT_EVIDENCE_INSTRUCTION;
  const endpointTemplate = options.evidenceAccess?.endpointTemplate?.trim() || DEFAULT_EVIDENCE_ENDPOINT;
  if (!/^[A-Za-z0-9._:-]{1,128}$/.test(producer)) {
    throw new InsightPlanSourceAdapterError("PLAN_SOURCE_SCHEMA_INVALID", "Plan Source producer 不合法");
  }
  if (instruction.length > 4000 || /\0/.test(instruction)) {
    throw new InsightPlanSourceAdapterError("PLAN_SOURCE_SCHEMA_INVALID", "Evidence instruction 不合法");
  }
  if (
    endpointTemplate.length > 2048
    || !endpointTemplate.startsWith("/")
    || /[\s\0]/.test(endpointTemplate)
  ) {
    throw new InsightPlanSourceAdapterError("PLAN_SOURCE_SCHEMA_INVALID", "Evidence endpoint template 不合法");
  }
  return { producer, instruction, endpointTemplate };
}
...
export function buildInsightPlanSource(
  ...
  options: InsightPlanSourceAdapterOptions = {},
): PlanSource {
  const adapterOptions = resolveAdapterOptions(options);
  if (
  ...
  const source: PlanSource = {
    ...
    source: {
      ...
      producer: adapterOptions.producer,
    ...
    extensions: {
      insight: {
        ...
        evidence_access: {
          ...
          instruction: adapterOptions.instruction,
          endpoint_template: adapterOptions.endpointTemplate,
        },
