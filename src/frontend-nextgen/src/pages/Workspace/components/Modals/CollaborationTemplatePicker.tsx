  ...
  return (
    <div className="mb-2 flex items-center gap-2">
        ...
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger asChild>
            <Button
              ...
              variant="secondary"
              aria-label="选择协作模板"
              className="h-8 min-w-[180px] flex-1 justify-between rounded-lg border-[var(--color-border)] bg-white px-2.5"
            >
                ...
                {selectedTemplate ? (
                  <>
                    ...
                    {selectedTemplate.tags.slice(0, 2).map((tag) => (
                      <span
                        key={tag}
                        className="max-w-[60px] truncate rounded-full border border-[var(--color-border)] px-1.5 py-0.5 text-[10px] leading-none text-[var(--color-muted)]"
                      >
                        {tagLabel(tag)}
                      </span>
                    ))}
                  ...
                  <>
                    <Loader2 className="h-3.5 w-3.5 animate-spin text-[var(--color-muted)]" aria-hidden />
                    <span className="flex-1 truncate text-xs text-[var(--color-muted)]">加载模板...</span>
                  </>
                ) : (
                  <span className="flex-1 truncate text-xs text-[var(--color-muted)]">选择模板</span>
                )}
              </span>
              <ChevronDown
                className={cn(
                      ...
                      <div className="flex w-full items-center gap-1.5">
                        ...
                        <div className="flex shrink-0 items-center gap-1">
                          {template.tags.slice(0, 2).map((tag) => (
                            <span
                              key={tag}
                              className="max-w-[64px] truncate rounded-full border border-[var(--color-border)] px-1.5 py-0.5 text-[10px] leading-none text-[var(--color-muted)]"
                            >
                              {tagLabel(tag)}
                            </span>
                          ))}
                        </div>
                        {active && <Check className="h-3.5 w-3.5 shrink-0 text-[var(--color-primary)]" aria-hidden />}
                      </div>
                      {template.description && (
                        <div className="line-clamp-2 text-[11px] leading-4 text-[var(--color-muted)]">
                          {template.description}
                        </div>
                      )}
                    </Button>
      ...
      {isTemplate && loadingYaml && (
        <span className="flex shrink-0 items-center gap-1 text-[11px] text-[var(--color-muted)]">
          <Loader2 className="h-3.5 w-3.5 animate-spin text-[var(--color-muted)]" aria-hidden />
          加载内容...
        </span>
      )}
