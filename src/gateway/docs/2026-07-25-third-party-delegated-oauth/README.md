...
1. ~~**Does Alipay IAM already act as an OAuth authorization server?**~~ **Decided: build our own.** BUService / IAM is an **authentication** service, **not** a third-party OAuth authorization server — it doesn't register third-party OAuth clients, run a third-party consent flow, or issue teamclaw-audience tokens. That's a **category mismatch**, not a missing feature, so the gateway builds its own `/authorize` + `/token` *on top of* it, using `iam.alipay.com` solely for the human-login step. (Community reaches the same build-our-own outcome by **design choice** — Google *can* be an authz server, but we use it as OIDC sign-in only; see SYSTEM-FLOW §11.)
