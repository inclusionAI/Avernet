//! SQLite migration repair helpers.
//!
//! Idempotent schema repair and additive column/index migrations invoked
//! between baseline tables and versioned-migration records. Used by the
//! runner in [`super::report`] and by the versioned migration body which
//! dispatches to a specific `add_sqlite_*` / `migrate_sqlite_*` routine.

use bcs_db_api::{
    DbError, DbPlugin, DbResult, DbStatement, DbTransactionStep, db_get_column,
};

use super::report::{table_exists, sqlite_table_columns};
...
/// Versioned migration 29 (`fixed_loop_runtime`): thirteen guarded DDL
/// statements for the fixed-Loop run/recovery schema. Ported verbatim from
/// dev's migrations/schema.rs during the 2026-09-23 rebase conflict
/// resolution (dev merged this migration as version 29; auth-session
/// versioning renumbered to 32 to stay strictly after dev's head).
