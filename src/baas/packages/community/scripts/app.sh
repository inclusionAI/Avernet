...
do_start() {
    ...
    if [[ -n "$debug_port" ]]; then
        ...
        SERVER_ENV="$env_name" APP_CONFIG_PATH="$config_file" nohup "$VENV_DIR/bin/python" -m debugpy --listen "0.0.0.0:$debug_port" "$WORK_DIR/src/secbaas/main.py" -c "$CONFIG_DIR" --mode "$APP_MODE" >> "$LOG_FILE" 2>&1 &
    else
