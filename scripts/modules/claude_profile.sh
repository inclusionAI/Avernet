#!/usr/bin/env bash
# scripts/modules/claude_profile.sh — validated single Claude bot profile input
...
claude_profile_dir() {
    ...
    configured_dir="${CLAUDE_PROFILE_DIR:-}"
    [ -n "$configured_dir" ] || return 1

    # Service modules intentionally change their working directory while they
    # start their own runtime.  Resolve a command-line relative profile from
    # the checkout, rather than from whichever service ran immediately before
    # this one (for example BAAS's source directory).
...
claude_profile_validate_config() {
    ...
    if [ ! -f "$manifest" ]; then
        log_error "Claude profile manifest not found: ${manifest}"
        return 1
    fi
...
bot = bots[0]
allowed = {"source", "profile", "name", "summary", "domains", "skills", "runtime"}
if set(bot) - allowed:
    raise SystemExit("Claude bot contains unsupported fields")
for field in ("source", "profile", "name", "summary", "domains", "skills"):
    value = bot.get(field)
...
if bot["source"] != "platform-data" or bot["profile"] != "platform-data-analyst":
    raise SystemExit("Claude merchant profile must define the platform-data analyst")

runtime = bot.get("runtime")
runtime_fields = {"type", "relay_port", "claude_config_dir", "workspace", "model", "permission_mode", "system_prompt_md"}
if not isinstance(runtime, dict) or set(runtime) - runtime_fields:
    raise SystemExit("Claude runtime contains unsupported fields")
if runtime.get("type") != "claude_code" or runtime.get("relay_port") != 18913:
    raise SystemExit("Claude runtime must be claude_code on relay port 18913")
if runtime.get("permission_mode") != "bypassPermissions":
...
if not isinstance(model, str) or any(c in model for c in "\t\r\n"):
    raise SystemExit("runtime.model must be a single-line string")
...
# A tab is whitespace to Bash `read`, which collapses an optional empty model
# field and shifts the prompt fields. Unit Separator preserves empty fields.
...
values = [
    ...
    runtime.get("model", ""),
    os.path.join(os.path.dirname(os.path.abspath(sys.argv[1])), runtime["system_prompt_md"]),
    runtime["permission_mode"],
]
if any("\x1f" in value or "\t" in value or "\n" in value or "\r" in value for value in values):
    raise SystemExit("unit-separator/tab/newline is not allowed in Claude profile values")
