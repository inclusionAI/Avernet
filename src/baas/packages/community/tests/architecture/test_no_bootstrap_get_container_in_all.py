...
``secbaas.bootstrap.get_container`` provides raw access to the DI container.
...
_KNOWN_DEBT: dict[str, list[tuple[str, int]]] = {
    "core": [
        ...
        ("core/service/paas/_local_paas_service.py", 772),
        ("core/service/paas/_local_paas_service.py", 1979),
    ],
    "plugins": [
        ("plugins/sandbox/utils/arca_utils.py", 91),
        ("plugins/sandbox/utils/arca_utils.py", 120),
    ],
}


def _collect_get_container_imports() -> dict[str, list[tuple[str, int, str]]]:
    """Scan all production .py files for imports of get_container from secbaas.bootstrap.
    ...
    for py_file in sorted(SECBAAS.rglob("*.py")):
        ...
        for node in ast.walk(tree):
            if not isinstance(node, ast.ImportFrom):
                continue
            if node.module != "secbaas.bootstrap":
                continue
...
def test_no_forbidden_get_container_imports():
    """Fail on any NEW import of get_container from secbaas.bootstrap.
