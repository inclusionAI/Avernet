...
export class TaskSourceService {
  constructor(
    private readonly repo: TaskSourceRepositoryPort,
    private readonly evidenceReader: FrozenEvidenceReader,
    private readonly adapterOptions: InsightPlanSourceAdapterOptions = {},
  ) {}
  ...
  async resolvePlanSource(taskId: string): Promise<PlanSourceDescriptor> {
    ...
    try {
      ...
      try {
        source = buildInsightPlanSource(sourceRef, evidence, this.adapterOptions);
      } catch (error) {
