...
def test_task_executor_run_bbs_delegates_to_bbs_runner():
    """TaskExecutor.run_bbs delegates to bbs_modal_executor.notify with correct args.
    ...
    g = MagicMock()
    g.task_id = "t1"
    with patch("agentclaw.community.core.task.task_runner.modal_executor.bbs_modal_executor.notify", new_callable=AsyncMock) as mock_notify:
        _run(exe.run_bbs(g))
