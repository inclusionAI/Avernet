//! Default impl for BcsServerState (test-only dietary setup).
//!
...
impl Default for BcsServerState {
    fn default() -> Self {
        ...
        let user_identity_port = Some(crate::identity_wiring::user_identity_port(&identity_stores));
        let auth_session_identities = Some(crate::identity_wiring::auth_session_identity_port(&identity_stores));
        let oauth_session_engine = auth_config
            ...
            .map(|oauth| {
                Arc::new(bcs_auth_oauth::OAuthSessionEngine::new(
                    ...
                    auth_session_identities.clone().expect("memory store wired"),
                    oauth.env.clone(),
        ...
        Self {
            ...
            // Test-default state: no `[api.auth]` section → compat mode.
            built_api_auth: None,
        }
    }
}
