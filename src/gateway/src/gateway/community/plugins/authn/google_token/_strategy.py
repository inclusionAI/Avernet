...
calling Google's userinfo endpoint directly (mirroring BCS's
``bcs-auth-google`` ``get_user_info``) and maps the response onto a
:class:`UserPrincipal` — no separate provider/abstraction.
...
from ._config import GOOGLE_USERINFO_URL, USERINFO_TIMEOUT_SECONDS
