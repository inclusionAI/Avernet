...
> **2026-09-22 写入契约更新**：本文保留为搜索/UI 改造的历史实施 spec。涉及 v1 报送、接收端补齐身份、写入别名绑定、旧 ACK 和裸 ID 发现清单的规定（尤其第 4、5 节及对应验收项），已由 [三元组 v2 接口升级说明](clawinsight-monitoring-composite-identity-v2.md) 替代。当前写入仅接受显式 `(botId, entityId, env)` 的 v2 DTO；URL 不变。UI、授权和现有三元组存储规则继续有效。
