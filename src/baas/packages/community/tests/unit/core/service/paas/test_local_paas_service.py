...
from secbaas.core.service.paas._local_paas_service import _normalize_message
...
def mock_desktop_sandbox_plugin():
    ...
    from secbaas.api.bot_runtime._ws_connection_info import WsConnectionInfo

    mock = MagicMock()
    mock.resolve_ws_conn_info.return_value = WsConnectionInfo(
        ws_url="wss://agentclawproxy-dev.alipay.com/wsrelay/test-session",
...
class TestGetDeviceInfoSameInstance:
    ...
    async def test_get_device_info_running(
        ...
        from secbaas.api.device_manage import LocalDeviceInfo
...
class TestResolveWsConnInfoRelay:
    ...
    async def test_sandbox_plugin_error_converted_to_device_creation_error(
        ...
        """Per D-04: Plugin SandboxPluginError converts to DeviceCreationError."""
        from secbaas.spi.sandbox.desktop._errors import SandboxPluginError
...
class TestResolveInvokeHttpInfo:
    ...
    async def test_returns_proxypass_url_not_localhost(self, local_paas_service):
        ...
        with (
            mock.patch(
                "secbaas.core.utils.proxypass_utils.build_proxypass_url",
                return_value="https://agentclawproxy-dev.alipay.com/proxypass/LOCAL_abc123--machine-001--user-001@1:8080/api/health",
            ),
            mock.patch(
                "secbaas.core.utils.proxypass_utils.generate_proxypass_jwt",
                return_value="eyJhbGciOiJIUzI1NiIs...",
            ),
            mock.patch("secbaas.bootstrap.get_container"),
        ):
    ...
    async def test_token_generated_via_proxypass_jwt(self, local_paas_service):
        ...
        with (
            mock.patch(
                "secbaas.core.utils.proxypass_utils.build_proxypass_url",
                return_value="https://agentclawproxy-dev.alipay.com/proxypass/LOCAL_abc123--machine-001--user-001@1:8080/",
            ),
            mock.patch(
                "secbaas.core.utils.proxypass_utils.generate_proxypass_jwt",
    ...
    async def test_target_appends_path_when_non_default(self, local_paas_service):
        ...
        with (
            mock.patch(
                "secbaas.core.utils.proxypass_utils.build_proxypass_url",
                return_value="https://agentclawproxy-dev.alipay.com/proxypass/LOCAL_abc123--machine-001--user-001@1:8080/api/v1/invoke",
            ) as mock_url,
            mock.patch(
                "secbaas.core.utils.proxypass_utils.generate_proxypass_jwt",
                return_value="mock-jwt-token",
    ...
    async def test_http_connection_info_three_fields(self, local_paas_service):
        ...
        with (
            mock.patch(
                "secbaas.core.utils.proxypass_utils.build_proxypass_url",
                return_value="https://agentclawproxy-dev.alipay.com/proxypass/LOCAL_abc123--machine-001--user-001@1:3000/",
            ),
            mock.patch(
                "secbaas.core.utils.proxypass_utils.generate_proxypass_jwt",
                return_value="mock-jwt-token",
...
class TestHandleContainerReadySuccess:
    ...
    async def test_successful_callback_sends_to_publish_service(
        ...
        with mock.patch(
            "secbaas.bootstrap.get_container",
...
class TestProcessPublishCallbackForDevice:
    ...
    async def test_repository_none_logs_error_and_returns(
        ...
        monkeypatch.setattr(
            "secbaas.core.service.publish_manage.DefaultPublishService.handle_device_callback",
            fake_handle,
        )
    ...
    async def test_success_path_sends_callback_with_heartbeat_stdout(
        ...
        caplog.set_level("INFO")
        with mock.patch("secbaas.bootstrap.get_container", return_value=mock_container):
