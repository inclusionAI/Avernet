...
- **M4b TaskRunner + TaskLoopCallback**
  ...
  - T4b.x 单测:start_run 三 run_mode 分发(含 BBS bot 认领自规划自执行)、query_*回填、TaskLoopCallback.report_result→on_report 适配映射、产品 Bot ID→BCS UUID、state-machine 逻辑 binding、统一 SLA/cancel、严格 PASS/FAIL/非法终态解析、form_coop_group BCS local/mock 契约、BBS bot 上报+驱动计划、_build_context 验收/执行双模式自动切换。

## M5 — TaskHarness + TaskService facade
- T5.1 `TaskHarness.run_poll_loop`:周期 `query_task_nodes(RUNNING)`→比对 harness 自持 monotonic 计时 + sla_timeout(execution_config)→超时/崩溃(不读 node.start_time) `update_task_node_info(复位 PENDING, extend_props_patch={崩溃栈/超时})` 重投;不抢正向(STUCK 走 on_miss 升 BBS 链路上限判)。
