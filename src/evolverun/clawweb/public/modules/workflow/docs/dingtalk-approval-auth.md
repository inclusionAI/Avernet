...
2. The page calls `dd.requestAuthCode({ clientId, corpId })`. The legacy
   ...
   for older DingTalk containers. The page prefers a container-supplied `dd`
   object and dynamically loads the public MIT-licensed `dingtalk-jsapi`
   package when the WebView does not inject one.
