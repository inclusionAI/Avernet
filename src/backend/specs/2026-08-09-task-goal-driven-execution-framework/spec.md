...
- **协程化(任务执行是耗时任务)**:`on_execute`/`on_report`/`on_miss`/`on_harness`/`start_run`/`form_coop_group`/`report_result`/`DeliveryPort.deliver` 全链路 `async def`(对齐 backend lifecycle async 模式)。编排写图(graph 内存同步快操作)在 per-task `threading.RLock` 锁内同步(**锁内不 await**);耗时 IO(投递 `start_run`/BCS 拉群 `form_coop_group`/`deliver`)全 `await` 在锁外(不阻塞事件循环)。`on_*` 锁内 collect(同步 side-effects)→ 锁外 `_drain` await run/group/miss/finish(锁内永不 await)。多节点投递并发 `asyncio.gather`+`Semaphore`(`_DELIVER_CONCURRENCY=8`)下沉 `TaskRunner.start_run` 内部(engine 批量调 `start_run`)。`TaskService.execute` 为 async,内部 `await engine.on_execute`。单测经 `asyncio.new_event_loop().run_until_complete` 驱动(不用 `@pytest.mark.asyncio`)。
