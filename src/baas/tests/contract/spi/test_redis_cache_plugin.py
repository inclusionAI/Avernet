"""SPI conformance contract tests for the baas Redis cache plugin.

Matches the ``CachePluginContract`` shape used for the stub plugin (and the
gateway Redis plugin) so every ``CachePlugin`` implementation is exercised
against the same behavioural contract. Redis TTL is enforced server-side
(``SET key value EX ttl``), so this suite uses a fake in-memory client and
verifies the shared contract, the TTL hand-off, and client release on close.
"""
