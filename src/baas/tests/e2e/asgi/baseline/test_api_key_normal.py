...
class TestCreateAPIKey:
    ...
    async def test_create_bot_key_succeeds(
        ...
        response = await api.client.post(
            ...
            json={
                "app_id": bot_app_id,
                "key_name": f"e2e-bot-key-{unique_id}",
            },
        )
        ...
        data = response.json()
        assert data["code"] == 0
        assert data["data"]["app_id"] == bot_app_id
        assert data["data"]["app_type"] == "bot"
        assert isinstance(data["data"]["api_key_prefix"], str)
        assert len(data["data"]["api_key_prefix"]) > 0
        assert isinstance(data["data"]["api_key"], str)
        assert len(data["data"]["api_key"]) > 0
...
class TestBotPermissions:
    ...
    @pytest.mark.asyncio
    async def test_grant_allowed_bot_succeeds(
        self, api: APITestHelper, unique_id: str, created_bot: dict
    ) -> None:
        bot_app_id = f"{created_bot['bot_uuid']}:000001"
        bot_key_resp = await api.client.post(
            ...
            json={
                "app_id": bot_app_id,
                "key_name": f"e2e-bot-grant-{unique_id}",
            },
        )
        assert bot_key_resp.status_code == 200
        bot_key_prefix = bot_key_resp.json()["data"]["api_key_prefix"]

        app_id = f"e2e-grant-app-{unique_id}"
        ...
        assert created_bot["bot_uuid"] in policy or bot_app_id in policy

    @pytest.mark.asyncio
    async def test_grant_allowed_bot_missing_bot_id(
        self, api: APITestHelper, unique_id: str
    ) -> None:
        app_id = f"e2e-missing-bot-{unique_id}"
        app_key_resp = await api.client.post(
            api.api_key_url(action="app"),
            params=api.params(),
            json={
    ...
    @pytest.mark.asyncio
    async def test_grant_allowed_bot_invalid_bot_id(
        self, api: APITestHelper, unique_id: str
    ) -> None:
        app_id = f"e2e-invalid-bot-{unique_id}"
        app_key_resp = await api.client.post(
            api.api_key_url(action="app"),
            params=api.params(),
            json={
        ...
        response = await api.client.post(
            api.api_key_url(app_key_prefix, action="allowed-bots/grant"),
            params=api.params(),
            json={"bot_id": f"invalid-format-{unique_id}"},
        )
