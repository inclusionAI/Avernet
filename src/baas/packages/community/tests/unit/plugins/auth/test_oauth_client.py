...
from secbaas.plugins.auth.oauth._oauth_client import OAuthClient
from secbaas.spi.auth import AuthUser


class TestOAuthClientInit:
    @patch("secbaas.config.ConfigLoader.load")
...
def _make_client():
    with patch("secbaas.config.ConfigLoader.load") as mock_load:
        mock_cfg = MagicMock()
        mock_cfg.user_config = {
...
class TestGetLoginUser:
    ...
    async def test_success_maps_user_fields(self):
        ...
        with patch(
            "secbaas.plugins.auth.oauth._oauth_client.httpx.AsyncClient",
            return_value=fake,
        ):
