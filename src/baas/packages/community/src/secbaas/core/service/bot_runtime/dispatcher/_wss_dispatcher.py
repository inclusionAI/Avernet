"""DefaultBotWssDispatcher — WebSocket connection resolution.

Extends BaseDispatcher to dispatch WebSocket connections to a bot's active device.
...
class DefaultBotWssDispatcher(BotBaseDispatcher, BotWssDispatcher):
    """WebSocket connection resolution for a bot.

    Implements WssDispatcher protocol for WS connection resolution.
    ...
    async def dispatch_bot_ws_conn_info(
        ...
        logger.info(
            f"Dispatching WS connection: bot_uuid={bot_uuid}, port={port}, path={path}"
        )
        ...
        conn_info = await self._paas_facade.resolve_ws_conn_info(
    ...
    async def _dispatch_for_specific_device(
        ...
        env = get_current_env()
        logger.info(
            f"Resolving WS for specific device: bot_uuid={bot_uuid}, "
        ...
        # 5. Resolve WS connection info via PaaS facade
        conn_info = await self._paas_facade.resolve_ws_conn_info(
            paas_device_id=paas_device_id,
            port=port,
            path=path,
        ...
        logger.info(
            ...
            f"device_uuid={device_uuid}, ws_url={conn_info.ws_url}, "
            f"target={conn_info.target}"
        )
