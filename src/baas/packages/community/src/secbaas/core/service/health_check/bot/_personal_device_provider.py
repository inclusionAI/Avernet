"""Personal 类型设备的 Provider 实现。

从 ac_entity_device_binding 表获取设备信息。
"""
...
if TYPE_CHECKING:
    ...
    from secbaas.core.service.device_binding_query import (
...
class PersonalDeviceProvider(DeviceSourceProvider):
    """Personal 类型设备 Provider。

    Personal Bot 只有一个设备，绑定关系存储在 ac_entity_device_binding 表。
    支持 arca 和 baas 两种 device_provider：
    - arca: 从 device_props.sandbox_id 获取 paas_device_id
    - baas: 通过 binding.device_id → baas_bot → baas_device 链路获取
    """
    ...
    async def list_paas_device_by_bot(
        ...
        """获取指定 Personal Bot 的 PaaS 设备信息。

        Args:
            bot_id: Bot ID (用于日志)
            entity_id: 实体 ID（未使用，保留用于接口一致性）
            binding_id: ac_entity_device_binding.id

        Returns:
            PaasDeviceInfo 列表
        """
        # Personal Bot: 从 ac_entity_device_binding 获取
        if self._query_service is not None:
            devices = self._query_service.list_paas_device_by_bot_personal(
                bot_id=bot_id,
                binding_id=binding_id,
            )
        else:
            devices = self._device_binding_repo.list_paas_device_by_bot_personal(
                bot_id=bot_id,
                binding_id=binding_id,
            )
        ...
        return [
            PaasDeviceInfo(
                ...
                source_table_id=str(device["source_table_id"])
                if device.get("source_table_id") is not None
                else None,
    ...
    async def extend_ttl_by_bot(
        ...
        """
        # 1. 获取设备列表
        devices = await self.list_paas_device_by_bot(bot_id, entity_id, binding_id)
        if not devices:
        ...
        for device in devices:
            ...
            if not device.paas_device_id:
                logger.warning(
                    f"[PersonalDeviceProvider] Skipping TTL extend: "
            ...
            if not should_extend:
                # 剩余时间充足，跳过续期
                logger.info(
                    f"[PersonalDeviceProvider] Skipping TTL extend for {device.paas_device_id}, "
            ...
            try:
                ...
                if ttl_info.success:
                    extended_count += 1
                    # 更新数据库中的 TTL（成功时重置 refresh_fail_count）
                    if ttl_info.new_expiration_time:
                        ttl_timestamp = int(
                    ...
                    logger.info(
                        f"[PersonalDeviceProvider] Extended TTL for {device.paas_device_id}: "
                    ...
                    logger.warning(
                        f"[PersonalDeviceProvider] Failed to extend TTL for {device.paas_device_id}: "
                        f"error={ttl_info.error}"
                    )
            except Exception as e:
                logger.error(
                    f"[PersonalDeviceProvider] Exception extending TTL for {device.paas_device_id}: {e}"
                )
                failed_count += 1
                device.refresh_fail_count += 1
                self._update_device_refresh_fail_count_to_db(device)
