...
- **Rule 7** — Core logic must be transport-agnostic.  Core must not
  import adapters (``community.adapters``) or web/RPC framework packages
...
- **Rule 14** — Configuration drives all wiring; environment-specific
  ...
  This test statically checks that ``from community.bootstrap`` imports
  occur only within bootstrap/ and adapters/.

KNOWN PRE-EXISTING VIOLATIONS (documented in RULES-MANIFEST.md):
- (was) 25+ core files imported ``community.infra.utils.env_utils`` — resolved via move to ``core.utils``
...
def test_core_does_not_import_adapters(project_architecture):
    ...
    rule = (
        Rule()
        .modules_that()
        .are_sub_modules_of("secbaas.community.core")
        .should_not()
        .import_modules_that()
        .are_sub_modules_of("secbaas.community.adapters")
    )
...
# from community.bootstrap at module level.
# ═══════════════════════════════════════════════════════════════════════════
...
def test_bootstrap_not_imported_from_core_api_spi(project_architecture):
    ...
    for layer in (
        ...
        "secbaas.community.api",
        "secbaas.community.spi",
    ):
        rule = (
            Rule()
            .modules_that()
            ...
            .should_not()
            .import_modules_that()
            .are_sub_modules_of("secbaas.community.bootstrap")
        )
        try:
            rule.assert_applies(project_architecture)
            ...
            raise AssertionError(
                f"\n{layer!r} must not import from community.bootstrap:"
                + "\n".join(lines)
            )
