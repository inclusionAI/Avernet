...
pub async fn bot_chat_async(
    ...
    let timeout_ms = req
        ...
        .min(24 * 60 * 60 * 1_000);
    let from_bot_id = match bcs_telemetry::observe_result("chat.auth.bot", resolve_bot_caller(&state, &headers)).await {
    ...
    let authenticated_staff_id = bcs_telemetry::observe_value("chat.auth.staff", authenticated_staff_id(&state, &headers, &uri)).await;
    ...
    let accepted_result = bcs_telemetry::observe_result("chat.accept", state
        .services
        .a2a_chat_runs
