...
export function createWorkflowsRouter(
  ...
  flowRunRepo: FlowRunRepository | null = null,
  nodeExecRepo: NodeExecutionRepository | null = null,
): Router {
  const router = Router();
  ...
  router.post("/save", asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const resolvedBotOwnerId = botOwnerId?.trim() || req.cookies?.staff_id?.trim() || "";
      ...
      if (isUpdate && botPermRepo && !req.isAdmin) {
        const hasPermission = await botPermRepo.hasEditPermission(permCheckId, resolvedBotOwnerId, resolvedBotId);
        if (!hasPermission) {
    ...
    try {
      ...
      if (workflowSpecRepo) {
        ...
        if (row) {
          ...
          if (updatedAtMs > 0) {
            (spec as any).updatedAt = updatedAtMs;
          }
  ...
  router.delete("/:workflowId", asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      // botOwnerId is required — fallback to cookie, but must not be empty
      const resolvedUserId = queryBotOwnerId?.trim() || req.cookies?.staff_id?.trim() || "";
      if (!resolvedUserId) {
        res.status(400).json({ error: "Bad Request", message: "botOwnerId is required" });
        return;
      }

      // Edit permission check: must have edit access to delete
      if (botPermRepo && !req.isAdmin) {
        const hasPermission = await botPermRepo.hasEditPermission(workflowId, resolvedUserId, queryBotId?.trim() || undefined);
        if (!hasPermission) {
          res.status(403).json({ error: "Forbidden", message: "No edit permission for this workflow" });
          return;
        }
  ...
  /** GET /:workflowId/bot-permissions — list bot permissions for a workflow (admin only) */
  router.get("/:workflowId/bot-permissions", asyncHandler(async (req: Request, res: Response) => {
    if (!req.isAdmin) {
      res.status(403).json({ error: "Forbidden", message: "Admin permission required to view workflow permissions" });
      return;
    }
    try {
      if (!botPermRepo) {
  ...
  /** PUT /:workflowId/bot-permissions — upsert bot permission for a workflow (admin only) */
  router.put("/:workflowId/bot-permissions", asyncHandler(async (req: Request, res: Response) => {
    if (!req.isAdmin) {
      res.status(403).json({ error: "Forbidden", message: "Admin permission required to modify workflow permissions" });
      return;
    }
    if (!botPermRepo) {
      res.status(503).json({ error: "Service Unavailable", message: "Database not configured" });
  ...
  /** DELETE /:workflowId/bot-permissions — delete bot permission for a workflow (admin only) */
  router.delete("/:workflowId/bot-permissions", asyncHandler(async (req: Request, res: Response) => {
    if (!req.isAdmin) {
      res.status(403).json({ error: "Forbidden", message: "Admin permission required to delete workflow permissions" });
      return;
    }
    if (!botPermRepo) {
      res.status(503).json({ error: "Service Unavailable", message: "Database not configured" });
    ...
    try {
      ...
      const deleted = await botPermRepo.delete(normalizedBotId, queryBotOwnerId, workflowId);
      if (!deleted) {
  ...
  /** GET /:workflowId/notification-config — get notification config for a workflow (admin only) */
  router.get("/:workflowId/notification-config", asyncHandler(async (req: Request, res: Response) => {
    if (!req.isAdmin) {
      res.status(403).json({ error: "Forbidden", message: "Admin permission required to view notification config" });
      return;
    }
    try {
      if (!notificationConfigRepo) {
      ...
      const row = await notificationConfigRepo.findByWorkflowId(String(req.params.workflowId));
      if (!row) {
        res.json(null);
        return;
      }
  ...
  /** PUT /:workflowId/notification-config — upsert notification config (admin only) */
  router.put("/:workflowId/notification-config", asyncHandler(async (req: Request, res: Response) => {
    if (!req.isAdmin) {
      res.status(403).json({ error: "Forbidden", message: "Admin permission required to modify notification config" });
      return;
    }
    if (!notificationConfigRepo) {
      res.status(503).json({ error: "Service Unavailable", message: "Database not configured" });
  ...
  /** DELETE /:workflowId/notification-config — delete notification config (admin only) */
  router.delete("/:workflowId/notification-config", asyncHandler(async (req: Request, res: Response) => {
    if (!req.isAdmin) {
      res.status(403).json({ error: "Forbidden", message: "Admin permission required to delete notification config" });
      return;
    }
    if (!notificationConfigRepo) {
      res.status(503).json({ error: "Service Unavailable", message: "Database not configured" });
  ...
  router.post("/:workflowId/callback-configs", asyncHandler(async (req: Request, res: Response) => {
    if (!req.isAdmin) {
      res.status(403).json({ error: "Forbidden", message: "Admin permission required to create callback config" });
      return;
    }
    if (!httpCallbackConfigRepo) {
      res.status(503).json({ error: "Service Unavailable", message: "Database not configured" });
  ...
  router.put("/:workflowId/callback-configs/:configId", asyncHandler(async (req: Request, res: Response) => {
    if (!req.isAdmin) {
      res.status(403).json({ error: "Forbidden", message: "Admin permission required to update callback config" });
      return;
    }
    if (!httpCallbackConfigRepo) {
      res.status(503).json({ error: "Service Unavailable", message: "Database not configured" });
  ...
  router.delete("/:workflowId/callback-configs/:configId", asyncHandler(async (req: Request, res: Response) => {
    if (!req.isAdmin) {
      res.status(403).json({ error: "Forbidden", message: "Admin permission required to delete callback config" });
      return;
    }
    if (!httpCallbackConfigRepo) {
      res.status(503).json({ error: "Service Unavailable", message: "Database not configured" });
  ...
  // ── Node Stats & Health (simplified — uses flow_runs data) ──────────

  /** GET /:workflowId/node-stats — per-node statistics from flow_runs + node_executions */
  router.get("/:workflowId/node-stats", asyncHandler(async (req: Request, res: Response) => {
    ...
    const fromTime = Math.floor((Date.now() - days * 86_400_000) / 1000);

    const runs = await flowRunRepo.findByWorkflowId(workflowId, { limit: 5000 });
    const runInPeriod = runs.filter((r) => r.started_at >= fromTime);
    const total = runInPeriod.length;
    const succeeded = runInPeriod.filter((r) => r.status === "succeeded").length;
    const failed = runInPeriod.filter((r) => r.status === "failed").length;

    const nodes = await nodeExecRepo.findByWorkflowId(workflowId, { limit: 10000 });
    const nodesInPeriod = nodes.filter((n) => n.started_at >= fromTime);

    const nodeStatsMap = new Map<string, { total: number; succeeded: number; failed: number; avgDurationMs: number; totalDurationMs: number }>();
    for (const n of nodesInPeriod) {
      const key = n.node_id;
      const existing = nodeStatsMap.get(key) ?? { total: 0, succeeded: 0, failed: 0, avgDurationMs: 0, totalDurationMs: 0 };
      const dur = (n.completed_at && n.started_at) ? n.completed_at - n.started_at : 0;
      nodeStatsMap.set(key, {
        total: existing.total + 1,
        succeeded: existing.succeeded + (n.status === "succeeded" ? 1 : 0),
        failed: existing.failed + (n.status === "failed" ? 1 : 0),
        avgDurationMs: (existing.totalDurationMs + dur) / (existing.total + 1),
        totalDurationMs: existing.totalDurationMs + dur,
      });
    }

    const nodesArray = Array.from(nodeStatsMap.entries()).map(([nodeId, stats]) => ({
      nodeId,
      total: stats.total,
    ...
    res.json({
      data: {
        workflowId,
        totalRuns: total,
        successRate: total > 0 ? (succeeded / total) * 100 : 100,
        nodes: nodesArray,
      },
    });
  }));

  /** GET /:workflowId/health — current health snapshot */
  router.get("/:workflowId/health", asyncHandler(async (req: Request, res: Response) => {
    ...
    const succeeded = runInPeriod.filter((r) => r.status === "succeeded").length;
    const failed = runInPeriod.filter((r) => r.status === "failed").length;
    const blocked = runInPeriod.filter((r) => r.status === "blocked").length;
    ...
    res.json({
      data: {
        ...
        score: total > 0 ? Math.max(0, Math.round((succeeded / total) * 100)) : 100,
      },
    });
  }));

  /** GET /:workflowId/health-trend — health score trend over time */
  router.get("/:workflowId/health-trend", asyncHandler(async (req: Request, res: Response) => {
    ...
    const runs = await flowRunRepo.findByWorkflowId(workflowId, { limit: 50000 });
    const now = Date.now();
    const trend: Array<{ snapshot_date: string; overall_score: number; success_rate: number }> = [];

    for (let d = days - 1; d >= 0; d--) {
      const dayStart = Math.floor((now - (d + 1) * 86_400_000) / 1000);
      const dayEnd = Math.floor((now - d * 86_400_000) / 1000);
      const dayRuns = runs.filter((r) => r.started_at >= dayStart && r.started_at < dayEnd);
      ...
      trend.push({ snapshot_date: dateStr, overall_score: rate, success_rate: rate });
    }
