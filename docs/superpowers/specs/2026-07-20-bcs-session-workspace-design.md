...
pub struct SessionFile {
    ...
    pub created_at: u64,          // 读时从 DB gmt_create 投影(unix 秒)；非存储列
    pub updated_at: u64,          // 读时从 DB gmt_modified 投影(unix 秒)；非存储列
}
```
...
`object_handle`、`status`、`gmt_create`、`gmt_modified`。
索引：`idx_session_files_session (env, session_id, gmt_create)`；唯一索引 `(env, session_id, file_id)` 与 `(env, file_id)`。
遵循现有 `env` + `gmt_create/gmt_modified` + utf8mb4 约定。**不另设 `created_at`/`updated_at` 列**——
领域 `SessionFile.created_at`/`updated_at` 由 store 读时从 `gmt_create`/`gmt_modified` 投影为 unix 秒
（MySQL `UNIX_TIMESTAMP` / SQLite `strftime('%s', …)`），与 `bcs-session-store` 一致。
...
由 BCS 在 `prepare_upload` 时生成，写入 `bcs_session_files.file_id`（PK）、
`(env, session_id, file_id)` 唯一索引，以及强制 `(env, file_id)` 全局唯一的 `uk_env_file_id` 唯一索引
（DB 层兜底，支撑 share token 仅凭 `file_id` 解析的 `get_by_file_id`）。
