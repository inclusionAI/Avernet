...
class TestPermissionErrors:
    ...
    @pytest.mark.asyncio
    async def test_grant_allowed_bot_for_app_type_bot_with_invalid_id(
        self, api: APITestHelper, unique_id: str
    ) -> None:
        app_id = f"e2e-grant-bot-type-{unique_id}"
        ...
        response = await api.client.post(
            ...
            params=api.params(),
            json={"bot_id": f"invalid-bot-{unique_id}"},
        )
        assert response.status_code == 400

    @pytest.mark.asyncio
    async def test_revoke_allowed_bot_for_app_type_bot(
        ...
        bot_key_resp = await api.client.post(
            ...
            json={
                "app_id": bot_app_id,
                "key_name": f"e2e-bot-revoke-test-{unique_id}",
            },
        )
        ...
        response = await api.client.post(
            api.api_key_url(bot_key_prefix, action="allowed-bots/grant"),
            params=api.params(),
