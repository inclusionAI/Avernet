...
_BCN_PLUGIN_REPO_PATH = Path("src") / "bcs" / "crates" / "plugins" / "openclaw-channel-bcn"
...
class ProcessEntry:
    ...
    adapter_port: int
    openclaw_process: Optional[subprocess.Popen] = None
    openclaw_port: int = 0
    hermes_process: Optional[subprocess.Popen] = None
    hermes_port: int = 0
...
class LocalProcessManager:
    ...
    def create_openclaw_config(
        ...
        *,
        ...
        oc_config.setdefault("agents", {}).setdefault("defaults", {})["workspace"] = str(workspace_dir)
        ...
        if bcn_entry_point.exists():
            ...
            oc_config["plugins"].setdefault("entries", {})["openclaw-channel-bcn"] = {"enabled": True}
        else:
            logger.info(
        ...
        logger.info(
            ...
            config_file, openclaw_port, workspace_dir,
        )
        return config_dir
    ...
    def create_hermes_config(
        ...
        *,
        ...
        logger.info(
            ...
            config_file, hermes_port, workspace_dir,
        )
        return config_dir
        ...
        * ``aicoding`` — skip the engine spawn entirely (teamclaw-aicoding-
        ...
        try:
            ...
            logger.info(
                ...
                device_id, adapter_port, engine, engine_port, bot_id,
            )
    ...
    def stop(self, device_id: str) -> bool:
        ...
        with self._lock:
            ...
            if entry is None:
                logger.warning("No process entry for device %s, nothing to stop", device_id)
                return True

        self._kill_process(entry.adapter_process, f"adapter(port={entry.adapter_port})")
        self._kill_process(entry.openclaw_process, f"openclaw(port={entry.openclaw_port})")
    ...
    def is_healthy(self, device_id: str) -> bool:
        ...
        openclaw_alive = (
            entry.openclaw_process is None
            or entry.openclaw_process.poll() is None
        )
        hermes_alive = (
            entry.hermes_process is None
            or entry.hermes_process.poll() is None
        )
        return adapter_alive and openclaw_alive and hermes_alive

    def get_entry(self, device_id: str) -> Optional[ProcessEntry]:
    ...
    def _spawn_openclaw(
        ...
        *,
        ...
        logger.info(
            ...
            profile_name, openclaw_port, config_dir, log_path,
        )
        ...
        try:
            process = subprocess.Popen(
                [
                    "openclaw",
                    "gateway", "run", "--port", str(openclaw_port),
                ],
                env=env,
                stdout=log_fh,
        ...
        logger.info("OpenClaw gateway healthy: port=%s, pid=%s", openclaw_port, process.pid)
        return process
    ...
    def _spawn_hermes(
        ...
        *,
        ...
        logger.info(
            ...
            bot_id, hermes_port, config_dir, log_path,
        )
        ...
        logger.info("Hermes Dashboard healthy: port=%s, pid=%s", hermes_port, process.pid)
        return process
    ...
    def _spawn_adapter(
        ...
        *,
        ...
        if engine == "openclaw":
            ...
            # defeats wildcard rules like `*.stable.teamclaw.net` — `localhost`
            ...
            env["ENGINE_OPENCLAW_PROCESS_HEALTHCHECK_TCP"] = (
        ...
        if engine == "openclaw":
            logger.info(
                ...
                engine, adapter_port, engine_port, log_path,
            )
        elif engine == "hermes":
            logger.info(
            ...
            relay_url = env.get("AICODING_RELAY_URL") or env.get("CLAUDE_CODE_RELAY_URL", "")
            logger.info(
                "Spawning engine adapter: engine=%s port=%s, relay_url=%s, log=%s",
                engine, adapter_port, relay_url, log_path,
            )
        ...
        try:
            process = subprocess.Popen(
                [
                    engine_python, "-m", "uvicorn", "engine.community.api.app:app",
        ...
        logger.info("Engine adapter healthy: port=%s, pid=%s", adapter_port, process.pid)
        return process
    ...
    def _register(
        ...
        *,
        ...
        with self._lock:
            self._processes[device_id] = ProcessEntry(
                device_id=device_id,
                bot_id=bot_id,
                adapter_process=adapter_process,
                adapter_port=adapter_port,
    ...
    def _setup_skills(adapter_port: int, symbol_json: str) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info("Set up %d skill symlink(s) via adapter :%s", len(symlinks), adapter_port)
            else:
                logger.warning(
    ...
    @staticmethod
    def _kill_process(proc: Optional[subprocess.Popen], label: str) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.info("Process %s terminated gracefully (pid=%s)", label, proc.pid)
            except subprocess.TimeoutExpired:
                proc.kill()
                try:
                    proc.wait(timeout=3)
                except subprocess.TimeoutExpired:
                    logger.warning("Process %s still alive after SIGKILL (pid=%s)", label, proc.pid)
                else:
                    logger.warning("Process %s killed forcefully (pid=%s)", label, proc.pid)
        except Exception as e:
