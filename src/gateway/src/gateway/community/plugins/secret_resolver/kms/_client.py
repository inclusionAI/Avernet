...
endpoint. Only the ``GetSecretValue`` action the resolver requires is
...
Mirrors ``secbaas.community.plugins.secret.kms._client`` so the two independent
services share the same proven, dependency-free KMS client shape.
"""
...
class KmsSecretNotFoundError(KmsError):
    """Raised when Aliyun KMS reports that the requested secret does not exist.

    Distinct from :class:`KmsError` so callers can map "absent secret" to the
    SPI ``None`` contract while still letting genuine transport/authorization
    errors propagate.
    """
...
def _is_not_found_code(code: str) -> bool:
    """Whether an Aliyun KMS error code means the secret does not exist."""
    return (
        "ResourceNotFound" in code
        or "NotFound" in code
        or "NoSuch" in code
        or code == "ResourceNotFoundException"
    )
...
class KmsClient:
    ...
    def get_secret_value(self, request: KmsGetSecretValueRequest) -> object:
        ...
        Returns:
            ...
            response shape the resolver consumes.
        ...
        if secret_data is None:
            raise KmsSecretNotFoundError(
                f"KMS secret {request.secret_name} was not found"
            )
        return _SecretValueResult(secret_data=cast(str, secret_data))
    ...
    def _call(self, params: dict[str, str]) -> dict[str, object]:
        ...
        if error and error != "OK":
            ...
            if _is_not_found_code(str(error)):
                raise KmsSecretNotFoundError(
                    f"KMS secret not found ({error}): {message}"
                )
