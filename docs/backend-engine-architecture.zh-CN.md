...
**是我们的 —— 走第 ② 条：我们提供适配层，引擎方只实现适配层协议。** 容器我们创建、镜像我们构建、进程我们拉起，于是可以在那个进程里放一层适配层。引擎方就不必直接实现标准引擎协议，只需要实现**适配层协议** —— 按它自己本来的接口形状定义的一层 —— 由适配层负责把它翻译成标准引擎协议。仓库里的 claude_code 和 openclaw 走的都是这条。
...
前面那张表列的是「怕什么」。但光有规则不解决问题 —— 规则写在文档里，靠人在评审时想起来，就一定会漏。**唯一真正有效的办法，是把每一条规则翻译成一个 CI 测试，让坏代码根本进不来。**

这里面有一个不那么直觉、但很关键的做法：

> **写测试来保证测试。** 重点在前面那个「测试」 —— 你真正要守住的，往往不是业务代码写得对不对，而是**测试本身有没有在真的测东西**。而这种守卫测试必须做到一件事：**一眼能看明白。**

举一个仓库里的实际例子，这条链路有四层：

**第一层：要求每个接口都有端到端测试。**
端到端测试的定义是硬的 —— 装上全部依赖注入模块，从路由一路执行到最底层，不 mock 任何东西。这样覆盖的是真实生产代码路径，负责人扫一眼输入输出就知道对不对。CI 上有一道门禁：每个活着的接口都必须有成功和失败两种用例，**没有就红**。存量缺口冻结成一份基线清单，新增缺口立刻失败，而且基线条目一旦被补上就必须删掉 —— 清单只能变短。

**第二层：但人会在端到端测试里 mock。**
这一下就把第一层的意义抵消掉了：路径还在，但中间被替身截断了，测试照样绿，生产代码照样烂。

**第三层：于是再写一个测试，保证端到端测试里没有 mock。**
它扫描端到端测试目录的语法树，禁掉 mock 库的导入与调用、`monkeypatch` / `mocker` 这类夹具，以及 `setattr` 这种绕过写法。想替换东西？只能走依赖注入的替换点 —— 那是系统的**边缘**，边缘之内全部保持真实。

**第四层：还有一个更隐蔽的洞，再补一个。**
有人不导入 mock 库，直接把依赖注入拿到的实例上的方法赋值掉。所以还有一个兄弟测试专门盯这个写法。

**最后一步，也是最关键的一步：把这些守卫测试放在需要负责人审批的位置。**
这些文件所在的目录带 OWNERS 声明。于是一个想在端到端测试里 mock 的人，要让 CI 变绿，就只能去改这个守卫测试本身 —— 而那需要负责人点头。而且守卫测试只有一个、逻辑很短，**负责人一眼就能看出它是不是被削弱了**。

这就是整套治理的核心机制，可以抽象成三句话：

1. **规则必须是可执行的断言，不是文档里的一段话。**
2. **守卫本身也会被绕过，所以守卫要有守卫 —— 而且要少、要短、要一眼能看懂。**
3. **把最后那道守卫放在需要审批的位置。** 绕过它不是不可能，但必须有人签字，而且签字的人看得懂自己在签什么。

第三条是这套机制成立的真正原因：它不指望人不犯错，只要求**绕过的动作是显式的、有署名的**。
...
6. **规则不落到 CI 上就等于没有，而守卫本身也需要守卫。** 写在文档里、靠人在评审时想起来的规则，一定会漏。把每条规则翻译成一个会红的测试；然后接受一个事实 —— 守卫会被绕过，所以还要有守卫的守卫。这类测试要少、要短、要一眼能看懂，并且放在需要负责人审批的位置：**绕过不必不可能，但必须是显式的、有署名的。**
