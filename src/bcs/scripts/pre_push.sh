...
bcs_parallel_gates() {
  ...
  printf '  e2e:  src/bcs/scripts/e2e_coverage.sh --bcs-line-min 39.8 --bcs-method-min 36 --force-rebuild   (log: %s)\n' "$e2e_log"
  echo ""
  ...
  _launch_tagged e2e "$e2e_exit" \
    "$bcs_dir/scripts/e2e_coverage.sh" --bcs-line-min 39.8 --bcs-method-min 36 --force-rebuild
...
if [[ $unit_on -eq 0 && $e2e_on -eq 0 ]]; then
  ...
  run_required "$bcs_dir/scripts/e2e_coverage.sh" --bcs-line-min 39.8 --bcs-method-min 36 --force-rebuild
fi
