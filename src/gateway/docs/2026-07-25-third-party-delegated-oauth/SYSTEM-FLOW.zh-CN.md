...
- **方案 A（自建授权服务器 / login-with-avernet）** **无外部依赖** —— 只需 (i) 我们自己的 `/authorize` + 授权 + `/token`，(ii) 我们本就在跑的真人登录，(iii) 签发我们自己的 JWT。**永远可由我们独立达成。** 它是底线。
- **方案 B（借 antbuservice / Google 当授权服务器）** 是*为了少建而做的优化* —— 仅当该外部方能签发 **tc-audience、tc-consented** 令牌（不止 SSO 透传）才可行。可能达不成；这是团队问题。
...
1. **A vs B（工作量）：** **antbuservice / Google** 能否充当签发 **tc-audience、tc-consented** 令牌的授权服务器？*能* → 更轻（B）。*不能* → 建 A。（无论如何 A 是保底基线。）
2. **推进 vs 推迟：** 若团队判断当下不值得优先做这个干净修复，唯一的替代是*有据可查地*接受透传反模式 —— 而非"我们别无选择"。
