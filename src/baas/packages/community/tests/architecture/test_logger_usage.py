...
All application code SHOULD use ``secbaas.logger.get_logger()`` for
...
Using ``secbaas.logger.get_logger()`` ensures:
...
def test_no_direct_logging_getlogger_outside_legacy_layers():
    ...
    Using ``secbaas.logger.get_logger()`` keeps logging swappable.
    """
    allowed_prefixes = {
        "spi/logger",
        "plugins/logger",
