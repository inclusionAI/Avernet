...
- **地址是网关的，不是它背后那一跳的。** 对外发布的 origin 取自 `gateway` 配置块
  ...
  _2026-07-31 更正。_
- **URL 由本端点拼出，不是原样透传。** 获取设备连接时仍然请求
  **`ws_conn_mode="relay"`**，provider 也仍然会给出一条完整 URL，但那条 URL 指向的是
  网关背后那一跳，不是我们对外发布的东西；我们改用手上已有的 target 和引擎 path
  对着网关重新拼。若 provider 给回的形状是 `/engine` 前缀无法表达的 —— BaaS 的
  LOCAL 平台返回 `/wsrelay/{session_id}` 并且完全忽略 path —— 则直接拒绝而不是发布，
  这样错误的假设会在服务端暴露，而不是变成一条连不上的 socket。_2026-07-31 更正。_
