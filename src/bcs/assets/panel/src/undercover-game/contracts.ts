import type { PlayerActor, PublicHistoryRound, PublicVoteRound, UndercoverGamePanelParams, UndercoverGamePanelProps, VoteCandidate } from './types';
...
function normalizeHistory(value: unknown): PublicHistoryRound[] { if (!Array.isArray(value)) return []; return value.flatMap((round) => { if (!round || typeof round !== 'object') return []; const r = round as Record<string, unknown>; if (!Array.isArray(r.speeches)) return []; return [{ round: typeof r.round === 'number' ? r.round : 0, stage: r.stage === 'pk' ? 'pk' as const : 'regular' as const, speeches: r.speeches.flatMap((speech) => { if (!speech || typeof speech !== 'object') return []; const x = speech as Record<string, unknown>; if (typeof x.text !== 'string' || !x.text.trim()) return []; return [{ actorId: requireText(x.actorId, 'publicHistory.actorId'), seatNumber: positiveInt(x.seatNumber ?? x.seat, 1), displayName: requireText(x.displayName ?? x.player, 'publicHistory.displayName'), text: x.text.trim() }]; }) }]; }); }
function normalizeVoteHistory(value: unknown): PublicVoteRound[] {
  if (!Array.isArray(value)) return [];
  return value.map((item) => {
    const r = item as Record<string, unknown>;
    if (!r || !Number.isInteger(r.round) || Number(r.round) < 1 || !['regular', 'pk'].includes(String(r.stage))
      || !Array.isArray(r.votes) || !Array.isArray(r.counts)) throw new PanelParamsError('voteHistory is invalid.');
    return { round: Number(r.round), stage: r.stage as 'regular' | 'pk',
      votes: r.votes.map((v) => ({ seatNumber: positiveInt(v.seatNumber, 1), displayName: requireText(v.displayName, 'voteHistory.displayName'), text: requireText(v.text, 'voteHistory.text') })),
      counts: r.counts.map((c) => { if (!Number.isInteger(c.votes) || c.votes < 0) throw new PanelParamsError('voteHistory.counts is invalid.'); return { seatNumber: positiveInt(c.seatNumber, 1), votes: c.votes }; }),
    };
  });
}
export function normalizePanelParams(props: UndercoverGamePanelProps): UndercoverGamePanelParams {
  ...
  if (raw.resultFile !== undefined && (typeof raw.resultFile !== 'string' || !/^undercover-result-v1-r[1-9]\d*(?:-pk)?-a[1-9]\d*\.json$/.test(raw.resultFile))) throw new PanelParamsError('resultFile is invalid.');
  const pkCandidates = Array.isArray(raw.pkCandidates) ? raw.pkCandidates.map((id) => requireText(id, 'pkCandidates actorId')) : [];
  if (phase.startsWith('pk_') && (pkCandidates.length < 2 || new Set(pkCandidates).size !== pkCandidates.length
    || pkCandidates.some((id) => !playerMap.has(id) || playerMap.get(id)?.alive === false))) throw new PanelParamsError('PK requires at least two living candidates.');
  const rules = raw.rules && typeof raw.rules === 'object' ? raw.rules as UndercoverGamePanelParams['rules'] : {};
  return { runId, groupId, sessionId, resultFile: raw.resultFile as string | undefined, openingAnnouncement: typeof raw.openingAnnouncement === 'string' && raw.openingAnnouncement.trim() ? raw.openingAnnouncement.trim() : undefined, gameSessionId: typeof raw.gameSessionId === 'string' && raw.gameSessionId.trim() ? raw.gameSessionId.trim() : undefined, phase, round, attempt: positiveInt(raw.attempt, 1), deadlineAt: typeof raw.deadlineAt === 'number' ? raw.deadlineAt : undefined, host, seatOrder, turnOrder, players, nodeActorMap, publicHistory: normalizeHistory(raw.publicHistory), pkCandidates, voteHistory: normalizeVoteHistory(raw.voteHistory), rules, voteCandidates: normalizeCandidates(raw.voteCandidates), apiBaseUrl: typeof raw.apiBaseUrl === 'string' && raw.apiBaseUrl.trim() ? raw.apiBaseUrl.trim() : DEFAULT_API_BASE_URL, currentViewerActorId: typeof raw.currentViewerActorId === 'string' ? raw.currentViewerActorId : undefined, currentAction, display: raw.display && typeof raw.display === 'object' ? raw.display as UndercoverGamePanelParams['display'] : {}, pollingInterval: typeof raw.pollingInterval === 'number' && raw.pollingInterval > 0 ? raw.pollingInterval : DEFAULT_POLLING_INTERVAL, autoRefresh: raw.autoRefresh !== false, maxResponseBytes: typeof raw.maxResponseBytes === 'number' && raw.maxResponseBytes > 0 ? raw.maxResponseBytes : DEFAULT_MAX_RESPONSE_BYTES };
}
