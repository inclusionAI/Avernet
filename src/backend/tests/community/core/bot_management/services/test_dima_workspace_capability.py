...
class TestHasDimaWorkspaceEnabled:
    ...
    # ── 扁平节点畸形：声明即唯一事实源，取值非 Mapping 按全关闭，不回退嵌套 ──

    def test_false_when_flat_node_malformed_do_not_fall_through(self):
        assert has_dima_workspace_enabled(
            {
                "capabilities": None,
                "bot_template_config": {"capabilities": {"dima_workspace": True}},
    ...
    def test_false_when_flat_node_non_mapping_value(self):
        assert has_dima_workspace_enabled(
            {"capabilities": ["dima_workspace"]}
        ) is False
