...
def _candidate_bcs_session_files() -> list[Path]:
    candidates = [_path_expand("~/.openclaw/.bcs/session.json")]
...
def resolve_runtime_user_id(
    ...
    argument, then local credential/session metadata fallbacks.  OS environment
    variables are intentionally ignored so effective inputs are traceable.
    """
    if explicit_user_id:
