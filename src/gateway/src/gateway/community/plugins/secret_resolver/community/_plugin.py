...
A dependency-free resolver that reads two env vars per secret — ``{NAME}_USER``
and ``{NAME}`` — and returns an object exposing ``.secret_user`` /
``.secret_value``. When the value var is unset the secret is absent and the
lookup returns ``None``, preserving the Protocol's "absent ⇒ None" contract.
"""
