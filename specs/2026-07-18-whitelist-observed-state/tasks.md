...
- **Done when:**
  - [ ] `GovernanceStatus.OBSERVED = "observed"` 加入枚举。
  - [ ] `TERMINAL_STATUSES = frozenset({CLOSED, OBSERVED})`;`ACTIVE_STATUSES` 确认仍不含 OBSERVED。
  - [ ] `TICKET_TRANSITIONS`:OPEN/SCHEDULED/WAITING_REVIEW 可转集加 OBSERVED;新增 `OBSERVED: frozenset({CLOSED})`。
  - [ ] 单测:`transition_to(OBSERVED)` 从三活跃态成功;从 OBSERVED 仅可转 CLOSED;OBSERVED→其他抛 `IllegalTicketTransitionError`。
- **Depends on:** Task 4
...
- **Done when:**
  - [ ] `ticket.py:575` `approve_whitelist` 分支:`transition_to(CLOSED)` → `transition_to(OBSERVED)`;`close_reason = close_reason or CloseReason.WHITELIST_APPROVED`(替字面量 `"whitelisted"`)。
  - [ ] 保留:清 `remind_at`、释放 `active_worker`、不设 cooldown、取消 pending(在 review_ticket 驱动侧)。
  - [ ] docstring L550 `approve_whitelist → CLOSED` 改为 `→ OBSERVED`。
  - [ ] `test_domain_model.py:954` 断言改 `close_reason == CloseReason.WHITELIST_APPROVED`(或 `== "whitelist_approved"`)。
  - [ ] grep 确认无其他读 `close_reason == "whitelisted"` 的生产代码(测试已仅此一处)。
  - [ ] 单测:approve_whitelist 后 `governance_status == OBSERVED`,`close_reason == WHITELIST_APPROVED`。
- **Depends on:** Task 5
...
- **Done when:**
  - [ ] `close_for_whitelist_hit`(L217):转的目标 CLOSED → OBSERVED;`CloseReason.SCAN_WHITELISTED` 不变;`_cancel_pending` 副作用不变。
  - [ ] docstring 同步改"→ OBSERVED(scan_whitelisted)"。
  - [ ] 单测:白名单命中且有活跃单 → 单转 OBSERVED、close_reason=SCAN_WHITELISTED、pending 通知被取消。
- **Depends on:** Task 5
...
- **Done when:**
  - [ ] `find_observed_ticket(worker_id)`:`governance_status == OBSERVED`、按 `gmt_modified DESC`、`worker_id == worker_id`(用 worker_id 非 active_worker,对齐 find_latest_closed 口径)。
  - [ ] 加入 `TaskRecordRepositoryProtocol`(protocols.py)。
  - [ ] 单测:有 OBSERVED 单返回最近一条;无返回 None;不影响 find_active_ticket。
- **Depends on:** Task 5
...
- **Done when:**
  - [ ] `open_observed_ticket(*, ticket: GovernanceTicket) -> str`:建 `GovernanceTicket.create` → 直接 `transition_to(OBSERVED)`(不经 OPEN)→ `save_ticket`;**不调** notify_repo、**不调** update_delivery_status。
  - [ ] Protocol `GovernanceLifecycleServiceProtocol` 加该方法签名。
  - [ ] 单测:建出的单 status=OBSERVED、notify_log 零新增、delivery_status 未被写。
...
- **Done when:**
  - [ ] `process_record` Step2-4 重排:`is_whitelisted` 命中时先 `find_observed_ticket`;有 OBSERVED 单 → 复用 `refresh_snapshot` 链路(状态不变、dt_version guard 生效、不发通知);无活跃单无 OBSERVED 单 → `lifecycle_svc.open_observed_ticket`。
  - [ ] 有活跃单(非白名单场景)仍走原 `_handle_active_ticket_refresh`。
  - [ ] OBSERVED 刷新审计 action 用 `AuditAction.WHITELIST_OBSERVED`(Task11 加枚举)。
  - [ ] 单测三路各一例:①白名单+有OBSERVED→刷新+状态不变+notify零增;②白名单+无单→建观察单+notify零增;③非白名单→原流程。
...
- **Done when:**
  - [ ] **新增** `AuditAction.WHITELIST_OBSERVED`(已定)——用于观察刷新(白名单 bot 已有 OBSERVED 单时刷新其快照)。与 `SCAN_WHITELISTED`(无活跃单命中加白)区分:`WHITELIST_OBSERVED`=持续刷新观察单;`SCAN_WHITELISTED`=scan 兜底关残留/记加白命中。
  - [ ] Task 10 的观察刷新审计改用 `AuditAction.WHITELIST_OBSERVED`(替原"复用 STILL_ACTIONABLE"的暂定)。
  - [ ] 删白收尾 OBSERVED→CLOSED 的 close_reason 用 `CloseReason.WHITELIST_APPROVED`(复用,不新加,语义=加白结束)。
  - [ ] `AuditAction.WHITELIST_OBSERVED` 新枚举值的单测覆盖(断言其 value 字符串、可在审计行写入读回)。
- **Depends on:** Task 10
...
- **Done when:**
  - [ ] whitelist_service 注入 `GovernanceLifecycleServiceProtocol`(单向,验证无循环依赖)。
  - [ ] `delete_whitelist_entry`:`remove` 之后 best-effort `find_observed_ticket(worker_id)` → 命中则 `close_observed_for_removal`(OBSERVED→CLOSED,不设 cooldown);无单跳过。
  - [ ] `lifecycle_service` 加 `close_observed_for_removal(ticket_id, *, now)`。
  - [ ] 单测:删白→OBSERVED 单转 CLOSED;再 off-batch(is_whitelisted=False)→ 建 OPEN 新单 + 发通知。
...
- **Done when:**
  - [ ] `list_review_tickets` 的 statuses 入参校验把 `observed` 加进合法集(若有校验)。
  - [ ] 不新增端点;评审按 status=observed 可筛出观察单。
  - [ ] 单测:GET 列表带 statuses=observed 返回观察单及其最新快照。
- **Depends on:** Task 10
...
- **Files:** —
- **Done when:**
  - [ ] Step1 验收:谓词收口纯重构、全量 community 绿、单独 commit。
  - [ ] 状态机:OBSERVED 进出 transitions 符合 spec;属 closed 族,ACTIVE_STATUSES 不含。
  - [ ] 三路产 OBSERVED 各一例绿。
  - [ ] 不发通知守卫:OBSERVED 单全周期 notify_log 零增量。
  - [ ] cooldown 隔离:OBSERVED 不被 find_latest_closed_by_worker 命中;删白后无 cooldown 立即重建。
  - [ ] 删白路径:OBSERVED→CLOSED→off-batch 重建 OPEN。
  - [ ] 画像可见:workflow 列表可见 OBSERVED + 最新快照。
  - [ ] `DEPLOY_PROFILE=test uv run pytest tests/community -q` 全绿。
  - [ ] `DEPLOY_PROFILE=corp_test uv run pytest tests/corp -q` 全绿(corp 若有依赖)。
- **Depends on:** all
