const SENSITIVE_FIELD_PATTERN = /(?:authorization|cookie|token|secret|api[_-]?key|access[_-]?key|password|credential)/i;
...
const CLI_CREDENTIAL_PATTERN = /--(?:authorization|cookie|token|secret|api[_-]?key|access[_-]?(?:key|token)|password|credential)(?:\s+|=)[^\s,;]+/gi;
...
const SIGNED_QUERY_CREDENTIAL_PATTERN = /[?&](?:x[-_]?amz[-_]?(?:credential|signature)|x[-_]?oss[-_]?signature|x[-_]?goog[-_]?signature|ossaccesskeyid|signature|sig)=(?:[^&#\s]+)/iu;
...
export function redactText(value: unknown, maxBytes = 32 * 1024): string {
  const input = String(value ?? "")
    ...
    .replace(CLI_CREDENTIAL_PATTERN, "[REDACTED]")
...
export function redactPersistableText(value: unknown, maxBytes = 32 * 1024): string {
  ...
  return containsSecret(redacted) || RAW_SECRET_LITERAL.test(redacted)
    ? REDACTED_SECRET_TEXT
    : redacted;
}
...
export function containsSecret(value: unknown, seen = new Set<object>()): boolean {
  if (typeof value === "string") return APPROVAL_SECRET_TEXT.test(value) || SIGNED_QUERY_CREDENTIAL_PATTERN.test(value);
  if (value == null || typeof value !== "object") return false;
  if (seen.has(value)) return false;
  seen.add(value);
  if (Array.isArray(value)) {
    return value.some((item) => containsSecret(item, seen));
  }
  return Object.entries(value as Record<string, unknown>).some(([key, child]) => {
    const normalized = key.toLowerCase().replace(/[^a-z0-9]/g, "");
    return [...APPROVAL_SECRET_KEYS].some((key) => normalized === key || normalized.endsWith(key))
      || containsSecret(child, seen);
  });
}
