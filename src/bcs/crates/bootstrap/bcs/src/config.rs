...
pub struct BcsConfig {
    ...
    /// Exact Provider identities that may assign a configured stable Bot ID.
    /// Empty list means Provider Bot IDs are generated by BCS.
    #[serde(default)]
    pub trusted_provider_bot_id_overrides: Vec<bcs_http::state::TrustedProviderBotIdOverride>,
...
impl Default for BcsConfig {
    fn default() -> Self {
        Self {
            ...
            trusted_provider_bot_id_overrides: Vec::new(),
...
impl BcsConfig {
    ...
    pub fn validate_trusted_provider_bot_id_overrides(&self) -> Result<(), String> {
        let mut seen = std::collections::HashSet::new();
        for override_rule in &self.trusted_provider_bot_id_overrides {
            if override_rule.provider_name.trim().is_empty()
                || override_rule.created_by.trim().is_empty()
                || override_rule.provider_bot_ref.trim().is_empty()
                || override_rule.bot_uuid.trim().is_empty()
            {
                return Err(
                    "trusted_provider_bot_id_overrides cannot contain blank fields".to_string(),
                );
            }
            if !override_rule
                .provider_bot_ref
                ...
                || override_rule.provider_bot_ref.len() > 256
            {
                return Err(format!(
                    "trusted_provider_bot_id_overrides has invalid provider_bot_ref: {}",
                    override_rule.provider_bot_ref
                ));
            }
            if !override_rule
                .bot_uuid
                ...
                || override_rule.bot_uuid.len() > 256
            {
                return Err(format!(
                    "trusted_provider_bot_id_overrides has invalid bot_uuid: {}",
                    override_rule.bot_uuid
                ));
            }
            let identity = (
                override_rule.provider_name.as_str(),
                override_rule.created_by.as_str(),
                override_rule.provider_bot_ref.as_str(),
            );
            if !seen.insert(identity) {
                return Err(format!(
                    "duplicate trusted_provider_bot_id_override for provider_bot_ref: {}",
                    override_rule.provider_bot_ref
                ));
            }
        }
...
fn validate_loaded_config(config: &BcsConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    config
        .validate_trusted_provider_bot_id_overrides()
        .map_err(|e| {
            Box::new(std::io::Error::new(std::io::ErrorKind::InvalidInput, e))
                as Box<dyn std::error::Error>
        })?;
...
mod tests {
    ...
    #[test]
    fn trusted_provider_bot_id_overrides_accept_exact_stable_local_identity() {
        let toml = r#"
            bots_base_dir = "/bots"

            [[trusted_provider_bot_id_overrides]]
            provider_name = "singlebox-merchant-claude"
            created_by = "001"
            provider_bot_ref = "merchant-platform-data:mock-user"
            bot_uuid = "平台数据分析"
        "#;
        let config: BcsConfig =
            toml::from_str(toml).expect("parse trusted Provider Bot ID override");

        config
            .validate_trusted_provider_bot_id_overrides()
            .expect("validate trusted Provider Bot ID override");
        assert_eq!(config.trusted_provider_bot_id_overrides.len(), 1);
        assert_eq!(
            config.trusted_provider_bot_id_overrides[0].bot_uuid,
            "平台数据分析"
        );
    }

    #[test]
    fn trusted_provider_bot_id_overrides_reject_invalid_provider_ref() {
        let mut config = BcsConfig::default();
        config.trusted_provider_bot_id_overrides =
            vec![bcs_http::state::TrustedProviderBotIdOverride {
                provider_name: "singlebox-merchant-claude".to_string(),
                created_by: "001".to_string(),
                provider_bot_ref: "merchant/platform-data".to_string(),
        ...
        assert!(
            config
                ...
                .contains("invalid provider_bot_ref")
        );
    }
