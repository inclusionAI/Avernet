// @asset-migrated: Avernet src/bcs/assets/panel/src/StateMachineRunView.tsx (引擎自带副屏模块，方案B本地注册)
...
export type StateMachineRunStatus = 'pending' | 'running' | 'completed' | 'failed' | 'aborted';
...
export type StateMachineNodeSubStatus = 'awaiting_response' | 'judging';
...
export interface StateMachineRunViewProps extends StateMachineRunViewData {
  ...
  className?: string;
  style?: CSSProperties;
  onInteraction?: (payload: { type: string; node?: StateMachineNode; run?: StateMachineRun }) => void;
}
...
// 与 manifest 一致，走 TeamClaw Gateway 的 /api/v1/collaboration/** 同源反代：
//   - 部署态由 Tern 同源反代（config/internal/runtime/config.ts 的 tern.proxy 表）转发到
//     teamclawgw-{pre,prod}，无需跨域 CORS，cookie 鉴权（withCredentials）作用域同源不裂。
//   - 本地 dev 由 config.local.ts 的 /api/v1/collaboration 代理规则转发到同一 gateway。
//   - gateway 实际路由 /api/v1/collaboration/state-machine-runs/**，响应统一为 {code,message,data} 信封。
// 历史：早期沿用 ocb /bcnproxy 前缀，但部署态 Tern 反代表无 /bcnproxy 条目，相对路径会落到前端站
// 本身被 CORB 拦截，故随 manifest 一并迁到 /api/v1/collaboration。远程 CDN UMD 仍用各自 /bcnproxy
// 约定（属 CDN 侧事项，由调用方经 params.apiBaseUrl 注入或 CDN UMD 自身约定）。
const DEFAULT_BASE_URL = '/api/v1/collaboration';
const DEFAULT_POLLING_INTERVAL = 3000;
const MAX_TRANSIENT_RETRIES = 3;
const MAX_HUMAN_RESPONSE_BYTES = 64 * 1024;

/**
 * 解包 gateway 信封响应 `{code,message,data}`。
 * /api/v1/collaboration/state-machine-runs/** 由 teamclaw gateway 提供，响应统一包在 `data` 里；
 * 这里取 `json.data`，并对非信封（裸响应）兜底回原 json，保持向后兼容。
 */
async function parseEnvelope<T>(response: Response): Promise<T> {
  const json = (await response.json()) as { data?: T } | T;
  const enveloped = json as { data?: T };
  return (enveloped && enveloped.data !== undefined ? enveloped.data : json) as T;
}
...
const RUN_TERMINAL_STATUSES = new Set<string>(['completed', 'failed', 'aborted']);
const NODE_ACTIVE_STATUSES = new Set<string>(['pending', 'ready', 'running', 'retry_scheduled']);
const NODE_TERMINAL_STATUSES = new Set<string>(['completed', 'failed', 'skipped']);
...
const Header = styled.header`
  ...
  border-radius: 12px;
  padding: 16px 18px;
  background: linear-gradient(180deg, rgba(255, 255, 255, 0.95) 0%, rgba(248, 250, 252, 0.9) 100%), #ffffff;
  box-shadow: 0 1px 3px rgba(15, 23, 42, 0.04), 0 4px 12px rgba(15, 23, 42, 0.02);
  backdrop-filter: blur(8px);
  transition: box-shadow 200ms ease, border-color 200ms ease;

  &:hover {
    box-shadow: 0 2px 8px rgba(15, 23, 42, 0.06), 0 6px 20px rgba(15, 23, 42, 0.03);
...
const GraphShell = styled.div`
  ...
  border-radius: 12px;
  background: radial-gradient(circle at 50% 50%, rgba(248, 250, 252, 0.5) 0%, transparent 70%),
    linear-gradient(#f1f5f9 1px, transparent 1px), linear-gradient(90deg, #f1f5f9 1px, transparent 1px), #fafbfd;
...
const NodeDetailHero = styled.div`
  ...
  border-radius: 12px;
  padding: 14px;
  background: linear-gradient(180deg, rgba(255, 255, 255, 0.95) 0%, rgba(248, 250, 252, 0.8) 100%), #ffffff;
  box-shadow: 0 1px 3px rgba(15, 23, 42, 0.03), 0 0 0 1px rgba(255, 255, 255, 0.6) inset;
`;
...
const NodeTag = styled.span<{ $tone: 'type' | 'role' }>`
  ...
  gap: 6px;
  align-items: center;
  border: 1px solid ${(props) => (props.$tone === 'type' ? '#dbeafe' : '#fed7aa')};
  border-radius: 999px;
  padding: 4px 9px;
...
const NodeInfoIcon = styled.span<{ $tone: 'time' | 'attempt' | 'duration' }>`
  ...
  justify-content: center;
  border-radius: 8px;
  color: ${(props) => (props.$tone === 'duration' ? '#b45309' : props.$tone === 'attempt' ? '#2563eb' : '#94a3b8')};
  background: ${(props) =>
    props.$tone === 'duration' ? '#fef3c7' : props.$tone === 'attempt' ? '#dbeafe' : '#f1f5f9'};
`;
...
const RuntimeRows = styled.div`
  ...
  border-radius: 10px;
  overflow: hidden;
  background: linear-gradient(180deg, rgba(255, 255, 255, 0.95) 0%, rgba(248, 250, 252, 0.85) 100%), #ffffff;
...
const CriteriaItem = styled.div<{ $satisfied?: boolean }>`
  ...
  gap: 10px;
  border: 1px solid
    ${(props) => (props.$satisfied === undefined ? '#e2e8f0' : props.$satisfied ? '#86efac' : '#fca5a5')};
  border-radius: 8px;
  padding: 10px;
  background: ${(props) =>
...
const CriteriaMarker = styled.span<{ $satisfied?: boolean }>`
  ...
  border-radius: 8px;
  color: #ffffff;
  background: ${(props) => (props.$satisfied === undefined ? '#94a3b8' : props.$satisfied ? '#16a34a' : '#dc2626')};
  font-size: 10px;
  font-weight: 800;
  line-height: 1;
`;
...
function getNodeDisplayStatus(status?: string, subStatus?: StateMachineNodeSubStatus) {
  return normalizeStatus(status) === 'running' && subStatus ? subStatus : status;
}
...
function getStatusTone(status?: string): StatusTone {
  ...
  if (normalized === 'running' || normalized === 'awaiting_response' || normalized === 'retry_scheduled') {
    return {
      bg: '#eff6ff',
      border: '#bfdbfe',
      text: '#1d4ed8',
  ...
  if (normalized === 'failed' || normalized === 'aborted' || normalized === 'error') {
    return {
      bg: '#fef2f2',
      border: '#fecaca',
      text: '#b91c1c',
...
function getDisplayAttempt(attempt?: number, maxAttempts?: number) {
  ...
  if (maxAttempts !== undefined && maxAttempts > 0 && displayAttempt > maxAttempts) {
    return maxAttempts;
  }
...
function formatNodeAttempt(status: string | undefined, attempt: number | undefined, maxAttempts?: number) {
...
function getEdgeState(edge: StateMachineEdge, nodeById: Map<string, StateMachineNode>): EdgeState {
...
async function createRequestError(response: Response) {
  ...
  return new StateMachineRunRequestError(bodyMessage || fallback || 'Failed to load graph', response.status);
}
...
function resolveBaseUrl(props: StateMachineRunViewProps) {
  return props.apiBaseUrl || props.baseUrl || props.data?.apiBaseUrl || props.data?.baseUrl || DEFAULT_BASE_URL;
}
...
function normalizeJudgeDecision(decision?: JsonValue) {
  ...
  const outcome = readStringValue(root, 'outcome') || readStringValue(rawResponse, 'outcome');
  const reason = readStringValue(root, 'reason') || readStringValue(rawResponse, 'reason');
  const confidence = readNumberValue(root, 'confidence') ?? readNumberValue(rawResponse, 'confidence');
  const retryInstruction =
    readStringValue(root, 'retry_instruction') || readStringValue(rawResponse, 'retry_instruction');
...
function getJudgeOutcomeStatus(outcome?: string) {
  ...
  if (normalized === 'approved' || normalized === 'accepted' || normalized === 'pass' || normalized === 'passed') {
    return 'completed';
  }

  if (normalized === 'rejected' || normalized === 'reject' || normalized === 'retry' || normalized === 'failed') {
    return 'failed';
  }
...
function buildGraphLayout(
  ...
  const maxColumns = Math.max(1, ...sortedGroups.map(([, group]) => group.length));
  const width = PADDING * 2 + maxColumns * NODE_WIDTH + Math.max(0, maxColumns - 1) * COLUMN_GAP;
  const height = PADDING * 2 + sortedGroups.length * NODE_HEIGHT + Math.max(0, sortedGroups.length - 1) * LEVEL_GAP;
  ...
  const layoutById = new Map(layoutNodes.map((layoutNode) => [layoutNode.node.node_id, layoutNode]));
...
function DurationIcon({ size = 16 }: { size?: number }) {
  return (
    <svg aria-hidden="true" fill="none" height={size} viewBox="0 0 24 24" width={size}>
      <path d="M10 3h4M12 7v5l3 2" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" />
...
const StateMachineRunView: React.FC<StateMachineRunViewProps> = (props) => {
  ...
  const [pendingHumanNodes, setPendingHumanNodes] = useState<PendingHumanNode[]>([]);
  const [pendingHumanLoading, setPendingHumanLoading] = useState(false);
  const [pendingHumanError, setPendingHumanError] = useState<string | null>(null);
  ...
  const [humanResponseError, setHumanResponseError] = useState<string | null>(null);
  ...
  const pendingHumanNode = pendingHumanNodes.length === 1 ? pendingHumanNodes[0] : null;
    ...
    async (expectedNodeId: string) => {
      ...
      try {
        const response = await fetch(
          joinUrl(baseUrl, `/state-machine-runs/${encodeURIComponent(runId)}/pending-human-nodes`),
          {
            credentials: 'include',
        ...
        const data = await parseEnvelope<PendingHumanNode[]>(response);
        ...
        if (nextNodeId && pendingHumanNodeIdRef.current && nextNodeId !== pendingHumanNodeIdRef.current) {
        ...
        if (data.length > 1) {
          setPendingHumanError('检测到多个待处理的用户输入，当前版本不支持并发用户输入。');
        } else if (data.length === 1 && data[0].node_id !== expectedNodeId) {
          setPendingHumanError('待处理的用户输入与当前运行节点不一致，请刷新后重试。');
        }
      } catch (requestError) {
        if ((requestError as Error).name !== 'AbortError' && pendingHumanAbortRef.current === abortController) {
          const normalizedError = normalizeRequestError(requestError);
          setPendingHumanError(normalizedError.message || '加载用户输入信息失败');
        }
      } finally {
        if (pendingHumanAbortRef.current === abortController) {
  ...
  const fetchGraph = useCallback(
    async (mode: 'initial' | 'refresh' = 'initial') => {
      ...
      try {
        const response = await fetch(joinUrl(baseUrl, `/state-machine-runs/${encodeURIComponent(runId)}/graph`), {
          credentials: 'include',
          signal: abortController.signal,
        });
        ...
        const data = await parseEnvelope<StateMachineRunGraph>(response);
        ...
        setSelectedNodeId((current) => {
          ...
          const runningNode = data.nodes.find((node) => normalizeStatus(node.status) === 'running');
          const activeNode = data.nodes.find((node) => isNodeActiveStatus(node.status));

          return runningNode?.node_id || activeNode?.node_id || data.nodes[0]?.node_id || null;
        });
      } catch (requestError) {
        if ((requestError as Error).name !== 'AbortError') {
          ...
          const shouldRetry =
            ...
            (!graphRef.current || isRunActiveStatus(graphRef.current.run.status));
  ...
  const fetchNodeDetail = useCallback(
    async (nodeId: string, options: { keepPrevious?: boolean } = {}) => {
      ...
      try {
        const response = await fetch(
          joinUrl(baseUrl, `/state-machine-runs/${encodeURIComponent(runId)}/nodes/${encodeURIComponent(nodeId)}`),
          {
            credentials: 'include',
            signal: abortController.signal,
          },
        ...
        const data = await parseEnvelope<StateMachineNodeDetailResponse>(response);
        ...
        if ((requestError as Error).name !== 'AbortError') {
          ...
          if (nodeDetailAbortRef.current === abortController) {
            setNodeDetailError(normalizedError.message || 'Failed to load node detail');
          }
        }
      } finally {
  ...
  const handleHumanResponseSubmit = useCallback(
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      if (new TextEncoder().encode(content).length > MAX_HUMAN_RESPONSE_BYTES) {
        setHumanResponseError(`用户输入不能超过 ${MAX_HUMAN_RESPONSE_BYTES} UTF-8 bytes。`);
        return;
      }
      ...
      try {
        const response = await fetch(
          joinUrl(
            baseUrl,
            `/state-machine-runs/${encodeURIComponent(runId)}/nodes/${encodeURIComponent(
              pendingHumanNode.node_id,
            )}/respond`,
          ),
          {
            method: 'POST',
        ...
        if ((requestError as Error).name !== 'AbortError' && humanResponseAbortRef.current === abortController) {
          const normalizedError = normalizeRequestError(requestError);
          setHumanResponseError(normalizedError.message || '提交用户输入失败');
    ...
    [baseUrl, fetchGraph, humanResponseSubmitting, humanResponseText, pendingHumanNode, runId],
  );
  ...
  useEffect(() => {
    if (!runId || !selectedNodeId || !nodeDetailModalOpen || nodeDetailLoading) {
      return;
    }
  ...
  }, [fetchNodeDetail, nodeDetail?.node?.node_id, nodeDetailLoading, nodeDetailModalOpen, runId, selectedNodeId]);
  ...
  useEffect(() => {
    ...
    const retryDelay =
      retryCount > 0
        ? Math.min(pollingInterval * Math.pow(2, retryCount - 1), DEFAULT_POLLING_INTERVAL * 10)
        : pollingInterval;
  ...
  const layout = useMemo(() => {
    ...
    return buildGraphLayout(graph.nodes, graph.edges, graph.definition.initial_nodes);
  }, [graph]);
  ...
  const selectedNodeStatus = selectedRuntimeNode?.status || selectedNode?.status || undefined;
  const selectedNodeSubStatus = selectedNodeDetail?.sub_status || selectedNode?.sub_status;
  const selectedNodeDisplayStatus = getNodeDisplayStatus(selectedNodeStatus, selectedNodeSubStatus);
  const selectedNodeAttempt = selectedRuntimeNode?.attempt ?? selectedNode?.attempt;
  const selectedNodeAttemptText = formatNodeAttempt(
    selectedNodeStatus,
    selectedNodeAttempt,
    selectedRuntimeNode?.max_attempts,
  );
  const selectedNodeStarted = selectedRuntimeNode?.started_at || selectedNode?.started_at;
  const selectedNodeCompleted = selectedRuntimeNode?.completed_at || selectedNode?.completed_at;
  ...
  const selectedNodeDurationParts = formatDurationParts(selectedNodeStarted, selectedNodeCompleted);
  ...
  const taskOutputText = graph?.run.output !== undefined ? stringifyValue(graph.run.output) : '';
  const nodeArtifactText = selectedRuntimeNode?.artifact_text || '';

  const handleCopyText = useCallback(async (text: string, target: CopyTarget) => {
    if (!text || !target) {
      return;
    }
  ...
  const humanInputEditor = activeHumanNode ? (
    <HumanInputCard aria-live="polite">
      <HumanInputHeader>
        <HumanInputHeading>
          <HumanInputTitle>等待用户输入</HumanInputTitle>
          <HumanInputNodeName>
            当前节点：
            {pendingHumanNode?.display_name || activeHumanNode.display_name || activeHumanNode.node_id}
          </HumanInputNodeName>
        </HumanInputHeading>
        <HumanInputBadge>需要你处理</HumanInputBadge>
      </HumanInputHeader>

      <HumanInputLead>请阅读审核说明并输入你的意见，提交后 Judge 会判定结果并继续执行。</HumanInputLead>

      {pendingHumanNode?.instruction ? <HumanInstruction>{pendingHumanNode.instruction}</HumanInstruction> : null}
      ...
      {pendingHumanNode?.timeout_deadline_ms ? (
        <HumanInputMeta>请在 {formatTime(pendingHumanNode.timeout_deadline_ms)} 前提交</HumanInputMeta>
      ) : null}

      {pendingHumanLoading && !pendingHumanNode ? <HumanResponseHint>正在加载用户输入信息…</HumanResponseHint> : null}

      {pendingHumanError ? <InlineNotice $danger>{pendingHumanError}</InlineNotice> : null}

      {pendingHumanNode && !pendingHumanError ? (
        <HumanResponseForm onSubmit={handleHumanResponseSubmit}>
          <HumanResponseLabel htmlFor={humanResponseInputId}>你的输入</HumanResponseLabel>
          ...
          />
          <HumanResponseHint>
            无需手动填写 approved / rejected，Judge 会根据你的自然语言输入进行判定。
          </HumanResponseHint>
          {humanResponseError ? <InlineNotice $danger>{humanResponseError}</InlineNotice> : null}
          <HumanResponseActions>
            <HumanResponseSubmitButton disabled={humanResponseSubmitting || !humanResponseText.trim()} type="submit">
              {humanResponseSubmitting ? '正在提交并判定…' : '提交用户输入'}
        ...
        <HumanArtifactList>
          {pendingHumanNode.upstream_artifacts.map((artifact) => (
            <div key={artifact.node_id}>
              <HumanArtifactLabel>上游输出 · {artifact.node_id}</HumanArtifactLabel>
  ...
  const nodeDetailContent = selectedNode ? (
    <>
      {selectedNode.node_id === activeHumanNode?.node_id ? <DetailSection>{humanInputEditor}</DetailSection> : null}
      ...
      <DetailSection>
        <NodeDetailHero>
          <NodeTagRow>
            <NodeTag $tone="type" title={selectedNode.kind || 'node'}>
              ...
              <NodeTagValue $tone="type">{selectedNode.kind || 'node'}</NodeTagValue>
            </NodeTag>
          ...
          <NodeTimeGrid>
            <NodeInfoCard>
              ...
              <NodeInfoValue title={formatTime(selectedNodeStarted)}>{formatTime(selectedNodeStarted)}</NodeInfoValue>
            </NodeInfoCard>
            <NodeSummaryArrow>
          ...
          <NodeMetricGrid>
            ...
            <NodeInfoCard>
              ...
              <NodeInfoStack>
                ...
                <NodeMetricValue>
                  <NodeInfoValue>{selectedNodeDurationParts.value}</NodeInfoValue>
                  {selectedNodeDurationParts.unit ? (
                    <NodeMetricUnit>{selectedNodeDurationParts.unit}</NodeMetricUnit>
                  ) : null}
                </NodeMetricValue>
              </NodeInfoStack>
            </NodeInfoCard>
          ...
          {selectedNode.final_output ? <FinalOutputNote>Final output node</FinalOutputNote> : null}
        </NodeDetailHero>
        ...
        {selectedNodeSubStatus === 'judging' ? (
          <InlineNotice>Response received. Judging is in progress.</InlineNotice>
        ) : null}

        {nodeDetailError ? <InlineNotice $danger>{nodeDetailError}</InlineNotice> : null}
      </DetailSection>
        ...
        <DetailSection>
          <CollapsibleHeader>
            ...
            {nodeArtifactExpanded ? (
              <CopyButton aria-label="Copy node output" type="button" onClick={handleCopyNodeArtifact}>
                <CopyIcon />
          ...
          {nodeArtifactExpanded ? (
            <OutputBlock id="state-machine-node-output">{selectedRuntimeNode.artifact_text}</OutputBlock>
          ) : null}
        </DetailSection>
      ) : null}
          ...
          {judgeOutputsExpanded ? (
            <JudgeOutputList id="state-machine-judge-outputs">
              {selectedNodeDetail.judge_outputs.map((judgeOutput, index) => {
                ...
                return (
                  <JudgeOutputCard
                    key={`${judgeOutput.node_id}-${judgeOutput.attempt ?? index}-${judgeOutput.created_at ?? index}`}
                  >
                    <JudgeMetaRow>
                      <JudgeMetaText>
                        Attempt
                        <JudgeMetaValue>{formatJudgeAttempt(judgeOutput.attempt)}</JudgeMetaValue>
                        <JudgeMetaSeparator>·</JudgeMetaSeparator>
                        <JudgeMetaValue>{formatTime(judgeOutput.created_at)}</JudgeMetaValue>
                      </JudgeMetaText>
                    ...
                    {decision ? (
                      <>
                        <DecisionSummary>
                          <DecisionHeader>
                            ...
                            {decision.confidence !== undefined ? (
                              <ConfidenceBadge>Confidence {Math.round(decision.confidence * 100)}%</ConfidenceBadge>
                            ) : null}
                          </DecisionHeader>
                          <DecisionReason>{decision.reason || 'No decision reason returned.'}</DecisionReason>
                          {decision.retryInstruction ? (
                            <RetryInstruction>
                              ...
                              <div>
                                ...
                                <RetryText>{decision.retryInstruction}</RetryText>
                              </div>
                            </RetryInstruction>
                          ) : null}
                        </DecisionSummary>
                        ...
                        {decision.checkedCriteria.length ? (
                          <>
                            ...
                            <CriteriaList>
                              {decision.checkedCriteria.map((criteria, criteriaIndex) => (
                                <CriteriaItem
                                  $satisfied={criteria.satisfied}
                                  key={`${criteria.criterion}-${criteriaIndex}`}
                                >
                                  <CriteriaMarker $satisfied={criteria.satisfied}>
                                    {criteria.satisfied === undefined ? '?' : criteria.satisfied ? 'OK' : 'NO'}
                                  </CriteriaMarker>
                                  <div>
                                    <CriteriaTitle>{criteria.criterion}</CriteriaTitle>
                                    {criteria.evidence ? (
                                      <CriteriaEvidence>{criteria.evidence}</CriteriaEvidence>
                                    ) : null}
                                  </div>
                                </CriteriaItem>
                              ))}
                      ...
                      <OutputBlock>{stringifyValue(judgeOutput.decision)}</OutputBlock>
                    )}
                  </JudgeOutputCard>
                );
              })}
      ...
      <DetailSection>
        <RuntimeRows>
          <RuntimeRow>
            ...
            <RuntimeStrongValue>
              {selectedRuntimeNode?.assignee_bot_id || selectedNode.assignee_bot_id || '-'}
            </RuntimeStrongValue>
          </RuntimeRow>
          <RuntimeRow>
            <RuntimeLabel>Timeout</RuntimeLabel>
            <RuntimeValue>{formatMilliseconds(selectedRuntimeNode?.node_timeout_ms)}</RuntimeValue>
          </RuntimeRow>
          <RuntimeRow>
            <RuntimeLabel>Delivery</RuntimeLabel>
            <RuntimeStrongValue>{selectedRuntimeNode?.delivery_request_id || '-'}</RuntimeStrongValue>
          </RuntimeRow>
          <RuntimeRow>
            <RuntimeLabel>Bot run</RuntimeLabel>
            <RuntimeStrongValue>{selectedRuntimeNode?.bot_delivery_run_id || '-'}</RuntimeStrongValue>
          </RuntimeRow>
        </RuntimeRows>
      </DetailSection>
    </>
    ...
    <Container className={props.className} style={props.style}>
      <Header>
        <HeaderTop>
          ...
          <Actions>
            {graph?.run.status ? (
              <StatusPill $tone={getStatusTone(graph.run.status)}>{getStatusLabel(graph.run.status)}</StatusPill>
            ) : null}
            <IconButton
        ...
        <HeaderInfo>
          <SubTitle>
            ...
            <InlineCopyButton aria-label="Copy run id" type="button" onClick={handleCopyRunId}>
              <CopyIcon size={11} />
            ...
            <ContentGrid>
              <GraphShell>
                {layout && layout.nodes.length > 0 ? (
                  ...
                  >
                    <defs>
                      <filter id="sm-node-shadow" x="-8%" y="-8%" width="116%" height="132%">
                        <feDropShadow
                          dx="0"
                          dy="2"
                          stdDeviation="3"
                      ...
                      <filter id="sm-node-shadow-selected" x="-8%" y="-8%" width="116%" height="132%">
                        <feDropShadow
                          dx="0"
                          dy="3"
                          stdDeviation="5"
                    ...
                    {layout.edges
                      .map((layoutEdge, index) => {
                        const edgeState = getEdgeState(layoutEdge.edge, nodeById);
                      ...
                      .sort(
                        (a, b) => EDGE_RENDER_ORDER[a.edgeState] - EDGE_RENDER_ORDER[b.edgeState] || a.index - b.index,
                      )
                      .map(({ edge, source, target, edgeState }) => {
                        ...
                        return (
                          <g key={`${edge.source}-${edge.outcome}-${edge.target}`}>
                            <path
                            ...
                            {edge.outcome ? (
                              <text
                                fill={edgeTone.label}
                                fontSize="12"
                                fontWeight={edgeState === 'executed' || edgeState === 'blocked' ? 650 : 500}
                                textAnchor="middle"
                                x={(sourceX + targetX) / 2}
                                y={midY - 6}
                              >
                    ...
                    {layout.nodes.map((layoutNode) => {
                      ...
                      const displayStatus = getNodeDisplayStatus(node.status, node.sub_status);
                      ...
                      return (
                        <NodeGroup
                          key={node.node_id}
                          aria-label={`Node ${node.display_name || node.node_id} ${nodePhase}`}
                          onClick={() => {
                            requestedNodeDetailIdRef.current = null;
                        ...
                        >
                          <rect
                            fill={nodePhase !== 'unknown' ? tone.fill : selected ? '#f8fbff' : '#ffffff'}
                            filter={selected ? 'url(#sm-node-shadow-selected)' : 'url(#sm-node-shadow)'}
                            height={height}
                            rx="10"
                            stroke={tone.stroke}
                          ...
                          <text
                            ...
                            y={y + 22}
                          >
                            {truncateText(node.display_name || node.node_id, 22)}
                          </text>
                          <g
                            stroke="#ffffff"
                            strokeLinecap="round"
                          ...
                          >
                            ...
                            <circle cx="0" cy="0" fill={tone.stroke} r="7" stroke="#ffffff" strokeWidth="1.5" />
                            {renderNodeStatusMarker(displayStatus)}
                          </g>
                          <foreignObject height="18" width={width - 24} x={x + 12} y={y + 34}>
                            <NodeMetaRow>
                              <BotIdText title={botId}>{truncateText(botId, 20)}</BotIdText>
                              {role ? <RoleTag title={role}>{role}</RoleTag> : null}
                            </NodeMetaRow>
                          </foreignObject>
                  ...
                  <DetailSection>
                    <HumanInputStatusNotice aria-live="polite" role="status">
                      ...
                      <HumanInputStatusBody>
                        <HumanInputStatusTitle>等待用户输入</HumanInputStatusTitle>
                ...
                <DetailSection>
                  <SectionHeader>
                    ...
                    {graph.run.output !== undefined ? (
                      <CopyButton aria-label="Copy task output" type="button" onClick={handleCopyTaskOutput}>
                        <CopyIcon />
                  ...
                  {graph.run.output !== undefined ? (
                    ...
                    <OutputPlaceholder>{taskOutputPlaceholder}</OutputPlaceholder>
                  )}
                </DetailSection>
      ...
      {nodeDetailModalOpen && selectedNode ? (
        ...
        >
          <ModalDialog aria-labelledby="state-machine-node-detail-title" aria-modal="true" role="dialog">
            <ModalHeader>
              <ModalTitleGroup>
              ...
              <ModalHeaderActions>
                {nodeDetailLoading ? <NodeDetailSpinner aria-label="Loading node detail" role="status" /> : null}
                <ModalCloseButton
