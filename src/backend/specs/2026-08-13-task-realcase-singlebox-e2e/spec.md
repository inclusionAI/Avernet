...
2. **未接线到运行时**：`TaskService` **未被任何 DI module 装配、没有任何 HTTP router 暴露**（`grep` 无 `task_module.py`、无 `/openapi/v1/task/*` 路由、`TaskService(` 仅在自身 core 构造）。即框架在 singlebox 运行态根本不可达，无法被真实 API 驱动。
...
- **结果回收分两种**：
  ...
  - **execute/verify 经 `report_result` 异步回投**（投递给 bot → bot 凭已装 skill 执行完上报 → `POST /openapi/v1/task/callback/report` → `on_report` 翻态推进）。
...
- 新 `adapters/http/task/router.py`（thin）：`POST /openapi/v1/task/execute`、`GET /openapi/v1/task/dashboard`、`POST /openapi/v1/task/callback/report`（回投 → `callback.report_result`）。Router 只翻译协议，不持领域策略。
...
### G4. 真实 case 端到端用例
- 用例：`POST /openapi/v1/task/execute` 提交 `gwqie46v7hzr1w6h`（存储行业尽调）TaskInfo；real `on_execute` 跑：规划 skill 拆解 → 搜推 skill 匹配 worker bots → 投递执行 → 验收 skill 验收 → 回投翻态 → FAIL治愈/MISS升BBS（按案例）→ 根 DONE。
- 轮询 `GET /openapi/v1/task/dashboard` 至终态，断言分解树、节点状态、run_mode、传播、补救、终态与权威剧本一致。
...
- **AC-2**：`POST /openapi/v1/task/execute` 提交 `gwqie46v7hzr1w6h` TaskInfo 后，框架经 **真实 skill 调用**（非 stub）完成首帧：规划 skill 拆出第一批子节点 → 搜推 skill 为其决出执行者 → 投递给真实 bot。
- **AC-3**：执行结果经真实回投链路（`/openapi/v1/task/callback/report` → `callback.report_result` → `on_report`）翻态、传播、补救；`GET /openapi/v1/task/dashboard` 反映分解树与状态推进。
...
- **R2 回投路径**：真实引擎异步回调 vs 适配层同步回投。为确定性与 singlebox 可行性，倾向 **适配层时效内回投**（`deliver` 调 skill 取结果 → `POST /openapi/v1/task/callback/report`），但真实异步回调仍需验证。需 plan 决策。
