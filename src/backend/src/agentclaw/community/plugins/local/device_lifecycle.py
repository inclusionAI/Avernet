...
from agentclaw.community.plugins.local.database import SessionLocal
...
def release_all_stale_bindings() -> None:
    ...
    try:
        ...
        if not stale_bindings:
            logger.info("[release_all_stale_bindings] No stale local bindings found")
            return

        binding_ids = [b.id for b in stale_bindings]
        logger.info(
            f"[release_all_stale_bindings] Found {len(stale_bindings)} stale "
            f"local device binding(s): {binding_ids}"
        )
...
def reallocate_orphaned_bots(bot_service) -> None:
    ...
    try:
        ...
        if not orphaned_bots:
            logger.info("[reallocate_orphaned_bots] No orphaned PENDING bots found")
            return
        ...
        if skipped:
            logger.info(
                f"[reallocate_orphaned_bots] Skipped {skipped} desktop bot(s) "
