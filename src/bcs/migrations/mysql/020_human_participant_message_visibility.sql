...
ALTER TABLE `bcs_group_participants`
  ADD COLUMN `message_view_scope` varchar(32) NOT NULL DEFAULT 'full'
  AFTER `tags_json`;

ALTER TABLE `bcs_group_sessions`
  ADD COLUMN `message_visibility_version` tinyint(4) NOT NULL DEFAULT '0'
  AFTER `session_kind`;

ALTER TABLE `bcs_messages`
  ADD COLUMN `visibility_domain` varchar(32) DEFAULT NULL
    AFTER `owner_bot_id`,
  ADD COLUMN `audience_kind` varchar(32) DEFAULT NULL
    AFTER `visibility_domain`,
  ADD COLUMN `audience_actor_ids_json` text DEFAULT NULL
    AFTER `audience_kind`,
