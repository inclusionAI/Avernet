...
from secbaas.adapters.web.routers.bot_service.http_conn_router import router
...
async def test_get_http_info_success(mock_dispatcher):
    mock_dispatcher.dispatch_bot_http_conn_info.return_value = HttpConnectionInfo(
        http_url="http://agentclawproxy-pre.alipay.com/proxypass/ARCA_sandbox123:8080/api/health",
    ...
    assert (
        data["http_url"]
        == "http://agentclawproxy-pre.alipay.com/proxypass/ARCA_sandbox123:8080/api/health"
    )
...
async def test_get_http_info_with_device_affinity(mock_dispatcher):
    mock_dispatcher.dispatch_bot_http_conn_info.return_value = HttpConnectionInfo(
        http_url="http://agentclawproxy-pre.alipay.com/proxypass/ARCA_sandbox456:443/api/health",
        token="token-abc",
    )
