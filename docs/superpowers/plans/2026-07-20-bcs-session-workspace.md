...
- 迁移文件 `006_session_files.sql`（mysql）+ SQLite parity in `crates/bootstrap/bcs/src/migrations.rs`；表名 `bcs_session_files`，标准 `id`/`gmt_create`/`gmt_modified`/`env` 四元组 + utf8mb4。只保留 DB 托管的 `gmt_create`/`gmt_modified`（不另设 `created_at`/`updated_at` 列）；唯一索引 `(env, session_id, file_id)` + `(env, file_id)`（file_id 为 ULID 全局唯一，后者保障 share token 按 file_id 解析），索引 `(env, session_id, gmt_create)`。
...
- Produces（供后续所有任务依赖）：
  ...
  - `bcs_domain::SessionFile`（领域结构，含 `file_id: String`、`session_id: String`、`file_name: String`、`mime_type: String`、`size: u64`、`sha256: Option<String>`、`owner: ActorRef`、`storage_backend: String`、`object_handle: String`、`status: FileStatus`、`created_at: u64`、`updated_at: u64`）—— `created_at`/`updated_at` **不落库**，由 store 读时从 DB 托管的 `gmt_create`/`gmt_modified` 投影为 unix 秒（MySQL `UNIX_TIMESTAMP` / SQLite `strftime('%s', …)`）；内存 store 用应用层 `now_secs()`。
...
> 时间戳只保留 DB 托管的 `gmt_create`/`gmt_modified`（与库里其它表一致），**不设** 应用层 `created_at`/`updated_at` 列；领域 `SessionFile.created_at`/`updated_at` 由 store 读时从 `gmt_*` 投影为 unix 秒。`uk_env_file_id` 强制 `(env, file_id)` 唯一（file_id 为 ULID 全局唯一），支撑 share token 仅按 `file_id` 解析（`get_by_file_id`）。
...
`src/mysql.rs`：镜像 `bcs-session-store/src/mysql.rs` 的 `MySqlSessionStore { db: Arc<dyn DbPlugin>, env: String, flavor: DbSqlFlavor }` + `new(db, env)`（Mysql）+ `sqlite(db, env)`（Sqlite）+ `with_flavor(db, env, flavor)`。SQL 用 `?` 占位 + `DbValue::from(...)` 绑定；`gmt_create`/`gmt_modified` 由 DB 默认值维护，**不另设 `created_at`/`updated_at` 列**——领域 `created_at`/`updated_at` 读时从 `gmt_*` 投影为 unix 秒（`select_cols()` 按 flavor 拼 `CAST(UNIX_TIMESTAMP(gmt_create) AS SIGNED) AS created_at` / SQLite `CAST(strftime('%s', gmt_create) AS INTEGER) AS created_at`）。`list` 用 offset 分页，`ORDER BY gmt_create, file_id`。`list_expired_pending` 用小写 `json_extract(object_handle, '$.expires_at')`（MySQL/SQLite 通用）。`flavor` 仅用于时间戳投影这一处方言分支。`is_duplicate_key()` 检测 insert 冲突（`uk_env_file_id` / `uk_session_file` 理论不应发生，ULID 全局唯一）。
...
impl SessionFileRepoPort for MySqlSessionFileStore {
    async fn insert(&self, params: NewSessionFileParams) -> ServiceResult<SessionFile> {
        ...
        // DB-managed `gmt_create`/`gmt_modified`. The returned row's timestamps are the
        // application now (≈ DB gmt_create); re-reads (get/list) project them from
        // `gmt_*`. `expires_at` lives inside `object_handle` JSON.
        let stmt = DbStatement::new(format!(
            "INSERT INTO bcs_session_files \
             (env, file_id, session_id, owner_actor_kind, owner_actor_id, file_name, mime_type, \
              size, storage_backend, object_handle, status) \
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Pending')",
        ))
    ...
    async fn get(&self, session_id: &str, file_id: &str) -> ServiceResult<Option<SessionFile>> {
        // `created_at`/`updated_at` are projected from `gmt_create`/`gmt_modified`
        // (epoch seconds) in a flavor-aware way: MySQL `CAST(UNIX_TIMESTAMP(gmt_create)
        // AS SIGNED)`, SQLite `CAST(strftime('%s', gmt_create) AS INTEGER)`. Shared
        // `select_cols()`/`select_sql(&self, ..)` helpers build the column list so every
        // read path uses the same projection.
        let stmt = DbStatement::new(format!(
...
- `update_object_handle_and_status`：`UPDATE bcs_session_files SET object_handle=?, status=?, size=? WHERE env=? AND session_id=? AND file_id=?` 后 SELECT 回读（`gmt_modified` 由 DB `ON UPDATE` 自动维护，无需写 `updated_at`）。
- `update_status`：`UPDATE ... SET status=? WHERE env=? AND session_id=? AND file_id=?` 后回读。
- `delete`：`DELETE FROM bcs_session_files WHERE env=? AND session_id=? AND file_id=?`，返回 affected>0。
- `list`：`SELECT {select_cols()} WHERE env=? AND session_id=? [AND file_name LIKE ?] [AND status=?] ORDER BY gmt_create, file_id LIMIT ? OFFSET ?`，`prefix` 转 `prefix%` 用 `LIKE`；同时跑 `COUNT(*)` 取 total。limit clamp 100/1000。
- `list_expired_pending`：`SELECT {select_cols()} WHERE env=? AND status='Pending' AND CAST(json_extract(object_handle,'$.expires_at') AS INTEGER) < ? LIMIT ?`（小写 `json_extract` MySQL/SQLite 通用）。
- `delete_all_for_session`：先 `SELECT {select_cols()} WHERE env=? AND session_id=?` 收集，再 `DELETE ... WHERE env=? AND session_id=?`，返回被删行。
