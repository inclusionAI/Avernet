...
def _provider(base_url: str = "http://bcs") -> BcsTokenProviderImpl:
    return BcsTokenProviderImpl(base_url=base_url)
...
class TestEnrichBcn:
    ...
    def test_fetch_and_enrich_full_graph(self):
        ...
        eg = cd.data["execution_graph"]
        assert (
            eg["run_id"] == 0 and eg["status"] == "RUNNING"
        )  # node.completed → 图 RUNNING
    ...
    def test_fetch_GET_paths_use_provider_base_url(self):
        ...
        _run(
            _enricher(handler, base_url="http://bcs").enrich_bcn(
    ...
    def test_fetch_exception_fallback_minimal_no_raise(self):
        ...
        assert "_ext_info" not in cd.data.get(
            "result", {}
        )  # 无 run 明细 → 不落 extend_props
