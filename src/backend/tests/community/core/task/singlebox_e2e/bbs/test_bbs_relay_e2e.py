...
def _dashboard_tasks(c: TestClient, task_id: str) -> tuple[dict, dict[str, dict]]:
    """GET /openapi/v1/task/dashboard → (graph_data, {node_id: node_dto})。"""
    data = c.get("/openapi/v1/task/dashboard", params={"task_id": task_id}).json()["data"]
...
def test_c_two_bots_claim_same_bbs_task_exactly_one_wins(client):
    ...
    r1 = c.post("/openapi/v1/task/bbs/claim", json={"task_id": task_id, "bot_id": "botA"})
    r2 = c.post("/openapi/v1/task/bbs/claim", json={"task_id": task_id, "bot_id": "botB"})
...
def test_b_partial_handoff_relay(client):
    ...
    assert c.post("/openapi/v1/task/bbs/claim", json={"task_id": task_id, "bot_id": "botA"}).status_code == 200
    r_attach_a = c.post("/openapi/v1/task/bbs/attach", json=_attach_body(task_id, task_id, "botA"))
    assert r_attach_a.status_code == 200, r_attach_a.text
    node_a = r_attach_a.json()["data"]["node_id"]
    assert node_a.startswith("bbs-")
    r_fail = c.post("/openapi/v1/task/bbs/result", json=_result_body(
    ...
    assert c.post("/openapi/v1/task/bbs/claim", json={"task_id": task_id, "bot_id": "botB"}).status_code == 200
    r_attach_b = c.post("/openapi/v1/task/bbs/attach", json=_attach_body(task_id, task_id, "botB"))
    ...
    r_pass = c.post("/openapi/v1/task/bbs/result", json=_result_body(
...
def test_d_crash_lease_relay(client):
    ...
    assert r_attach_b.status_code == 200, r_attach_b.text
    node_b = r_attach_b.json()["data"]["node_id"]
    assert c.post("/openapi/v1/task/bbs/result", json=_result_body(
