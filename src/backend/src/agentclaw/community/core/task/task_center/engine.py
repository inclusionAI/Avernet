...
_STATIC_MOCK_SUMMARY: dict[str, str] = {
    ...
    # ---- merchant-operations-goal-to-sop 节点 mock(理发店18周年店庆) ----
    "frame_goal": (
        "18周年店庆目标卡:周期一个月,门店为理发店,新客王牌剪发引流+老客护理套餐。"
        "结果目标:客流提升+到店转化提升+套餐复购提升,品质不变。已知约束:活动贡献毛利率≥3%,"
        "促销预算≤3000元,备货现金占用≤5000元。需平台营销/会员/客流产能/供应履约细化,风险评审后汇总SOP。"
    ),
    "design_marketing": (
        "18周年庆营销打法:新客王牌剪发引流券(门市80/实付40,平台补贴24/商家承担16);"
        "老客护理套餐券(门市360/实付330,平台补贴6/商家承担24)。预售10.15起定金翻倍,"
        "11.4-11.11品类券,11.11-11.15返场。统一主视觉,优惠规则前置公示。"
        "无法独立闭环:人群未分层需会员细化;客流产能需校验;护理耗材需供应核验;比价/负面舆情需风控。"
    ),
    "design_membership_service": (
        "会员与服务承接:老客分三层(高价值/沉睡/流失召回),高价值推护理套餐+专属预约,"
        "沉睡推唤醒券(护理体验价),流失召回推王牌剪发回归券。服务组合:护理套餐含2次护理+1次修剪60分钟,"
        "预约制分流工位。权益兑现:核销有效期30天,过期延期一次。"
        "无法闭环:客流产能需校验工位饱和;护理耗材需核验库存;风险需评审。"
    ),
    "assess_capacity": (
        "客流产能核验:日常客流约40人/天,活动期预估新增15-20人/天。工位4个,日均产能约60人,"
        "活动期排班增1人可覆盖。预约错峰(工作日午间+晚间),剪发走walk-in。建议每日承接上限55人,超限预警。"
        "无法闭环:护理耗材库存需供应核验;排班增人成本需财务确认;风险需评审。"
    ),
    "assess_fulfillment": (
        "供应履约核验:护理耗材(护理膏/精华液/毛巾)活动期需求约50份,现有20份,缺口30份。"
        "Plan A:常规采购(3工作日,够用);Plan B:加急(1工作日,成本+15%)。品质:耗材批次一致,保质期内。"
        "剪发耗材日常库存充足。无法闭环:采购金额是否在备货上限内需店主确认;品质风险需评审;客诉赔付需风控。"
    ),
    "risk_review_group": (
        "风险评审群结论(业务风险+舆情两路):业务风险5项——①护理耗材采购超预算(中,Plan A/B+备货上限约束);"
        "②排班增人成本(低);③护理套餐核销逾期客诉(中,延期+退款预案);④品质下降(中,批次校验+时长监控);"
        "⑤客流超产能(中,每日上限55+预约分流)。舆情4项——①负面评价扩散(中,差评监控+48h响应);"
        "②虚假宣传(低,规则前置公示);③平台规则合规(中,券面额/核销/有效期符合政策);④消费者预期管理(中,预售规则醒目提示)。"
        "条件通过含9项约束。无法闭环:舆情监测系统无对接需研发补。"
    ),

    "synthesize_plan": (
        "18周年店庆经营方案与执行计划(已汇总):新客引流券(门市80/实付40)+老客护理套餐(门市360/实付330),"
        "预售10.15定金翻倍,11.4-11.11品类券,11.11-11.15返场,每日上限55人,预约错峰。"
        "护理耗材Plan A采购30份(在备货上限内)。5项业务风险有约束通过,4项舆情风险有监测。"
        "执行计划:活动前(备货+排班+券配置),活动中(每日监控客流/产能/舆情),异常(阈值触发→降量/限购/赔付/公关)。"
        "待外部执行:真实投券/采购/排班系统接入。"
    ),
}
