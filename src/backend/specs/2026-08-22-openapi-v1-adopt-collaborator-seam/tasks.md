...
- **Done when:**
  ...
  - [x] **The bar calls STAY, and `_check_operable_permission` with them.** They were deleted because only `openapi_v1/editors/router.py` reaches these methods — true, and not the test. `AGENTS.md` says delivery adapters do not own domain policy, and who may remove an editor is collaboration policy, not a transport concern. Raised as a P2 in review round 5, after the identical argument had been accepted for the publication facade in round 4. Restoring `remove_editor`'s inline ADMIN refusal is the one with teeth: without it a MEMBER could delete another editor at the service boundary. **`require_capability` and `require_team_space_member` stay** — they are capability and space checks, not collaborator bars. `remove_editor` still resolves `operator_level`, because it answers a second question: an ADMIN may not remove themselves while an OWNER may. `check_permission` stays public — the internal `/api` collaborator methods still use it, and `add_collaborator` still re-checks ADMIN underneath `add_editor`.
  ...
  - [x] Tests: `test_unsupported_bot_is_masked_until_after_actor_authorization` is **restored** — the ordering it pins is assertable from inside the service again now that the bars are back. Alongside it, `test_the_service_itself_refuses_below_each_editor_bar` drives every bar through the service with no seam in front of it, and `test_the_five_editor_rows_declare_the_same_bars` reads the rows. The two halves do not substitute for each other, and both were checked for vacuity. `collaborator_service` writes no audit rows of its own, so the seam's is the only one.
- **Depends on:** Task 13
