...
_ENGINE_RUNTIME_EXEMPT_REASON = (
    ...
    "/openapi/v1 route answers 401 in singlebox, so a flow could only assert "
    "401s and would prove nothing. The verifier has landed and require_principal "
    "is real; what is still missing is a minter. It accepts only a token signed "
    "with the gateway's key, singlebox runs the backend without a gateway, and "
    "AVERNET_PRINCIPAL_SIGNING_KEY is unset there — which the verifier treats as "
    "deny. This is the same blocker, and deliberately the same answer, as "
    "_GATEWAY_PRINCIPAL_EXEMPT_REASON above: the auth workstream chose to wait "
    "for a gateway in the box rather than have tests forge tokens against a "
    "shared key, and this module follows that ruling rather than inventing a "
    "second one. Covered meanwhile by relay/connection unit tests, endpoint "
    "tests per group, and a 16-route cross-tenant isolation sweep. Drain this "
    "entry together with that one, when singlebox fronts the backend with a "
    "gateway — the same event that unblocks the whole track's definition of done."
)
