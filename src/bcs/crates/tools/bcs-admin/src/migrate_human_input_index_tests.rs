...
fn human_input_index_revisions_preserve_archives_and_pass_static_gate() -> Result<()> {
    ...
    let migrations = load_selected_migrations(&index_migration_args(vec![1, 8, 28]))?;
...
#[test]
fn human_input_index_compatibility_keeps_old_records_and_requires_new_increment() -> Result<()> {
    let migrations = load_selected_migrations(&index_migration_args(vec![1, 8, 28]))?;
    ...
    assert_eq!(report.pending_versions, vec![plans[2].clone()]);
...
async fn check_human_input_index_migrations(db: &dyn DbPlugin) -> Result<()> {
    ...
    anyhow::ensure!(!original_records.iter().any(|row| [1, 8, 28].contains(&row.version)), "test migration versions already present");
    let result = async {
        ...
        // Fresh installations also run 028. Simulate loss of the version
        // record after its successful DDL and confirm a replay is safe.
        apply_mysql_migration(db, &migrations[2], &plans[2]).await?;
        db.execute(DbStatement::new("DELETE FROM bcs_schema_migrations WHERE version = 28")).await?;
        apply_mysql_migration(db, &migrations[2], &plans[2]).await?;
        assert_scope_rows_and_uniqueness(db).await?;
        db.execute(DbStatement::new("DELETE FROM bcs_schema_migrations WHERE version = 28")).await?;
        // MySQL cannot create the original utf8mb4 index. utf8mb3 permits its
        // exact unprefixed shape, providing an existing-index upgrade fixture;
        // this does not claim to reproduce an OceanBase deployment.
        db.execute(DbStatement::new("ALTER TABLE bcs_human_input_requests CONVERT TO CHARACTER SET utf8mb3")).await?;
        db.execute(DbStatement::new("ALTER TABLE bcs_human_input_requests DROP INDEX idx_human_input_scope_status, ADD INDEX idx_human_input_scope_status (reply_scope_key, status, deadline_ms, created_at)")).await?;
        ...
        assert_eq!(report.pending_versions, vec![plans[2].clone()]);
        // A missing index is schema drift: propagate DDL failure without
        // recording 028 or changing either historical migration row.
        db.execute(DbStatement::new("ALTER TABLE bcs_human_input_requests DROP INDEX idx_human_input_scope_status")).await?;
        assert!(apply_mysql_migration(db, &migrations[2], &plans[2]).await.is_err());
        assert_eq!(load_applied_mysql_migrations(db).await?, before);
        db.execute(DbStatement::new("ALTER TABLE bcs_human_input_requests ADD INDEX idx_human_input_scope_status (reply_scope_key, status, deadline_ms, created_at)")).await?;
        apply_mysql_migration(db, &migrations[2], &plans[2]).await?;
        assert_human_input_scope_index(db, 700).await?;
        assert_eq!(human_input_column_metadata(db).await?, column_before);
        assert_scope_rows_and_uniqueness(db).await?;
        let after = load_applied_mysql_migrations(db).await?;
        assert_eq!(after.iter().filter(|row| row.version != 28).cloned().collect::<Vec<_>>(), before);
    ...
    if mysql_schema_migrations_exists(db).await? {
        db.execute(DbStatement::new("DELETE FROM bcs_schema_migrations WHERE version IN (1, 8, 28)")).await?;
