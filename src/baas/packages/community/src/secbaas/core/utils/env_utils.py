...
from secbaas.config import ConfigLoader, ConfigPath, get_config, get_config_by_path
...
def _get_deploy_env_var() -> str:
    cfg = get_config()
    env_var = get_config_by_path(cfg, ConfigPath.DEPLOY_ENV_VAR)
    if not env_var:
        raise RuntimeError(
            f"Config path '{ConfigPath.DEPLOY_ENV_VAR.value}' not set — "
            "deploy env var name is required"
        )
    return str(env_var)


def _get_env_or_empty() -> str:
    deploy_env_var = _get_deploy_env_var()
    return (
        os.getenv("SERVER_ENV")
        or os.getenv("REAL_SERVER_ENV")
        or (os.getenv(deploy_env_var) if deploy_env_var else "")
        or ""
    ).lower()


def is_empty_env() -> bool:
    env = _get_env_or_empty()
    return not env
