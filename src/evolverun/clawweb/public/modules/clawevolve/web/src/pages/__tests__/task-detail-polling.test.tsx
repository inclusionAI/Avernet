...
const api = vi.hoisted(() => ({ evolve: { capabilities: async () => ({ skillManagement: true, stageCustomization: true }), getTask: vi.fn(), listTaskLogArchives: vi.fn(), getTaskSkillDiff: vi.fn(), getSkillAsset: vi.fn(), getSkillAssetHistory: vi.fn(), getSkillVersionContent: vi.fn(), getSkillVersionDiff: vi.fn() } }))
...
describe('Task detail live updates', () => {
  ...
  it('filters Stage test preparation and artifacts without introducing a second visual section', async () => {
    ...
    const history = { ...task('completed', true), task_type: 'stage_test', config: { ...task('completed', true).config, stageTest: { stage: 'diagnose', mode: 'replace' } },
      steps: [step('PREP', 'skill_prepare'), { ...step('MAIN', 'stage_extension'), output: { summary: '前置完成结果' }, stageExtension: { stage: 'diagnose', mode: 'replace', implementationId: 'IMPL-HOST', displayName: '自定义诊断实现' } }, step('FINAL', 'skill_finalize')],
    ...
    const record = view.container.querySelector('#step-MAIN')!.textContent!
    expect(record.indexOf('历史回答')).toBeLessThan(record.indexOf('前置完成结果'))
