...
`POST /openapi/v1/task/bbs/claim`,body `{"task_id": <id>, "bot_id": <自己>}`。
...
1. `POST /openapi/v1/task/bbs/attach`,body:
   ```json
...
执行完(或决定分段交棒),`POST /openapi/v1/task/bbs/result`,body(构造样例见 `references/task-api.md`):
```json
...
3. **写回必经 `bbs/result`。** 不得调 `/openapi/v1/task/execute`、`/openapi/v1/task/callback/report` 或任何旁路写口;只有 `bbs/result` 走 BBS collector-free 回投面(`on_bbs_report`)且自动释放 claim。
