...
Success uses ApiResponse with data `{path, file_count, elapsed_ms}`. The path is the original request, count and elapsed are nonnegative integers. Relative paths use `workspace_root().parent`; contained absolute paths are accepted. Hidden files, ZIP files and each hardlink entry count once. Directories, symlinks and special files are excluded. Ignore rules do not apply. No contents or file lists are returned. Counts observe a live tree and are not atomic snapshots.
...
The worker opens `/`, then every configured-root and requested component using directory descriptors and O_NOFOLLOW. It retains and verifies ancestry, compares child inode/device identity and rejects observed directory mutations. Links in the configured root are forbidden too. Do not pass a symlink alias for the configured workspace; configure its actual absolute path. `..` components are rejected even if their normalized result would be contained. `.` is the root. Empty, NUL and >4096 character inputs are invalid; request_id is required and at most 128 characters.

Limits per Engine process: two concurrent workers, no pending queue (immediate 503 on saturation), ten seconds from admission through worker output. Timeout/cancellation terminates the process, allows 0.5 seconds to exit, then kills and reaps it before releasing capacity. Cleanup can add this grace interval to response time. No thread or process continues scanning after cleanup. Worker executable/module are fixed and user paths are argv values, never shell text.
