...
- **T4.5** **assert happy（AC-3/AC-4）**：分解树 root→N_overview→{N_market,N_tech,N_compete,N_customer}→N_practice_bbs→N_report（结构由 skill 产出）；run_mode/assignee 由搜推 skill 决出（多 bot 维度→`coop_group`+`GroupFormation`；其余 `single_bot`）；执行结果经 `POST /api/v1/collaboration/tasks/callback/report`→`on_report` 翻态/传播；终态 root `DONE`、图 `output.result=all_done`；`GET /openapi/v1/collaboration/tasks/dashboard` 反映分解树推进。
