...
class TestWssConnInfo:
    ...
    @pytest.mark.asyncio
    async def test_get_wss_conn_info(
        self, api: APITestHelper, created_bot: dict
    ) -> None:
        """GET /bots/{bot_uuid}/wss-conn returns WSS connection info."""
        response = await api.client.get(
            api.bot_wss_conn_url(created_bot["bot_uuid"]),
...
class TestCmdEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_post_cmd(self, api: APITestHelper, created_bot: dict) -> None:
        """POST /bots/{tenant}/{bot_uuid}/execute-command accepts a command body."""
        response = await api.client.post(
            api.bot_cmd_url(created_bot["bot_uuid"]),
            params=api.params(),
            json={"cmd": "ls"},
        )
