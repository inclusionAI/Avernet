...
function RepairDetail({
  ...
  return (
    <div className="mx-auto w-full max-w-[1440px] px-4 py-7 sm:px-6 lg:px-8">
      ...
      {(pendingContextCalls.length > 0 || relayNotice) && task.status === 'running' && <div role="status" className="mt-5 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800"><p className="font-medium">正在补充 Context</p><p className="mt-1 text-xs leading-5">{relayNotice || (ownerCanOperate ? '页面会自动使用当前登录态完成受控 OCB 请求，AIS 容器保持运行。' : '任务正在等待 Owner 页面使用登录态完成受控 OCB 请求，AIS 容器保持运行。')}</p></div>}
