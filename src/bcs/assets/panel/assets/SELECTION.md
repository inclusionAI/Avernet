...
| Interior wall | Original geometry | `scripts/generate-atlases.mjs` | Slate panels; replaces the removed Urban wall reference. |
| Interior floor | Original geometry | `scripts/generate-atlases.mjs` | Walnut parquet tile with staggered joins. |
| Table and directional chairs | RPG Urban Pack 1.0 + local bridge geometry | `Tiles/tile_0274.png` | The original stepped brass/felt table replaces the narrow source reference; chair directions use exact 90° rotations. |
...
No file from a rejected or deferred source is present under `assets/source` or `src/undercover-game/assets`.
