...
class TestGetSymlinkMappings:
    ...
    def test_local_path_with_absolute_path(self, skill_set_service, mock_skill_set_repo, mock_skill_repo):
        ...
        mock_skill_set_repo.get_skills_in_set.return_value = [
            {'id': '1', 'name': 'cct-zbb-instraction-query',
        ...
        symlinks = skill_set_service.get_symlink_mappings(user_id='100015', bolt_id='default')
    ...
    def test_git_path_standard_format(self, skill_set_service, mock_skill_set_repo, mock_skill_repo):
        """测试 git:// 标准格式 - 使用新接口的绝对路径软链"""
        mock_skill_set_repo.get_all_active_skill_sets.return_value = [
            {'id': '1', 'name': 'Test Skill Set', 'is_default': False}
        ]
        mock_skill_set_repo.get_skills_in_set.return_value = [
            {'id': '1', 'name': 'query-user-trade-record-list',
             'git_path': 'git://business/riskinsight/query-user-trade-record-list'}
        ]
        symlinks = skill_set_service.get_symlink_mappings(user_id='100015', bolt_id='default')
        assert len(symlinks) == 1
        assert symlinks[0].source.endswith("/skills-repo/business/riskinsight/query-user-trade-record-list")
        assert symlinks[0].target.endswith("/skills/query-user-trade-record-list")

    def test_local_path_with_trailing_slash(self, skill_set_service, mock_skill_set_repo, mock_skill_repo):
...
class TestAddRemoveSkillSync:
    ...
    @pytest.mark.asyncio
    async def test_add_skills_to_active_set_triggers_sync(self, skill_set_service, mock_skill_set_repo, mock_skill_repo):
        """测试：在激活的技能集中添加技能会触发软链同步到设备"""
        # Arrange: 设置技能集为激活状态（is_active=True）
        mock_skill_set_repo.get_by_id.return_value = {'id': '1', 'name': 'TestSet', 'is_default': False, 'is_active': True}
        mock_skill_set_repo.get_skills_in_set.return_value = []
        ...
        with patch.object(skill_set_service, '_sync_symlinks_to_device_if_needed') as mock_sync:
            with patch.object(skill_set_service.skill_service, 'activate_skill', new_callable=AsyncMock, return_value=True):
                with patch('agentclaw.community.core.skill_center.services.skill_set_service.SkillSetMetadataWriter'):
                    # Act
                    result = await skill_set_service.add_skills_to_set('1', ['123'], user_id='100015')
    ...
    @pytest.mark.asyncio
    async def test_add_skills_to_inactive_set_no_sync(self, skill_set_service, mock_skill_set_repo, mock_skill_repo):
        """测试：在非激活的技能集中添加技能不会触发同步"""
        # Arrange: 设置技能集为非激活状态（is_active=False）
        mock_skill_set_repo.get_by_id.return_value = {'id': '1', 'name': 'TestSet', 'is_default': False, 'is_active': False}
        mock_skill_set_repo.get_skills_in_set.return_value = []

        mock_skill_repo.get_by_id.return_value = {'id': '123', 'name': 'test-skill', 'bolt_id': 'test_bot'}

        with patch.object(skill_set_service, '_sync_symlinks_to_device_if_needed') as mock_sync:
            with patch('agentclaw.community.core.skill_center.services.skill_set_service.SkillSetMetadataWriter'):
                # Act
                await skill_set_service.add_skills_to_set('1', ['123'], user_id='100015')

        # Assert: 同步方法没有被调用
        mock_sync.assert_not_called()

    @pytest.mark.asyncio
    async def test_remove_skill_from_active_set_triggers_sync(self, skill_set_service, mock_skill_set_repo, mock_skill_repo):
        """测试：在激活的技能集中移除技能会触发软链同步（包括空列表情况）"""
        # Arrange
        mock_skill_set_repo.get_by_id.return_value = {'id': '1', 'name': 'TestSet', 'is_default': False}
        ...
        with patch.object(skill_set_service, '_sync_symlinks_to_device_if_needed') as mock_sync:
            with patch.object(skill_set_service.skill_service, 'deactivate_skill', new_callable=AsyncMock, return_value=True):
                with patch.object(skill_set_service.skill_service, 'get_link_name', return_value='business_test_skill'):
                    with patch('agentclaw.community.core.skill_center.services.skill_set_service.SkillSetMetadataWriter'):
                        # Act
                        result = await skill_set_service.remove_skill_from_set('1', '123', user_id='100015')

        # Assert: 即使移除后技能集为空，也会触发同步（清空设备软链）
        mock_sync.assert_called_once()
        assert result is True

    def test_sync_symlinks_with_device_proxy(self, skill_set_service, mock_skill_set_repo):
        ...
        with patch.object(skill_set_service, 'get_symlink_mappings', return_value=[]) as mock_get_mappings:
            # Act
            result = skill_set_service._sync_symlinks_to_device_if_needed(user_id='100015')
    ...
    def test_sync_skips_when_sync_returns_failure(self, skill_set_service):
        """测试：当同步失败时返回 False"""
        mock_device_sync = MagicMock()
        mock_device_sync.sync_symlinks.return_value = {'success': False, 'message': 'error'}
    ...
    @pytest.mark.asyncio
    async def test_add_skill_already_in_other_set_rejected(self, skill_set_service, mock_skill_set_repo, mock_skill_repo):
        """测试：同一 bot 下，一个 skill 不能同时属于多个 skill set"""
        # Target set we're adding to
        mock_skill_set_repo.get_by_id.return_value = {'id': '2', 'name': 'Set2', 'is_default': False, 'is_active': False}
        ...
        mock_skill_repo.get_by_id.return_value = {'id': '999', 'name': 'shared-skill', 'bolt_id': 'test_bot'}

        # list_skill_sets returns two sets: set '1' has the skill, set '2' is target
        with patch.object(skill_set_service, 'list_skill_sets', return_value=[
            {'id': '1', 'name': 'Set1'},
            {'id': '2', 'name': 'Set2'},
        ]):
            with patch('agentclaw.community.core.skill_center.services.skill_set_service.SkillSetMetadataWriter'):
                result = await skill_set_service.add_skills_to_set('2', ['999'], user_id='100015')
