...
async def revoke_authorized_app(
    ...
    **How much this withdraws depends on who asks**, and it has to: since a
    ...
    from this parameter.
    """
    # Follow-up (#951): the owner's withdrawal is all-or-nothing and should not
    # be. An owner wanting to cut off *one* colleague's delegation — the
    # colleague who left the team, say — currently has to withdraw every
    # delegation of that application on the bot, their own included. The record
    # supports the narrower operation already (`revoke` is keyed on the
    # delegating user); what is missing is a way to name whose delegation to
    # withdraw, which is a new public operation rather than a fix. Kept out of
    # the docstring because that text is published to external callers, and an
    # unshipped plan is not part of the contract they are reading.
    del principal
    resolved_owner, _ = resolve_delegable_bot(
        bot_service, collaborators, bot_id=bot_id, caller_id=user_id,
        owner_id=owner_id,
