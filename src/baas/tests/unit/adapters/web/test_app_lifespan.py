...
from secbaas.community.adapters.web.app import app, lifespan, load_config
from secbaas.community.config import Config


class TestAppNoCoreImport:
    """Verify app.py does not import from secbaas.community.core for lifecycle wiring."""

    def test_no_core_import_in_app(self):
        """WHEN inspecting app.py source, THEN no `from secbaas.community.core` import exists.

        Note: ``secbaas.community.core.utils`` is exempt — cross-layer utility allowed
        for adapters/plugins per architecture layer rules.
        """
        source = inspect.getsource(
            __import__("secbaas.community.adapters.web.app", fromlist=["app"])
        )
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom) and node.module:
                ...
                ) or node.module.startswith("secbaas.community.core.utils"), (
...
class TestLifespanOrder:
    ...
    async def test_lifespan_calls_initialize_then_shutdown(
        ...
        with (
            patch(
                "secbaas.community.adapters.web.app.load_config",
                return_value=minimal_config,
            ),
            patch(
                "secbaas.community.adapters.web.app.ApplicationContainer",
                return_value=container,
            ),
            patch(
                "secbaas.community.adapters.web.app.initialize_services",
                side_effect=mock_initialize,
            ),
            patch(
                "secbaas.community.adapters.web.app.shutdown_services",
                side_effect=mock_shutdown,
            ),
        ):
    ...
    async def test_shutdown_called_on_exception(self, app_mock, minimal_config):
        ...
        with (
            ...
            patch("secbaas.community.adapters.web.app.initialize_services"),
            patch(
                "secbaas.community.adapters.web.app.shutdown_services",
                side_effect=mock_shutdown,
            ),
...
class TestLoadConfig:
    ...
    def test_loads_config_and_returns_config_object(self, tmp_path):
        ...
        with patch(
            "secbaas.community.adapters.web.app.ConfigLoader.load",
