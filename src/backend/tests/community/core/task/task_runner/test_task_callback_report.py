    ...
    *,
    ...
    rd = (
        ...
        if run_detail is not None
            ...
            "run": {"status": "completed", "output": {"final": "ok"}},
            "nodes": [],
        }
    )
    gd = (
...
class _TaskStateErrorEngine(_RecordingEngine):
    ...
    async def on_report(self, patch: TaskNodePatch) -> TaskNodePatch:
        raise TaskStateError(
            "node in terminal state" if self._terminal else "bad transition"
        )
...
class _FakeRunInfoRepo:
    ...
    def __init__(
        self, by_session: dict[str, TaskNodeRunInfoRecord] | None = None
    ) -> None:
...
def _node(task_id: str, node_id: str, status: Status = Status.RUNNING) -> TaskNode:
    spec = TaskSpec(
        context=Context(background="", title="t"),
    ...
    return TaskNode(
        ...
        node_run_graph=None,  # node_run_graph 仅作引用,测试不读
    )
...
class _CallbackTestService(TaskService):
    ...
    def __init__(
        ...
        *,
        ...
        run_info_repo: _FakeRunInfoRepo,
    ) -> None:
        self._test_engine = engine
        super().__init__(
            graph,
            callback_repo=callback_repo,
            task_node_run_info_repo=run_info_repo,
        )
...
def _default_enricher():
    """真 ``CallbackDataEnricher``(BcsTokenProviderImpl);``enrich_bcn`` 短连 httpx 由 ``_install_fake_bcs`` 打桩。"""
    return CallbackDataEnricher(BcsTokenProviderImpl(base_url="http://bcs"))
...
class TestClawMind:
    ...
    def test_claw_mind_persists_full_fields_and_advances_nothing(self):
        ...
        assert (
            rec.status == "DONE"
        )  # flow_runs.status=succeeded → task Status.DONE(非顶层 node_succeeded)
        ...
        assert (
            eg["tasks"][0]["task_spec"]["context"]["title"] == "N1"
        )  # 无 node_title → 退 node_id
...
class TestBCNManagerWorker:
    ...
    @staticmethod
    def _mw_event(
        event_type: str, *, session_id="s-1", group_id="g1", task_id="task-1", data=None
    ) -> dict:
    ...
    def test_manager_worker_events_are_acknowledged_without_persistence(self):
        ...
        events = [
            self._mw_event(
                "group.created",
                data={"status": "active", "name": "G", "group_kind": "normal"},
            ),
            self._mw_event(
                "session.created", data={"status": "active", "session_kind": "task"}
            ),
            self._mw_event(
                "task.assigned",
                task_id="t-a",
...
class TestBCNStateMachine:
    ...
    @staticmethod
    def _sm_event(
        event_type: str, *, run_id="run-1", session_id="s-1", node_id=None, data=None
    ) -> dict:
    ...
    def test_node_completed_with_run_still_running_persists_but_no_converge(
        self, monkeypatch
    ):
        run_detail = {
        ...
        assert (
            rec.run_id == "run-1" and rec.node_id == ""
        )  # req4:node_id 恒空(忽略事件 data.node_id)
        assert rec.main_session_id == "s-1"
        ...
        assert (
            eg["tasks"][0]["task_spec"]["context"]["title"] == "Step1"
        )  # display_name
        assert eg["tasks"][0]["run_info"]["assignee"] == "b1"
        assert eg["tasks"][0]["run_info"]["extend_props"] == {
            "attempt": 1,
            "outcome": "success",
            "status": "completed",
        }
    ...
    def test_fetch_failure_falls_back_to_raw_cloudevent_but_still_converges(
        self, monkeypatch
    ):
    ...
    def test_bcs_non_200_fetch_falls_back_to_raw_event(self, monkeypatch):
        ...
        ev = self._sm_event(
            "state_machine.run.completed", data={"completed_at": "T", "output": {}}
        )
        svc, engine, repo, _ri = _make_svc()
        _run(
            _dispatch_call(
    ...
    def test_handled_event_without_node_uses_run_id_as_loop_task_id(self, monkeypatch):
        _install_fake_bcs(
            monkeypatch, run_detail={"run": {"status": "running"}, "nodes": []}
        )
        ev = self._sm_event(
...
def _parsed(
    event_type: str, *, task_id="t-a", session_id="s-1", group_id="g1", data=None
) -> dict:
    return {
        "event_id": "evt-1",
        "event_type": event_type,
...
class TestManagerWorkerMerge:
    ...
    def test_parse_returns_none_for_non_manager_worker_events(self):
        for et in (
            ...
            assert (
                parse_manager_worker_bcn(_bcn_event(et, scope={"group_id": "g"}))
                is None
            )
    ...
    def test_group_created_sets_group_status(self):
        merged = merge_manager_worker_execution_graph(
            None, _parsed("group.created", data={"status": "active", "name": "G"})
        )
    ...
    def test_session_created_defaults_active_status(self):
        merged = merge_manager_worker_execution_graph(
            None, _parsed("session.created", data={})
        )
    ...
    def test_task_assigned_then_completed_accumulates_in_order(self):
        state = merge_manager_worker_execution_graph(
            None,
            _parsed(
                "task.assigned",
                task_id="t-a",
        ...
        state = merge_manager_worker_execution_graph(
            ...
            _parsed(
                "task.completed",
                task_id="t-a",
                data={"result": {"size_bytes": 9}, "completed_at": "T"},
            ),
        )
    ...
    def test_task_completed_survives_late_assigned(self):
        ...
        state = merge_manager_worker_execution_graph(
            ...
            _parsed(
                "task.completed",
                task_id="t-a",
                data={"result": {"ok": 1}, "completed_at": "T"},
            ),
        )
    ...
    def test_different_tasks_do_not_clobber_each_other(self):
        ...
        state = merge_manager_worker_execution_graph(
            ...
            _parsed(
                "task.assigned",
                task_id="t-b",
                data={"manager_id": "m2", "worker_id": "w2"},
            ),
        )
...
class TestDispatchSelectionAndAuth:
    ...
    def test_claw_mind_routes_to_ingest_only(self):
        ...
        _run(
            _dispatch_call(
                _req(_CLAW_MIND_BODY), svc, auth, InMemoryCallbackCorrelationRegistry()
            )
        )
    ...
    def test_common_task_route_does_not_invoke_auth(self):
        ...
        body = {"task_id": "t1", "node_id": "c1", "status": "DONE", "output": {"r": 1}}
        svc, engine, _repo, _ri = _make_svc()
        auth = _RecordingAuth()
        _run(
            _dispatch_call(_req(body), svc, auth, InMemoryCallbackCorrelationRegistry())
        )
        assert auth.sources == []
        assert len(engine.reports) == 1
    ...
    def test_callback_report_never_calls_start_run(self, monkeypatch):
        ...
        for body in bodies:
            ...
            assert engine.starts == [], (
                f"start_run 不应在 /callback/report 被调用: {body}"
            )
