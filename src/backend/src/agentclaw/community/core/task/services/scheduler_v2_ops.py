"""v2 全生命周期 NodeType-aware tick(plan §3.1/§7.2/§12A,tasks T-16..T-20)。

从 ``task_scheduler.py`` 抽出以守 architecture 1000-line cap。混入
:class:`TaskScheduler`。**加性、门控**:仅当图含 v2 动作节点(任一
``node_type is not NodeType.DISPATCH``)时走 ``_tick_v2``;否则回退旧 tick,
保既存 30+ scheduler 测试不破。
...
def is_v2_graph(task: Task) -> bool:
    """门控:图含任一非 DISPATCH 动作节点 → v2 链路。

    旧 ``spawn_build_dag`` 产的节点 ``node_type`` 默认 DISPATCH → 判否 → 走旧 tick。
    v2 动作图含 BOT_SEARCH/DECOMPOSITION/... → 判是 → 走 ``_tick_v2``。
    """
    g = task.execution_graph
    if g is None:
        return False
    return any(n.node_type is not NodeType.DISPATCH for n in g.nodes)


class SchedulerV2OpsMixin:
    ...
    def _v2_children(self, task: Task, node_id: str) -> list[str]:
    ...
    def _v2_descendants(self, task: Task, node_id: str) -> list[str]:
        """全部后代(BFS,沿出边)。"""
        g = task.execution_graph
        if g is None:
            return []
    ...
    def _tick_v2(self, task: Task) -> dict:
        ...
        for n in list(g.nodes):
            if n.status is not NodeStatus.PENDING:
                continue
            if not self._v2_unlocked(task, n.node_id):
                continue
            acted = self._v2_advance_node(task, n)
            if acted:
                progressed = True
        ...
        task = self._svc.get(task_id)
        if self._v2_detect_and_aggregate(task):
            progressed = True

        task = self._svc.get(task_id)
        # task 终态判定:root 聚合 DONE → GOAL_VERIFY
        if self._v2_maybe_goal_verify(task):
            progressed = True
    ...
    def _v2_unlocked(self, task: Task, node_id: str) -> bool:
    ...
    def _v2_bot_search(self, task: Task, n: Node) -> bool:
        ...
        if rec.candidates:
            ...
            return True
        # 未匹配
        depth = self._v2_subtask_depth(task, n.node_id)
        if depth >= MAX_RECURSION_DEPTH:
            self._v2_add_child(task, n.node_id, f"{n.node_id}_hang", "mark-hang", NodeType.MARK_HANG)
            self._v2_set_done(task, n.node_id)
            ...
            return True
        self._v2_add_child(task, n.node_id, f"{n.node_id}_dec", "decomposition", NodeType.DECOMPOSITION)
        self._v2_set_done(task, n.node_id)
        logger.info("[SchedulerV2] bot-search miss node=%s → decomposition", n.node_id)
        return True

    def _v2_decomposition(self, task: Task, n: Node) -> bool:
        """分解:decompose_subtasks(spec, state with 父 depth)→ children BOT_SEARCH(depth+1)。

        children depth≥MAX → MARK_HANG(不落 children)。"""
        parent_depth = self._v2_subtask_depth(task, n.node_id)
    ...
    def _v2_dispatch(self, task: Task, n: Node) -> bool:
        """派发:claim + fire ExecutionPort(单 bot/群);Node→RUNNING + SubtaskState→RUNNING。

        完成由 skill 经 on_event 异步回投;此处只起跑(动作维度 RUNNING + 实体维度 fold)。"""
        task_id = task.id
        if n.assignee:
            try:
                self._svc.claim_node(task_id, n.node_id, n.assignee)
            except Exception:  # noqa: BLE001 — claim 已被占等,跳过本 tick
                return False
            rec = self._discover.recommend(task_id, n.node_id)
            if rec.run_mode is RunMode.COOP_GROUP and rec.candidates:
                self._execution.coop_group(task_id, n.node_id, [c.bot_id for c in rec.candidates])
            else:
                self._execution.dispatch_single_bot(task_id, n.node_id, n.assignee)
        else:
        ...
        if st is not None and st.status is not NodeStatus.RUNNING:
            ...
            self._svc._task_repo.save(fresh)  # noqa: SLF001
        logger.info("[SchedulerV2] dispatch node=%s assignee=%s", n.node_id, n.assignee)
        return True

    def _v2_mark_hang(self, task: Task, n: Node) -> bool:
    ...
    def _v2_bbs_dispatch(self, task: Task, n: Node) -> bool:
    ...
    def _v2_detect_and_aggregate(self, task: Task) -> bool:
        ...
        for n in list(g.nodes):
            ...
            if any(
                ...
                for cid in self._v2_children(task, n.node_id)
            ):
                continue
            # 收集后代 DISPATCH 叶子(实际执行的派发节点)
            leaves = [
                did for did in self._v2_descendants(task, n.node_id)
    ...
    def _v2_maybe_goal_verify(self, task: Task) -> bool:
...
__all__ = ["SchedulerV2OpsMixin", "is_v2_graph", "MAX_RECURSION_DEPTH"]
