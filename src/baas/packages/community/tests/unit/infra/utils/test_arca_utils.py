...
from secbaas.plugins.sandbox.utils import arca_utils
...
class TestConstants:
    ...
    def test_arca_proxy_base_url_pre(self):
        """WHEN accessing ARCA_PROXY_BASE_URL_PRE, THEN returns pre-prod URL."""
        assert (
            arca_utils.ARCA_PROXY_BASE_URL_PRE
            == "https://agentclawproxy-pre.alipay.com"
        )

    def test_arca_proxy_base_url_prod(self):
        """WHEN accessing ARCA_PROXY_BASE_URL_PROD, THEN returns prod URL."""
        assert (
            arca_utils.ARCA_PROXY_BASE_URL_PROD
            == "https://agentclawproxy-prod.alipay.com"
        )
...
class TestGetArcaProxyBaseUrl:
    ...
    def test_returns_pre_url_when_env_is_pre(self, monkeypatch):
        """WHEN current env is 'pre', THEN returns pre-prod URL."""
        monkeypatch.setenv("SERVER_ENV", "pre")
        result = arca_utils._get_arca_proxy_base_url()
        assert result == arca_utils.ARCA_PROXY_BASE_URL_PRE

    def test_returns_prod_url_when_env_is_prod(self, monkeypatch):
        """WHEN current env is 'prod', THEN returns prod URL."""
        monkeypatch.setenv("SERVER_ENV", "prod")
        result = arca_utils._get_arca_proxy_base_url()
        assert result == arca_utils.ARCA_PROXY_BASE_URL_PROD

    def test_returns_prod_url_when_env_is_gray(self, monkeypatch):
        """WHEN current env is 'gray' (which maps to prod), THEN returns prod URL."""
        monkeypatch.setenv("SERVER_ENV", "gray")
        result = arca_utils._get_arca_proxy_base_url()
        assert result == arca_utils.ARCA_PROXY_BASE_URL_PROD

    def test_returns_prod_url_when_env_is_dev(self, monkeypatch):
        """WHEN current env is 'dev' (not 'pre'), THEN returns prod URL."""
...
class TestGetArcaTarget:
    ...
    def test_formats_target_with_sandbox_id(self):
        """WHEN _get_arca_target is called with sandbox_id, THEN returns formatted target."""
        result = arca_utils._get_arca_target("sb-12345")
        assert result == f"ARCA_sb-12345:{arca_utils.BOLT_PORT}"

    def test_formats_target_with_numeric_sandbox_id(self):
        """WHEN called with numeric sandbox_id, THEN formats correctly."""
        result = arca_utils._get_arca_target("987654")
        assert result == "ARCA_987654:20003"

    def test_formats_target_with_hyphenated_sandbox_id(self):
        """WHEN called with hyphenated sandbox_id, THEN formats correctly."""
        result = arca_utils._get_arca_target("arca-sb-test-001")
...
class TestGetBoltUrl:
    ...
    def test_builds_correct_bolt_url(self, monkeypatch):
        """WHEN _get_bolt_url is called, THEN builds correct full URL."""
        monkeypatch.setenv("SERVER_ENV", "pre")
        result = arca_utils._get_bolt_url("sb-1", "/api/file/read")
        expected = "https://agentclawproxy-pre.alipay.com/proxypass/ARCA_sb-1:20003/api/file/read"
        assert result == expected

    def test_builds_correct_bolt_url_with_upload_path(self, monkeypatch):
        """WHEN _get_bolt_url is called in prod, THEN builds correct URL."""
        monkeypatch.setenv("SERVER_ENV", "prod")
        result = arca_utils._get_bolt_url("sandbox-x", "/api/file/upload")
        expected = "https://agentclawproxy-prod.alipay.com/proxypass/ARCA_sandbox-x:20003/api/file/upload"
        assert result == expected
...
class TestProxypassTokenAndHeaders:
    ...
    def test_get_proxypass_token_builds_jwt(self):
        """WHEN _get_proxypass_token is called, THEN builds JWT from MIST secret."""
        mock_secret_plugin = MagicMock()
        mock_secret_plugin.get_secret.return_value = "test-secret"
        mock_container = MagicMock()
        mock_container.plugins.secret_plugin.return_value = mock_secret_plugin

        with (
            patch("secbaas.bootstrap.get_container", return_value=mock_container),
            patch.object(
            ...
            result = arca_utils._get_proxypass_token("sb-42")
            assert result == "fake-jwt-token"
            mock_jwt.assert_called_once_with("ARCA_sb-42:20003", "test-secret", ttl=300)

    def test_get_proxypass_token_respects_custom_ttl(self):
        ...
        with (
            ...
            result = arca_utils._get_proxypass_token("sb-42", ttl=120)
            assert result == "fake-jwt-token"
            mock_jwt.assert_called_once_with("ARCA_sb-42:20003", "test-secret", ttl=120)

    def test_get_proxypass_headers_builds_correct_structure(self):
        ...
        with (
            ...
            headers = arca_utils._get_proxypass_headers("sb-1")
...
class TestUploadToArca:
    ...
    @pytest.mark.asyncio
    async def test_uploads_successfully(self, monkeypatch):
        ...
        with (
            ...
            result = await arca_utils.upload_to_arca(b"hello", "/data/test.txt", "sb-1")
            assert result == {"status": "ok", "path": "/data/test.txt"}

    @pytest.mark.asyncio
    async def test_raises_value_error_for_none_sandbox_id(self):
        """WHEN sandbox_id is None, THEN raises ValueError."""
        with pytest.raises(ValueError, match="Sandbox ID is required"):
            await arca_utils.upload_to_arca(b"data", "/path", None)
    ...
    async def test_raises_on_http_error(self, monkeypatch):
        ...
        with (
            ...
            with pytest.raises(httpx.HTTPStatusError):
                await arca_utils.upload_to_arca(b"data", "/path", "sb-1")
...
class TestArcaUtilsEndToEnd:
    ...
    def test_full_url_building_chain_pre_env(self, monkeypatch):
        """WHEN building URLs in pre env, THEN the full chain works correctly."""
        monkeypatch.setenv("SERVER_ENV", "pre")

        base = arca_utils._get_arca_proxy_base_url()
        target = arca_utils._get_arca_target("sb-test")
        bolt_url = arca_utils._get_bolt_url("sb-test", "/api/file/read")

        assert base == "https://agentclawproxy-pre.alipay.com"
        assert target == "ARCA_sb-test:20003"
        assert (
            bolt_url
            == "https://agentclawproxy-pre.alipay.com/proxypass/ARCA_sb-test:20003/api/file/read"
        )

    def test_full_url_building_chain_prod_env(self, monkeypatch):
        """WHEN building URLs in prod env, THEN the full chain works correctly."""
        monkeypatch.setenv("SERVER_ENV", "prod")

        base = arca_utils._get_arca_proxy_base_url()
        target = arca_utils._get_arca_target("prod-sb")
        bolt_url = arca_utils._get_bolt_url("prod-sb", "/api/file/upload")

        assert base == "https://agentclawproxy-prod.alipay.com"
        assert target == "ARCA_prod-sb:20003"
        assert bolt_url.startswith("https://agentclawproxy-prod.alipay.com")
