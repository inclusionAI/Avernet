/** Generic Evolve application configuration. Values are JSON; consumers own their schemas. */
...
const SELECT_COLUMNS = `id, config_key, config_json, version, enabled, description, updated_by, gmt_create, gmt_modified`;
...
export class AppConfigRepository {
  ...
  async listAll(enabledOnly = false): Promise<AppConfigRow[]> {
    const sql = enabledOnly
      ? `SELECT ${SELECT_COLUMNS} FROM ce_app_config WHERE enabled = 1 ORDER BY config_key`
      : `SELECT ${SELECT_COLUMNS} FROM ce_app_config ORDER BY config_key`;
  ...
  async findByKey(configKey: string): Promise<AppConfigRow | null> {
    const rows = await this.db.query<AppConfigRow>(
      `SELECT ${SELECT_COLUMNS} FROM ce_app_config WHERE config_key = ?`,
      [configKey],
    );
    return rows[0] ?? null;
  }
  ...
  async create(input: CreateAppConfigInput): Promise<AppConfigRow> {
    ...
    await this.db.exec(
      `INSERT INTO ce_app_config (config_key, config_json, version, enabled, description, updated_by, gmt_create, gmt_modified)
       VALUES (?, ?, 1, 1, ?, ?, ?, ?)`,
      [
        input.config_key,
        input.config_json,
  ...
  async update(configKey: string, input: UpdateAppConfigInput): Promise<AppConfigRow | null> {
    ...
    // Revision counter; application-layer gmt_modified (no trigger).
    sets.push("version = ?");
    ...
    await this.db.exec(
      `UPDATE ce_app_config SET ${sets.join(", ")} WHERE config_key = ?`,
      values,
    );
  ...
  async delete(configKey: string): Promise<boolean> {
    const result = await this.db.exec(
      "DELETE FROM ce_app_config WHERE config_key = ?",
      [configKey],
    );
