...
/** 列宽须与 BotTable 列顺序一致；团队空间额外显示 Owner 列。 */
const COLUMN_WIDTHS = ['w-[280px]', '', 'w-[96px]', 'w-[80px]', 'w-[220px]', 'w-[140px]', 'w-[360px]', 'w-[72px]'];
...
const BotTableSkeleton: React.FC<BotTableSkeletonProps> = ({ rows = 6, showOwner = false }) => {
  const widths = showOwner ? COLUMN_WIDTHS : COLUMN_WIDTHS.filter((_, index) => index !== 5);
  return (
