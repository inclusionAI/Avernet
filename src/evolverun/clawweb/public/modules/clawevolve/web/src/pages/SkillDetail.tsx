...
export default function SkillDetail() {
  ...
  return <div className="mx-auto max-w-6xl px-4 py-7 sm:px-6 lg:px-8">
    ...
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-medium text-blue-600">Skill 详情</p><h1 className="mt-1 text-2xl font-semibold text-gray-950">{asset?.name ?? '加载中…'}</h1>{asset && <p className="mt-1 font-mono text-xs text-gray-400">{asset.botId} / {asset.skillId}</p>}</div>{asset && <button onClick={() => navigate(`/evolve/new?type=full&target=skill&assetId=${encodeURIComponent(asset.assetId)}`)} className="rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-medium text-white">发起 Skill 自进化</button>}</div>
