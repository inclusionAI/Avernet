...
from secbaas.plugins.logger.bare import BareLoggerPlugin, _resolve_log_level
from secbaas.spi.logger._protocols import LoggerPlugin
...
class TestBaasLoggerPlugin:
    ...
    def test_configure_creates_file_handlers(self) -> None:
        ...
        with (
            patch("os.makedirs"),
            patch(
                "secbaas.plugins.logger.bare.TimedRotatingFileHandler"
            ) as mock_handler_cls,
        ):
    ...
    def test_configure_creates_root_stream_handler(self) -> None:
        ...
        with (
            patch("os.makedirs"),
            patch("secbaas.plugins.logger.bare.TimedRotatingFileHandler"),
        ):
            plugin.configure(app_name="test", log_dir="/tmp/test_logs")
    ...
    def test_trace_id_filter_injects_traceid(self) -> None:
        ...
        with patch("secbaas.spi.tracer.get_tracer_plugin", return_value=mock_tracer):
            from secbaas.plugins.logger.bare import _TraceIdFilter
