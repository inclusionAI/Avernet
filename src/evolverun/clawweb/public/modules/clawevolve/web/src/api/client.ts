...
export const api = {
  ...sharedApi,
  evolve: {
    capabilities: () => fetchJson<{ skillManagement: boolean; stageCustomization: boolean }>('/api/evolve/capabilities'),
    ...sharedApi.evolve,
    ...
    getStageFeedbackDownloadUrl(taskId: string, stepId: string, interactionId: string, fileIndex: number): Promise<{ url: string; filename: string; size: number }> {
      return fetchJson(`/api/evolve/tasks/${encodeURIComponent(taskId)}/steps/${encodeURIComponent(stepId)}/interactions/${encodeURIComponent(interactionId)}/files/${fileIndex}/download-url`)
    },
