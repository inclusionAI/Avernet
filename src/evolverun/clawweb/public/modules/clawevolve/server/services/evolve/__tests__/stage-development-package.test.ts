...
describe("Stage Skill developer package", () => {
  ...
  it("documents the optional platform feedback loop without adding a package manifest", async () => {
    const archive = await createStageDevelopmentPackage({
      stage: "hardening",
      mode: "replace",
      flow: "skill_hardening",
    });
    const zip = await JSZip.loadAsync(archive);

    expect(Object.keys(zip.files)).toEqual(["SKILL.md"]);
    const guide = await zip.file("SKILL.md")!.async("string");
    expect(guide).toContain("### 基于用户反馈继续处理（可选）");
    expect(guide).toContain('"action": "request_feedback"');
    expect(guide).toContain('"previous_result"');
    expect(guide).toContain('"user_feedback"');
    expect(guide).toContain('"round": 2');
    expect(guide).toContain('"path": "平台下载后的本地文件路径"');
    expect(guide).toContain("平台只会重新执行当前 Stage 实现");
    expect(guide).toContain("未返回 loop 时，本次结果按正常完成处理");
    expect(guide).not.toMatch(/implementation\.(json|ya?ml)|agent-task|contract\.json/);
  });
  ...
  it.each(["bot_evolution", "skill_evolution"] as const)("omits feedback Loop from %s Optimize replacement while retaining HITL", async (flow) => {
    const zip = await JSZip.loadAsync(await createStageDevelopmentPackage({ flow, stage: "optimize", mode: "replace" }));
    const guide = await zip.file("SKILL.md")!.async("string");
    expect(guide).toContain('"hitl": true');
    expect(guide).toContain("answer.answers");
    expect(guide).not.toMatch(/request_feedback|previous_result|user_feedback|loop\.round/);
    for (const mode of ["preprocess", "postprocess"] as const) {
      const extensionZip = await JSZip.loadAsync(await createStageDevelopmentPackage({ flow, stage: "optimize", mode }));
      expect(await extensionZip.file("SKILL.md")!.async("string")).toContain("request_feedback");
    }
  });
    ...
    for (const stage of officialStageContracts.stages.filter((item) =>
      ...
      for (const mode of stage.extensionModes) {
        it(`documents actual ${flow}/${stage.stage}/${mode} inputs and business output`, async () => {
          ...
          const nativeBusiness = mode === "replace" && ["diagnose", "plan", "optimize"].includes(stage.stage);
          if (!nativeBusiness) {
          ...
          expect(guide).toContain('"format": "form"');
          expect(guide).toContain('"format": "text"');
          expect(blocks).toContainEqual({ hitl: true, question: {
            format: "text", tag: "execution_confirmation", contentFile: "confirmation.md",
          } });
          expect(guide).toContain("contentFile 与 content 不能同时提供");
          expect(guide).toContain(nativeBusiness ? "hitl.answer.content" : "human_input.content");
          expect(guide).not.toContain("format 固定为 form");
          expect(guide).toContain(nativeBusiness ? "answer.answers" : "human_input.answers");
          expect(guide).not.toContain("<form>");
