...
function needsExplicitInjectReplay(entry: SessionHistoryMessage): boolean {
  ...
  // An inject received before the first model turn has no native Claude JSONL
  // session to append to. Replay it exactly once as ordinary model input.
...
export const handleChatSend: ChatHandler = async (ctx, frame, deps) => {
  ...
  const systemPrompt = [deps.systemPromptPrefix, conversationContext].filter(Boolean).join('\n\n');
