...
import type { OcbRequestIdentity, OcbSpacePort } from "../internal/module-api.js";

export function spaceRequestIdentity(req: Request): OcbRequestIdentity | null {
  const userId = String(req.header("X-User-Id") ?? "").trim();
  return userId ? { userId, authorization: req.header("Authorization"), cookie: req.header("Cookie") } : null;
}

export function createEvolveSpacesRouter(port?: OcbSpacePort): Router {
  const router = Router();
  router.get("/spaces", asyncHandler(async (req, res) => {
    ...
    if (!port) { res.status(503).json({ error: "OCB 空间服务未配置" }); return; }
