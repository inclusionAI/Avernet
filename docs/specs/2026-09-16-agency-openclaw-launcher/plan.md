...
## Follow-up: standalone third-party launcher and team selection (2026-09-21)

User-authorized scope: move the complete launcher, helpers, tests and usage guide
into `src/bcs/third-party/agency-agent`, without importing or invoking the plugin's
install-instructions scripts. No BCS wire/protocol or runtime plugin changes.

- [x] Add regression tests for copying/running the directory on its own, engine
  admission, renamed switches, team/profile parsing and sorted team expansion.
- [x] Move the implementation and tests; leave the legacy single-Bot installer
  untouched and update its documentation links to the standalone entry point.
- [x] Add `--engine` (default/only supported: openclaw), `--overwrite-profile`,
  `--reregister`, and repeatable `--team`; remove the retired switch spellings.
  Qualified selectors normalize to the same Markdown-relative identity; mixed
  profile/team selections form a deterministic first-selected deduplicated list.
- [x] Use `~/.avernet/bcs/third-party` for persistent state, with the source cache
  at `agency-agent`. Preserve existing path-derived instance IDs and assigned
  ports across restarts and single-profile/team selection. Explicit old state
  directories remain reusable; do not silently move user credentials/workspaces.
- [x] Run standalone tests, the old installer's regression, lint, syntax, secret
  scan and source-size checks; document compatibility and unverified live paths.

### Verification (2026-09-21)

- Standalone suite: **46 tests passed** (original lifecycle/interaction regressions
  plus directory-copy execution, engine validation, team/profile selection,
  deduplication, default layout, legacy record compatibility and directory reuse).
- Original plugin installer: **6 cases passed**, production install.sh unchanged.
- Ruff, Bash syntax, Python compilation, added-file whitespace, repository secret
  scanner and file-size checks passed. Largest touched source: 491 lines.
- Read-only parse of the local agency-agents engineering team loaded 64 profiles
  with 64 distinct stable instance IDs, including both requested example roles.
- During validation the existing global re-registration test intermittently failed
  on a loopback HTTP request. Temporary diagnostic wrappers did not reproduce it
  in 20 consecutive attempts; the final uninstrumented full suite passed. No
  production retry or assertion was weakened to hide the event.
- No real team, Gateway, BCS registration or model call was performed for this
  change. No user state was moved; no Rust code changed and Cargo/E2E stack gates
  were not run. Changes are left uncommitted on the current feature branch.

## Follow-up: confirm team launches above five agents (2026-09-21)

- Team selections now show the total expanded/deduplicated count and ask once with
  default No when more than five agents are selected, including existing agents
  and explicit profiles outside the teams. The prompt precedes instance changes,
  profile/BCS identity prompts, plugin installs and registrations.
- No/empty/EOF cancels without instance changes; non-interactive runs fail closed.
  The overwrite/reregister flags do not grant consent to a large-team launch.
  Exactly five agents and profile-only commands keep their previous behavior.
- Red/green regression confirmed the prompt was absent before the change. Seven
  new tests cover the threshold, deduplication, mixed selection counts, acceptance,
  cancellation, non-interactive refusal and preservation of existing state.
- Final verification: **53 standalone tests passed**, **6 legacy installer cases
  passed**, Ruff, syntax, source-size, whitespace and credential checks passed.
  No real BCS registration, Gateway launch or model call was performed.

## Follow-up: colored output and bounded parallel startup (2026-09-21)

Approved design: terminal-only status colors; default concurrency 4 with
`--parallel 1` restoring serial behavior. User confirmations and BCS registration
remain serial, preserving the existing batch re-registration safety barrier.

- [x] Add red tests that measure real executable overlap and concurrency limits,
  terminal versus pipe/NO_COLOR output, install failures and Ctrl+C cancellation.
- [x] Add standalone `agency_console.py` and `agency_parallel.py`; keep the whole
  bundle independent of Avernet internals and existing install scripts.
- [x] Make OpenClaw command wait loops cancellation-aware and serialize spawning
  against cancellation. Join all workers before cleaning up owned Gateways.
- [x] Run full regression and finish negative-path verification, docs, lint and
  source/credential checks. No real BCS or model requests are part of validation.

### Verification (2026-09-21)

- Final full suite: **66 tests passed**; the original installer: **6 cases passed**.
  Ruff, Bash/Python syntax, repository credential scanner, whitespace and source
  size checks passed (largest touched source: 493 lines).
- Executable tracing proved overlap and upper bounds for default 4/custom 2
  workers and serial behavior for 1. Phase barriers preserve ordered registration
  after all installs; cancellation tests verify no queued starts or surviving
  install/probe/Gateway processes.
- Real PTYs verified color output; NO_COLOR, TERM=dumb and pipe output stay plain.
  Output writes are synchronized, JSON and private diagnostics remain unstyled.
- Stress testing exposed a process-exit/signal race on macOS. Cleanup now reaps
  exited leaders, distinguishes real permission errors, and sends TERM once per
  Gateway group before reaping. Twenty repeated six-agent start/stop cycles passed.
- Captured the previously intermittent mock HTTP failure as ConnectionResetError:
  the test server closed /register connections without draining their POST body.
  Fixed the server fixture (not production retries), eliminating that source of
  TCP resets. Metadata responses now match Bot names rather than arrival order.
- No real agent instances, remote BCS registrations or model calls were performed;
  changes remain uncommitted on the current feature branch.

## Follow-up: engine-scoped state and offline legacy migration (2026-09-21)

Approved layout: `~/.avernet/bcs/agency-agent/agency-agent` is the shared source
checkout; instances and runtime locks belong under `<root>/<engine>/`. Profile
...
- Final suite: **77 tests passed**; legacy installer: **6 cases passed**; Ruff,
  Bash/Python syntax, repository credential scanning, whitespace and source-size
  checks passed (largest modified source is 494 lines).
