...
async fn message_less_final_fails_attempt_and_schedules_retry() {
    ...
    let started = runtime
        .start_state_machine_run(StartStateMachineRunCommand {
            ...
            definition: None,
            definition_ref: None,
            input: json!({"question": "silence should retry"}),
            caller_id: None,
        })
        .await
        .expect("start run");
    ...
    let frontend_commands = frontend_delivery.commands.lock().await;
    assert_eq!(frontend_commands.len(), 2);
    let silent_event: Value =
        serde_json::from_str(&frontend_commands[1].event_json).expect("silent final event json");
    assert_eq!(silent_event["payload"]["state"], "final");
    assert_eq!(silent_event["payload"]["stop_reason"], "silent");
    assert!(silent_event["payload"].get("message").is_none());
