...
impl InteractionService for InteractionManagement {
    ...
    async fn resolve(
        ...
        if let Err(message) = validate_resolution(&record, &command.resolution) {
            warn!(
                bcs_run_id = %record.key.bcs_run_id,
            ...
            return Err(InteractionServiceError::InvalidRequest(message));
        }

        // 前端 resolve 只发 values；BCS 对照 requested options 把值分类为
        // Provider 所需的 values/customValues，并补入权威 question/header。
        // fingerprint 与 Provider 转发均使用规范化后的版本。
...
fn validate_questions(payload: &Value) -> Result<(), String> {
    ...
    for question in questions {
        ...
        if let Some(options) = options {
            ...
            {
                return Err("ask_user allowOther must be a boolean when present".to_string());
            }
...
fn validate_ask_user_resolution(
    ...
    for question in questions {
        ...
        if answer.get("customValues").is_some() {
            return Err(format!(
                "ask_user answer {question_id} must put submitted selections in values; customValues is generated by BCS"
            ));
        }
        ...
        if let Some(options) = question.get("options").and_then(Value::as_array) {
            ...
            if has_custom_value && !allow_other {
                return Err(format!(
                    "ask_user question {question_id} does not allow custom answers; select one of the provided options"
                ));
            }
        } else if values.len() > 1 {
            return Err(format!(
                "free-text answer {question_id} accepts at most one value"
            ));
        }
    }
    Ok(())
...
/// 前端 resolve 只发 `values`。对于选择题，BCS 用 requested options 的 value
/// 精确匹配并拆成声明过的 `values` 与自定义的 `customValues`；自由文本题不拆分。
/// question/header 始终由 BCS 用权威存储值覆盖，header 缺失时保持缺失，不从
/// questionId 或前端输入合成。canonical resolution 同时用于 fingerprint 与
/// Provider 转发。cancel / exec / mode_switch 原样返回。
fn augment_ask_user_resolution(record: &InteractionRecord, mut resolution: Value) -> Value {
    ...
    let Some(answers) = resolution.get_mut("answers").and_then(Value::as_object_mut) else {
        return resolution;
    };
    for question in questions {
        ...
        if let Some(answer) = answers.get_mut(question_id).and_then(Value::as_object_mut) {
            ...
            answer.remove("customValues");
            if let (Some(options), Some(values)) = (
                question.get("options").and_then(Value::as_array),
                answer.get("values").and_then(Value::as_array),
            ) {
                let offered = options
                    .iter()
                    .filter_map(|option| option.get("value").and_then(Value::as_str))
                    .collect::<HashSet<_>>();
                let (declared_values, custom_values): (Vec<_>, Vec<_>) = values
                    .iter()
                    .cloned()
                    .partition(|value| value.as_str().is_some_and(|value| offered.contains(value)));
                answer.insert("values".to_string(), Value::Array(declared_values));
                if !custom_values.is_empty() {
                    answer.insert("customValues".to_string(), Value::Array(custom_values));
                }
            }
            answer.insert(
                "question".to_string(),
...
mod tests {
    ...
    #[tokio::test]
    async fn ask_user_submit_accepts_empty_array_as_explicit_skip() {
        let (service, _store, provider, _frontend) = service(true);
        ...
        ask.payload = json!({
            ...
            "questions":[
                ...
                },
                {
                    ...
                    "question":"When should deletion happen?",
                    "options":[
                        {"value":"人确认后才删","label":"人确认后才删"},
                        {"value":"自动删除","label":"自动删除"}
                    ]
                }
            ]
        });
    ...
    #[tokio::test]
    async fn ask_user_submit_rejects_missing_non_string_and_blank_values() {
        for (index, (invalid_answer, expected_message)) in [
            (
                json!({}),
                "ask_user answer question_1 requires values",
            ),
            (
                json!({"values":[null]}),
                "ask_user answer question_1 values must be non-empty strings",
            ),
            (
                json!({"values":[""]}),
                "ask_user answer question_1 values must be non-empty strings",
        ...
        {
            let (service, _store, provider, _frontend) = service(true);
            let interaction_id = format!("ask-invalid-value-{index}");
            ...
            let mut command = resolve(&interaction_id, &format!("idem-invalid-value-{index}"));
            command.resolution = json!({
                "action":"submit",
                "answers":{"question_1":invalid_answer}
            });

            assert_eq!(
                service.resolve(command).await.unwrap_err(),
                InteractionServiceError::InvalidRequest(expected_message.to_string())
            );
            assert!(provider.calls.lock().await.is_empty());
        }
    }
    ...
    async fn ask_user_submit_defaults_missing_allow_other_to_true() {
        ...
        command.resolution = json!({
            ...
            "answers":{
                "target":{
                    "values":["private cloud"]
                },
                "components":{
                    "values":["web", "scheduler"]
                }
            }
        });
    ...
    #[tokio::test]
    async fn ask_user_submit_rejects_custom_value_when_allow_other_is_false_and_logs_context() {
        let (service, _store, provider, _frontend) = service(true);
        let mut ask = requested("ask-disallowed");
        ask.kind = InteractionKind::AskUser;
        ask.payload = json!({
            "runId":"provider-run-1",
            "phase":"requested",
            "interactionId":"ask-disallowed",
            "kind":"ask_user",
            "questions":[{
                "questionId":"target",
                "question":"Where?",
        ...
        let mut command = resolve("ask-disallowed", "idem-invalid-custom");
        command.resolution = json!({
            "action":"submit",
        ...
        assert_eq!(
            result.unwrap_err(),
            InteractionServiceError::InvalidRequest(
        ...
        for expected in [
            "interaction resolution rejected",
            "bcs_run_id=bcs-run-1",
            "interaction_id=ask-disallowed",
            "bcs_session_id=session-1",
            "group_id=group-1",
            "bot_id=bot-1",
            "provider_run_id=provider-run-1",
            "resolver_actor_id=human_1",
            "ask_user question target does not allow custom answers; select one of the provided options",
        ] {
            assert!(
