...
| 响应头 | `Deprecation: Sat, 15 Aug 2026 00:00:00 GMT` 与 `Sunset: Sun, 15 Aug 2027 00:00:00 GMT`（RFC 9745 / RFC 8594），在该地址的**每一个**响应上，包括失败响应 |
...
- **2026-08-15** —— **Agent 在前的寻址。** 每个带 Agent 作用域的操作都迁到
  ...
  `session_key`。`TODO(#960)` 随之关闭：Agent 都在路径上之后，`require_granted_bot` 为
  每一个操作检查授权，不再有 handler 自查。带 Agent 作用域的组件名离开了 `{bot_id}` 那
