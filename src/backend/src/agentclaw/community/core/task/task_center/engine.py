...
_STATIC_MOCK_SUMMARY: dict[str, str] = {
    ...
    "launch": (
        "投放实施执行报告(18周年店庆,活动窗口2026.10.15-11.15共32天,"
        "目标新客到店1500/券核销≥1000/护理售卖200/新增会员800,促销预算≤20万)。"
        "一、自主授权范围内已执行投放(均记实施记录):"
        "①体验券分批投放——首批50张大额体验券(成本25/售价48达标、补贴29%≤30%),"
        "线上社群推送+线下传单同步,按剪发位产能分批放量冲1500新客,已记批次/数量/渠道/时间;"
        "②传单制作与分发——体验券+护理套餐简介,门店周边3公里,已执行;"
        "③展位布置——门店入口展位+易拉宝/展示架,营业时段10:00-22:00内,已执行;"
        "④线上推送——社群定时推送体验券活动(10:00-12:00/14:00-16:00),已执行。"
        "二、需审批项(已挂起未放量):护理套餐放量(需店主审批)、扩产能临时技师+2(需店主审批),均已提交审批申请暂未执行。"
        "三、待取舍项(已标暂停):低价引流款低于毛利底线,待店主决定是否保留。"
        "四、能力缺口(unhandled_tasks→BBS研发接力):到店核销防重复领工具缺失——"
        "现有发券与核销工具无法可靠判断同一用户是否已领取过同类活动券,影响核销准确性、"
        "重复领券、促销预算超支与核销ROI统计,需补齐扫码核销+领券去重+核销记录回传能力,"
        "当前无系统支持、不假装已完成,转BBS由研发bot接力补齐。"
        "结论:自主实施部分已完成并记实施记录,能力缺口交BBS研发接力,需审批/待取舍项按授权边界挂起暂停。"
    ),
}
