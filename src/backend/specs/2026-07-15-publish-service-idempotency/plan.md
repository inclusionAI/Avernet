...
- **B. Release legs** — first/upgrade release (verify + online) onto the
  runner; approve consolidation (remove the internal approve in
  `BotBuildService.release()`; approve failures become step failures);
  `retry()` decision from ledger state.
...
| `state` | varchar(32) | `PENDING → ID_RECORDED → APPROVED → COMPLETED`, plus `FAILED`, `ABANDONED` |
...
| Destroy-by-stage (incl. after-online verify teardown) | stop → approve → binding release, best-effort | `destroy_stage` runner op so approve/binding-release are resumable steps |
...
| `retry()` | `is_online_release_recorded` heuristic | reads the online op's ledger state: `ID_RECORDED`+approved/terminal ⇒ BaaS-restart branch; `PENDING`/unapproved ⇒ resume the op (which re-drives approve rather than blindly restarting) |
...
BaaS logs), and per-step suffix for the follow-up call:
`...{base}.approve`. Deterministic per logical operation, distinct per step,
...
- **Crash-window harness** (`tests/community/core/service_bot/services/test_publish_crash_windows.py`):
  ...
  the workflow id is in the ledger, approve was called at most once and the
  workflow ended approved, and the publish record reached its expected
  status. Existing-bot ops also get an "in-doubt + already-terminal" case
