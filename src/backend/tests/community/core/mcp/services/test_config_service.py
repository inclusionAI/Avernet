...
class TestMCPConfigServiceBuildPayload:
    ...
    def test_validate_user_update_checks_owner_bots_outside_the_delivery_entity(self):
        center = MagicMock()
        center.get_mcp_detail.return_value = {
            "serverCode": "mcp.test", "runMode": "REMOTE",
            "endpoints": [{"env": "PROD", "networkType": "OFFICE", "transportProtocol": "SSE"}],
        }
        bot_repo = MagicMock()
        bot_repo.list_live_bot_ids_by_owner.return_value = ["personal-bot"]
        bot_repo.get_by_id_and_owner.return_value = {
            "bot_id": "personal-bot", "owner_id": "caller", "active_engine": "openclaw",
        }
        bot_repo.list_by_entity.return_value = (1, [{
            "bot_id": "team-bot", "owner_id": "team-owner", "active_engine": "openclaw",
        }])
        ...
        capability_reader.effective_mcp_server_codes.return_value = frozenset({"mcp.test"})
        bot_configs = MagicMock()
        bot_configs.list_by_owner_and_server_code.side_effect = lambda **kwargs: {
            "team-bot": {"endpoint_env": "PROD", "transport_protocol": "SSE"}
            if kwargs["owner_id"] == "team-owner" else {}
        }
        user_repo = MagicMock()
        user_repo.get_by_user_and_server_code.return_value = None
        svc = MCPConfigService(
            user_mcp_config_repo=user_repo, bot_mcp_config_repo=bot_configs,
            mcp_center=center, bot_repo=bot_repo, capability_reader=capability_reader,
            mcp_runtime_credentials=McpRuntimeCredentialsConfig(), secret_resolver=MagicMock(),
        )

        result = svc.validate_user_config_update(
            user_id="caller", server_code="mcp.test", api_key=None, headers=None,
            endpoint_env="PRE", transport_protocol=None,
            entity_id="team-42", entity_type="team",
        )

        assert result["valid"] is False
        assert "personal-bot" in result["error"]
