"""Unified Skill / SkillSet repositories (prod ZDAS + local SQLite).
...
This is a faithful ORM port of the **prod** ``ZDASSkillRepository`` /
``ZDASSkillSetRepository`` — prod behavior is the contract and
...
def _skill_to_dict(s) -> dict:
    """Reproduces ZDASSkillRepository._row_to_dict from a Skill ORM row."""
    git_path = s.git_path
...
def _skill_set_to_dict(ss) -> dict:
    """Reproduces ZDASSkillSetRepository._row_to_dict from a SkillSet row."""
    return {
        "id": str(ss.id),
        "name": ss.name,
...
class SkillRepository:
    ...
    def delete_bot_local_skill(
        ...
        *,
        ...
        """Atomically delete one Local Skill and enqueue its quarantine purge."""
        from agentclaw.community.plugins.local.sqlite_models import (
            DefaultSkillsetSkillExclusion,
        )
        ...
        with self._db.transactional_orm_session() as db:
            ...
            self._delete_skill_set_associations(db, int(skill_id))
            db.delete(skill)
            cleanup = LocalSkillCleanupWorkModel(
                env=get_current_env(),
                owner_id=owner_id,
                bot_id=bot_id,
                skill_id=int(skill_id),
                package_locator=quarantine_locator,
                package_locator_hash=sha256(quarantine_locator.encode("utf-8")).hexdigest(),
