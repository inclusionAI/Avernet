...
def test_resolve_bracketed_ipv6_loopback_target_wins_over_baas_url():
    """A bracketed IPv6 loopback target classifies the same as 127.0.0.1
    (shared is_loopback_target) and wins over the BaaS facade URL."""
    transport = InMemoryDeviceAdapterTransport()
    resolved = transport._resolve_adapter_url({
        "url": "http://127.0.0.1:8890/api/v1/bots/team_claw/BOT-x/invoke-http/20003",
        "target": "[::1]:20015",
    })
    assert resolved == "http://[::1]:20015"
