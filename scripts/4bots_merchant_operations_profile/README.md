...
```text
私聊建账并一次补齐人类专属决定 → manager-worker 首轮分工并形成三份五行业务卡/交接 → 公开候选与初始问题集
→ Manager 按本轮 Worker 回执动态编排当前 session 有界 one-shot SOP → 运行内由店长自治补齐信息、三方复核并收敛契约
→ 四项专业检查全部通过后店主 HumanInput 最终验收；否则完整输出阻断结果 → 可选持久执行群 → 复盘/再协商
```

完整任务已要求多 Agent 协商和可执行交付时，店长应主动推进到一次性 SOP，不要求店主逐步提示“拉群”或“建自定义协作”。one-shot 运行中不插入人类问题：授权内由店长按保守、可逆原则决定，执行时事实转成带失败动作的前置条件，无法安全收口时以 BLOCKED 完整结束。只有明确需要跨日运行、重复调用或长期复用时才创建持久执行群。
...
- 单服务数据回归输入为需求 5,400 分钟、同口径可用产能 6,720 分钟时，结果必须是剩余 1,320 分钟、缺口 0；但若同一批员工/时段另有护理增量 2,160 分钟，则整体需求必须合计为 7,560、缺口 840，不能把剪发与护理当独立产能池后返回 PASS。不得写“缺口 -1,320”，也不得在没有排期证据时把 60 天完整义务按 `1/3` 推成首周需求。供应链缺少计划级 SKU、授权渠道或品质标准时不得 PASS；实际批次可作为收货前置条件，缺工作日历时只保留相对交期，不生成具体日期。
...
- manager 生成公开候选版本，并为当前回执或 carry-forward 保存来源证据。“通过”可以携带字段完整的执行前置条件和监控项，但不能与硬阻断、未决 manager/owner 决定或失败校验并存。所有店主专属决定必须在 run 前一次补齐；运行中不得新建人类问题。
...
- 超时回归：`runtime.state_machine.defaults.node_timeout_ms` 和每个 bot_task 的有效值都必须至少 300000ms，HumanInput 至少 600000ms；故意放入 60000/120000/180000 时应在本地预检失败。accepted/blocked marker 必须是工具零调用单行节点，长总结只能放在唯一 final output。
...
- Human 时点回归：店主专属决定只允许在 run 前补齐；HumanInput 只位于全部专业节点完成且最终就绪之后。run 可以先启动并完成专业节点，店主随后加入群做末端验收；不得绑定 `human_001`，也不得用运行前普通群聊的“接受”预填 HumanInput。`collaborate run` 必须是启动激活的最后一次工具调用，提交后立刻结束回复，释放 manager session 给入口节点。
...
- 缺少目标、超授权阈值或店主独有的关键事实时，店长在 one-shot 启动前一次询问；缺少可代理的经营信息时，优先由 owner Agent 补充、店长在授权包络内保守决策，或转为带失败动作的执行前置条件。平台行业基准只能回答其明确覆盖的聚合指标，不能给当前门店字段填空。
