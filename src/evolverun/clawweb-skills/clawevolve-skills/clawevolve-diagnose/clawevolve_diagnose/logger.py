...
_FIELD_VALUE_LIMIT = 6000


def configure(log_file: str | Path | None = None, secrets: Iterable[str] | None = None) -> None:
    """Configure diagnose progress logging.

    Logs are written to stderr by default and, when a path is provided, also to
    a run-local log file.  Secret values are redacted before output.
    """
    ...
    with _LOCK:
        ...
        if _LOG_FILE:
            try:
                ...
                # Logging must never break the skill pipeline.
                pass
...
def _format_line(level: str, message: str, fields: dict[str, object]) -> str:
    ...
    if fields:
        ...
        suffix = " " + " ".join(parts)
    return _redact(f"{ts} [clawevolve-diagnose] {level.upper()} {message}{suffix}")
...
def _safe_value(value: object) -> str:
    ...
    if len(text) > _FIELD_VALUE_LIMIT:
        return text[:_FIELD_VALUE_LIMIT] + f"...<truncated {len(text) - _FIELD_VALUE_LIMIT} chars>"
    return text
