...
| T4 | `POST /openapi/v1/task/bbs/claim` 路由 + `BbsClaimDTO` | T3 | 路由 200,第二 claim 409 | ☐ |
...
| T6 | `POST /openapi/v1/task/bbs/attach` 路由 + `BbsAttachDTO` | T5 | 路由返回 `node_id`;非 owner/前提不满足 409 | ☐ |
...
| T8 | `POST /openapi/v1/task/bbs/result` 路由 + `BbsResultDTO`/`AcceptanceResultDTO` | T7 | root_verified DONE;非 owner 409 | ☐ |
