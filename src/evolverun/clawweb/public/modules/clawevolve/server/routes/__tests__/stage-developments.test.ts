...
describe("independent Stage development record", () => {
  ...
  it("freezes the new Plan business contract from the owned development without upgrading old versions", async () => {
    ...
    for (const flow of ["skill_evolution", "bot_evolution"]) {
      ...
      expect(current.staticValidation.executionContract).toBe("clawevolve.plan-business/v1");
    ...
    for (const flow of ["skill_evolution", "bot_evolution"]) {
      for (const [stage, name] of [["diagnose", "诊断"], ["plan", "规划"], ["optimize", "优化"]]) {
        for (const [mode, suffix] of [["preprocess", "前置"], ["postprocess", "后置"], ["replace", ""]]) {
          ...
          const planBusiness = stage === "plan" && mode === "replace";
          if (planBusiness) {
          ...
          if (mode === "replace" && stage === "diagnose") {
            expect(input.phase).toBe("session_analysis");
            expect(input.input.session).toHaveProperty("session_id");
          }
          if (mode === "replace" && stage === "optimize") {
            expect(Object.keys(input.artifacts)).toEqual(["tune_report.md", "changed_files.txt", "diff.patch", "change_manifest.json"]);
            expect(guide).not.toContain("request_feedback");
          }
          expect(Boolean(input.target_skill)).toBe(flow === "skill_evolution" && mode !== "replace");
  ...
  it.each(
    ...
    if (mode === "replace") {
      if (stage === "plan") expect(inputs[0]).toHaveProperty("phase", "discovery");
      if (stage === "diagnose") expect(inputs[0]).toHaveProperty("phase", "session_analysis");
      if (stage === "optimize") expect(inputs[0]).toHaveProperty("artifacts");
  ...
  it("delivers the three native Plan business input/output pairs through the developer package route", async () => {
    const test = await setup();
    ...
    expect(outputs[2]).toEqual({ goal_analysis: expect.any(Object), prospective_cases: [expect.any(Object)], discovery: expect.any(Object) });
    expect(guide).not.toContain("objective_document");
    expect(guide).not.toContain("spec_document");
    expect(guide).toContain("目标与方案文档渲染");
    expect(guide).toContain("loop.previous_result");
    expect(guide).toContain("contentFile");
  });
