...
describe('profile system prompt expansion', () => {
  ...
  it('expands only relative Markdown imports and returns non-content metadata', () => {
  ...
  it('rejects imports outside profile root, cycles, and nesting deeper than five', () => {
    fs.writeFileSync(path.join(root, 'outside.md'), 'outside', 'utf8');
    write('CLAUDE.md', '@../outside.md');
    assert.throws(() => resolveSystemPromptFile(path.join(profile, 'CLAUDE.md'), profile), /relative path inside the configured profile root/);

    write('CLAUDE.md', '@WORKFLOW.md');
    write('WORKFLOW.md', '@CLAUDE.md');
    assert.throws(() => resolveSystemPromptFile(path.join(profile, 'CLAUDE.md'), profile), /cycle/);
    ...
    write('E.md', '@F.md');
    write('F.md', 'too deep');
    assert.throws(() => resolveSystemPromptFile(path.join(profile, 'CLAUDE.md'), profile), /depth exceeds 5/);
  });
