...
pub async fn handle_connection(
    ...
    let leadership_shutdown = state.bot_connections.connection_epoch.subscribe();
    ...
    let mut write_handle = tokio::spawn(async move {
        ...
        if write_shutdown.is_cancelled() {
            ...
            // closed channel (delivery reported as failed) instead of enqueuing
            // into a socket that is already closing for leadership loss.
            drop(client_rx);
    ...
    // Stop the write loop
    if leadership_shutdown.is_cancelled() {
        // The writer sends a retryable close independently of registry cleanup.
        if !write_handle.is_finished() {
            let _ = tokio::time::timeout(std::time::Duration::from_secs(2), &mut write_handle).await;
        }
    }
    write_handle.abort();
    metrics_hook
