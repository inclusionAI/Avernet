...
- 金庸的 ``arch-analysis`` 是真实 LLM 推理,经 ``OpenApiBotAdapter``(直连 singlebox 上金庸的
  引擎 ws)live 调用;金庸本体由 ``LiveTaskBotProvisioner`` 真实建 bot + 装 skill。
...
_OWNER_BOT_NAME = (
    "task-owner-bot"  # 对齐 test_task_integration_e2e.py:owner 做 planning+search
)
_JY_BOT_NAME = "金庸"
        ...
        task_spec=TaskSpec(
            ...
            goal=Goal(
                ...
                acceptances=[
                    AcceptanceCriteria(
                        id="ac1",
                        description="覆盖基础架构/中间件/数据/业务等主要技术架构方向",
                    ),
                    AcceptanceCriteria(
                        id="ac2",
                        description="每个方向给出架构师清单(姓名或角色 + 职责)",
                    ),
                ],
            ),
        ),
...
class TestBbsRelayE2ELive(unittest.TestCase):
    ...
    async def _run(self) -> None:
        ...
        await prov._aclose()
        print(
            f"[provision] owner={owner_id} ← planning+search ; 金庸={jy_id} ← arch-analysis+bbs-relay-pickup"
        )
        ...
        print(
            f"[seed] task={TASK_ID} source=owner({owner_id}) bbs_mode=True root=PLANNING(可恢复)"
        )
        ...
        def _dash() -> dict:
            return client.get(
                "/openapi/v1/collaboration/tasks/dashboard", params={"task_id": TASK_ID}
            ).json()["data"]
        ...
        try:
            ...
            for i, sub in enumerate(SUB_DOMAINS):
                ...
                r = client.post(
                    ...
                    json={"task_id": TASK_ID, "bot_id": jy_id},
                )
                ...
                if i == 0:
                    r2 = client.post(
                        "/api/v1/collaboration/tasks/bbs/claim",
                        json={"task_id": TASK_ID, "bot_id": owner_id},
                    )
                    self.assertEqual(
                        r2.status_code,
                        409,
                        f"第二 bot claim 未被 CAS 拒(应 409):{r2.text}",
                    )
                ...
                self.assertTrue(
                    node_id.startswith("bbs-"), f"node_id 非 bbs- 前缀:{node_id}"
                )
                ...
                self.assertEqual(
                    nd["status"],
                    "RUNNING",
                    f"attach 后节点非 RUNNING(@{sub}):{nd['status']}",
                )
                self.assertEqual(
                    (nd["run_info"] or {}).get("run_mode"),
                    "bbs",
                    f"节点非 run_mode=bbs(@{sub})",
                )
                self.assertEqual(
                    (nd["run_info"] or {}).get("assignee"),
                    jy_id,
                    f"节点 assignee 非金庸(@{sub})",
                )
                ...
                try:
                    ...
                    if status != "COMPLETED" or not content:
                        finding_text = (
                            f"<arch-analysis status={status} error={run.get('error')}>"
                        )
                        ...
                        n_arch = (
                            len((parsed or {}).get("architects", [])) if parsed else 0
                        )
                        total_architects += n_arch
                        print(
                            f"[relay@{sub}] arch-analysis status={status} parsed={n_arch} "
                ...
                self.assertEqual(
                    nd["status"],
                    "SUCCESS",
                    f"result 后节点非 SUCCESS(@{sub}):{nd['status']}",
                )
                root = _node(g, TASK_ID)
                self.assertIsNone(
                    (root["run_info"]["extend_props"] or {}).get("bbs_owner"),
                ...
                print(
                    f"[relay@{sub}] result 200 node=SUCCESS root={root['status']} bbs_owner=释放 ✓"
                )
        finally:
            try:
        ...
        for n in bbs_nodes:
            ri = n["run_info"] or {}
            self.assertEqual(
                n["status"], "SUCCESS", f"scoped 未 SUCCESS:{n['node_id']}"
            )
            self.assertEqual(
                ri.get("assignee"), jy_id, f"scoped 非 金庸 接力:{n['node_id']}"
            )
            self.assertTrue(
        ...
        print(
            f"[final] graph={g['status']} 金庸接力段={len(bbs_nodes)} 架构师(parsed)={total_architects}"
        )
