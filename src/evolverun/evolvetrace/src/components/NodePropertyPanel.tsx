...
import type { WorkflowNode, PostAction, KnowledgeItem, RetryConfig, NodeAlerting } from '../types'
...
export default function NodePropertyPanel() {
  ...
  const tabs: { id: TabId; label: string; badge?: string }[] = [
    ...
    { id: 'advanced', label: 'Advanced', badge: hasAdvancedConfig(node) ? 'configured' : undefined },
    { id: 'actions', label: 'Actions', badge: hasPostActions(node) ? `${(node.onSuccess?.length ?? 0) + (node.onFailure?.length ?? 0)}` : undefined },
    { id: 'alerts', label: 'Alerts', badge: node.alerting ? 'on' : undefined },
  ]
      ...
      <div className="p-4 space-y-3">
        ...
        {activeTab === 'basic' && (
          <>
            ...
            <BranchIdField
              value={(node as any).branchId as string | undefined}
              onChange={(val) => updateNode(node.id, { branchId: val } as any)}
            />
            ...
            {executorType === 'human' && (
              <CollapsibleSection title="门控动作" badge={node.executor?.actions ? 'configured' : undefined} defaultOpen={!!node.executor?.actions}>
        ...
        {activeTab === 'advanced' && (
          <>
            ...
            <CollapsibleSection
              title="重试"
              badge={node.retry ? 'configured' : undefined}
            ...
            <CollapsibleSection
              title="输出契约"
              badge={node.outputContract ? 'configured' : undefined}
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    value={JSON.stringify(
                      (({ required, ...rest }) => rest)(node.outputContract as Record<string, unknown>),
                      null,
                      2,
                    )}
            ...
            <CollapsibleSection
              title="模拟输出"
              badge={node.mock ? 'configured' : undefined}
            ...
            {(executorType === 'collaboration' || executorType === 'approval') && (
              <CollapsibleSection
                title="反馈处理"
                badge={node.onFeedback ? 'configured' : undefined}
