...
try{
 // Production has state-machine routes but no session messages at this API base.
 const routeCalls=[];let detailFails=false;let routeAttempt=1;
 globalThis.fetch=async(url)=>{
  const path=new URL(url,'http://local.test').pathname;routeCalls.push(path);
  if(path==='/api/v1/collaboration/state-machine-runs/run-component/graph')return response({run:{run_id:'run-component',status:'running'},nodes:[{node_id:'node-b',status:'completed',attempt:routeAttempt}]});
  if(path.endsWith('/pending-human-nodes'))return response([]);
  if(path.endsWith('/nodes/node-b'))return detailFails?response({message:'detail unavailable'},503):response({node:{node_id:'node-b',status:'completed',attempt:routeAttempt,artifact_text:routeAttempt===1?'状态机公开发言':'重试后的公开发言'}});
  return response({},404);
 };
 let routes;await act(async()=>{routes=mount({...base,apiBaseUrl:'/api/v1/collaboration'});await settle(5)});
 assert.match(text(routes),/状态机公开发言/);
 assert.equal(routes.container.querySelector('[role="alert"]'),null);
 assert.equal(routeCalls.some(path=>path.includes('/sessions/')),false);
 detailFails=true;routeAttempt=2;await act(async()=>{button(routes,'刷新').click();await settle(5)});
 assert.match(routes.container.querySelector('[role="alert"]').textContent,/发言内容加载失败/);
 assert.match(text(routes),/状态机公开发言/,'last successful room survives a detail failure');
 detailFails=false;await act(async()=>{button(routes,'刷新').click();await settle(5)});
 assert.equal(routes.container.querySelector('[role="alert"]'),null);
 assert.match(text(routes),/重试后的公开发言/);
 assert.doesNotMatch(text(routes),/状态机公开发言/);
 await act(async()=>routes.unmount());

 const speechInstruction=`请发言\n[UNDERCOVER_UI_CONTEXT_V1]\n{"action":"speech","round":1,"seatNumber":1,"word":"苹果","maxChars":5,"forbidOwnWord":true,"bluntness":4}\n[/UNDERCOVER_UI_CONTEXT_V1]`;const posts=[];globalThis.fetch=fetchFor([{graph:{run:{run_id:'run-component',status:'running'},nodes:[{node_id:'node-a',status:'ready',attempt:1},{node_id:'node-b',status:'completed'}]},pending:[{node_id:'node-a',attempt:1,instruction:speechInstruction,timeout_deadline_ms:Date.now()+60000,upstream_artifacts:[{node_id:'node-b',text:'上游发言'}]}],artifacts:{}},{graph:{run:{run_id:'run-component',status:'running'},nodes:[{node_id:'node-a',status:'completed'},{node_id:'node-b',status:'completed'}]},pending:[]}],posts);
 ...
 globalThis.fetch=async(url)=>{const path=new URL(url,'http://local.test').pathname;if(path.endsWith('/graph')){currentGraphAttempt=attemptSnapshot++===0?1:2;return response({run:{run_id:'run-attempt',status:'running',updated_at:1},nodes:[{node_id:'node-a',status:'completed',attempt:currentGraphAttempt}]})}if(path.endsWith('/pending-human-nodes'))return response([]);if(path.endsWith('/messages'))throw new Error('Unexpected session messages request');const attempt=currentGraphAttempt;attemptCalls.push(`${path.split('/').at(-1)}:${attempt}`);return response({node:{node_id:'node-a',run_id:'run-attempt',status:'completed',attempt,artifact_text:attempt===1?'第一次尝试':'第二次尝试'}})};
 ...
 globalThis.fetch=async(url)=>{const path=new URL(url,'http://local.test').pathname;if(path.endsWith('/graph'))return response({run:{run_id:'run-recoverable',status:'running'},nodes:[{node_id:'node-a',status:'completed',attempt:1}]});if(path.endsWith('/pending-human-nodes'))return response([]);recoverableDetailCalls+=1;if(recoverableDetailCalls===1)return response({message:'temporary'},500);return response({node:{node_id:'node-a',run_id:'run-recoverable',status:'completed',attempt:1,artifact_text:'重试后可见'}})};
 ...
 globalThis.fetch=async(url)=>{const path=new URL(url,'http://local.test').pathname;const runId=path.includes('run-new')?'run-new':'run-old';if(path.endsWith('/graph'))return response({run:{run_id:runId,status:'running'},nodes:[{node_id:'node-a',status:'completed',attempt:1}]});if(path.endsWith('/pending-human-nodes'))return response([]);return response({node:{node_id:'node-a',run_id:runId,status:'completed',attempt:1,artifact_text:runId==='run-old'?'旧运行气泡':'新运行气泡'}})};
 ...
 globalThis.fetch=async(url)=>{const path=new URL(url,'http://local.test').pathname;if(path.endsWith('/graph'))return response({run:{run_id:'run-vote-detail',status:'running'},nodes:[{node_id:'node-b',status:'completed',attempt:1}]});if(path.endsWith('/pending-human-nodes'))return response([]);return response({node:{node_id:'node-b',run_id:'run-vote-detail',status:'completed',attempt:1,artifact_text:'{"kind":"vote","target_actor_id":"a"}'}})};
 ...
 const hostSnapshot=content=>({graph:finishGraph,pending:[],artifacts:{'node-host':content}});
 for(const snapshot of [
   ...
   {...hostSnapshot(finale),graph:{...finishGraph,run:{run_id:'run-finale',status:'running'}}},
 ]){
 ...
 globalThis.fetch=fetchFor([{graph:{run:{run_id:'bubble-run',status:'running'},nodes:[{node_id:'node-a',status:'completed',completed_at:1},{node_id:'node-b',status:'completed',completed_at:2}]},pending:[],artifacts:{'node-a':'一号的公开线索','node-b':'二号的最新线索'}}],[]);
 ...
 globalThis.fetch=fetchFor([{graph:{run:{run_id:'marker-run',status:'running'},nodes:[{node_id:'node-a',status:'running'},{node_id:'node-b',status:'completed',completed_at:2},{node_id:'node-c',status:'completed',completed_at:3}]},pending:[],artifacts:{'node-a':'a的公开发言','node-b':'b的公开发言','node-c':'c的公开发言'}}],[]);
