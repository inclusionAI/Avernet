...
def test_real_create_prepares_only_personal_and_keeps_original_apply(bot_type):
    svc, _ = _make_service()
    device = svc._device_service_provider()
    device.prepare_bot_storage_policy.return_value = {"device_provider": "baas"}
    events = []
    original_result = device.apply_device.return_value
    device.prepare_bot_storage_policy.side_effect = lambda **kw: (
    ...
    assert events == (["prepare", "apply"] if bot_type == "personal" else ["apply"])
    ...
    if bot_type == "personal":
        ...
        device.prepare_bot_storage_policy.assert_not_called()
        assert "device_provider" not in kwargs
...
def test_real_create_stops_before_allocation_if_policy_preparation_fails():
    ...
    svc, _ = _make_service()
    device = svc._device_service_provider()
    device.prepare_bot_storage_policy.side_effect = RuntimeError("policy write failed")
...
@pytest.mark.parametrize("new_bot", [True, False])
def test_service_never_reads_rollout_or_policy_even_when_unavailable(storage, new_bot):
    baas = make_baas(storage)
    storage[2].get_config = MagicMock(
        side_effect=AssertionError("service must not read policy")
    )
    storage[3].get_config.side_effect = AssertionError("service must not read rollout")
    allocate(baas, bot_type="service", new_bot=new_bot)
    assert latest_storage(baas)["type"] == "nas"
    storage[2].get_config.assert_not_called()
    storage[3].get_config.assert_called_once_with(
...
def test_backend_readiness_uses_default_volume(storage, env, specific):
    ...
    baas._bot_storage_service = MagicMock()
    readiness = []
    ...
    baas._bot_storage_service.initialize.side_effect = initialize_policy
