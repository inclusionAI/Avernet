...
export function createFacadesRouter(facadeRepo: FacadeBindingRepository | null): Router {
  const router = Router();
  ...
  router.delete("/:command", asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      const deleted = await facadeRepo.deleteByCommand(String(req.params.command));
      if (!deleted) {
