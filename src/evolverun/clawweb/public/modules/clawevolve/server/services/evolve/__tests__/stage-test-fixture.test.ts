...
import { freezeDiagnosePlanSource } from "../plan-business.js";

function sourceForQueries(queries: string[]) {
  const cases = queries.map((query, i) => ({ case_id: `case-${i}`, case_type: "good", session_id: `session-${i}`,
    query, context: { original_query: query }, evidence: { observed: "actual source" } }));
  return freezeDiagnosePlanSource({ schema_version: "plan-source/v2", generated_at: "2026-09-11T00:00:00Z",
    source: { type: "diagnose", id: "diagnose:EV-SOURCE", producer: "clawevolve-diagnose", bot_id: "bot", version: "2" },
    problem: { title: "日报", user_guidance: null }, cases,
    analysis: { case_distribution: { good: queries.length }, root_cause_clusters: [] }, planning_hints: {}, extensions: {},
  }, { taskId: "EV-SOURCE", stepId: "STEP-SOURCE", userId: "user", botId: "bot" },
  { cases: { items: cases.map((item) => ({ caseId: item.case_id, type: item.case_type })) } });
}

describe("fixed Stage test Skill fixture", () => {
  it("matches every Source case, leaves the document unchanged, and keeps legacy selection", () => {
    const source = sourceForQueries(["/daily-report-zh 单日记录", "/daily-report-zh 多日记录"]);
    const snapshot = JSON.stringify(source);
    expect(selectStageTestFixtureVersion("skill_evolution", "plan", "replace", source)).toBe(3);
    ...
    expect(selectStageTestFixtureVersion("bot_evolution", "plan", "replace", source)).toBeUndefined();
    expect(selectStageTestFixtureVersion("skill_evolution", "plan", "postprocess", source)).toBeUndefined();
    for (const other of ["/other-skill 日报", "无显式调用", "/daily-report-zh 日报\n/other-skill 另一任务"]) {
      expect(() => selectStageTestFixtureVersion("skill_evolution", "plan", "replace",
        sourceForQueries(["/daily-report-zh 第一条", other]))).toThrow(/daily-report-zh/);
    }
  });
