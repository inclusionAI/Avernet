...
sys.modules.setdefault("secbaas.infra", MagicMock())
sys.modules.setdefault("secbaas.infra.utils", MagicMock())
sys.modules.setdefault("secbaas.infra.utils.secret_utils", _mock_secret_utils)
sys.modules.setdefault("secbaas.infra.utils.env_utils", _mock_env_utils)
sys.modules.setdefault("secbaas.infra.utils.arca_utils", _mock_arca_utils)

from datetime import UTC

from secbaas.plugins.sandbox.desktop._real import (
    RealDesktopSandbox,
...
class TestRealDesktopSandboxPluginResolveWsConnInfo:
    ...
    def test_returns_ws_connection_info_with_all_fields(self, cm: MagicMock) -> None:
        ...
        with (
            patch(
                "secbaas.plugins.sandbox.utils.arca_utils._generate_proxypass_jwt",
                return_value="mock-jwt-token",
            ),
            patch(
                "secbaas.core.utils.env_utils.get_current_env",
                return_value="dev",
            ),
        ):
    ...
    def test_ws_url_format_dev_env(self, cm: MagicMock) -> None:
        """ws_url 格式包含 wss://agentclawproxy-dev/wsrelay/session_id。"""
        ...
        assert (
            "wss://agentclawproxy-dev.alipay.com/wsrelay/my-session-123"
            == result.ws_url
        )
    ...
    def test_ws_url_format_pre_env(self, cm: MagicMock) -> None:
        ...
        assert result.ws_url.startswith("wss://agentclawproxy-pre.alipay.com/wsrelay/")
