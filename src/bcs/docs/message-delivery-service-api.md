...
已结束段、当前段和原始 delta 均按原始字节直接连接，不插入或移除换行，不做任意子串或模糊去重。
...
群路由优先使用非空会话成员；缺少会话、未配置会话服务、查询失败或会话成员为空时，
保留原有群成员回退。
...
`source_im_message_id` 精确绑定原始用户消息，同一消息的多目标状态聚合。消息进入完成、取消、失败或容量拒绝终态时清除非过期状态表情。取消、失败和
不确定状态继续提供简短说明；inject 和普通成功不额外提示。
多目标聚合时，仍有目标排队则保持“排队中”；没有排队目标但仍在处理则显示“思考中”；仅当没有排队或处理中的目标且至少一个目标过期时显示“已过期”。
每个 100ms 扫描周期最多尝试一次外部 IM 状态投递，单次最多读取一条来源消息并等待 provider 2 秒，随后返回事件循环继续消费已提交状态变化。
