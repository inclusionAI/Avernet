    ...
    *,
    ...
    if relay_execution:
        ...
        parts.extend([
            ...
            f"3. 若产生下一步节点，POST {backend}/api/v1/collaboration/tasks/search，传 task_id、下一步 node_id、holder_id、relay_turn，取得 catalog_id 与候选集。搜索接口只给候选，必须由 search skill 决策。",
            "4. 将 search skill 决策作为 SEARCH_RESULT POST callback/report：payload 至少含 catalog_id、outcome。HIT_SINGLE 还需 assignee；HIT_MULTI_BOTS 还需 bot_ids、collab_mode、group_name、members_info；MISS 还需 miss_reason，并在 failure_reason 写明搜推失败原因。progress_reason 必须说明为什么选择该 Bot/协作群。",
