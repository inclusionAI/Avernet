...
- **Rule 15** — Plugin isolation.  Plugins are independently deployable
  ...
  composition root (``secbaas.bootstrap``) is allowed to wire plugins
  ...
  2. ``secbaas.bootstrap`` IS allowed to import from plugins (positive
    ...
    for plugin in plugins:
        for other in plugins:
            ...
            rule = (
                Rule()
                .modules_that()
                .are_sub_modules_of(f"secbaas.plugins.{plugin}")
                .should_not()
                .import_modules_that()
                .are_sub_modules_of(f"secbaas.plugins.{other}")
            )
            try:
...
def test_plugins_do_not_import_each_other(project_architecture):
    ...
    imports into any other plugin sub-package under ``secbaas.plugins``.
    """
...
def test_bootstrap_may_import_plugins(project_architecture):
    """Rule 15 (composition-root exemption): ``secbaas.bootstrap`` IS
    permitted to import from ``secbaas.plugins`` so it can wire the
    ...
    rule = (
        ...
        .should()
        .import_modules_that()
        .are_sub_modules_of("secbaas.plugins")
    )
    rule.assert_applies(project_architecture)
