"""Bare logger plugin — stdlib logging with trace-id injection.

Creates per-logger file handlers replicating the SOFA output format so the
open-source edition produces compatible log lines without ``sofapy_base``.

For each named logger, four handlers are attached:
``{name}.log``, ``{name}-error.log``, ``{name}-fatal.log`` and
``common-error.log``, all written to ``~/logs/{app_name}/``.
"""
...
class _TraceIdFilter(logging.Filter):
    ...
    (e.g. ``BareTracerPlugin`` or the enterprise ``SofaTracerPlugin``).
    """
...
class BareLoggerPlugin(LoggerPlugin):
    """Logger plugin for bare (non-SOFA) mode."""

    def __init__(self) -> None:
        self._level: int = logging.INFO
        self._log_dir: str = ""
        self._app_name: str = "gateway"
