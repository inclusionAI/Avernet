...
_PROXY_HOST_MAP: dict[str, ConfigPath] = {
...
def _get_agentclawproxy_host() -> str:
    """Get agentclawproxy host from config based on current env."""
    env = get_current_env()
    path = _PROXY_HOST_MAP.get(env, ConfigPath.AGENTCLAW_PROXY_HOST_DEV)
...
def build_proxypass_url(target: str, path: str, scheme: str = "wss") -> str:
    ...
    Returns:
        ...
        {scheme}://agentclawproxy-{env}.example.com/proxypass/{target}{path}
    """
    host = _get_agentclawproxy_host()
