...
export default function SkillCenter() {
  ...
  return <div className="w-full px-3 py-6 sm:px-4 lg:px-5">
    <PageTitle title="技能管理" description="登记技能，查看版本内容，并发起 Skill 自进化。" action={<button onClick={() => { setSpaceId(''); setShowRegister(true) }} className={primaryButton}><Icon name="plus" />登记 Skill</button>} />
    <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[1050px] table-fixed text-left text-sm">
        ...
        <tbody className="divide-y divide-gray-100">{filtered.slice((visiblePage - 1) * skillListPageSize, visiblePage * skillListPageSize).map((asset) => <tr key={asset.assetId} className="group transition hover:bg-gray-50/70">
          ...
          <td className="sticky right-0 border-l border-gray-100 bg-white px-3 py-4 text-center shadow-[-8px_0_12px_-12px_rgba(15,23,42,0.35)] group-hover:bg-gray-50"><div className="flex items-center justify-center gap-3 whitespace-nowrap"><button className="text-xs font-medium text-blue-600" onClick={() => setLaunch({ asset, action: 'diagnose' })}>诊断</button><button className="text-xs font-medium text-blue-600" onClick={() => setLaunch({ asset, action: 'optimize' })}>优化</button><button onClick={() => navigate(`/evolve/skills/${encodeURIComponent(asset.assetId)}`)} className="inline-flex items-center justify-center whitespace-nowrap rounded-md border border-blue-100 bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-700 transition hover:border-blue-200 hover:bg-blue-100">查看</button></div></td>
        </tr>)}</tbody>
      </table></div>
