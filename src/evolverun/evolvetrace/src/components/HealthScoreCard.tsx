import { useWorkflowHealth, useWorkflowHealthTrend } from '../api/hooks'
...
export function HealthScoreCard({ workflowId, onNodeClick }: { workflowId: string; onNodeClick?: (nodeId: string) => void }) {
  ...
  const metric = (label: string, value: string, color = 'text-gray-700') => (
  ...
  return (
    <div className={`rounded-xl border ${health.overallScore >= 80 ? 'border-green-200' : health.overallScore >= 60 ? 'border-orange-200' : 'border-red-200'} bg-white p-5 shadow-sm`}>
      <div className="flex items-center gap-5">
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-2 flex items-center gap-2">
            ...
            <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-medium ${s.label}`}>{s.desc}</span>
          </div>
          <div className="grid grid-cols-2 gap-x-4 gap-y-0.5">
            {metric('成功率', `${health.successRate}%`, health.successRate >= 80 ? 'text-green-600' : health.successRate >= 60 ? 'text-orange-500' : 'text-red-600')}
            {metric('节点失败率', `${health.nodeFailureRate}%`)}
            {metric('P95 耗时', formatDuration(health.p95DurationMs))}
            {metric('平均重试', `${health.retryRate}`)}
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 space-y-1.5 border-t border-gray-100 pt-3">
          ...
          <div className="flex items-center gap-3 text-xs">
            ...
            {health.fragileNode && (
              <button
                ...
                className="inline-flex items-center gap-1 text-red-500 hover:text-red-600 hover:underline"
              >
        ...
        <p className="text-xs leading-relaxed text-gray-400">{health.recommendation}</p>
      </div>
    </div>
  )
}
