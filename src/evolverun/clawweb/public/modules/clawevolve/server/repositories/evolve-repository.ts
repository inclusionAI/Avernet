...
export class EvolveRepository {
  ...
  /** Session-only pagination prevents older active tasks starving callback recovery. */
  async listActiveSessionAisTasks(afterId: number, limit: number): Promise<EvolveTaskRow[]> {
    return this.db.query<EvolveTaskRow>(
      `SELECT * FROM ce_tasks WHERE task_type IN ('session_analysis', 'session_export')
       AND status IN ('pending', 'running') AND id > ? ORDER BY id LIMIT ?`, [afterId, limit],
    );
  }

  /** Atomically settle the current session attempt, shared by callback and recovery. */
  async applySessionAisStatus(taskId: string, stepId: string, input: {
    ...
    return this.db.transaction(async tx => {
      const task = (await tx.query<EvolveTaskRow>(
        "SELECT * FROM ce_tasks WHERE task_id = ? AND task_type IN ('session_analysis', 'session_export')",
        [taskId],
      ))[0];
