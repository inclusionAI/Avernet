...
一个**内容 skill `bbs-relay-pickup` + 现有 task API 的最小加法 REST 扩展**,让任意引擎的 bot 被唤醒后自主完成 6 步:①经 `GET /openapi/v1/collaboration/tasks/list`(筛 `bbs_mode=True`)+ `GET /openapi/v1/collaboration/tasks/dashboard` 发现 BBS 升级任务并取整图 → ②`POST /openapi/v1/collaboration/tasks/bbs/claim` CAS 独占该任务根节点(恰一赢、输者 409 让出)→ ③读根 `Goal`+`Acceptance` 与 DONE 叶子、自判剩余里能做哪部分 → ④把"能做的那部分"新建一个 `run_mode="bbs"` 节点挂到图上、执行 → ⑤经 `POST /openapi/v1/collaboration/tasks/bbs/result` → `on_bbs_report`(collector-free,见 FR-EXT-03)写回结果与最新状态 → ⑥本次 pass 完成隐式释放 claim,下个 bot 按新的 DONE 叶子继续接力。
...
- 本期不做 BBS 接单路径的鉴权:`POST /openapi/v1/collaboration/tasks/*` 现无鉴权,`/openapi/v1/collaboration/tasks/callback/*` 已 HMAC 鉴权;BBS 新路由本期沿用 `/openapi/v1/collaboration/tasks/*` 裸奔现状,鉴权另系分(见 §7.2)。
...
- **FR-PICK-04 挂节点 + 执行(步④)**:把"能做的那部分"封装成 `TaskSpec` → `POST /openapi/v1/collaboration/tasks/bbs/attach`(`add_task_nodes` 薄封装,仅 claim 持有者可调)在**根下新建一个 `run_mode="bbs"` 子节点** + 翻 `PENDING→RUNNING`(`assignee=bot_id`,attach 合并 create+start)→ 原生能力执行这一个节点。**前提**:任务图空闲(无 `RUNNING`)、根 `PLANNING`(可委托);图级 `HUNG`(`root_stuck`/`loop_exhausted`)= 硬终态,跳过。
- **FR-PICK-05 写回(步⑤)**:经 `POST /openapi/v1/collaboration/tasks/bbs/result` → `engine.on_bbs_report(TaskNodePatch{acceptance_result?, output_patch?, exec_error?}, root_verified)`(**collector-free**,见 FR-EXT-03:不跑 `_on_pass_collect`/`_on_fail_collect`/`_drain`,避免框架经 owner-bot 重规划与接力冲突)。`verdict=PASS`→节点 `DONE`;`verdict=FAIL`+gaps→节点 `FAILED`;**根目标满足时 bot 带 `root_verified=true`** → 根 acceptance `PASS`→根 `DONE` + 图 `DONE`;报完**自动清根 `bbs_owner`** 释放。长活须周期性 `output_patch` 作 checkpoint(见 FR-IDEM-02)。
...
- **FR-EXT-01 bbs/claim 路由(新)**:`POST /openapi/v1/collaboration/tasks/bbs/claim {task_id, bot_id}` → 条件写根 `bbs_owner`;恰一赢,输者 409;赢者返回 claim 句柄(含 root `node_id`)。
- **FR-EXT-02 bbs/attach 路由(新)**:`POST /openapi/v1/collaboration/tasks/bbs/attach {task_id, parent_node_id, task_spec, bot_id}` → 仅 claim 持有者可调;封装 `TaskGraphService.add_task_nodes` 在根下新建 `run_mode="bbs"` 子节点 + 翻 `PENDING→RUNNING`(`assignee=bot_id`);返回 `node_id`。守卫保证框架 dispatch/drain 不自动消费该节点。
- **FR-EXT-03 bbs/result 路由(新)**:`POST /openapi/v1/collaboration/tasks/bbs/result {task_id, node_id, bot_id, acceptance_result?, output_patch?, exec_error?, root_verified=false}` → 仅 claim 持有者可调;→ 新增 `engine.on_bbs_report(TaskNodePatch, root_verified)`(**collector-free**:仅 `update_task_node_info` 翻 scoped 节点终态 + 可选根收口 + 清根 `bbs_owner`,不跑 `_on_pass_collect`/`_on_fail_collect`/`_drain`)。`root_verified=true` → 根 acceptance `PASS`→根 `DONE` + 图 `DONE`。不直调 `on_report`(其 `_on_pass_collect` 会经 owner-bot 重规划+派发,与 §10.4 接力冲突)。
...
| 待定项 | 说明 |
|---|---|
| **BBS 接单鉴权** | `POST /openapi/v1/collaboration/tasks/*` 现裸奔;本期 `bbs/*` 沿用现状(同 `/openapi/v1/collaboration/tasks/*`)。`/openapi/v1/collaboration/tasks/callback/*` 已 HMAC 鉴权,但 BBS 走 `/openapi/v1/collaboration/tasks/bbs/*` 不走 callback。prod 部署前置网关/`AuthPlugin` 可能挡,app 内不校验。鉴权另系分 |
...
- 不改 ocb bot 生命周期/会话/BCS 协作基建;不动 `/openapi/v1/collaboration/tasks/callback/*` 既有鉴权与回调契约。
...
│   → POST /openapi/v1/collaboration/tasks/bbs/claim(CAS 占根)              │
│   → (自判:根goal+DONE叶子 → 能做哪部分)             │
│   → POST /openapi/v1/collaboration/tasks/bbs/attach(挂 run_mode=bbs 节点) │
│   → [干活 + 周期 bbs/result output_patch checkpoint] │
│   → POST /openapi/v1/collaboration/tasks/bbs/result(acceptance 回投)      │
...
| ② CAS 占根 | `POST /openapi/v1/collaboration/tasks/bbs/claim`(根 `bbs_owner` CAS) | FR-EXT-01 |
| ③ bot 自判 | agent LLM;rubric 在 `references/` | 纯 skill |
| ④ 挂节点 + 执行 | `POST /openapi/v1/collaboration/tasks/bbs/attach`(`add_task_nodes` + start)+ 原生能力 | FR-EXT-02(+ FR-EXT-06 守卫) |
| ⑤ 写回结果+状态 | `POST /openapi/v1/collaboration/tasks/bbs/result` → `on_bbs_report`(`acceptance_result`/`output_patch`/`exec_error`/`root_verified`,collector-free) | FR-EXT-03 |
...
- 干活前必须 `POST /openapi/v1/collaboration/tasks/bbs/claim {task_id, bot_id}` 成功。
...
- 新路由:`POST /openapi/v1/collaboration/tasks/bbs/claim`、`POST /openapi/v1/collaboration/tasks/bbs/attach`、`POST /openapi/v1/collaboration/tasks/bbs/result`。
