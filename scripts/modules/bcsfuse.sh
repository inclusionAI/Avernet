...
bcsfuse_load_env() {
    ...
    # Layer 2: BCSFuse-specific env (overrides Layer 1)
    # This file is generated by bcsfuse_setup() from .env.example and
    # contains BCSFUSE_AUTH_TOKEN, LLM/Embedding config, feature flags, etc.
...
bcsfuse_ensure_schema() {
    ...
    if [ -f "$schema_script" ]; then
        ...
        cd "${BCSFUSE_DIR}"
        if ! uv run python "$schema_script" setup; then
