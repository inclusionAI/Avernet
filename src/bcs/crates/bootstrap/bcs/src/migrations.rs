...
const SQLITE_VERSIONED_MIGRATIONS: &[SqliteMigration] = &[
    ...
    SqliteMigration {
        version: 15,
        name: "permission_request_public_id",
    },
];
...
async fn apply_sqlite_migration_body(
    ...
    match migration.version {
        ...
        14 => Ok(()),
        // External string `request_id` column on permission_requests (kept
        // bigint `id` as the internal PK/FK). ALTERs in idempotently so both
        // fresh and pre-existing DBs converge.
        15 => add_sqlite_permission_request_public_id(db).await,
        _ => Ok(()),
    }
}
...
async fn add_sqlite_edge_permission_schema(db: &dyn DbPlugin) -> DbResult<()> {
    ...
    for stmt in [
        ...
        "CREATE TABLE IF NOT EXISTS permission_requests (id INTEGER PRIMARY KEY AUTOINCREMENT, edge_id INTEGER, env TEXT NOT NULL, from_id TEXT NOT NULL, to_id TEXT NOT NULL, request_kind TEXT NOT NULL, requested_ref_id INTEGER, requested_rules TEXT, message TEXT, status TEXT NOT NULL DEFAULT 'pending', decision_reason TEXT, created_by TEXT NOT NULL, decided_by TEXT, decided_at TEXT, gmt_create TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, gmt_modified TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)",
...
/// v15: add the external string `request_id` column to `permission_requests`.
///
/// `id` (bigint auto-increment) stays the internal PK/FK join key; `request_id`
/// (a bare UUID v4, simple form, generated by the service via
/// `Uuid::new_v4().simple().to_string()`) is the client-facing stable id. ALTERs
/// in idempotently so both fresh DBs (v13 created the table without it) and
/// pre-existing DBs converge. Legacy rows are backfilled with `CAST(rowid AS
/// TEXT)` (SQLite has no `UUID()`; rowid is unique per row) before the unique
/// index is created.
async fn add_sqlite_permission_request_public_id(db: &dyn DbPlugin) -> DbResult<()> {
    if !table_exists(db, "permission_requests").await? {
        return Ok(());
    }
    let columns = sqlite_table_columns(db, "permission_requests").await?;
    if columns.iter().any(|column| column == "request_id") {
        return Ok(());
    }
    db.execute(DbStatement::new(
        "ALTER TABLE permission_requests ADD COLUMN request_id TEXT NOT NULL DEFAULT ''",
    ))
    .await?;
    db.execute(DbStatement::new(
        "UPDATE permission_requests SET request_id = CAST(rowid AS TEXT) WHERE request_id = ''",
    ))
    .await?;
    db.execute(DbStatement::new(
...
"add_bot_task_modes".to_string(),
                ...
                (
                    15,
                    "permission_request_public_id".to_string(),
                    "sqlite".to_string()
                )
            ]
        );
