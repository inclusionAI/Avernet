...
describe('Skill center asset list and recorded events', () => {
  ...
  it('shows one business event per task with HITL action and exact Skill detail selection', async () => {
    api.evolve.listSkillEvents.mockResolvedValue({ items: [
      ...
      { eventId: 'EVENT-OPTIMIZE', assetId: 'ASSET-1', name: 'Evidence Skill', description: 'Host event skill description', ownerId: 'owner', botId: 'BOT-1', type: 'optimization', status: 'completed', outcome: '指标提升', taskId: 'TASK-1', actorId: null, actorType: 'system', versionFrom: { versionId: 'VERSION-1', version: 'v1' }, versionTo: { versionId: 'VERSION-2', version: 'v2' }, waitingInteractionId: null, summary: '完成两轮优化', startedAt: 1789060001, completedAt: 1789060010, updatedAt: 1789060010, testBench: null },
    ...
    expect(screen.getByText('Host event skill description')).toBeTruthy()
  ...
  it('creates a new Skill version from direct editing or package upload', async () => {
    const upgraded = {
      ...
      versions: [{ versionId: 'VERSION-3', version: 'v3', creationKind: 'edit' }, ...(asset.versions ?? [])],
    }
    api.evolve.getSkillAsset.mockResolvedValueOnce(asset).mockResolvedValue(upgraded)
    api.evolve.getSkillVersionContent.mockImplementation(async (_assetId, versionId) => ({
      files: [{ path: 'SKILL.md', text: true }], selected: { path: 'SKILL.md', content: `Frozen ${versionId}` },
    }))
    api.evolve.getSkillVersionDiff.mockResolvedValue({ baseline: null, files: [] })
    api.evolve.createSkillVersion.mockResolvedValue({ versionId: 'VERSION-3', version: 'v3', creationKind: 'edit' })
    render(<MemoryRouter initialEntries={['/evolve/skills/ASSET-1']}><SkillDetail /></MemoryRouter>)
    await screen.findByText('Frozen VERSION-2')

    fireEvent.click(screen.getByRole('button', { name: '编辑 Skill' }))
    expect(screen.getByRole('dialog', { name: '编辑 Skill' })).toBeTruthy()
    await waitFor(() => expect((screen.getByRole('textbox', { name: '文件内容' }) as HTMLTextAreaElement).value).toBe('Frozen VERSION-2'))
    fireEvent.change(screen.getByRole('textbox', { name: '文件内容' }), { target: { value: '# Edited Skill\n' } })
    fireEvent.click(screen.getByRole('button', { name: '创建新版本' }))
    await waitFor(() => expect(api.evolve.createSkillVersion).toHaveBeenCalledWith('ASSET-1', {
      mode: 'edit', baseVersionId: 'VERSION-2', edits: [{ path: 'SKILL.md', content: '# Edited Skill\n' }],
    }))
    await screen.findByRole('button', { name: /v3.*当前版本/ })
    ...
    api.evolve.uploadSkillVersion.mockResolvedValue({ versionId: 'VERSION-4', version: 'v4', creationKind: 'upload' })
    fireEvent.click(screen.getByRole('button', { name: '上传并创建新版本' }))
    await waitFor(() => expect(api.evolve.uploadSkillVersion).toHaveBeenCalledWith('ASSET-1', 'VERSION-3', file))
  })

  it('always edits the latest version even while a historical version is selected', async () => {
    api.evolve.getSkillAsset.mockResolvedValue(asset)
    api.evolve.getSkillVersionContent.mockImplementation(async (_assetId, versionId) => ({
      files: [{ path: 'SKILL.md', text: true }], selected: { path: 'SKILL.md', content: `Frozen ${versionId}` },
    }))
  ...
  it('keeps a shared Skill readable without exposing version write controls', async () => {
    api.evolve.getSkillAsset.mockResolvedValue({ ...asset, canEdit: false })
    api.evolve.getSkillVersionContent.mockResolvedValue({
      files: [{ path: 'SKILL.md', text: true }], selected: { path: 'SKILL.md', content: 'Shared frozen content' },
    })
    api.evolve.getSkillVersionDiff.mockResolvedValue({ baseline: null, files: [] })
    render(<MemoryRouter initialEntries={['/evolve/skills/ASSET-1']}><SkillDetail /></MemoryRouter>)
    await screen.findByText('Shared frozen content')
  ...
  it('rolls a historical version forward after confirmation and shows its source lineage', async () => {
    const rolledBack = {
      ...asset, currentVersion: 'v3',
      versions: [
        { versionId: 'VERSION-3', version: 'v3', creationKind: 'rollback', sourceVersion: { versionId: 'VERSION-1', version: 'v1' } },
        ...(asset.versions ?? []),
      ],
    }
    api.evolve.getSkillAsset.mockResolvedValueOnce(asset).mockResolvedValue(rolledBack)
    api.evolve.getSkillVersionContent.mockImplementation(async (_assetId, versionId) => ({
      files: [{ path: 'SKILL.md', text: true }], selected: { path: 'SKILL.md', content: `Frozen ${versionId}` },
    }))
    api.evolve.getSkillVersionDiff.mockResolvedValue({ baseline: { sha256: 'baseline' }, files: [] })
    api.evolve.createSkillVersion.mockResolvedValue({
      versionId: 'VERSION-3', version: 'v3', creationKind: 'rollback',
      sourceVersion: { versionId: 'VERSION-1', version: 'v1' },
    })
    render(<MemoryRouter initialEntries={['/evolve/skills/ASSET-1']}><SkillDetail /></MemoryRouter>)
    await screen.findByText('Frozen VERSION-2')
    fireEvent.click(screen.getByRole('button', { name: /^v1/ }))
    ...
    expect(screen.getByRole('dialog', { name: '确认回滚到 v1？' })).toBeTruthy()
    expect(screen.getByText('将基于 v1 的内容创建新的最新版本，不会覆盖或删除任何历史版本。')).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: '确认创建新版本' }))

    await waitFor(() => expect(api.evolve.createSkillVersion).toHaveBeenCalledWith('ASSET-1', {
      mode: 'rollback', baseVersionId: 'VERSION-2', sourceVersionId: 'VERSION-1',
    }))
    await screen.findByRole('button', { name: /v3.*当前版本.*回滚自 v1.*VERSION-1/ })
