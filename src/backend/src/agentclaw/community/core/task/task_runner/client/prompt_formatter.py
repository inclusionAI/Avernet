...
def _static_relay_closure() -> str:
    ...
    """
    return "\n".join([
        "【执行闭环·严格按 step1→step2→step3 顺序逐步产出,三步缺一不可】",
        "严格按上方【接力执行·三步闭环】顺序逐步执行并输出,每步以 \"step1\"/\"step2\"/\"step3\" 开头分明,不得跳过、合并或只用一句话概括代替:",
        "- step1 接力上下文:复述接自谁、上游规划交你执行的下一步任务是什么、你的承接边界;",
        "- step2 执行产出:真正推理完成本节点任务指令,给出完整执行结论与产出,不得只复述任务不产出;",
        "- step3 gap 与交接:获取上方最新统一上下文(上游产出正文+本步产出),自己对照目标计算 gap、规划下一步任务,并按问题智能匹配能力、交接派发给下一步执行 bot;本链路终点节点则在 step3 声明接力终结、不再派发。",
