...
class LocalBot(BaseModel):
    ...
    bot_type: Literal["desktop"] = Field(
        default="desktop", description="Local Bot discriminator, always desktop."
    )
    ...
    bot_desc: str = Field(
        description="Description of what the local bot is for; may be empty."
    )
    engine: str = Field(description="Engine used by the local bot.")
    status: str = Field(
        description="Current lifecycle status reported for the local bot."
    )
    owner_entity_id: str = Field(description="User who owns the local bot.")
    machine_id: str | None = Field(
        default=None, description="Host device identifier, when assigned."
    )
    mount_path: str | None = Field(
        default=None, description="Workspace path mounted for the bot, when configured."
    )
...
class LocalBotCreate(BaseModel):
    ...
    bot_desc: str | None = Field(
        default=None, description="Optional description of the bot's purpose."
    )
    mount_path: str | None = Field(
        default=None, description="Optional workspace path to mount for the bot."
    )
    avatar_url: str | None = Field(
        default=None, description="Optional avatar URL for the bot."
    )
    engine: str = Field(
        default="openclaw", description="Engine to run on the local device."
    )
...
class LocalBotAuthPending(BaseModel):
    ...
    iframe_url: str = Field(
        default="",
        description="URL to embed for completing authorization; empty when unavailable.",
    )
    redirect_url: str = Field(
        default="",
        description="URL to open for completing authorization; empty when unavailable.",
    )
...
class LocalBotAuthStatus(BaseModel):
    ...
    message: str | None = Field(
        default=None, description="Additional status detail, when available."
    )
    bot: LocalBot | None = Field(
        default=None,
        description="Created bot after authorization completes; otherwise null.",
    )
...
class LocalDevice(BaseModel):
    ...
    machine_name: str = Field(
        default="", description="User-facing device name; may be empty."
    )
    ...
    ip_address: str = Field(
        default="", description="Reported device IP address; may be empty."
    )
    last_alive_at: str | None = Field(
        default=None, description="Most recent heartbeat time, when reported."
    )
    created_at: str | None = Field(
        default=None, description="Device registration time, when reported."
    )
...
class LocalDirectoryEntry(BaseModel):
    ...
    children: list["LocalDirectoryEntry"] | None = Field(
        default=None, description="Child entries for a directory; null for a leaf."
    )
...
class LocalOpenFolder(BaseModel):
    ...
    folder_path: str | None = Field(
        default=None,
        max_length=512,
        description="Folder to open, relative to the bot workspace; omit for the workspace root.",
    )
...
class LocalLifecycleResult(BaseModel):
    ...
    status: str | None = Field(
        default=None, description="Resulting lifecycle status, when reported."
    )
    result: dict | None = Field(
        default=None, description="Additional engine result fields, when reported."
    )
