...
class UserIdentityInfo:
    ...
    Every field except ``work_no`` is ``str | None``: ``None`` is an intentional
    contract state (the directory genuinely supplies no value), null rather
    than invented when absent. An all-``None`` info (identity and dept all null)
    means "looked up, no record" — the ``200`` + null answer, distinct from a
    :class:`DeptLookupError` ("directory down", 5xx). Used by the
    ``GET /openapi/v1/org/user?user_id=`` directory-lookup branch to return
    another user's identity + dept; the whoami no-param branch still reads
    identity off the verified principal and uses :meth:`get_dept_by_work_no` for
...
class StaffDeptPlugin(Plugin, Protocol):
    ...
    def get_user_by_work_no(self, *, work_no: str) -> UserIdentityInfo:
        ...
        all-``None`` shape. Only an unreachable or erroring service raises
        :class:`DeptLookupError` — so the caller can tell "no record" (200 +
        null) from "directory down" (5xx), the same split
        :meth:`get_dept_by_work_no` makes.

        Used by the ``GET /openapi/v1/org/user?user_id=`` directory-lookup branch
        to return another user's identity + dept; the whoami no-param branch
        still reads identity off the verified principal and uses
        :meth:`get_dept_by_work_no` for dept only.
        """
        ...
