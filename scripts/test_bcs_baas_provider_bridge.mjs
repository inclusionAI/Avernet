#!/usr/bin/env node
// Contract tests for the loopback BCS Provider -> BaaS bridge.
...
async function waitForHealth(port) {
  for (let attempt = 0; attempt < 40; attempt += 1) {
    try {
      ...
      // Bridge may still be binding its local h2c listener.
    }
    await sleep(50);
  }
...
try {
  ...
  response = await h1Request(bridgePort, providerBody('chat.inject'), 'wrong-token');
  ...
  assert.match(bridgeOutput, /bridge\.reject reason=provider_bot_mismatch/);
  assert.match(bridgeOutput, /bridge\.reject reason=unauthorized/);
  assert.match(bridgeOutput, /bridge\.reject reason=unsupported_method/);
  assert.doesNotMatch(bridgeOutput, /message-not-logged/);
