...
class TestSessionRoutingAffinityPrefix:
    ...
    def test_teclaw_no_eval_id_returns_structured_key(self, service):
        """teclaw 生产流量也统一构造结构化 key（延迟物化 get-or-create）。"""
        assert (
            plan_session_id(
                engine_type="teclaw",
                tc_bot_id=BOT_UUID,
...
class TestCreateSessionInvokerInjection:
    ...
    async def test_metadata_tenant_fallback_when_no_context(
        ...
        # 无显式 session_id 时 run_id 现场生成 uuid，亲和键含该 uuid
        consistency_key = (
            "session:00000000-0000-0000-0000-000000000004:user:" + BOT_UUID
        )
...
class TestMaterializeSession:
    ...
    async def test_teclaw_reuses_get_or_create(self, service):
        """teclaw：复用 _get_or_create_teclaw_session 的探测语义，不直接 create。"""
        session_client = _make_session_client_stub()
        binding = _make_binding_info(engine_type="teclaw")
        teclaw = AsyncMock(return_value=(PLANNED_SESSION_ID, True))
        with (
            patch.object(
                service,
                "_resolve_ws_connection",
            ...
            patch.object(service, "_get_or_create_teclaw_session", teclaw),
            patch.object(service, "_persist_session_create", return_value=None),
        ):
            metadata = {"tenant": TENANT}
            await service._materialize_session(
            ...
            teclaw.assert_awaited_once_with(
                session_client, PLANNED_SESSION_ID, BOT_UUID, metadata, BOT_UUID
            )
