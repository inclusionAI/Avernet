...
class TaskService:
    ...
    def __init__(self, graph, harness=None, *, bot=None, bcs=None, discover=None,
                 bcs_identity=None, task_provider_id: str = "",
        ...
        bot/bcs/discover:传输端口(DI 从配置注入 local/prod/double 实现传给引擎;
        省略=stub 路径/纯内核单测)。
        task_provider_id:任务模式 roster 圈定的 provider(空=圈定关闭,旧行为);
        透传给引擎派发策略。

        ``task_info_repo``(可选):task_info 持久化协议;``callback_repo``(可选):
        可重放回投落库协议;``task_id_provider``:task_id 生成器(默认 uuid4)。
        ``task_node_repo``/``task_node_run_info_repo``(可选):workflow/yaml 分支的
        节点与运行信息持久化。所有 repository 为 ``None`` 时保留纯内核/单测路径。"""
        self._graph = graph
