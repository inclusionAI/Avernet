...
def _capture_conversion_logs(monkeypatch, caplog):
    conversion_logger = logging.getLogger("test.default-converter-messages")
...
def _conversion_messages(caplog, logger) -> list[str]:
    return [
        record.getMessage()
        for record in caplog.records
        if record.name == logger.name and record.getMessage().startswith("[convert]")
    ]


def _conversion_payloads(message: str) -> tuple[dict, dict | None]:
    raw_input, raw_output = message.split(" input=", 1)[1].split(" output=", 1)
    return json.loads(raw_input), json.loads(raw_output)


def test_conversion_logging_is_disabled_by_default(monkeypatch, caplog):
    conversion_logger = _capture_conversion_logs(monkeypatch, caplog)

    event = DefaultStreamConverter().convert(
        StreamChunk(type="delta", content="hello"),
        run_id="run-log",
    )

    assert event is not None
    assert _conversion_messages(caplog, conversion_logger) == []


def test_enabled_conversion_logging_records_input_and_output(monkeypatch, caplog):
    conversion_logger = _capture_conversion_logs(monkeypatch, caplog)
    converter = DefaultStreamConverter(conversion_logging_enabled=True)
    ...
    assert event is not None
    messages = _conversion_messages(caplog, conversion_logger)
    assert len(messages) == 1
    raw_input, raw_output = messages[0].split(" input=", 1)[1].split(" output=", 1)
    assert "source=bcn_downlink run_id=run-log" in messages[0]
    ...
    assert output["event"] == "chat"
    assert json.loads(output["data"])["deltaText"] == "hello"


def test_enabled_conversion_logging_records_filtered_input(monkeypatch, caplog):
    conversion_logger = _capture_conversion_logs(monkeypatch, caplog)
    converter = DefaultStreamConverter(conversion_logging_enabled=True)

    event = converter.convert(
        StreamChunk(type="usage", usage={"input_tokens": 12}),
        run_id="run-log",
    )
    ...
    assert len(messages) == 1
    raw_input, raw_output = messages[0].split(" input=", 1)[1].split(" output=", 1)
    assert json.loads(raw_input)["usage"] == {"input_tokens": 12}
    assert json.loads(raw_output) is None


@pytest.mark.parametrize(
    ("chunk_type", "output_field"),
    (("delta", "deltaText"), ("error", "errorMessage")),
)
def test_chat_scalar_bodies_are_previewed_only_in_logs(
    monkeypatch,
    caplog,
    chunk_type,
    output_field,
    ...
    conversion_logger = _capture_conversion_logs(monkeypatch, caplog)
    full_body = "0123456789abcdefghij"
    expected_preview = "0123456789…<truncated:20>"
    chunk = StreamChunk(type=chunk_type, content=full_body)
    converter = DefaultStreamConverter(conversion_logging_enabled=True)

    event = converter.convert(chunk, run_id="run-preview")

    logged_input, logged_output = _conversion_payloads(
        _conversion_messages(caplog, conversion_logger)[0]
    )
    assert logged_input["content"] == expected_preview
    assert json.loads(logged_output["data"])[output_field] == expected_preview
    assert chunk.content == full_body
    assert json.loads(event.data)[output_field] == full_body


def test_chat_final_body_is_previewed_only_in_logs(monkeypatch, caplog):
    conversion_logger = _capture_conversion_logs(monkeypatch, caplog)
    full_body = "0123456789abcdefghij"
    expected_preview = "0123456789…<truncated:20>"
    chunk = StreamChunk(type="final", content=full_body)
    converter = DefaultStreamConverter(conversion_logging_enabled=True)
    ...
    assert logged_input["content"] == expected_preview
    logged_text = json.loads(logged_output["data"])["message"]["content"][0]["text"]
    assert logged_text == expected_preview
    assert chunk.content == full_body
    assert json.loads(event.data)["message"]["content"][0]["text"] == full_body


def test_aborted_input_body_is_previewed_only_in_logs(monkeypatch, caplog):
    conversion_logger = _capture_conversion_logs(monkeypatch, caplog)
    full_body = "0123456789abcdefghij"
    chunk = StreamChunk(type="aborted", content=full_body)
    converter = DefaultStreamConverter(conversion_logging_enabled=True)

    event = converter.convert(chunk, run_id="run-preview")

    logged_input, _ = _conversion_payloads(
        _conversion_messages(caplog, conversion_logger)[0]
    )
    assert logged_input["content"] == "0123456789…<truncated:20>"
    assert chunk.content == full_body
    assert json.loads(event.data)["state"] == "aborted"


def test_thinking_bodies_are_previewed_only_in_logs(monkeypatch, caplog):
    conversion_logger = _capture_conversion_logs(monkeypatch, caplog)
    full_delta = "0123456789abcdefghij"
    full_text = "ABCDEFGHIJklmnopqrst"
    chunk = StreamChunk(
        type="agent",
        metadata={
            "engine_frame": {
                "stream": "thinking",
                "data": {"delta": full_delta, "text": full_text},
            }
        },
    )
    ...
    logged_input_data = logged_input["metadata"]["engine_frame"]["data"]
    logged_output_data = json.loads(logged_output["data"])
    assert logged_input_data["delta"] == "0123456789…<truncated:20>"
    assert logged_input_data["text"] == "ABCDEFGHIJ…<truncated:20>"
    assert logged_output_data["delta"] == "0123456789…<truncated:20>"
    assert logged_output_data["text"] == "ABCDEFGHIJ…<truncated:20>"
    assert chunk.metadata == original_metadata
    assert json.loads(event.data)["delta"] == full_delta
    assert json.loads(event.data)["text"] == full_text


def test_tool_payloads_remain_complete_in_logs(monkeypatch, caplog):
    conversion_logger = _capture_conversion_logs(monkeypatch, caplog)
    full_argument = "tool argument longer than ten characters"
    chunk = StreamChunk(
        type="agent",
    ...
    event = converter.convert(chunk, run_id="run-tool")

    logged_input, logged_output = _conversion_payloads(
        _conversion_messages(caplog, conversion_logger)[0]
    )
    logged_input_args = logged_input["metadata"]["engine_frame"]["data"]["args"]
    logged_output_args = json.loads(logged_output["data"])["args"]
    assert logged_input_args == {"command": full_argument}
    assert logged_output_args == {"command": full_argument}
    assert json.loads(event.data)["args"] == {"command": full_argument}


def test_interaction_payloads_remain_complete_in_logs(monkeypatch, caplog):
    conversion_logger = _capture_conversion_logs(monkeypatch, caplog)
    full_question = "Sensitive secret question body"
    chunk = _interaction_chunk(
    ...
    event = converter.convert(chunk, run_id="run-interaction")

    logged_input, logged_output = _conversion_payloads(
        _conversion_messages(caplog, conversion_logger)[0]
    )
    logged_input_question = logged_input["metadata"]["payload"]["payload"]["questions"][
        0
    ]
    logged_output_question = json.loads(logged_output["data"])["questions"][0]
    assert logged_input_question["question"] == full_question
    assert logged_input_question["secret"] is True
    assert logged_output_question["question"] == full_question
    assert "secret" not in logged_output_question
    assert json.loads(event.data)["questions"][0]["question"] == full_question
...
def test_log_serialization_failure_cannot_abort_conversion():
    ...
    assert event is not None
    assert json.loads(event.data)["deltaText"] == "hello"


def test_info_backend_failure_cannot_abort_conversion(monkeypatch):
    failing_logger = _FailingLogBackend()
    monkeypatch.setattr(
        "secbaas.community.core.service.sse._default_converter.logger",
        failing_logger,
    )
    ...
    assert event is not None
    assert json.loads(event.data)["deltaText"] == "hello"
    assert failing_logger.info_calls == 1


def test_conversion_error_is_logged_and_preserved(monkeypatch, caplog):
    conversion_logger = _capture_conversion_logs(monkeypatch, caplog)
    ...
    monkeypatch.setattr(
        "secbaas.community.core.service.sse._default_converter._transform_chunk",
        _raise_conversion_error,
    )
    converter = DefaultStreamConverter(conversion_logging_enabled=True)

    with pytest.raises(ValueError, match="convert boom"):
        converter.convert(
            StreamChunk(type="delta", content="hello"),
            run_id="run-log",
        )
    ...
    messages = _conversion_messages(caplog, conversion_logger)
    assert len(messages) == 1
    _, logged_output = _conversion_payloads(messages[0])
...
def test_exception_backend_failure_preserves_conversion_error(monkeypatch):
    failing_logger = _FailingLogBackend()
    ...
    assert failing_logger.exception_calls == 1
