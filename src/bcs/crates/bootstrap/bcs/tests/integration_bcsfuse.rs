...
async fn fuse_without_bcsfuse_uses_local_engine() {
    ...
    let participants = vec![
        ParticipantInfo { bot_uuid: bot1.bot_id.clone(), role: Some("driver".into()), tags: Vec::new(), message_view_scope: None },
        ParticipantInfo { bot_uuid: bot2.bot_id.clone(), role: Some("consultant".into()), tags: Vec::new(), message_view_scope: None },
    ];
    let group = client
