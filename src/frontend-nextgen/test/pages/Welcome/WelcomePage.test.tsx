...
describe('Welcome 欢迎页(Open 形态默认入口)', () => {
  ...
  it('ScenariosSection 渲染场景标题与截图资源(src/assets/Images/scenarios,四卡七图)', async () => {
    mockedGetCurrentAuthUser.mockRejectedValueOnce({ response: { status: 401 } });
    await act(async () => {
      render(<Welcome />);
    });
    ...
    // (与 Header/Footer 的 brand.Logo 同桩),故按 alt 而非 src 路径区分七张场景图。
    const SCENARIO_ALTS = [
      'Bot 发现示例',
      'Bot 协作示例一',
      'Bot 协作示例二',
    ...
    expect(scenarioImages).toHaveLength(7);
