...
def build_public_openapi() -> dict[str, Any]:
    """The BaaS OpenAPI narrowed to the public ``/openapi/v1`` surface.

    Both ``paths`` and ``components`` are narrowed: paths to ``/openapi/v1`` and
    components to only those transitively referenced by those paths.
    """
...
if __name__ == "__main__":  # pragma: no cover - CLI entry for CI
    ...
    dest = sys.argv[1] if len(sys.argv) > 1 else "baas.openapi.json"
    dump_openapi(dest)
