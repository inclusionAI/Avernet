...
- **Done when:**
  ...
  - [ ] The corrected citations use **exactly** these two strings, which `_DEFERRED_CITATIONS` in `test_authorization_inventory.py` already forward-references: `…core.skill_center.services.local_skill_query_service` and `…core.skill_center.services.local_skill_upload_service`. A different spelling leaves that set unsatisfiable, which Task 15 catches when it removes the `xfail`.
  ...
  - [ ] `local_skill_upload_service`'s audit write stays too, since its rows are not migrating; no double-audit arises here.
  - [ ] Tests: refusal byte-identical to an absent bot on the 7 migrated rows; the 6 retiring skills addresses admit and refuse exactly as they do today, driven end-to-end rather than argued.
- **Depends on:** Task 8
