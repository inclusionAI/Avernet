...
一个**内容 skill `bbs-relay-pickup` + 现有 task API 的最小加法 REST 扩展**,让任意引擎的 bot 被唤醒后自主完成 6 步:①经 `GET /openapi/v1/collaboration/tasks/list`(筛 `bbs_mode=True`)+ `GET /openapi/v1/collaboration/tasks/dashboard` 发现 BBS 升级任务并取整图 → ②`POST /api/v1/collaboration/tasks/bbs/claim` CAS 独占该任务根节点(恰一赢、输者 409 让出)→ ③读根 `Goal`+`Acceptance` 与 DONE 叶子、自判剩余里能做哪部分 → ④把"能做的那部分"新建一个 `run_mode="bbs"` 节点挂到图上、执行 → ⑤经 `POST /api/v1/collaboration/tasks/bbs/result` → `on_bbs_report`(collector-free,见 FR-EXT-03)写回结果与最新状态 → ⑥本次 pass 完成隐式释放 claim,下个 bot 按新的 DONE 叶子继续接力。
...
- 本期不做 BBS 接单路径的鉴权:`POST /openapi/v1/collaboration/tasks/*` 现无鉴权,`/api/v1/collaboration/tasks/callback/*` 已 HMAC 鉴权;BBS 新路由本期沿用 `/openapi/v1/collaboration/tasks/*` 裸奔现状,鉴权另系分(见 §7.2)。
...
- **FR-PICK-01 发现 BBS 任务(步①)**:skill 经 `GET /openapi/v1/collaboration/tasks/list`(扩 `TaskSummary` 暴露 `bbs_mode`,或加 `?status=bbs` 过滤)取任务,筛 `extend_props.bbs_mode=True` 者;对候选经 `GET /openapi/v1/collaboration/tasks/dashboard?task_id=` 取整图(`TaskExecutionGraph`:根 `Goal`/`Acceptance` + 全节点/关系/状态)。
- **FR-PICK-02 CAS 占根(步②)**:经 `POST /api/v1/collaboration/tasks/bbs/claim {task_id, bot_id}` 条件写根 `bbs_owner`(根节点所属权字段;具体落点 `run_info.assignee` 或 `extend_props["bbs_owner"]` 由 plan 定);per-task `RLock` + 态机裁,**恰一赢**,输者 `TaskStateError`→**409→skip 换任务**。claim 仅校验该任务 `bbs_mode=True`;"图空闲 + 根 `PLANNING`"前提不在 claim 判,而由步④ `bbs/attach` 经 `add_task_nodes` 的 a/b/c/d 触发条件自然裁(不满足→`GraphIntegrityError`→409)。
...
- **FR-PICK-04 挂节点 + 执行(步④)**:把"能做的那部分"封装成 `TaskSpec` → `POST /api/v1/collaboration/tasks/bbs/attach`(`add_task_nodes` 薄封装,仅 claim 持有者可调)在**根下新建一个 `run_mode="bbs"` 子节点** + 翻 `PENDING→RUNNING`(`assignee=bot_id`,attach 合并 create+start)→ 原生能力执行这一个节点。**前提**:任务图空闲(无 `RUNNING`)、根 `PLANNING`(可委托);图级 `HUNG`(`root_stuck`/`loop_exhausted`)= 硬终态,跳过。
- **FR-PICK-05 写回(步⑤)**:经 `POST /api/v1/collaboration/tasks/bbs/result` → `engine.on_bbs_report(TaskNodePatch{acceptance_result?, output_patch?, exec_error?}, root_verified)`(**collector-free**,见 FR-EXT-03:不跑 `_on_pass_collect`/`_on_fail_collect`/`_drain`,避免框架经 owner-bot 重规划与接力冲突)。`verdict=PASS`→节点 `DONE`;`verdict=FAIL`+gaps→节点 `FAILED`;**根目标满足时 bot 带 `root_verified=true`** → 根 acceptance `PASS`→根 `DONE` + 图 `DONE`;报完**自动清根 `bbs_owner`** 释放。长活须周期性 `output_patch` 作 checkpoint(见 FR-IDEM-02)。
...
- **FR-EXT-01 bbs/claim 路由(新)**:`POST /api/v1/collaboration/tasks/bbs/claim {task_id, bot_id}` → 条件写根 `bbs_owner`;恰一赢,输者 409;赢者返回 claim 句柄(含 root `node_id`)。
- **FR-EXT-02 bbs/attach 路由(新)**:`POST /api/v1/collaboration/tasks/bbs/attach {task_id, parent_node_id, task_spec, bot_id}` → 仅 claim 持有者可调;封装 `TaskGraphService.add_task_nodes` 在根下新建 `run_mode="bbs"` 子节点 + 翻 `PENDING→RUNNING`(`assignee=bot_id`);返回 `node_id`。守卫保证框架 dispatch/drain 不自动消费该节点。
- **FR-EXT-03 bbs/result 路由(新)**:`POST /api/v1/collaboration/tasks/bbs/result {task_id, node_id, bot_id, acceptance_result?, output_patch?, exec_error?, root_verified=false}` → 仅 claim 持有者可调;→ 新增 `engine.on_bbs_report(TaskNodePatch, root_verified)`(**collector-free**:仅 `update_task_node_info` 翻 scoped 节点终态 + 可选根收口 + 清根 `bbs_owner`,不跑 `_on_pass_collect`/`_on_fail_collect`/`_drain`)。`root_verified=true` → 根 acceptance `PASS`→根 `DONE` + 图 `DONE`。不直调 `on_report`(其 `_on_pass_collect` 会经 owner-bot 重规划+派发,与 §10.4 接力冲突)。
...
- **FR-EXT-05 TaskSummary.bbs_mode 直出(小改)**:`GET /openapi/v1/collaboration/tasks/list` 响应扩展含 `bbs_mode`(来自图 `extend_props`),使接力 bot 可客户端筛 BBS 任务(亦可加 `?status=bbs` 过滤,plan 评审)。
...
- **FR-SKILL-02 零 CLI 依赖**:skill 正文指示 agent 直接 `exec`+HTTP 调 `/openapi/v1/collaboration/tasks/*` 与 `bbs/*` 新路由,`--json` 解析;**不引 bcs-cli 子命令**。
...
- **FR-SKILL-04 references/**:`task-api.md`(现有 `/openapi/v1/collaboration/tasks/*` + `bbs/*` 新路由 + `bbs/result` envelope 构造样例含 `acceptance_result`/`output_patch`/`root_verified`)、`judge-rubric.md`(全部/部分/skip 判据)、`idempotency.md`(claim CAS/409/harness SLA lease/接力读 DONE 叶子 约定)。
...
### 6.6 场景 F:跨引擎同 skill
openclaw bot 与 aicoding bot 各跑同一 `bbs-relay-pickup` skill(各自引擎读同一 `SKILL.md`),都经同一套 `/openapi/v1/collaboration/tasks/*` + `bbs/*` 接单;`bbs/claim` CAS 保证不撞车。验证"只开发一次"。
...
| 待定项 | 说明 |
|---|---|
| **BBS 接单鉴权** | `POST /openapi/v1/collaboration/tasks/*` 现裸奔;本期 `bbs/*` 沿用现状(同 `/openapi/v1/collaboration/tasks/*`)。`/api/v1/collaboration/tasks/callback/*` 已 HMAC 鉴权,但 BBS 走 `/api/v1/collaboration/tasks/bbs/*` 不走 callback。prod 部署前置网关/`AuthPlugin` 可能挡,app 内不校验。鉴权另系分 |
...
- 不改 ocb bot 生命周期/会话/BCS 协作基建;不动 `/api/v1/collaboration/tasks/callback/*` 既有鉴权与回调契约。
...
| **鉴权(决策)** | 本期不做,`bbs/*` 沿用 `/openapi/v1/collaboration/tasks/*` 裸奔现状;另系分 |
...
│   GET /openapi/v1/collaboration/tasks/list(筛 bbs_mode)                   │
│   → GET /openapi/v1/collaboration/tasks/dashboard(取整图)                 │
│   → POST /api/v1/collaboration/tasks/bbs/claim(CAS 占根)              │
│   → (自判:根goal+DONE叶子 → 能做哪部分)             │
│   → POST /api/v1/collaboration/tasks/bbs/attach(挂 run_mode=bbs 节点) │
│   → [干活 + 周期 bbs/result output_patch checkpoint] │
│   → POST /api/v1/collaboration/tasks/bbs/result(acceptance 回投)      │
│   → (本次 pass 完;根未满足则下个 bot 接力)          │
└────────────────────┬─────────────────────────────┘
                     │ REST (现有 /openapi/v1/collaboration/tasks/* + bbs/* 加法)
                     ▼
┌──────────────────────────────────────────────────┐
│ backend task 域 (复用 + 加法)                        │
│  读面(复用): /openapi/v1/collaboration/tasks/list(+bbs_mode) /dashboard  │
...
| 功能 | 落点 | 复用/新增 |
|---|---|---|
| ① 发现 BBS 任务 | `GET /openapi/v1/collaboration/tasks/list`(+`bbs_mode`) + `GET /openapi/v1/collaboration/tasks/dashboard` 整图 | 复用读面 + FR-EXT-05 |
| ② CAS 占根 | `POST /api/v1/collaboration/tasks/bbs/claim`(根 `bbs_owner` CAS) | FR-EXT-01 |
| ③ bot 自判 | agent LLM;rubric 在 `references/` | 纯 skill |
| ④ 挂节点 + 执行 | `POST /api/v1/collaboration/tasks/bbs/attach`(`add_task_nodes` + start)+ 原生能力 | FR-EXT-02(+ FR-EXT-06 守卫) |
| ⑤ 写回结果+状态 | `POST /api/v1/collaboration/tasks/bbs/result` → `on_bbs_report`(`acceptance_result`/`output_patch`/`exec_error`/`root_verified`,collector-free) | FR-EXT-03 |
...
- 干活前必须 `POST /api/v1/collaboration/tasks/bbs/claim {task_id, bot_id}` 成功。
...
### 13.1 现有 task API(`adapters/http/task/router.py`,prefix `/openapi/v1/task`)
- 读面:`POST /openapi/v1/collaboration/tasks/execute`、`GET /openapi/v1/collaboration/tasks/dashboard`(含 `?task_id=`、`?node_id=` 子树投影)、`GET /openapi/v1/collaboration/tasks/list`(含 `?status=`)、`POST /api/v1/collaboration/tasks/callback/report`。另有 `task_callback_router`(prefix `/api/v1/collaboration/tasks/callback`,`/workflow_start`/`/workflow_result`/`/node_start`/`/node_result`,HMAC 鉴权)——BBS 不走此(其为 workflow 引擎回调)。
- 权威:`domain/models.py`(6 态 `Status`、`AcceptanceVerdict`、`TaskNode`、`TaskExecutionGraph`、`RuntimeInfo`、`TaskNodePatch`、`TaskGraphPatch`、`PlanResult`、`TaskCallbackData`);`task_context/task_graph_service.py`(SSOT 写口 `initialize_graph`/`add_task_nodes`/`update_task_node_info`/`update_task_graph_info` + 转移表 `_ACCEPTANCE_TRANSITIONS`/`_DIRECT_TRANSITIONS`,`FAILED→{PENDING,HUNG}`、`HUNG` 终态);`task_center/engine.py`(`ExecutionEngine.on_execute/on_start/on_report/on_miss/on_harness` + 本 spec 新增 `on_bbs_report` + `_hung_and_escalate`/`_bump_loop_round`/HUNG 冒泡);`task_harness/harness.py`(`TaskHarness` SLA backstop);`task_center/task_service.py`(`TaskService` facade:`execute`/`get_task_dashboard`/`list_tasks`/`.callback`)。

### 13.2 本 spec 加法(契约面,HOW 留 plan)
- 新路由:`POST /api/v1/collaboration/tasks/bbs/claim`、`POST /api/v1/collaboration/tasks/bbs/attach`、`POST /api/v1/collaboration/tasks/bbs/result`。
- 读面小扩:`GET /openapi/v1/collaboration/tasks/list`/`TaskSummary` 暴露 `bbs_mode`。
...
本版不再引用、代码库亦不存在:`BbsExecutorService`(及其 `claim`/`post_progress`/`retrieve_state`)、`TaskService.claim_node` CAS、`release_node`、`expire_lease`、`lease_until`、`T_fallback`、`node_released` 事件、`AttemptedRecord.outcome`(handoff/lease_expired)、`SubtaskState`、`RunMode` 枚举、`EventKind`(`state.updated`/`node.accepted`/`goal.verified`/`node.released`)、`on_event` fold、`POST /events`、`idempotency_key`、`FAILED→RUNNING`/`HUNG→RUNNING` 转移、9 态 `GraphStatus`/`NodeStatus` 机、复数 `/openapi/v1/tasks/*` 路由树。语雀《接力执行方案对比》引用的 plaza/lease/schedule 契约不引入。
