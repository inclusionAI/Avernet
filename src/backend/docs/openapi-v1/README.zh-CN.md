...
**要写调用这套接口的客户端？** 请从 **[`usage.zh-CN.md`](usage.zh-CN.md)**
（[English](usage.md)）开始 —— 面向调用方的使用文档：什么场景带什么凭证、应用身份如何
取得用户 Bot 的授权、每个操作在请求上要写什么，以及被拒绝时如何诊断。它完全站在网关
之外来写，刻意不提本 README 记录的任何内部机制——修改时请保持这一点：调用方看到的只有
一个地址和一种响应结构，而转发的 principal、路由表、准入模式恰恰是这个地址存在的意义
所在。
