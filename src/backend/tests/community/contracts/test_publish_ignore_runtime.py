...
async def test_consumer_calls_signed_pinned_runtime(world, engine_success):
    from tests.community.factories.publish_ignore import (
        seed_publish_ignore,
        assert_ignore_engine_called,
    )

    seed_publish_ignore(world, engine_success=engine_success)
    service = world.get(PublishIgnoreServiceProtocol)
    command = PublishIgnoreCommand(
        "ignore-bot", "ignore_owner", 3, "online", "add", "cache", "contract"
    )
