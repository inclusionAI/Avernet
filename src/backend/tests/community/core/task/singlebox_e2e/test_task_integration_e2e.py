        ...
        "task_spec": {
            ...
            "goal": {
                ...
                "acceptances": [
                    {
                        "id": "ac1",
                        "description": "明确存储行业当前是否具备中短期投资价值",
                    },
                    {
                        "id": "ac2",
                        "description": "明确最值得跟踪的细分赛道、公司类型和核心变量",
                    },
                    {
                        "id": "ac3",
                        "description": "提供市场规模、竞争格局、技术演进、客户需求四大维度的系统分析",
                    },
                    {"id": "ac4", "description": "至少形成 5 条核心投资判断"},
                    {
                        "id": "ac5",
                        "description": "每条投资判断需同时说明支持证据、风险因素、需要进一步验证的问题",
                    },
                ],
            },
        },
...
def _wake_prompt() -> str:
    ...
    return (
        ...
        "  步⑤ POST /api/v1/collaboration/tasks/bbs/result 写回(verdict=PASS、done 列出达成的 AC、"
...
class TestTaskIntegrationE2E(unittest.TestCase):
    ...
    async def _run(self, loop: asyncio.AbstractEventLoop) -> None:
        ...
        role_ids = {
            nid: await prov.create_bot(bot_name=name) for nid, name in ROLE_BOTS.items()
        }
                ...
                if ep.get("bbs_mode"):
                    ...
                    if (not busy and not held) or wakes == 0:
                        if wakes < _MAX_WAKES:
                            wakes += 1
                            print(
                                f"[wake#{wakes}] 唤醒 relay bot 自驱 bbs-relay-pickup ..."
                            )
                            try:
                                run = await adapter.send_and_wait_async(
                                    bot_id=relay_id,
                                    message=_wake_prompt(),
                                    timeout=600.0,
                                )
        ...
        for t in g.get("tasks") or []:
            ...
            print(
                f"  - {t.get('node_id'):28} {t.get('status'):8} "
                f"mode={ri.get('run_mode') or '-':11} assignee={str(ri.get('assignee') or '-')[:24]}"
            )

        self.assertEqual(
            g.get("status"), "DONE", f"全图未闭环 DONE:status={g.get('status')}"
        )
        ...
        self.assertEqual(
            (ov["run_info"] or {}).get("run_mode"),
            "single_bot",
            "N_overview 非 single_bot",
        )
        self.assertEqual(
            (ov["run_info"] or {}).get("assignee"),
            role_ids["N_overview"],
        ...
        for gid in ("N_market", "N_tech", "N_customer"):
            ...
            self.assertTrue(
                str((nd["run_info"] or {}).get("assignee") or "").startswith(
                    ("grp_", "bcs_grp_")
                ),
        ...
        group_count = sum(
            ...
            if (n.get("run_info") or {}).get("run_mode") == "coop_group"
        ...
        self.assertEqual(
            nm["status"], "HUNG", "N_field_interview 未 HUNG(应 MISS@MAX→HUNG 升 BBS)"
        )
        self.assertTrue(
            (g.get("extend_props") or {}).get("bbs_mode"), "图未置 bbs_mode(BBS 未升)"
        )
        ...
        for n in bbs_scoped:
            self.assertEqual(
                n.get("status"), "DONE", f"scoped 未 DONE:{n.get('node_id')}"
            )
            self.assertEqual(
                (n.get("run_info") or {}).get("assignee"),
                relay_id,
