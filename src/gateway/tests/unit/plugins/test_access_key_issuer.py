...
def issuer() -> AccessKeyIssuer:
    return AccessKeyIssuer(
        AccessKeyRepository(build_database()),
        BarePrincipalSigner(PrincipalSignerConfig(signing_key="k")),
        clock=lambda: _FIXED_NOW,
    )


async def test_issue_persists_row_and_returns_record(issuer: AccessKeyIssuer) -> None:
    ...
    rec = await AccessKeyRepository(build_database()).find_access_key_by_token(
        issued.token
    )
