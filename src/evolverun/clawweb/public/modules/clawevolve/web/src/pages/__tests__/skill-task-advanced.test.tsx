...
vi.mock('../../hooks/useEvolveModelConfig', () => ({
  useEvolveModelConfig: () => ({ defaultModel: 'GLM-5.2', models: ['GLM-5.1', 'GLM-5.2'] }),
}))
...
describe('fixed Skill advanced task form', () => {
  ...
  it.each(['diagnose', 'full'])('submits %s with defaults Owner, ignores forged URL identity and keeps model editable', async (type) => {
    ...
    expect(api.bots.list).toHaveBeenCalledWith({ ownerId: 'original-owner', status: 'all' })
    expect(api.bots.list.mock.calls.every(([input]) => input.ownerId === 'original-owner')).toBe(true)
  ...
  it.each(['missing', 'wrong-owner', 'failure'])('never falls back to another Bot when the fixed Bot is %s', async (kind) => {
    if (kind === 'failure') api.bots.list.mockRejectedValue(new Error('Bot 查询失败'))
    else api.bots.list.mockResolvedValue({ bots: [kind === 'missing' ? { ...bot, botId: 'other-bot' } : { ...bot, ownerId: 'viewer' }] })
    open()
  ...
  it('ignores a late response for the previous fixed Skill after query navigation', async () => {
    ...
    api.bots.list.mockResolvedValue({ bots: [{ ...bot, botId: 'second-bot', ownerId: 'second-owner' }] })
    open('full')
