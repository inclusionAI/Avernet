...
- `git://...`：Repo 内容定位；`services/git_sync.py::GitSyncService` 管理 bootstrap、周期同步、DB/缓存、OSS 散目录及下载包。bootstrap clone 默认在 20 秒超时，可由 `GIT_CLONE_TIMEOUT_SECONDS` 覆盖；超时清理半成品 bare repo 后进入既有 OSS fallback。发布的 Singlebox 默认配置不提供远程 Repo URL，启动时只从已有的 host-side `~/aiworkbench/skills-repo` 初始化 SQLite 和 MarketCache；本地目录不存在时记录缺失并继续启动。显式运行时 overlay 仍可配置 URL，用于需要远程仓库的本地集成。改 Git 供给时同时核对 `repository_catalog_service.py` 和实际消费端，避免只验证 DB。
