...
from secbaas.spi.bot_service import BotBindingData, LogRelationPayload
...
class TestLocalBotServicePlugin:
    ...
    async def test_report_logs_debug(self):
        ...
        with patch(
            "secbaas.plugins.bot_service.local._local_plugin.logger"
        ) as mock_logger:
...
class TestAiohttpBotServicePluginReport:
    ...
    async def test_report_http_error_logs_warning(self):
        ...
        with patch("secbaas.plugins.bot_service.real._plugin.logger") as mock_logger:
            await plugin.report(payload)
