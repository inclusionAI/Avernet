...
- [x] 2.1 写 SKILL.md frontmatter（`name=task-loop`、`version=1.0.0`、`author`、`tags=[task,loop,orchestrate]`、`description` 说明预装等价+5段）
- [x] 2.2 写 `# task-loop` 总述：预装即齐备 / 5 段 / 只跑命中段 / 不并段
- [x] 2.3 写 `## 路由规则`：5 触发词各→一段映射 + 优先级 + 未命中静默 no-op + "仅命中段"硬约束 + 反例
...
- [ ] 7.6 回归：装到 chat/owner/worker/relay 各触发一轮，输出与"单独装该段"一致（recognition 卡片 / planning `List[TaskSpec]`+has_gap / search 4 态 / acceptance result / bbs result）；无 cue 时静默  > ⏳ 待环境验证：需 Avernet singlebox + 4 类 bot(chat/owner/worker/relay) 触发一轮做端到端回归,本会话无该环境,留待联调环境补验。除 G7.6 外静态校验(段体逐字节 / frontmatter / 路由 / references)均通过。
