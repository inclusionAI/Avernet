"""TeClaw adapter 测试桩:Noop(安全零值) + Mock(记录调用)。"""
...
class NoopTeClawAdapter:
    """No-op TeClaw adapter:返回安全零值、不做任何 I/O。
    ...
    async def create_adapter_session(
        ...
        *,
        ...
        if os.getenv("BAAS_STUB_ENGINE_SESSION_ERROR"):
            raise RuntimeError("stub teclaw: simulated session creation failure")
        if os.getenv("BAAS_STUB_ENGINE_SESSION_SLOW"):
            await asyncio.sleep(2)
        return ("", True)


class MockTeClawAdapter:
    """内存版 TeClaw adapter:记录调用、返回可预期值,供单测断言。"""
    ...
    def __init__(
        self,
        *,
        session_result: tuple[str, bool] = ("mock-teclaw-session", False),
    ) -> None:
