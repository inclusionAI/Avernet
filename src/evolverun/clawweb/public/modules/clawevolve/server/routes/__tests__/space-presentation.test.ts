...
const personal: OcbSpace = { id: "personal-owner", name: "Personal", type: "PERSONAL", role: "ADMIN" };
...
beforeEach(async () => {
  ...
  const ocbSpaces: OcbSpacePort = { listAccessibleSpaces: vi.fn(async () => [team, personal]) };
  const app = express();
...
async function seed(targetSpace: OcbSpace = team) {
  await skills.createAsset({ assetId: "target", versionId: "target-v1", ownerUserId: actor,
    ...
    botId: "target-bot", ocbSkillId: "local-skill", displayName: "evidence-skill",
