...
- **Rule 7** — Adapters must remain thin.  The delivery layer (adapters/web/)
  ...
  4. Domain imports — adapters must not import from ``secbaas.core.service``
     or ``secbaas.domain`` at module level.
...
def test_no_domain_imports_in_adapters(project_architecture):
    """Rule 7: Adapters must not import from ``secbaas.core.service``
    or ``secbaas.domain``.
    ...
    rule = (
        Rule()
        .modules_that()
        .are_sub_modules_of("secbaas.adapters.web")
        .should_not()
        .import_modules_that()
        .are_sub_modules_of("secbaas.core.service")
    )
