...
class TestRouter:
    def test_workflow_result_success(self, client):
        c, svc = client
        r = c.post("/openapi/v1/collaboration/tasks/callback/workflow_result", json=_body(loop_task_id="t1::root1"))
        assert r.status_code == 200, r.text
        assert svc.callback.calls[0][0] == "result"

    def test_node_result_success(self, client):
        c, svc = client
        r = c.post("/openapi/v1/collaboration/tasks/callback/node_result", json=_body(node=True))
        assert r.status_code == 200, r.text
        assert svc.callback.calls[0][1].loop_task_id == "t1::c1"

    def test_workflow_start_success(self, client):
        c, svc = client
        r = c.post("/openapi/v1/collaboration/tasks/callback/workflow_start",
    ...
    def test_node_start_success(self, client):
        c, svc = client
        r = c.post("/openapi/v1/collaboration/tasks/callback/node_start", json=_body(node=True, status="RUNNING"))
        assert r.status_code == 200, r.text
        assert svc.callback.calls[0][0] == "start"
    ...
    def test_correlation_error_400(self, client):
        ...
        r = c.post("/openapi/v1/collaboration/tasks/callback/workflow_result", json=_body())  # 无 loop_task_id,registry 空
        assert r.status_code == 400, r.text

    def test_validation_422(self, client):
        c, _ = client
        r = c.post("/openapi/v1/collaboration/tasks/callback/node_result", json={"task_id": "t1"})  # 缺必填
