...
- **结果回收分两种**：
  ...
  - **execute/verify 经 `report_result` 异步回投**（投递给 bot → bot 凭已装 skill 执行完上报 → `POST /api/v1/collaboration/tasks/callback/report` → `on_report` 翻态推进）。
...
- 新 `adapters/http/task/router.py`（thin）：`POST /openapi/v1/collaboration/tasks/execute`、`GET /openapi/v1/collaboration/tasks/dashboard`、`POST /api/v1/collaboration/tasks/callback/report`（回投 → `callback.report_result`）。Router 只翻译协议，不持领域策略。
...
- **AC-3**：执行结果经真实回投链路（`/api/v1/collaboration/tasks/callback/report` → `callback.report_result` → `on_report`）翻态、传播、补救；`GET /openapi/v1/collaboration/tasks/dashboard` 反映分解树与状态推进。
...
- **R2 回投路径**：真实引擎异步回调 vs 适配层同步回投。为确定性与 singlebox 可行性，倾向 **适配层时效内回投**（`deliver` 调 skill 取结果 → `POST /api/v1/collaboration/tasks/callback/report`），但真实异步回调仍需验证。需 plan 决策。
