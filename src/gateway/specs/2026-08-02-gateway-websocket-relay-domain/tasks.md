...
- **Done when:**
  ...
  - [x] One `base_url` standard for **every** server, enforced by `Server.__post_init__`: a scheme is required (`http`/`https`/`ws`/`wss`), the authority must name a host, and the port (if written) must be numeric; a value missing any of these fails the boot, naming the server. `http_base_url` / `websocket_base_url` re-spell it per plane. This also fixed the shipped bare-host samples, which produced a relative URL with an empty host.
  - [x] The host is checked separately from the scheme: `https:///engine-proxy` satisfies a scheme check but has an empty authority, so it would fail at call time rather than at boot.
