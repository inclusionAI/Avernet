...
function parseFormQuestion(raw: Record<string, unknown>): StageSkillFormQuestion {
  ...
  if (!Array.isArray(raw.questions) || raw.questions.length < 1 || raw.questions.length > MAX_FORM_ITEMS) {
    throw new Error(`question.questions 必须包含 1 到 ${MAX_FORM_ITEMS} 个问题`);
  }
  ...
  const parsed = { format: "form" as const, title, ...(description ? { description } : {}), questions };
