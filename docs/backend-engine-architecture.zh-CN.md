...
| 1 | 骨架：微内核的七个角色 | 图 1 |
| 2 | 契约：两类接口，方向相反 | 图 2 |
| 3 | 引擎：异构接入怎么做到改动可控 | 图 3、图 4 |
| 4 | 链路：一次请求怎么穿透两个模块 | 图 5 |
| 5 | 治理：架构怎么才能不腐烂 | 图 6 |
| 6 | 收口：可迁移的五条经验 | — |
...
| --- | --- | --- | --- |
| **传输协议适配层** | 把协议报文翻译成领域调用，再把领域结果翻译回协议报文。只做翻译，不做任何业务判断 | 面向租户与前端的 HTTP 入口，40+ 个路由分组 | 面向单个 Bot 的 HTTP 与长连接入口 |
| **领域服务接口** | 内核对外提供的能力清单。进出的请求与响应都是**领域对象**，不是裸报文 —— 这是内核能与传输协议彻底解耦的前提 | 115 个领域服务接口 | 23 个能力域接口 |
| **领域内核** | 全部领域逻辑，不认识任何传输框架 | 55 个领域模块 | 领域模型 + 引擎适配层 |
| **系统边界契约** | 本系统与外部系统接触的每一处，都在这里定义成可替换的契约 —— 让同一份内核能落进不同的部署环境 | 35 处边界：数据库、缓存、对象存储、身份、审批 … | 每个引擎的原生接口 |
| **系统边界实现** | 契约的具体实现，可整套替换 | 开源 / 本地替身 / 企业，三套 | 每引擎一套传输实现 |
| **底座** | 各层共享的基础类型，不依赖任何上层 | 生命周期、跨层数据结构 | 线协议信封 |
| **依赖注入根（DI Root）** | 唯一挑选具体实现的地方 | 59 个装配模块，按部署形态选择 | 按引擎组装，每引擎一个 |

层数和依赖方向完全一致，不同的是**各自的系统边界通向哪里**：Backend 的边界通向基础设施（数据库、缓存、对象存储、身份、审批），Engine 的边界通向异构 Agent 引擎。同一套分层方法，套在两类完全不同的外部世界上。

### 1.2 底座：共享的基础类型只能往下放

Engine 的传输实现处在依赖链末端，规则上禁止它依赖领域内核。但传输层产生的线协议信封，恰恰是领域内核要消费的东西。

两边都要用它，谁也不能依赖谁，它就只能落在两边都够得着的下方。底座因此被定义成「不依赖本模块任何其他层」的一层，线协议信封是它的第一个住户。
...
宪法里最核心的一条：**领域服务接口和系统边界契约是两类不同的东西，必须分开定义、分开记录、分开版本化、分开测试。**

- **领域服务接口**：外部调内核。定义成独立于传输层的接口，进出都是领域对象。
- **系统边界契约**：内核调外部系统。每一处跨出本系统的调用，都要先有一份契约。
...
| | 领域服务接口 | 系统边界契约 |
| --- | --- | --- |
| **谁破坏谁** | 内核改接口 → 所有调用方受影响 | 契约变动 → 所有边界实现受影响 |
| **怎么测** | 一致性测试：实现是否满足接口 | 契约测试 + 替身：内核在各种外部行为下是否正确 |
| **谁可替换** | 内核实现可替换，调用方无感 | 边界实现可替换，内核无感 |
...
挑选发生在**唯一的依赖注入根**，由一个部署形态开关驱动，五档：开源、单机、测试、企业、企业测试。

两个设计细节值得说：

**开关是纯选择器。** 它只回答「哪一档」，不提供任何派生属性。「这一档装配了什么」由**装了哪些模块**回答，而不是由开关上的某个方法回答。
...
![异构引擎引擎适配层](images/arch/03-engine-acl.svg)
...
| 角色 | 接一个新引擎时 |
| --- | --- |
| 传输协议适配层 | **不动** —— 对外表面与引擎无关 |
| 领域服务接口（能力域接口） | **不动** —— 领域协议与引擎无关 |
| 引擎适配层 | **新增一套** —— 领域形状 ⇄ 该引擎原生形状 |
| 系统边界契约（引擎原生接口） | **新增一套** —— 该引擎的原生接口 |
| 边界实现（传输） | **新增一套** —— 连上去、收发帧 |
| 依赖注入根 | **新增一个** —— 把上面三者拼起来，并声明能力矩阵 |
...
引擎适配层位于领域内核之内，但它只依赖该引擎的**原生端口契约**；具体的传输实现由依赖注入根注入。因此「内核不依赖系统边界实现」这条约束对**整个内核成立，包括引擎适配层自己**。
...
每个引擎在自己的依赖注入根声明一份能力矩阵，分三态：

- **原生支持** — 引擎适配层为它接上了该引擎的原生能力；
- **受限 / 降级** — 支持但有约束，**声明时必须写明原因**；
- **未接入** — 上层不派发，前端直接隐藏功能。

核心规则是：**一个能力存在，当且仅当引擎适配层真的把它接上了。** 声明与实现的一致性在**装配阶段**交叉校验 —— 声明了却没接上，启动就失败，而不是等到线上第一次调用。
...
**Backend 侧**：① 调用方 → ② 传输协议适配（解析、鉴权、协议校验）→ ③ 领域服务接口（拿到的是契约，不是实现）→ ④ 领域编排（按属主解析 Bot、判定发布态、解析设备上下文）→ ⑤ 出口契约（唯一的跨系统抽象）
...
| 领域服务接口 | `api/`（定义在各领域模块内，此处转出） | `core/<domain>/protocol.py` |
...
| 我要做的事 | 应该动的位置 |
| --- | --- |
| 加一个公开接口 | 传输协议适配层加路由 → 注入对应的领域服务接口 |
| 加一个领域能力 | 领域模块内定义契约 + 实现 → 领域服务接口层转出 |
...
| `src/backend/src/agentclaw/community/api/README.md` | 领域服务接口层的设计理由 |
