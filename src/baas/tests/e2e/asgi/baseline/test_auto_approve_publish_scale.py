...
class TestScaleUpAutoApprove:
    ...
    async def test_scale_up_2_devices_auto_approve(
        ...
        """2→4 devices with hooks: auto_approve + 2 device callbacks → ACTIVE or SUCCESS."""
        t0 = time.monotonic()
        bot = await create_and_activate_bot(
        ...
        status = await wait_for_publish_status(
            api, publish_id, {"ACTIVE", "SUCCESS"}, timeout_seconds=0.5
        )
        assert status in ("ACTIVE", "SUCCESS"), (
            f"Expected ACTIVE or SUCCESS, got {status}"
        )
        if status == "SUCCESS":
            await _assert_replica_desired(api, bot["bot_uuid"], 4)
        else:
            tlog.info("Publish is ACTIVE — scale may still be provisioning")
