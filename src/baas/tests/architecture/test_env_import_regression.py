...
Core logic must NOT import from infrastructure layer (``community.infra``)
...
DEBT FULLY PAID: ``env_utils`` moved to ``community.core.utils.env_utils``
...
def test_device_manage_no_get_current_env_re_export():
    ...
    any external consumer relying on ``from community.core.service.device_manage
