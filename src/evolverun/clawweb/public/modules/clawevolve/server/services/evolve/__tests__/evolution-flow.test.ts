...
describe("fixed Evolve flows", () => {
  ...
  it("accepts disabled stages from the Skill Center unified stage-selection payload", () => {
    expect(botEvolutionFlow.resolveSelection(
      fullInput({ taskType: "diagnose" }),
      { diagnose: true, hardening: false, plan: true, optimize: false },
    )).toEqual({
      diagnose: true,
      hardening: false,
      plan: true,
    ...
    expect(() => botEvolutionFlow.resolveSelection(
      fullInput({ taskType: "diagnose" }),
      { diagnose: true, hardening: true, plan: true, optimize: false },
    )).toThrow("Stage 开关不合法: hardening");
  });
