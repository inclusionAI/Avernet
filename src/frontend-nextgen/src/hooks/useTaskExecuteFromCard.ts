 ...
 *
 * 卡片 task_ready 点「确认」→ aixBridge.submit('执行任务', {__taskAction:'execute', task})
 ...
 * `task_spec.context.title` 缺失时由平台层从 `task.goal` 派生「短摘要」。
...
export interface UseTaskExecuteFromCardOptions {
  ...
  /** relay 首棒必须触发指定 holder 回复；群聊实现需定向到 driver/manager，不能使用静默注入。 */
...
export function useTaskExecuteFromCard({
      ...
      void (async () => {
        try {
          ...
          const form: TaskComposerForm = {
            title: task.title?.trim() || deriveTaskTitle(task.goal),
            objective: task.goal ?? '',
            instruction: [
  ...
  }, [appendAssistantMessage, context, streamAssistantMessage, submitPanelMessage, submitTaskExecutionMessage]);
}
