...
- Timestamp-removal follow-up (2026-09-21): the user explicitly requests removing both Provider-specific timestamp columns from this unmerged PR's DB SQL and code. Remove the timestamps from BotProviderRecord and its consumers; retain Bot lifecycle timestamps and legacy binding response timestamps. This is a scoped correction of MySQL 029 / SQLite 030, not permission to rewrite earlier migrations or retained database history.
- Execution: first add failing schema and binding-timestamp compatibility tests in `bcs-bot-store/tests/bot_provider_migration.rs` and `bot_provider_storage.rs`; then remove the columns/record fields and timestamp CAS in `bot_provider_storage.rs`. The user clarified that no replacement version field or new explicit locks should be added. Retain business-state conditions, unique constraints and atomic gateway projection writes; repeated affiliation does not rewrite a saved webhook, and concurrent explicit webhook updates use database write ordering without a metadata version precondition. The preexisting gateway binding lock remains unchanged. Update Memory store, projection, core callers, shared conformance, bootstrap/admin schema assertions and the correction document. Run affected Cargo suites, bootstrap migration/HTTP tests and boundary gates; self-review only, no unrelated splitting or live database operations.
- Enum follow-up approved (2026-09-21): keep `ProviderBotConnectionMode::{Gateway, Plugin}` in `bcs-domain` as the only definition. Re-export it under the DTO, registration and Bot-storage names. All wire/token/DB values use `gateway`/`plugin`; retain gateway defaults at Provider-admin and explicit plugin defaults at token/register. This supersedes the draft `upstream` spelling below, not the upstream connection semantics.
- Follow-up plan: (1) add failing shared-type, serde and persisted-value tests; (2) replace duplicate enums, centralize DB string conversion, update consumers and the undeployed SQLite 030 constraint; (3) align OpenAPI/docs and run affected Cargo contract/module/migration suites, then self-review. Scope excludes earlier migrations, live databases, unrelated file splitting, commit and push. The user-approved undeployed-draft correction is the scoped exception to migration checksum preservation; retained development databases must be explicitly upgraded or recreated and draft tokens reissued before using this version.
- Enum follow-up verification: shared-type/wire tests first failed 2/5 and the raw persisted-value test failed on `upstream` versus `plugin`; all passed after unification. Domain/protocol/Bot/store/register/shared-contract suites passed 536 tests (5 ignored); the final storage/migration rerun passed 12 tests including the additional mode-constraint case. HTTP/OpenAPI/organization suites passed 754 tests; bootstrap migration tests passed 3 and real mounted registration passed 1. Author self-review only (subagents prohibited). All changed source line counts were checked; existing oversized files were not split, per user instruction. `git diff --check` passed. No real MySQL or full Singlebox run was performed. The unchanged dependency checker first hit a shell/locale error; `LC_ALL=C` exposed existing DEP-6/DEP-1 allowlist mismatches and DEP-8 unused-dependency findings. Cargo manifests/lockfiles and CI checks are unchanged. The user subsequently authorized commit and push with `--no-verify` to the existing PR head; no force push or parent-worktree mutation.

- Data-correction follow-up (2026-09-21): remove the dedicated backfill implementation, DB operation, report type, standalone example and backfill-only tests. Retain normal registration, metadata/projection writes and schema migrations. Document historical correction rules in the user's personal knowledge base for a separately approved work order; this change performs no data correction. Publication to the existing PR head with `commit --no-verify` and `push --no-verify` is explicitly authorized.
...
- Keep earlier upstream migrations frozen. Historical correction, audited preflight, writer fencing, consistency validation and rollback belong to a separate work order; ship no dedicated backfill DB method or executable. Consolidate only this PR's unused draft migrations per the cleanup decision above; do not change any live database.
...
**Files:** focused schema/store tests; Provider integration docs; affected `CONTEXT.md`; this plan and progress ledger. Correction execution is maintained outside the product repository.

1. Preserve tests for normal metadata/projection writes and upstream identity, credentials and ownership; remove tests exclusive to the retired backfill operation.
2. Remove the dedicated backfill implementation and command. Hand off correction rules to the separate work-order document; retain product documentation for additive schema, legacy-read dual writes, read-cutover prerequisites and rollback.
...
5. Self-review (subagents prohibited) and report actual checks, unavailable validations and remaining risks. Publish only under the user's explicit authorization recorded above.
...
- Original implementation checkpoint: all six tasks were implemented. Bot-backed membership, gateway-only dual writes, both read selectors, legacy mutation compatibility, scoped registration, additive schemas and audited binding backfill were present. The later data-correction follow-up removes the dedicated backfill capability. The unused registration-table draft was removed after confirming this PR has never been deployed; no production migration was performed.
