...
describe('persisted Skill operation audit', () => {
  ...
  const registration = { assetId: 'asset', versionId: 'v1', ownerUserId: 'owner', botId: 'bot',
    externalSkillId: '44', displayName: 'Skill', description: 'actual description', packageRef: 'base', packageSha256: 'base-sha' };
  ...
  it('keeps the first Stage input and preserves lifecycle and loop metadata', async () => {
    await tasks.createTask({ ...task, configJson: '{}' });
    await tasks.createStep({ taskId: 'task', stepId: 'stage', stepType: 'hardening', stepNo: 1,
      command: 'test', ext: { loop: { root_step_id: 'stage', round: 1 }, existing: 'kept' } });
    const before = (await tasks.findStep('stage'))!;
    const input = { goal: 'original goal', target_skill: { path: '/candidate/skill' } };
    expect(await tasks.freezeStageInput('stage', input)).toEqual(input);
    expect(await tasks.freezeStageInput('stage', { goal: 'changed goal' })).toEqual(input);
    const after = (await tasks.findStep('stage'))!;
    expect(after.status).toBe(before.status);
    expect(after.started_at).toBe(before.started_at);
    expect(after.completed_at).toBe(before.completed_at);
    expect(JSON.parse(after.ext_json!)).toEqual({
      loop: { root_step_id: 'stage', round: 1 }, existing: 'kept', initial_stage_input: input,
    });
  });

  it('serializes competing input freezes so callers observe the same snapshot', async () => {
    await tasks.createTask({ ...task, configJson: '{}' });
    await tasks.createStep({ taskId: 'task', stepId: 'stage', stepType: 'hardening', stepNo: 1, command: 'test' });
    const [first, second] = await Promise.all([
      tasks.freezeStageInput('stage', { goal: 'first' }),
      new EvolveRepository(db).freezeStageInput('stage', { goal: 'second' }),
    ]);
    expect(first).toEqual(second);
    expect(JSON.parse((await tasks.findStep('stage'))!.ext_json!).initial_stage_input).toEqual(first);
  });
