...
export class BotWorkflowPermissionRepository {
  ...
  async getViewByIdsForOwner(ownerId: string, botId?: string): Promise<{ restrictedIds: Set<string>; viewableIds: Set<string> } | null> {
    ...
    //   Query ALL can_view=1 records under this owner, regardless of bot_id value.
    //   This means the owner can see any workflow where they (or any of their bots) have view permission.
   ...
   *   3. Check owner-level permission (bot_id IS NULL OR bot_id = '') → allow if can_edit = 1
   *   4. If botId provided, check bot-level permission → allow if can_edit = 1
   *   5. Otherwise deny
   */
  async hasEditPermission(workflowId: string, userId: string, botId?: string): Promise<boolean> {
    ...
    // Step 3: Check owner-level permission (bot_id IS NULL OR empty)
    ...
    // Step 4: If botId provided, check bot-level permission
    if (botId) {
    ...
    // Step 5: Has records but no edit permission
    return false;
  }
