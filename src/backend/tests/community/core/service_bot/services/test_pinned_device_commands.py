...
def test_pinned_device_command_uses_existing_paas_endpoint():
    ...
    assert svc.exec_command_on_bot(bot_uuid='authorized-bot', paas_device_id='physical@template', cmd='probe') == result
...
def test_invalid_result_is_not_legacy(envelope):
    ...
    with pytest.raises(BaasServiceError):
        svc.exec_command_on_bot(bot_uuid='authorized-bot', paas_device_id='physical', cmd='probe')


def test_device_id_cannot_inject_url_path():
    svc = service({'code': 0, 'data': {'exit_code': 0}})
    svc.exec_command_on_bot(bot_uuid='authorized-bot', paas_device_id='device/other?x=1', cmd='probe')
