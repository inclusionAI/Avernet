...
#: any mode is actually selectable.
_APPROVAL_SET = "approval.set"
...
async def list_approval_modes(
    ...
    Answers 501 when the bot's engine cannot set an approval mode, so every
    value listed here is one the write endpoint accepts.
    """
    # Deliberate divergence: the engine's own modes route is its one route with
    # no capability gate, so on an engine declaring no approval capability it
    # advertises three modes while get and set both answer 501.
    #
    # Gated on the *write* capability, not the read. The engine defines
    # approval.get and approval.set independently and gates its own two routes
    # on one each, so an engine may declare the read without the write; keying
    # this route off the read would then advertise three selectable modes while
    # every PUT answers 501. What this route promises is the write's accept-set,
    # so the write's capability is the one that decides whether it has anything
    # to say.
    #
    ...
    if _APPROVAL_SET not in supported:
        raise EngineCapabilityUnsupportedError(
            f"engine does not declare {_APPROVAL_SET}"
        )
    return envelope(
        [
