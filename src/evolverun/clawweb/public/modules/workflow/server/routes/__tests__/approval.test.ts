...
it('uses the server-verified DingTalk user instead of a request empId for approval actions',async()=>{
 const {base}=await fixture([]);
 const submit=(authCode:string,empId?:string)=>fetch(`${base}/resolve`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({authCode,empId,action:'approve',comment:'checked'})});
 expect((await submit('stranger-code','reviewer')).status).toBe(403);
 expect((await submit('reviewer-code','stranger')).status).toBe(200);
 ...
 expect((await submit('reviewer-code')).status).toBe(409);
});

it('rejects URL-era empId-only submissions and exposes public DingTalk auth config',async()=>{
 const {base,row}=await fixture([]);
 const config=await(await fetch(base.replace(/\/1$/, '/auth/dingtalk/config'))).json();
 expect(config).toEqual({clientId:'test-client-id',corpId:'test-corp-id'});
...
it('keeps data resource section decisions in detail across resolve and detail reads', async () => {
 ...
 const response=await fetch(`${base}/resolve`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({authCode:'reviewer-code',action:'approve',comment:'已核对',detail})});
