...
export class InsightImprovementRepository {
  ...
  async listAdmin(input: {
    ...
    const scopeConditions = includeAll ? ["1 = 1"] : [`i.source_type IN (${GOVERNANCE_SOURCE_SQL})`];
    const scopeParams: unknown[] = [];
    if (input.ownerUserId) {
      scopeConditions.push("i.owner_user_id = ?");
      scopeParams.push(input.ownerUserId);
    }
    if (input.botId) {
      scopeConditions.push("i.bot_id = ?");
      scopeParams.push(input.botId);
    }
    if (input.status) {
      scopeConditions.push("i.status = ?");
      scopeParams.push(input.status);
    }
