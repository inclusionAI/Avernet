...
重点验证：POST 写接口必须经过 cookie 鉴权（get_op_ctx），GET 读接口不鉴权。
...
# ---------------------------------------------------------------------------
# POST /api/v1/cache/{key} — 鉴权行为
# ---------------------------------------------------------------------------


class TestSetCacheAuth:
    """POST /api/v1/cache/{key} 必须经过 get_op_ctx 鉴权。"""

    def test_set_cache_success_with_auth(self, authed_client, mock_cache):
        """鉴权通过时正常写入缓存。"""
    ...
    def test_set_cache_rejected_when_auth_fails(self, mock_cache):
        """get_op_ctx 鉴权失败（如无 cookie）时返回 401，且不写缓存。"""
        app = _build_app(
            mock_cache,
    ...
    def test_set_cache_depends_on_op_ctx(self):
        """set_cache 路由必须声明 get_op_ctx 依赖（防回归：误删鉴权参数）。"""
        post_route = next(
            r
...
# ---------------------------------------------------------------------------
# GET /api/v1/cache/{key} — 不鉴权
# ---------------------------------------------------------------------------


class TestGetCacheNoAuth:
    """GET /api/v1/cache/{key} 不依赖 get_op_ctx，无需 cookie。"""

    def test_get_cache_success_without_auth(self, mock_cache):
        """命中缓存返回 200，无需任何鉴权依赖。"""
    ...
    def test_get_cache_has_no_op_ctx_dependency(self):
        """get_cache 路由不应声明 get_op_ctx 依赖。"""
        get_route = next(
            r
            for r in iter_api_routes(router)
