...
> **状态更新（2026-09-10）：以下保留的是 2026-08-20 的历史设计，不是当前重启契约。**
> 当前有效契约见 [AICoding 重启模板快照契约，revision 2](../2026-09-10-aicoding-restart-template-snapshot/spec.md)。
> 新契约明确：模板由前端组合，后端不拉市场、不比较版本；策略复用既有创建或更新服务；
> 确认标记在 `extra_configs` 内，与 `template_config` 同级；保存错误由原主链路告警后继续重启。
> 下文有关“后端主动拉最新”“仅更高版本 update-only”“确认字段位于 body 顶层”及
> “保存失败向接口上抛”的内容不再适用；历史流程图和行号也不可当作现行实现依据。
