...
prepare_bcs_runtime_config() {
    ...
    if [ -n "${BCS_E2E_MOCK_BASE_URL:-}" ]; then
        ...
        sed_args+=(
            ...
            "-e" "/^\[bcsfuse\]/,/^\[/{s|^sync_timeout_ms = [0-9][0-9]*$|sync_timeout_ms = 10|;s|^sync_max_attempts = [0-9][0-9]*$|sync_max_attempts = 1|;s|^sync_retry_base_delay_ms = [0-9][0-9]*$|sync_retry_base_delay_ms = 10|;}"
