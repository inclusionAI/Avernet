...
class SkillSetControlPlaneService:
    @inject
    def __init__(self, repository: SkillSetControlPlaneRepository, bot_repo: BotRepository, runtime: SkillSetRuntimeReconcilerProtocol, legacy_factory: SkillSetServiceFactory, passport: PassportPlugin) -> None:
    ...
    def _bot(self, bot_id: str, actor_id: str) -> dict:
        bot = self._bot_repo.get_by_id_and_owner(bot_id, actor_id)
        if bot is None:
            raise LocalSkillNotFoundError()
        return bot
    ...
    async def add_skill(self, *, bot_id: str, actor_id: str, set_id: str, skill_id: str) -> dict:
        self._bot(bot_id, actor_id)
        return await self._reconcile(bot_id, actor_id, self._repository.add_skill(bot_id=bot_id, set_id=set_id, skill_id=skill_id))

    async def remove_skill(self, *, bot_id: str, actor_id: str, set_id: str, skill_id: str) -> dict:
        self._bot(bot_id, actor_id)
        return await self._reconcile(bot_id, actor_id, self._repository.remove_skill(bot_id=bot_id, set_id=set_id, skill_id=skill_id))

    async def activate(self, *, bot_id: str, actor_id: str, set_id: str) -> dict:
        self._bot(bot_id, actor_id)
        return await self._reconcile(bot_id, actor_id, self._repository.set_active(bot_id=bot_id, set_id=set_id, active=True))

    async def deactivate(self, *, bot_id: str, actor_id: str, set_id: str) -> dict:
        self._bot(bot_id, actor_id)
        return await self._reconcile(bot_id, actor_id, self._repository.set_active(bot_id=bot_id, set_id=set_id, active=False))
    ...
    def resources(self, *, bot_id: str, actor_id: str) -> list[dict]:
        ...
        try:
            default_clis = self._passport.query_passport_clis(bot_id, str(bot.get("entity_id") or owner_id))
        except Exception:
            default_clis = []
        return [
            {
    ...
    async def _reconcile(self, bot_id: str, actor_id: str, mutation: SkillSetMutation) -> dict:
        try:
            await self._runtime.reconcile(bot_id=bot_id, owner_id=actor_id)
