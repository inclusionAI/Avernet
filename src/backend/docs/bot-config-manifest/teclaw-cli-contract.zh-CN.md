...
**平台分两个时点完成校验**，但**两者都在构造 artifact 之前**——这是给你们的保证：
...
**物化时（归档已拉取并解包，此时才有目录树可查）：**

- 规范化（解析符号链接）之后**逃出 `path` 子树**的条目；
- 在解包结果中**不存在**的路径；
- 不是**普通文件**的路径（目录、符号链接、设备文件等）。

> 分开写是因为「`PUT` 时全部拒绝」做不到：URL 形式的归档在 `PUT` 时还没有被拉取，
> 没有目录树可以检查存在性或解析符号链接。写成那样等于给你们一个我们兑现不了的
> 保证。

不管在哪一步拒绝，**到达引擎的每一个 `entrypoints` 值，都是 `path` 目录内一个已
存在的普通文件的相对路径**——这一点不变。
...
- **字段缺席 ≠ 空数组。**没有 `cli_tools` 字段的含义是**「保持现状，不要动」**，
  ...
  这与 §3.4 第 4 条的全量覆盖不冲突：覆盖的前提是平台**发了**这个字段。
