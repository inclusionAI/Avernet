...
Usage::
    ...
    # SOFA mode (default)
    python src/secbaas/main.py --config configs/ --mode sofa

    # Bare mode (no MOSN/Layotto)
    python src/secbaas/main.py --config configs/ --mode bare
...
if __name__ == "__main__":
    ...
    if args.mode == MODE_SOFA:
        # Try enterprise runner first (secbaas-enterprise package installed),
        # fall back to community runner (monolith or community-only).
        try:
            from secbaas_enterprise.plugins.runner.sofa import (
                SofaAppRunnerPlugin,
            )
        except ImportError:
            raise

        SofaAppRunnerPlugin().run(config_path)
    elif args.mode == MODE_BARE:
        from secbaas.plugins.runner.bare import BareAppRunnerPlugin

        BareAppRunnerPlugin().run(config_path)
    else:
        raise RuntimeError("Unsupported run mode %s", args.mode)
