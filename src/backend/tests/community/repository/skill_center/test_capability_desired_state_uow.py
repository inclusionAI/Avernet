...
def test_deactivate_all_sets_converges_to_default_capabilities_only():
    """Ordinary Sets inactive; every skill row retired (the Default's members
    return through the flush); MCP rows retired only where a Set claims them."""
    db = _Database()
    repository = CapabilityDesiredStateRepository(db)
    default, default_skill = _seed_default_with_member(db)
    with db.transactional_orm_session() as session:
        ordinary = SkillSet(
        ...
        session.add_all(
            [
                SkillSetSkill(
                    skill_set_id=ordinary.id, skill_id=set_skill.id, env="dev"
                ),
                SkillSetMCPServer(
                    skill_set_id=ordinary.id, server_code="mcp.claimed",
                    name="claimed", env="dev",
                ),
                BotSkillInstallation(
                    bot_id="bot", owner_id="owner", skill_id=set_skill.id,
                    env="dev",
                ),
                BotSkillInstallation(
                    bot_id="bot", owner_id="owner", skill_id=direct_skill.id,
                    env="dev",
                ),
                BotMCPInstallation(
                    bot_id="bot", owner_id="owner", server_code="mcp.claimed",
                    env="dev",
                ),
                BotMCPInstallation(
                    bot_id="bot", owner_id="owner", server_code="mcp.direct",
                    env="dev",
                ),
            ]
        )
    ...
    with db.orm_session() as session:
        ...
        assert {
            row.server_code for row in session.query(BotMCPInstallation).all()
        } == {"mcp.direct", "mcp.member"}

    # The flush brings back exactly the Default Set's members.
    plan = repository.flush_installations(
        bot_id="bot", owner_id="owner", env="dev",
        engine_type="openclaw", default_engine_types=("openclaw",),
    )
    assert plan.skills_to_install == frozenset({int(default_skill.id)})
    with db.orm_session() as session:
        assert {
            int(row.skill_id) for row in session.query(BotSkillInstallation).all()
        } == {int(default_skill.id)}
