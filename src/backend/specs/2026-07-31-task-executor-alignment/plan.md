...
12. **retry/reroute 统一到 `tick` 驱动 + reroute 交 skill**(修订:原 `_handle_node_failed` 旁路退场)。原 `_handle_node_failed` retry 仅 `set_node_status(RUNNING)` 不 dispatch(等下 tick 重新搜推,可能复选失败 bot),且重派不经 `claim_node` → `attempted_executors` 计数不涨 → 死循环同执行方重派;reroute 用 `driver.redispatch(C5)` 计数规则,与 spec FR-GRAPH-08"由失败方 exec-bot skill 发起 gap bot-search 判 reroute"不一致;且生产 `POST /events` 只调 `TaskService.on_event`(落态 fold)未接 `Scheduler.on_event` → 旁路生产走不到。**重构**:`_tick` 放开"只推进 PENDING",对 FAILED-Dispatch 调 `_retry_failed`(attempts<max 经 `claim_node` 同执行方 re-claim+fire,推进计数;到 max 派 `ExecutionPort.probe` 给失败方 skill 判 reroute,guard 只派一次);reroute = skill 经 `task-plan-skill` 发起 gap `bot-search` → `add_node(BOT_SEARCH)` → 后续 tick 处理,非 scheduler C5 规则;`Scheduler.on_event` 改为 NODE_FAILED 落态后泵一次 `tick`(tick 为唯一驱动权威);router `/events` 补 `Scheduler.on_event` 双调。详见 tasks T-13。
