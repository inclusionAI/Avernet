...
from secbaas.community.spi.sandbox import DesktopSandbox, DesktopSandboxPlugin
...
class MockDesktopSandboxPlugin:
    ...
    def resolve_ws_conn_info(
        ...
        """
        return {
            "ws_url": f"wss://agentclawproxy-dev.service.test/wsrelay/{session_id}",
...
class TestPublicApiExports:
    """Verify DesktopSandbox and DesktopSandboxPlugin are exported from secbaas.community.spi.sandbox."""

    def test_desktop_device_importable(self) -> None:
        """THEN DesktopSandbox is importable from secbaas.community.spi.sandbox."""
        from secbaas.community.spi.sandbox import DesktopSandbox as DD

        assert DD is DesktopSandbox

    def test_desktop_sandbox_plugin_importable(self) -> None:
        """THEN DesktopSandboxPlugin is importable from secbaas.community.spi.sandbox."""
        from secbaas.community.spi.sandbox import DesktopSandboxPlugin as DDP

        assert DDP is DesktopSandboxPlugin

    def test_both_in_all(self) -> None:
        """THEN both classes are listed in __all__."""
        from secbaas.community.spi.sandbox import __all__ as exported
