...
mod transaction;
pub use transaction::{DbExecuteResult, DbStatement, DbTransactionParam, DbTransactionStep, DbTransactionStepResult};
...
pub trait DbPlugin: Send + Sync + 'static {
    ...
    ///
    /// Validate step options before starting. Commit when every reached step
    /// succeeds, or a plain Execute with `with_transaction_stop_on_no_rows`
    /// affects zero rows. That explicit early success commits the prefix and
    /// returns only reached results, including the zero-row Execute result.
    /// Otherwise results correspond one-to-one to steps. Any execution or
    /// commit failure is an error; execution failures roll back earlier writes.
    /// An ordinary zero-row query or Execute continues to the next step.
