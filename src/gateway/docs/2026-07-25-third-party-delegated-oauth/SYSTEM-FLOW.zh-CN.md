...
- **A vs B 已定（2026-07-25）→ 建 A。** 没有外部提供方（corp 的 antbuservice、community 的 Google）能签发 **tc-audience、tc-consented** 令牌，故自建授权服务器（方案 A）就是两种 flavor 下的设计。更轻的"借 IdP"分支已出局。

**待议 —— 需团队**

1. **推进 vs 推迟：** 若团队判断当下不值得优先做这个干净修复，唯一的替代是*有据可查地*接受透传反模式 —— 而非"我们别无选择"。（注意：既然 B 已排除，推迟即意味着接受反模式；不存在可等待的更轻的干净选项。）
