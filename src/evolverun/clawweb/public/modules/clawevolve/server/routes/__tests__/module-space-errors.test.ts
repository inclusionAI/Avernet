...
let baseUrl: string;
const listAccessibleSpaces = vi.fn<SpaceDirectory["listAccessibleSpaces"]>();
const exportLocalSkill = vi.fn<BotSkillGateway["exportLocalSkill"]>();
...
beforeEach(async () => {
  ...
  await new SkillAssetRepository(db).createAsset({ assetId: "target", versionId: "target-v1", ownerUserId: actor,
    botId: "bot", externalSkillId: "existing", displayName: "Existing", packageRef: "fixture:existing", packageSha256: "fixture-checksum" });
});
...
async function startHost(configured = true) {
  module = createClawevolveModule({ db, dispatch,
    ...
    ...(configured ? { hostSpaces: { listAccessibleSpaces } } : {}),
  });
  const app = express();
...
describe("ClawEvolve module space errors under a generic-500 host", () => {
  ...
  it.each(["/stage-developments", "/skill-assets"] as const)("preserves explicit-space 503 when Host spaces are unconfigured for %s", async endpoint => {
  ...
  it.each([
    { code: "HOST_SPACE_UNAVAILABLE", status: 503, field: "status" },
    { code: "HOST_SPACE_REQUEST_FAILED", status: 403, field: "statusCode" },
    { code: "HOST_SPACE_INVALID_RESPONSE", status: 502, field: "status" },
    { code: "HOST_PERSONAL_SPACE_NOT_INITIALIZED", status: 409, field: "statusCode" },
  ])("preserves safe $code/$status across all mounted public space routes", async ({ code, status, field }) => {
    listAccessibleSpaces.mockRejectedValue(Object.assign(new Error("可安全展示的 宿主空间错误"), {
    ...
    for (const endpoint of endpoints) {
      ...
      expect(await response.json()).toEqual({ code, error: "可安全展示的 宿主空间错误" });
    }
    expect(hostErrors).not.toHaveBeenCalled();
    await expectUnchanged(before);
  });
  ...
  it.each([
    ...
    { name: "unknown code with status 403", fields: { code: "UNKNOWN_Host_ERROR", status: 403 } },
    { name: "safe code but string status", fields: { code: "HOST_SPACE_UNAVAILABLE", status: "503" } },
    { name: "safe code but non-error status", fields: { code: "HOST_SPACE_UNAVAILABLE", status: 200 } },
    { name: "safe code but out-of-range status", fields: { code: "HOST_SPACE_UNAVAILABLE", status: 600 } },
    { name: "safe code but fractional status", fields: { code: "HOST_SPACE_UNAVAILABLE", status: 503.5 } },
    { name: "safe code but non-string message", fields: { code: "HOST_SPACE_UNAVAILABLE", status: 503, message: { privateDetail } } },
