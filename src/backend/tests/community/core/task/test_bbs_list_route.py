"""BBS 任务列表 HTTP 路由契约测试:GET /openapi/v1/collaboration/tasks/bbs/list。
...
def _client(records: list[BbsTaskOverviewRecord]) -> TestClient:
    ...
    app.dependency_overrides[require_principal] = lambda: {"user_id": "bbs-list-owner"}
    app.dependency_overrides[require_user_id] = lambda: "bbs-list-owner"
...
def test_bbs_list_route_returns_envelope_with_parsed_fields(client):
    r = client.get("/openapi/v1/collaboration/tasks/bbs/list")
...
def test_bbs_list_route_empty_when_no_bbs():
    c = _client([])
    r = c.get("/openapi/v1/collaboration/tasks/bbs/list")
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["code"] == 200000
...
def test_bbs_list_route_paginates_by_page_and_page_size():
    ...
    first = c.get("/openapi/v1/collaboration/tasks/bbs/list", params={"page": 1, "page_size": 2})
    assert first.status_code == 200, first.text
    page1 = first.json()["data"]
    assert page1["total"] == 3
    assert [it["task_id"] for it in page1["items"]] == ["bbs-1", "bbs-2"]

    second = c.get("/openapi/v1/collaboration/tasks/bbs/list", params={"page": 2, "page_size": 2})
...
def test_bbs_list_route_page_beyond_range_empty_with_total():
    ...
    r = c.get("/openapi/v1/collaboration/tasks/bbs/list", params={"page": 5, "page_size": 10})
    assert r.status_code == 200, r.text
...
def test_bbs_list_route_rejects_invalid_page_params():
    ...
    assert c.get("/openapi/v1/collaboration/tasks/bbs/list", params={"page": 0}).status_code == 422
    assert (
        c.get("/openapi/v1/collaboration/tasks/bbs/list", params={"page_size": 0}).status_code == 422
    )
    assert (
        c.get("/openapi/v1/collaboration/tasks/bbs/list", params={"page_size": 101}).status_code == 422
    )
...
def test_bbs_list_route_filters_by_status():
    ...
    r = c.get("/openapi/v1/collaboration/tasks/bbs/list", params={"status": "RUNNING"})
    assert r.status_code == 200, r.text
    page = r.json()["data"]
    assert page["total"] == 2
...
def test_bbs_list_route_rejects_invalid_status():
    ...
    c = _client([_canned_record()])
    assert (
        c.get("/openapi/v1/collaboration/tasks/bbs/list", params={"status": "NOPE"}).status_code == 400
    )
...
def test_bbs_list_route_rejects_multi_value_status():
    ...
    assert (
        c.get(
            "/openapi/v1/collaboration/tasks/bbs/list", params={"status": "RUNNING,DONE"}
        ).status_code
        == 400
    )
...
def test_bbs_list_route_filters_by_search_word_task_spec():
    ...
    r = c.get("/openapi/v1/collaboration/tasks/bbs/list", params={"search_word": "alphaunique"})
    assert r.status_code == 200, r.text
    page = r.json()["data"]
    assert page["total"] == 1
...
def test_bbs_list_route_filters_by_search_word_extend_props():
    ...
    r = c.get("/openapi/v1/collaboration/tasks/bbs/list", params={"search_word": "zoeunique"})
    assert r.status_code == 200, r.text
    page = r.json()["data"]
    assert page["total"] == 1
    assert page["items"][0]["task_id"] == "bbs-1"
...
def test_bbs_list_route_search_word_case_insensitive():
    ...
    r = c.get("/openapi/v1/collaboration/tasks/bbs/list", params={"search_word": "mixedcasetitle"})
    assert r.status_code == 200, r.text
    assert r.json()["data"]["total"] == 1
...
def test_bbs_list_route_search_word_no_match():
    ...
    c = _client([_canned_record()])
    r = c.get("/openapi/v1/collaboration/tasks/bbs/list", params={"search_word": "zzznotfound"})
    assert r.status_code == 200, r.text
...
def test_bbs_list_route_search_word_blank_treated_as_omitted():
    ...
    c = _client([_record("bbs-1"), _record("bbs-2")])
    r = c.get("/openapi/v1/collaboration/tasks/bbs/list", params={"search_word": "   "})
    assert r.status_code == 200, r.text
    assert r.json()["data"]["total"] == 2
...
def test_bbs_list_route_combines_status_and_search_word():
    ...
    r = c.get(
        "/openapi/v1/collaboration/tasks/bbs/list",
