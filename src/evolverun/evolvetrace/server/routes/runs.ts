...
import { sendIntervention } from "../services/baas-intervention.js";
...
export function createRunsRouter(
  ...
  router.get("/", asyncHandler(async (_req: Request, res: Response) => {
    ...
    try {
      ...
      const countOptions = { status, workflowId, from, to, inputQuery, ...originFilter };
      ...
      const statusCounts = !status && !inputQuery
  ...
  router.post("/:flowId/poll-message", asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const { resolveBaasConfig } = await import("../db.js");
      ...
      // Build URL — baseUrl must be provided via BAAS_BASE_URL; no hardcoded origin.
      if (!config.baseUrl) {
        res.status(400).json({ error: "Bad Request", message: "BaaS baseUrl 未配置" });
        return;
      }
      const pollUrl = `${config.baseUrl}/openapi/v1/messages/${encodeURIComponent(messageId)}`;
