...
CALLBACK_TIMEOUT_SECONDS = int(os.environ.get("CLAWBENCH_CALLBACK_TIMEOUT_SECONDS", "180"))
CALLBACK_RETRY_ATTEMPTS = int(os.environ.get("CLAWBENCH_CALLBACK_RETRY_ATTEMPTS", "3"))
CALLBACK_RETRY_BASE_DELAY_SECONDS = float(os.environ.get("CLAWBENCH_CALLBACK_RETRY_BASE_DELAY_SECONDS", "1"))


def _callback_env(name: str, default: str = "") -> str:
...
def _callback_owner_id() -> str:
    return _callback_env("CLAWBENCH_OWNER_ID") or _callback_env("OWNER_ID")


def _callback_enabled() -> bool:
    return (
        _callback_env("CLAWBENCH_CALLBACK_ENABLED").lower() in {"1", "true", "yes"}
        and bool(_callback_env("CLAWWEB_URL"))
        and bool(_callback_env("CLAWBENCH_BENCH_RUN_ID"))
        and bool(_callback_owner_id())
    )


def _callback_retry_delay(attempt: int) -> float:
    jitter = random.uniform(0, 0.25)
    return CALLBACK_RETRY_BASE_DELAY_SECONDS * (2 ** max(0, attempt - 1)) + jitter


def _callback_json(method: str, url: str, payload: Dict[str, Any] | None = None) -> Dict[str, Any]:
    data = None if payload is None else json.dumps(payload).encode("utf-8")
    headers = {"Content-Type": "application/json", "X-User-Id": _callback_owner_id()}
    max_attempts = max(1, CALLBACK_RETRY_ATTEMPTS)
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            with urllib.request.urlopen(req, timeout=CALLBACK_TIMEOUT_SECONDS) as resp:
                ...
                if attempt > 1:
                    logger.info(
                        "[clawweb-callback] retry succeeded attempt=%s/%s method=%s url=%s",
                        attempt,
                        max_attempts,
                        method,
                        url,
            ...
            last_error = err
            delay = _callback_retry_delay(attempt)
            logger.warning(
                "[clawweb-callback] retryable HTTP error attempt=%s/%s nextDelay=%.1fs method=%s url=%s status=%s",
                attempt,
                max_attempts,
                delay,
                method,
            ...
            delay = _callback_retry_delay(attempt)
            logger.warning(
                "[clawweb-callback] retryable error attempt=%s/%s nextDelay=%.1fs method=%s url=%s error=%s",
                attempt,
                max_attempts,
                delay,
                method,
...
def _callback_ok(method: str, url: str, payload: Dict[str, Any]) -> bool:
    try:
        _callback_json(method, url, payload)
        return True
    except Exception as exc:
        logger.warning("[clawweb-callback] %s", exc)
        return False


def _merge_run_summary(clawweb_url: str, bench_run_id: str, patch: Dict[str, Any]) -> None:
    try:
        run = _callback_json("GET", f"{clawweb_url}/api/bench/runs/{bench_run_id}")
        summary = run.get("summary") if isinstance(run, dict) else {}
        if not isinstance(summary, dict):
            summary = {}
        summary.update(patch)
        _callback_ok("PUT", f"{clawweb_url}/api/bench/runs/{bench_run_id}", {"summary": summary})
    except Exception as exc:
        logger.warning("[clawweb-callback] failed to update run summary: %s", exc)


def _summarize_transcript_file(path: Path) -> Dict[str, Any]:
    event_count = 0
    first_event_type = ""
    last_event_type = ""
    session_id = ""
    ...
    try:
        with path.open("r", encoding="utf-8", errors="replace") as fh:
            for line in fh:
                ...
                # Extract workflow metadata from manifest
...
def _normalize_task_status(raw: Any, score: float | None = None, max_score: float | None = None) -> str:
    ...
    if status in {"error", "failed"}:
        return "failed"
    if status == "pending" and score is not None and max_score is not None and max_score > 0:
        return "succeeded" if score >= max_score else "scored"
    return status


def _notify_clawweb_task_completed(
    *,
    task: Task,
    task_entry: Dict[str, Any],
    task_grading: Dict[str, Any],
    task_index: int,
    task_total: int,
    ...
    uploaded_transcript_paths: set[str],
) -> None:
    if not _callback_enabled():
        return

    clawweb_url = _callback_env("CLAWWEB_URL").rstrip("/")
    bench_run_id = _callback_env("CLAWBENCH_BENCH_RUN_ID")
    now = int(time.time())
    score = float(task_grading.get("mean", 0.0) or 0.0)
    max_score = 1.0
    result_status = _normalize_task_status(task_entry.get("status"), score, max_score)
    completed = len(grades_by_task_id)
    succeeded = sum(1 for entry in task_entries if _normalize_task_status(entry.get("status")) == "succeeded")
    failed = sum(1 for entry in task_entries if _normalize_task_status(entry.get("status")) == "failed")
    score_so_far = sum(float(g.get("mean", 0.0) or 0.0) for g in grades_by_task_id.values())
    max_score_so_far = float(completed)
    ...
    progress = {
        ...
        "lastUpdatedAt": now,
        "scoreSoFar": score_so_far,
        "maxScoreSoFar": max_score_so_far,
        "passRateSoFar": (score_so_far / max_score_so_far) if max_score_so_far else None,
        "tokenUsageSoFar": {
    ...
    _merge_run_summary(clawweb_url, bench_run_id, {"progress": progress})

    result_json = dict(task_entry)
    _callback_ok(
        "POST",
        f"{clawweb_url}/api/bench/runs/{bench_run_id}/results",
        {
            "results": [
                {
                    "resultId": f"res_{bench_run_id}_{task.task_id}",
                    "taskId": task.task_id,
                    "taskName": task.frontmatter.get("name", task.task_id),
                    "status": result_status,
                    "score": score,
                    "maxScore": max_score,
                    "gradingType": task.grading_type,
                    "executionTimeMs": int(float(task_entry.get("execution_time", 0.0) or 0.0) * 1000),
                    "transcriptPath": str(transcript_path) if transcript_path.is_file() else None,
                    "workspacePath": task_entry.get("workspace"),
    ...
    if transcript_path.is_file():
        ...
        if resolved not in uploaded_transcript_paths:
            try:
                size_bytes = transcript_path.stat().st_size
                if size_bytes <= 10 * 1024 * 1024:
                    content_text = transcript_path.read_text(encoding="utf-8", errors="replace")
                    summary = _summarize_transcript_file(transcript_path)
                    usage = task_entry.get("usage")
                    ...
                    if _callback_ok(
                        ...
                        uploaded_transcript_paths.add(resolved)
                        logger.info("[clawweb-callback] uploaded session artifact for %s", task.task_id)
                else:
                    logger.warning("[clawweb-callback] skip transcript over 10MB: %s", transcript_path)
            except Exception as exc:
                logger.warning("[clawweb-callback] failed to upload transcript %s: %s", transcript_path, exc)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Skip uploading to external leaderboard (default: True, will switch to internal leaderboard later)",
    )
    parser.add_argument(
        "--upload",
        type=str,
    ...
    parser.add_argument(
        "--judge-base-url",
        default=None,
        help="Custom OpenAI-compatible API base URL for judge (bypasses standard providers)",
    )
    parser.add_argument(
        "--judge-api-key",
        default=None,
        help="API key for judge custom endpoint (default: $OPENAI_API_KEY env var)",
    )
    parser.add_argument(
        "--judge-timeout-seconds",
        type=float,
        default=DEFAULT_JUDGE_TIMEOUT_SECONDS,
        help=f"Judge call timeout in seconds (default: {DEFAULT_JUDGE_TIMEOUT_SECONDS})",
    )
    parser.add_argument(
        "--judge-max-retries",
        type=int,
        default=DEFAULT_JUDGE_MAX_RETRIES,
        help=f"Maximum judge call attempts (default: {DEFAULT_JUDGE_MAX_RETRIES})",
    )
    parser.add_argument(
        "--base-url",
        default=None,
    ...
    parser.add_argument(
        ...
        type=str,
        default="pinchbench",
        help="Benchmark path (relative to tasks/ or absolute). Domain is extracted from the last directory name.",
    )
    parser.add_argument(
        "--scene",
        default="default",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Copy TOOLS.md and AGENTS.md from the main OpenClaw workspace "
            "(~/.openclaw/workspace/) into each benchmark agent's workspace. "
            "This ensures the benchmark agent has the same MCP tool mappings "
            "and agent configuration as the main workspace."
        ),
    )
    parser.set_defaults(link_skills=True)
    parser.add_argument(
        "--link-skills",
        dest="link_skills",
        action="store_true",
        help=(
            "Symlink ~/.openclaw/workspace/skills into each benchmark workspace "
            "instead of copying skill directories (default)."
        ),
    )
    parser.add_argument(
        "--copy-skills",
        dest="link_skills",
        action="store_false",
        help="Copy skill directories into each benchmark workspace instead of symlinking.",
    )
    return parser.parse_args()


def _build_grade_kwargs(
    task: Task, execution_result: Dict[str, Any], skill_dir: Path, args: argparse.Namespace
) -> Dict[str, Any]:
    grade_kwargs = dict(
        task=task,
...
def _should_use_api_judge(args: argparse.Namespace) -> bool:
    return bool(getattr(args, "judge_base_url", None) or getattr(args, "judge_api_key", None))
...
def _generate_run_id() -> str:
    """Generate unique run ID using timestamp + UUID for concurrent safety."""
...
def main():
    ...
    # Parse args first to get benchmark name
    args = _parse_args()

    # Determine tasks directory based on --benchmark parameter
    script_dir = Path(__file__).parent
    skill_root = script_dir.parent  # Parent of scripts/ is the skill root
    tasks_dir = skill_root / "tasks" / args.benchmark
    ...
    # Use UUID to isolate concurrent benchmark runs for the same model
    run_uuid = uuid.uuid4().hex[:8]
    agent_id = task_scoped_agent_id(f"bench-{model_slug}", run_uuid)
    # Scene-based workspace for concurrent isolation
    agent_workspace = Path(f"/tmp/pinchbench/{args.scene}/{run_id}/{run_uuid}/agent_workspace")
    ...
    incremental_path = scene_dir / f"{run_id}_{model_slug}_benchmark_report.json"
    ...
    def _write_incremental_results():
        task_entries = [
            {
                ...
                "interaction_events": r.get("interaction_events", []),
        ...
        # Extract domain from benchmark path (last directory name)
    ...
    uploaded_transcript_paths: set[str] = set()
    for i, task in enumerate(tasks_to_run, 1):
        task_grades = []
        task_results = []
        for run_index in range(runs_per_task):
            ...
            try:
                result = execute_openclaw_task(
                    ...
                    skill_dir=skill_dir,
                    output_dir=scene_dir / f"{run_id}_{model_slug}_transcripts",
                    verbose=args.verbose,
                    copy_main_workspace_md=args.copy_main_workspace_md,
                    link_skills=args.link_skills,
                    judge_model=args.judge or DEFAULT_JUDGE_MODEL,
                    judge_backend="api" if _should_use_api_judge(args) else "openclaw",
            ...
            try:
                grade_kwargs = _build_grade_kwargs(task, result, skill_dir, args)
                grade = grade_task(**grade_kwargs)
            except Exception as exc:
                traceback_preview = traceback.format_exc()[-4000:]
                if execution_error:
                    note = f"EXECUTION_FAILED: {execution_error}; AUTOMATED_GRADING_FAILED: {type(exc).__name__}: {exc}; Traceback preview: {traceback_preview}"
                else:
                    note = f"AUTOMATED_GRADING_FAILED: {type(exc).__name__}: {exc}; Traceback preview: {traceback_preview}"
                logger.warning(
                    "Task grading failed for %s, continuing: %s\n%s",
                    task.task_id,
                    exc,
                    traceback_preview,
                )
            ...
            if result.get("status") == "error":
                ...
                if stderr_preview:
                    execution_note = (
                        f"OPENCLAW_AGENT_MESSAGE_FAILED: OpenClaw exited with code {exit_code}. "
        ...
        task_entries, efficiency = _write_incremental_results()
        task_entry = next((entry for entry in reversed(task_entries) if entry.get("task_id") == task.task_id), None)
        if task_entry:
            transcript_path = scene_dir / f"{run_id}_{model_slug}_transcripts" / f"{task.task_id}.jsonl"
            _notify_clawweb_task_completed(
                task=task,
                task_entry=task_entry,
                task_grading=grades_by_task_id[task.task_id],
                task_index=i,
                task_total=len(tasks_to_run),
                task_entries=task_entries,
                grades_by_task_id=grades_by_task_id,
                efficiency=efficiency,
                transcript_path=transcript_path,
                uploaded_transcript_paths=uploaded_transcript_paths,
            )

    output_path = scene_dir / f"{run_id}_{model_slug}_benchmark_report.json"
        ...
        task_entries = [
            {
                ...
                "interaction_events": result.get("interaction_events", []),
