...
// Only external OCB ports are mocked; ZIP validation and artifact IO stay real.
const owner = "registrar";
const member = "teammate";
const outsider = "outsider";
const team: OcbSpace = { id: "team-97", name: "Aurora Research", type: "TEAM", role: "MEMBER" };
const otherTeam: OcbSpace = { id: "team-other", name: "Other Team", type: "TEAM", role: "ADMIN" };
...
let ocbSpaces: OcbSpacePort;
let ocbLocalSkills: OcbLocalSkillPort;
...
beforeEach(async () => {
  ...
  ocbSpaces = {
    listAccessibleSpaces: vi.fn(async ({ identity }) => [
      ...
      // not whichever space OCB happens to return first.
  ...
  ocbLocalSkills = {
    ...
    listLocalSkills: vi.fn(async () => [
      { skillId: "local-skill", displayName: "Evidence Skill", description: "OCB description" },
    ]),
    exportLocalSkill: vi.fn(async () => ({ packageBytes,
      sha256: createHash("sha256").update(packageBytes).digest("hex"),
      displayName: "Evidence Skill", description: "OCB description" })),
    replaceLocalSkill: vi.fn(async () => { throw new Error("Registration must not replace an OCB Skill"); }),
  };
  const app = express();
  app.use(express.json());
  app.use("/api/evolve", createSkillAssetsRouter({ repo: assets, artifactStore: store, ocbLocalSkills, ocbSpaces }));
  app.use("/api/evolve", createStageSkillsRouter({ repo: stages, artifactStore: store, ocbSpaces }));
...
describe("Space registration with real repositories", () => {
  ...
  it("preserves owner-only access for historical records with no space, even without current memberships", async () => {
    ...
    await assets.createAsset({ assetId: "legacy-asset", versionId: "legacy-version", ownerUserId: owner,
      botId: "bot-fixture", ocbSkillId: "legacy-skill", displayName: "Historical Skill",
    ...
    vi.mocked(ocbSpaces.listAccessibleSpaces).mockResolvedValue([]);
