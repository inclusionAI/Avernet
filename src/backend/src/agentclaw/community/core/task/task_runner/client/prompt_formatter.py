...
def _static_relay_closure() -> str:
    ...
    return "\n".join([
        ...
        "【协作群多轮接力(仅当上方【## 群组成】列出除你之外的其它 Bot 成员)】上述三步按多轮执行,不得合并成一轮;你是 driver(协调者)时按 1-4 执行,你是被派发成员(consultant 等)时按 3 执行:",
        "1) 承接(driver 先单独回复):复述接自谁、上游交你的下一步任务、你的承接边界,并说明本群各成员 Bot 分工(含 driver 自己的职责);此轮只发承接,不执行、不派发;",
        "2) 派发 + driver 自执行(driver):对【## 群组成】中每个非人类 Bot 成员用 bcs_assign_task(主从群)或 bcs_route(chat 群)派发;派发消息必须把上方【## 上游产出正文】全文复制进去,并在消息内声明“此即本次完整输入,不得请求更多详情”;human 仅为观察者,不得向 human 派发;driver 自己也是执行者,须并行完成自己职责范围内的产出,不得把全部执行甩给成员;",
        "3) 成员执行(被派发成员):上方【## 上游产出正文】及派发消息带入的正文即为完整输入,视为已提供全部所需上下文;无论详略,立即基于此 + 自身合理假设给出你职责内的完整审查结论或修订条件;严禁以“缺详情/需完整方案”为由请求追加输入、要求同步方案、暂停或拒绝执行——一律直接产出结论。",
        "4) 汇总与交接(driver):收齐各成员回复并合并自己已完成的产出,综合成本群执行产出(执行产出步),再算 gap 给出交接(gap与交接步);在正文描述下一步交给谁、做什么,不路由群外。",
