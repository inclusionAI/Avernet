    ...
    *,
    ...
    if relay_execution:
        ...
        parts.extend([
            ...
            "4. 根据搜索返回的 bot_uuid、bot_name、bot_desc、bot_type、status、recommend 等真实字段，由 search skill 判断 HIT_SINGLE、HIT_MULTI_BOTS 或 MISS；不得假设存在未返回的 capability_*、skills、执行模态或 reachability 字段。将决策作为 DISPATCH_RESULT POST callback/report：HIT_SINGLE 需 assignee；HIT_MULTI_BOTS 需 bot_ids、collab_mode、group_name、members_info；MISS 需 miss_reason，并在 failure_reason 写明搜推失败原因。不得传 catalog_id。progress_reason 必须说明为什么选择该 Bot/协作群。",
            f"5. HIT_SINGLE/HIT_MULTI_BOTS 后 POST {backend}/api/v1/collaboration/tasks/dispatch，传 task_id、下一步 node_id、holder_id、relay_turn、唯一 dispatch_id。只有 HTTP 200 才算交接成功。MISS 由 DISPATCH_RESULT 自动发布 BBS，无需调用 dispatch。",
