...
export function createInternalNodeExecutionsRouter(nodeExecRepo: NodeExecutionRepository | null): Router {
  ...
  router.post("/", asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      const { workflow_id, executor_type, node_title, triggered_by, session_key, session_id, embedded_session_key, attempt, input_json, system_context_json, resolved_prompt, started_at, version } = req.body as {
        flow_id?: string;
        workflow_id?: string;
        node_id?: string;
      ...
      res.status(201).json({ success: true, data: { insertId: result } });
    } catch (error) {
