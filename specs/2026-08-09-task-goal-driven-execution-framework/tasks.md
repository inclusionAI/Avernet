...
> 权威:`spec.md`(WHAT/WHY)、`plan.md`(HOW)。v4.4 结构性重构(对齐最新 classDiagram + 5 模块文档):
> ① `Relation` 一等公民(`depends_on` 退场);② `PLANNING` 新态(6 态);③ 模型瘦身(`task_id`/`node_run_graph`/`run_id`/`relations`/`run_mode` str/删 collab_mode/SLA/Scope/RunMode/CollabMode 枚举/`AcceptanceCriteria.type`/`AcceptanceResult` 无 verifier);④ facade 6→2 API + `TaskGraphService` 独立;⑤ 事件驱动 on_* + 状态条件触发(删 `ExecutionEngine.drive` fixpoint 泵)。
...
- **写网关收口**:图谱原子变更只走 `TaskGraphService` 4 API + 内部派生查询;旁路同写口。
- **框架零 case 知识**:任何具体节点名(`N_overview`/`N_market` 等)只允许出现在 case 的 DecomposerPort 产出或测试 stub,**禁止出现在框架代码**。
- **不并存两套**:本轮即合并,删除并行包与旧模型失效代码(含 v4.3 `ExecutionEngine.drive`/6 facade/`depends_on` 字段/`TaskGraphStore` 合并设计)。

## 依赖与落地序(M0 模型 → M1 图谱 → M2 编排核 → M3 planner → M4 dispatcher/runner → M5 harness/facade → M6 singlebox E2E)

```
M0  领域模型 v4.4(Relation/6态/瘦身后字段) + 中间类型(patch/criteria/op_result/callback_data)
 → M1  TaskGraphService 独立(4 API + 内部派生查询;relations 依赖派生)
...
- T0.5 派生规则注释:dependencies_satisfied/depth/decompose_children 均从 `relations.type=DEPENDENCY` 派生(非持久)。
- ✅ 验收:模型与最新 classDiagram 1:1;`grep` 无 `depends_on`/`collab_mode`/`SLA`/`Scope`/`RunMode`/`CollabMode`/`verifier`/`NodeRuntimePatch`/`TaskGraphInfo` 残留。

## M1 — TaskGraphService(独立图谱 SSOT,4 API + 内部派生)
...
- T1.5 内部派生查询(只读,`_` 前缀):`_query_nodes`、`_decompose_children`、`_compute_parents`、`_node_depth`、`_execution_config`——均从 `relations` 派生。
- T1.x 单测:状态流转合法性、add a/b/c 条件触发校验、relations 依赖派生(deps_satisfied/depth)、PLANNING 语义、传播规则、单层同构护栏。

## M2 — 编排核 on_*(事件驱动 + 状态条件)
- T2.1 `_Orchestrator.on_execute(task_id)`:initialize_graph 后,条件 a(根 PENDING)→ plan→add→dispatch→start_run。
...
- T2.x 单测:on_execute 首帧、on_report PASS/FAIL 分支、on_miss 闸门、on_harness 不抢正向、串行化、loop_round 递增。
...
- T3.2 `TaskPlanner.plan(graph)->list[TaskNode]`:**触发条件**(图谱有更新 ∧ 无 RUNNING 节点 ∧ 有 PLANNING 节点);不满足返回 []。读图自发现目标(FAIL 叶子/PLANNING 父),委托 decompose;规划原则硬约束(派发/执行中节点不可改含前序依赖;只对失败+子全DONE自身PLANNING父);纯读图去重;步进式 deps 满足才产。**删除** v4.3 写死节点。
...
- **M4a TaskDispatcher + BotDiscoverPort**
  ...
  - T4.2 `TaskDispatcher.dispatch(to_do_list)->list[DispatchOutcome]`:无 graph 入参;前序依赖(relations)双检全 DONE;search→update_task_node_info(run_mode str/assignee,RUNNING);HIT_MULTI_BOTS→`runner.form_coop_group`;MISS→DispatchOutcome(miss)交编排核;完成派发目标落库后 batch 交 `start_run`。
  - T4.3 `SearchBasedDispatchRule`(`OptimizerRule`)委托 `BotDiscoverPort`+`TaskRunner`。
  - T4a.x 单测:四 outcome 派发目标落库、MISS 不决策、collab_mode 来自 search(内部)、DISPATCHED 必 RUNNING、前序依赖双检、start_run 由 Dispatcher 触发(批量)。
- **M4b TaskRunner + TaskLoopCallback**
  ...
  - T4.8 输入/验收投影:`start_run` 内部 `_project_output(task_id,node,accept_type)` 用 `_compute_parents`/`_query_nodes`/`_decompose_children` 组合按 NODE/SUBTREE/TASK 聚合 DONE output 注入执行体(accept_type 用 str,原 Scope 退场)。
  - T4b.x 单测:start_run 三 run_mode 分发(含 BBS 只挂悬赏)、query_*回填、TaskLoopCallback.report_result→on_report 适配映射、form_coop_group BCS local/mock 契约、BBS 自主回投用例、_project_output 三 scope。

## M5 — TaskHarness + TaskService facade
- T5.1 `TaskHarness.run_poll_loop`:周期 `_query_nodes(RUNNING)`→比对 start_time + sla_timeout(execution_config/extend_props)→超时/崩溃 `update_task_node_info(HUNG/FAILED, extend_props_patch)`;不抢正向。
...
- T5.3 `TaskService.__init__` 组合根:注入 decomposer/discover/runner_ctor/bbs/harness;Avernet 用 stub/singlebox double,corp adapter 红线。
...
## Risks / 待实现期定的项
- 旧 `core/task` ORM repo 与 v4.4 图模型适配:in-memory 优先,ORM 适配按需(M1)。
...
## 里程碑
- ⬜ **M0** 领域模型 v4.4(结构性重构,先落模型)。
- ⬜ **M1** TaskGraphService 独立(4 API + relations 派生)。
