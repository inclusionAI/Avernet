...
async function startRouter(overrides: Record<string, unknown> = {}) {
  ...
  const skillAssetRepo = {
    findAsset: vi.fn(async () => ({
      ...
      external_skill_id: "local-skill", display_name: "local-skill",
    })),
  };
  ...
  app.use("/api/evolve", createStageSkillsRouter({
    ...
    hostLocalSkills: hostLocalSkills as never,
    dispatch,
    ...overrides,
  }));
  ...
  return {
    ...
    evolveRepo, stageRepo, skillAssetRepo, hostLocalSkills, putObject, getObject, dispatch,
  };
}
