...
AUTHORIZATION: dict[tuple[str, str], Authorization] = {
    ...
    # The user's own account-level authorizations: no bot to address, and the
    # user is the verified caller by construction (``UserIdDep``).
    ("POST", "/openapi/v1/org/user/authorized-apps"):
        NoCheck("the named user's own account-level authorizations"),
    ("GET", "/openapi/v1/org/user/authorized-apps"):
        NoCheck("the named user's own account-level authorizations"),
    ("DELETE", "/openapi/v1/org/user/authorized-apps/{app_id}"):
        NoCheck("the named user's own account-level authorizations"),
